...
/* Compute the int n for which we have: phi ~= (n * phiSubUnitWidth +
 * phiSubUnitStart). n = 0 for the module with the lowest Phi > -Pi/2. n = 0 for
 * the module with the lowest Phi > Pi/2.
 */
const int computePhiUnitRef(const double phi, const int numPhiUnits,
  ...
  if (fabs((phiUnitRefExact - round(phiUnitRefExact))) <
      inner_cabling_roundingTolerance)
    phiUnitRef = fabs(round(phiUnitRefExact));
  else
    phiUnitRef = std::floor(phiUnitRefExact);
...
/*
 * If a module is on the negative cabling side, it is seen here as from a
 * rotation of 180 deg around CMS_Y.
 */
const double computeStereoPhi(const double phi, const bool isPositiveZEnd) {
  const double stereoPhi =
      (isPositiveZEnd ? phi : femod(M_PI - phi, 2. * M_PI));
  return stereoPhi;
}
 ...
 * 4: (-X) side, (-Z) end
 */
const int computeInnerTrackerQuarterIndex(const bool isPositiveZEnd,
...
const int computeSubDetectorIndex(const std::string subDetectorName) {
  ...
  if (subDetectorName == inner_cabling_tbpx)
    ...
    logINFO(any2str("Unknown subDetector name : ") + any2str(subDetectorName));
  }
...
/*
 * This identifies the ring number (associated to a given radius) + the (Z) side
 * of the ring on which the module is located. The '(Z) side of a ring'
 * identifies whether a module is at the low |Z| or at the high |Z| within the
 * ring.
 */
const int computeHalfRingIndex(const int ringNumber,
                               const bool isSmallerAbsZHalfRing) {
  const int isSmallerAbsZHalfRingIndex = (!isSmallerAbsZHalfRing);
  const int halfRingIndex =
      (ringNumber < 1 ? 0 : (ringNumber - 1) * 2 + isSmallerAbsZHalfRingIndex);
 ...
 */
const int computeNumELinksPerModule(const std::string subDetectorName,
                                    const int layerOrRingNumber) {
  int numELinksPerModule = 0;

  // tbpx
  if (subDetectorName == inner_cabling_tbpx) {
    if (layerOrRingNumber == 1)
      numELinksPerModule = inner_cabling_numELinksPerModuleBarrelLayer1;
    else if (layerOrRingNumber == 2)
      numELinksPerModule = inner_cabling_numELinksPerModuleBarrelLayer2;
    else if (layerOrRingNumber == 3)
      numELinksPerModule = inner_cabling_numELinksPerModuleBarrelLayer3;
    else if (layerOrRingNumber == 4)
      numELinksPerModule = inner_cabling_numELinksPerModuleBarrelLayer4;
    else {
  ...
  // tfpx
    ...
    if (layerOrRingNumber == 1)
      numELinksPerModule = inner_cabling_numELinksPerModuleForwardRing1;
    else if (layerOrRingNumber == 2)
      numELinksPerModule = inner_cabling_numELinksPerModuleForwardRing2;
    else if (layerOrRingNumber == 3)
      numELinksPerModule = inner_cabling_numELinksPerModuleForwardRing3;
    else if (layerOrRingNumber == 4)
      numELinksPerModule = inner_cabling_numELinksPerModuleForwardRing4;
    else {
  ...
  // tepx
    ...
    if (layerOrRingNumber == 1)
      numELinksPerModule = inner_cabling_numELinksPerModuleEndcapRing1;
    else if (layerOrRingNumber == 2)
      numELinksPerModule = inner_cabling_numELinksPerModuleEndcapRing2;
    else if (layerOrRingNumber == 3)
      numELinksPerModule = inner_cabling_numELinksPerModuleEndcapRing3;
    else if (layerOrRingNumber == 4)
      numELinksPerModule = inner_cabling_numELinksPerModuleEndcapRing4;
    else if (layerOrRingNumber == 5)
      numELinksPerModule = inner_cabling_numELinksPerModuleEndcapRing5;
    else {
      logERROR(any2str("Found ring number ") + any2str(layerOrRingNumber) +
