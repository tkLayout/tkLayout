...
InnerBundle::InnerBundle(const int bundleId, const bool isPositiveZEnd,
  ...
  plotColor_ = computePlotColor(isBarrel_, isPositiveZEnd, layerDiskNumber,
                                myBundleIndex);
};
 ...
 */
void InnerBundle::addGBT(GBT *myGBT) { GBTs_.push_back(myGBT); }

/*
 * Compute bundle color on website.
 * Bundles next to each other in space, must be of different colors.
 */
const int InnerBundle::computePlotColor(const bool isBarrel,
