...
IrradiationMap::IrradiationMap(std::string irradiationMapFile)
    : rhoMin(0), rhoMax(0), rhoBinWidth(0), rhoBinNum(0), zMin(0), zMax(0),
      zBinWidth(0), zBinNum(0), invFemUnit(1) {
...
void IrradiationMap::ingest(std::string irradiationMapFile) {
  ...
  // change the min and max values because we are interested in the center
  // points of the bins
  zMin += zBinWidth / 2;
  zMax -= zBinWidth / 2;
  ...
  if (!found_rhoMin || !found_rhoMax || !found_rhoBinWidth ||
      !found_rhoBinNum || !found_zMin || !found_zMax || !found_zBinWidth ||
      !found_zBinNum || !found_invFemUnit) {
    std::ostringstream tempSS;
    tempSS << "Error while parsing irradiation map values: found_rMin "
           << found_rhoMin << "; found_rMax " << found_rhoMax
           << "; found_rBinWidth " << found_rhoBinWidth << "; found_rBinNum "
           << found_rhoBinNum << "; found_zMin " << found_zMin
           << "; found_zMax " << found_zMax << "; found_zBinWidth "
           << found_zBinWidth << "; found_zBinNum " << found_zBinNum
           << "; found_invFemUnit " << found_invFemUnit;
    logERROR(tempSS);
  }
}

double IrradiationMap::binArea() const { return zBinWidth * rhoBinWidth; }

bool IrradiationMap::operator<(const IrradiationMap &confrontedMap) const {
  return (binArea() < confrontedMap.binArea());
}

bool IrradiationMap::isInRegion(
    const std::pair<double, double> &coordinates) const {
  return ((zMin <= coordinates.first) && (zMax >= coordinates.first) &&
          (rhoMin <= coordinates.second) && (rhoMax >= coordinates.second));
}

double IrradiationMap::calculateIrradiation(
    const std::pair<double, double> &coordinates) const {
  double z = 0;
  double rho = 0;
  double z1 = 0;
  ...
  if (isInRegion(coordinates)) {
    ...
    // if the point is in a intersection of the grid formed by the map bin
    // centers
    if ((z1 == z2) && (rho1 == rho2)) {
      // single value
    ...
    // if is in a z line
      ...
      irrxy = irr1 / (rho2 - rho1) * (rho2 - rho) +
              irr2 / (rho2 - rho1) * (rho - rho1);
    }
    ...
    // if is in the middle
      ...
      irrxy = irr11 / ((z2 - z1) * (rho2 - rho1)) * (z2 - z) * (rho2 - rho) +
              irr21 / ((z2 - z1) * (rho2 - rho1)) * (z - z1) * (rho2 - rho) +
              irr12 / ((z2 - z1) * (rho2 - rho1)) * (z2 - z) * (rho - rho1) +
              irr22 / ((z2 - z1) * (rho2 - rho1)) * (z - z1) * (rho - rho1);
    }
  } else {
    logERROR(
        "Error while calculating module irradiation, module out of region");
  }
