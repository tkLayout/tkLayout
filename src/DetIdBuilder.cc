...
BarrelDetIdBuilder::BarrelDetIdBuilder(bool isPixelTracker,
...
void BarrelDetIdBuilder::visit(Barrel &b) {
  ...
  // !!! Barrel level
  if (!isPixelTracker_) {
    geometryHierarchyIds_[1] = 205 % 100;
  } // Outer Tracker : always 5.
  // In CMSSW, Phase 2 Outer Tracker Barrel is identified by 205, and its id is
  // 205 % 100.
  else {
    geometryHierarchyIds_[1] = 201 % 100;
  } // Inner Tracker : always 1.
    // In CMSSW, Phase 2 Inner Tracker Barrel is identified by 201, and its id
    // is 201 % 100.
...
void BarrelDetIdBuilder::visit(Layer &l) {
  ...
  if (!isTiltedLayer_)
    numRings_ = numFlatRings_;
}

void BarrelDetIdBuilder::visit(RodPair &r) {
  double phiSegment =
      2 * M_PI / numRods_; // Phi interval between 2 consecutive rods.
  double startAngle = femod(
      r.Phi(),
      phiSegment); // Calculates the smallest positive rod's Phi in the Layer.
...
void BarrelDetIdBuilder::visit(BarrelModule &m) {
  ...
  if (!m.isTilted()) {
    ...
    // !!! Inner Tracker : Rod level
      ...
      geometryHierarchyIds_[4] = phiRef_;
    }

    // Calculated ring identifier.
    // Ring numbering starts from 1 from lowest Z, and increments with
    // increasing Z.
    if (isCentered_)
      ringRef = (side > 0 ? (m.uniRef().ring + numFlatRings_ - 1)
                          : (1 + numFlatRings_ - m.uniRef().ring));
    else
      ringRef = (side > 0 ? (m.uniRef().ring + numFlatRings_)
                          : (1 + numFlatRings_ - m.uniRef().ring));

    // !!! Module level
    if (!isPixelTracker_) {
      geometryHierarchyIds_[6] = ringRef;
    }
  ...
  // Tilted part
    ...
    if (!isPixelTracker_) {
      // !!! Category level
      uint32_t category =
          (side > 0 ? 2 : 1); // tilted part : 2 on +Z side, 1 on -Z side.
      geometryHierarchyIds_[4] = category;

      // !!! Ring level
      ringRef = (side > 0 ? (m.uniRef().ring - numFlatRings_)
                          : (1 + numRings_ - m.uniRef().ring));
      geometryHierarchyIds_[5] = ringRef;
    ...
    // No scheme for tilted Inner Tracker exists in CMSSW.
      ...
      logWARNING("Tilted Pixel DetIds not supported yet.");
    }
  }
  ...
  if (!isPixelTracker_) {
    ...
    geometryHierarchyIds_[6] = sensorRef;
  };
...
void BarrelDetIdBuilder::visit(Sensor &s) {
  if (s.subdet() == ModuleSubdetector::BARREL) {
    ...
    // !!! Sensor level
    if (!isPixelTracker_) {
      uint32_t sensorRef = (s.innerOuter() == SensorPosition::LOWER
                                ? 1
                                : 2); // Lower sensor 1, upper sensor 2.
      geometryHierarchyIds_[7] = sensorRef;
    }
    // !!! Inner tracker : Sensor level
    else {
      geometryHierarchyIds_[6] = 0;
    }
...
EndcapDetIdBuilder::EndcapDetIdBuilder(bool isPixelTracker,
                                       std::vector<int> geometryHierarchySizes)
    : isPixelTracker_(isPixelTracker),
      geometryHierarchySizes_(geometryHierarchySizes) {}
...
void EndcapDetIdBuilder::visit(Endcap &e) {
  ...
  // !!! Endcap level
  if (!isPixelTracker_) {
    geometryHierarchyIds_[1] = 204 % 100;
  } // Outer Tracker : always 4.
  // In CMSSW, Phase 2 Outer Tracker Endcap is identified by 204, and its id is
  // 204 % 100.
  else {
    geometryHierarchyIds_[1] = 202 % 100;
  } // Inner Tracker : always 2.
    // In CMSSW, Phase 2 Inner Tracker Endcap is identified by 202, and its id
    // is 202 % 100.
}
...
void EndcapDetIdBuilder::visit(EndcapModule &m) {
  double phiSegment =
      2 * M_PI / numModules_; // Phi interval between 2 consecutive modules.
  double startAngle =
      femod(m.center().Phi(),
            phiSegment); // Calculates the smallest positive Phi in the Ring.

  // Calculates Phi identifier.
  uint32_t phiRef_ =
      1 + round(femod(m.center().Phi() - startAngle, 2 * M_PI) / phiSegment);
...
void EndcapDetIdBuilder::visit(Sensor &s) {
  if (s.subdet() == ModuleSubdetector::ENDCAP) {
    ...
    // std::cout << s.myDetId() << " " << test << " " << "rho = " <<
    // s.hitPoly().getCenter().Rho() << " z = " <<  s.hitPoly().getCenter().Z()
    // << " phi = " <<  (s.hitPoly().getCenter().Phi() * 180. / M_PI) <<
    // std::endl;
  }
}
