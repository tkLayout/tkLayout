...
int main(int argc, char *argv[]) {
  ...
  cout << "mod_d p_cut eff_1 inef_" << efficient_pt
       << " pt(1%) pt(90%) geom_ineff" << endl;
  for (mod_d = start; mod_d < stop; mod_d += step) {
    ...
    // cout << 100 * myError.probabilityInside(cur_cut, curvature, cur_error) *
    // myError.geometricEfficiency() << " ";
    printf("%.2f ",
           100 * myError.probabilityInside(1 / p_cut, curvature, cur_error) *
               myError.geometricEfficiency());

    p = efficient_pt;
    curvature = 1 / p;
    cur_error = myError.computeError(p) * curvature;
    // cout << 100-100 * myError.probabilityInside(cur_cut, curvature,
    // cur_error) * myError.geometricEfficiency() << " ";
    printf(
        "%.2f ",
        100 - 100 * myError.probabilityInside(1 / p_cut, curvature, cur_error) *
                  myError.geometricEfficiency());
...
int main(int ac, char* av[]) {
  try {
    ...
    optional.add_options()
      ...
      ("maxRange,r", po::value<double>(&range), "sets the vertical range for the
material plot")
      ("output,o", po::value<string>(&outputName), "output file name (.png will
be added)")
      ;
