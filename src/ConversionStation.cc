...
const std::map<std::string, ConversionStation::Type>
    ConversionStation::typeString = {{"flange", FLANGE}, {"second", SECOND}};
...
void ConversionStation::routeConvertedElements(
    MaterialObject &localOutput, MaterialObject &serviceOutput,
  ...
  for (const MaterialObject::Element *currElement : serviceElements_) {
    ...
    // Find whether the material need to be converted (flange station, or endcap
    // station with right destination)
    if ((stationType_ == FLANGE) ||
        (stationType_ == SECOND && currElement->destination.state() &&
         currElement->destination().compare(stationName_()) == 0)) {
      ...
      for (const std::unique_ptr<Conversion> &currConversion : conversions_) {
        ...
        if (myInput) {
          const std::vector<std::unique_ptr<MaterialObject::Element>>
              &myInputElements = myInput->elements();
          if (myInputElements.size() == 1) {
            inputElement = myInputElements.front().get();
          }
        }

        if (inputElement && currElement) {
          // If incoming material and conversion input element are the same: do
          // conversion!
          if (inputElement->elementName().compare(currElement->elementName()) ==
              0) {
            converted = true;

            // Calculate conversion factor
            multiplier = currElement->quantityInUnit(inputElement->unit(),
                                                     inactiveElement) /
                         inputElement->quantityInUnit(inputElement->unit(),
                                                      inactiveElement);

            // Compute the output and store result
            Inoutput *myOutput = currConversion->outputs();
            if (myOutput) {
              for (const std::unique_ptr<MaterialObject::Element>
                       &outputElementUPtr : myOutput->elements()) {
                MaterialObject::Element *outputElement =
                    outputElementUPtr.get();
                if (outputElement) {
                  // newElement is the converted Element.
                  // newElement is assigned the properties from OUTPUT Element.
                  // Notably, in the rare case where outputElement has a
                  // componentName (see conversion station cfg file), that name
                  // is assigned to newElement.
                  MaterialObject::Element *newElement =
                      new MaterialObject::Element(*outputElement, multiplier);

                  // Here, new Element is assigned additional info from the
                  // INPUT Elements. newElement is assigned the same
                  // componentName as in input (except if outputElement has a
                  // componentName).
                  if (currElement->componentName.state() &&
                      !outputElement->componentName.state()) {
                    newElement->componentName(currElement->componentName());
                  }
                  // If we end up with a converted element belonging to no
                  // component, there is a problem !
                  if (!newElement->componentName.state()) {
                    logWARNING("Element " + newElement->elementName() +
                               "which is at output of station" +
                               stationName_() +
                               "has no assigned componentName.");
                  }
                  if (currElement
                          ->debugInactivate()) { // apply the inactivation also
                                                 // to converteds
                    newElement->debugInactivate(true);
                  }
                  // TODO: check if is ok to do this
                  if (currElement->destination.state() &&
                      !newElement->destination
                           .state()) { // apply the same destination of
                                       // converted element (only if not defined
                                       // in output conversion rule)
                  ...
                  // We add it to the relevant volumes (these volumes depend
                  // whether the newElement is a service or local).
    ...
    // WARNINGS / INFOS on materials that were assigned in cfg to be converted,
    // but that were eventually not converted!!
    if (!converted) {
      ...
      if (stationType_ != SECOND) {
        ...
        // If I was the target of the failed conversion, then it's a warning,
        // otherwise it's an info
        if (currElement->destination.state() &&
            currElement->destination().compare(stationName_()) == 0)
          doWarning = true;
        else
      ...
      if (doInfo)
        infoMaterials.insert(currElement->elementName());
    }
  }

  for (auto &warningMaterial : warningMaterials) {
    logWARNING("Element \"" + warningMaterial + "\" ignored by station \"" +
               stationName_() + "\".");
  }
  for (auto &infoMaterial : infoMaterials) {
    logINFO("Element \"" + infoMaterial + "\" ignored by station \"" +
            stationName_() + "\".");
  }

  // MATERIALS INDEPENDENT FROM THE CONVERSIONS
  for (const std::unique_ptr<NonConvertedMaterials> &it :
       nonConvertedMaterials_) {
    for (const std::unique_ptr<MaterialObject::Element>
             &nonConvertedOutputElementUPtr : it->elements()) {

      // We now have our non-converted output Element :)
      // Amount of its material is INDEPENDENT from the input.
      // We add it to the relevant volumes (these volumes depend whether the
      // output Element is a service or local).
      MaterialObject::Element *outputElement =
          nonConvertedOutputElementUPtr.get();
...
void ConversionStation::buildConversions() {
  ...
  for (const auto &node : nonConvertedMaterialsNode_) {
    std::unique_ptr<NonConvertedMaterials> newNonConverted(
        new NonConvertedMaterials(subdetectorName_));
...
/*
 * Add the output of the conversion station (whether in converted or
 * non-converted category, local or service) to the relevant volumes. The
 * 'relevant volumes" are: local: MaterialObject& localOutput (assigned to the
 * station volume). service: MaterialObject& serviceOutput (routed from the
 * station volume).
 */
void ConversionStation::addOutputElements(
    MaterialObject::Element *outputElement, MaterialObject &localOutput,
    MaterialObject &serviceOutput) {
  ...
  if (outputElement->service()) {
    if (outputElement->unit().compare("g/m") == 0) {
      ...
      logERROR(any2str("Services (") + any2str(outputElement->elementName()) +
               any2str(") in ") + any2str(outputElement->unit()) +
...
void ConversionStation::Conversion::build() {
  ...
  if (inputNode_.size() > 0) {
    ...
    const std::vector<std::unique_ptr<MaterialObject::Element>>
        &myInputElements = input_->elements();
    if (myInputElements.size() != 1) {
      logERROR("Conversion with " + any2str(myInputElements.size()) +
               " elements in input, only 1 element is supported.");
    } else {
...
void ConversionStation::Inoutput::build() {
  ...
  for (auto &currentElementNode : elementsNode_) {
    std::unique_ptr<MaterialObject::Element> newElement(
        new MaterialObject::Element(elementMaterialType_, subdetectorName_));
...
ConversionStation::NonConvertedMaterials::NonConvertedMaterials(
