...
bool MessageLogger::addMessage(string sourceFunction, string message,
  ...
  if (level <= screenLevel_) {
    std::cout << "(" + shortLevelCode[level] + ") " << sourceFunction << ": "
              << message << std::endl;
  }

  if (level == DEBUG) { // TODO: this could be more efficient
    for (int i = sourceFunction.length(); i < 20; ++i)
      message = " " + message;
...
bool MessageLogger::addMessage(string sourceFunction, ostringstream &message,
                               int level /*=UNKNOWN*/, bool unique /*=false*/) {
...
string MessageLogger::getLatestLog(int level) {
  ...
  if ((level >= 0) && (level < NumberOfLevels)) {
    ...
    for (itMessage = logMessageV.begin(); itMessage != logMessageV.end();) {
...
string MessageLogger::getLatestLog() {
  ...
  while (itMessage != logMessageV.end()) {
    result += "(" + shortLevelCode[itMessage->level] + ") " +
              itMessage->message + "\n";
    messageCounter[itMessage->level]--;
    logMessageV.erase(itMessage);
