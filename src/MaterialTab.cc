...
namespace material {
  ...
  const double ChemicalElement::computeInteractionLength(const double atomicWeight) {
    ...
    //const double interactionLength = 35. * pow(atomicWeight, 1./3.);     // GEANT4 formula
    ...
    if (mattabStream.good()) {
      while (!mattabStream.eof()) {
        ...
        if (material[0] != '#') {
          ...
          insert(std::make_pair(material, std::make_tuple(density, radiationLength, interactionLength)));
        }
