...
ChemicalBase::ChemicalBase(const int materialsTreeHierarchyLevel,
                           const double density)
    : materialsTreeHierarchyLevel_(materialsTreeHierarchyLevel),
      density_(density) {}

ChemicalElement::ChemicalElement(const double density, const int atomicNumber,
                                 const double atomicWeight)
    : ChemicalBase(0, density),
      // A ChemicalElement has materialsTreeHierarchyLevel_ = 0.
      atomicNumber_(atomicNumber), atomicWeight_(atomicWeight) {
 ...
 */
const double
ChemicalElement::computeRadiationLength(const int atomicNumber,
                                        const double atomicWeight) {
  ...
  const double f_Z = a_2 * (1 / (1 + a_2) + 0.20206 - 0.0369 * a_3 +
                            0.0083 * a_4 - 0.002 * a_6);
  ...
  const double radiationLength =
      716.408 * atomicWeight /
      (pow(atomicNumber, 2.) * (L_Z - f_Z) + atomicNumber * L_prime_Z);
  return radiationLength;
}

const double
ChemicalElement::computeInteractionLength(const double atomicWeight) {
...
ChemicalMixture::ChemicalMixture(const double density,
  ...
  // ChemicalBaseMap alreadyDefinedMaterials(allChemicalElements.begin(),
  // allChemicalElements.end());
  ChemicalBaseMap alreadyDefinedMaterials;
  alreadyDefinedMaterials.insert(allChemicalElements.begin(),
                                 allChemicalElements.end());

  const std::pair<double, double> &radiationAndInteractionLengths =
      computeRadiationAndInteractionLengths(fractions_,
                                            alreadyDefinedMaterials);
  radiationLength_ = radiationAndInteractionLengths.first;
  interactionLength_ = radiationAndInteractionLengths.second;
}

ChemicalMixture::ChemicalMixture(const double density,
                                 const MassComposition &fractions,
                                 const ChemicalBaseMap &alreadyDefinedMaterials)
    : ChemicalBase(computeMaterialsTreeHierarchyLevel(fractions,
                                                      alreadyDefinedMaterials),
                   density) {
...
const MassComposition ChemicalMixture::computeMassComposition(
    const ChemicalFormula &formula,
    const ChemicalElementMap &allChemicalElements) const {
  ...
  for (const auto &elementIt : formula) {
    ...
    if (found == allChemicalElements.end()) {
      std::cout << "Tried to create molecule made of unknow atom(s) name."
                << std::endl;
    } else {
...
const MassComposition ChemicalMixture::computeMassComposition(
    const MassComposition &fractionsFromCfg) const {
...
void ChemicalMixture::normalizeMassComposition(
    MassComposition &fractions) const {
...
void ChemicalMixture::checkMassCompositionSum(
    const MassComposition &fractions) const {
  double fractionSum = 0.;
  for (const auto &fractionIt : fractions) {
    const double chemicalBaseMassFraction = fractionIt.second;
    fractionSum += chemicalBaseMassFraction;
  ...
  if (fabs(fractionSum - 1.) > insur::mat_negligible) {
    std::cout << "Error defining Chemical mixture: sum of mass fractions is "
                 "not equal to 1."
              << std::endl;
...
const int ChemicalMixture::computeMaterialsTreeHierarchyLevel(
    const MassComposition &fractions,
    const ChemicalBaseMap &alreadyDefinedMaterials) const {
  ...
  for (const auto &fractionIt : fractions) {
    ...
    if (found == alreadyDefinedMaterials.end()) {
      std::cout << "Tried to create mixture made of unknow chemical element / "
                   "chemical compound / mixture:"
                << chemicalBaseName << std::endl;
    } else {
      const ChemicalBase &alreadyDefinedMaterial = found->second;
      materialsTreeHierarchyLevel =
          std::max(materialsTreeHierarchyLevel,
...
ChemicalMixture::computeRadiationAndInteractionLengths(
    const MassComposition &fractions,
    const ChemicalBaseMap &alreadyDefinedMaterials) const {
  ...
  for (const auto &fractionIt : fractions) {
    ...
    if (found == alreadyDefinedMaterials.end()) {
      ...
      if (fabs(radiationLength) < insur::mat_negligible)
        std::cout << "Found a null radiation length for " << chemicalBaseName
                  << std::endl;
      else {
      ...
      if (fabs(interactionLength) < insur::mat_negligible)
        std::cout << "Found a null interaction length for " << chemicalBaseName
                  << std::endl;
      else {
        invertedInteractionLength +=
            chemicalBaseMassFraction / interactionLength;
      }
  ...
  if (fabs(invertedRadiationLength) < insur::mat_negligible)
    std::cout << "Mixture with infinite radiation length!!" << std::endl;
  else {
    radiationLength = 1. / invertedRadiationLength;
  }

  double interactionLength = 0.;
  if (fabs(invertedInteractionLength) < insur::mat_negligible)
    std::cout << "Mixture with infinite interaction length!!" << std::endl;
  else {
    interactionLength = 1. / invertedInteractionLength;
  }
...
MaterialsTable::MaterialsTable() {
  // std::string mattabFile(mainConfigHandler::instance().getMattabDirectory() +
  // "/" + insur::default_mattabfile);

  // CHEMICAL ELEMENTS
  std::ifstream chemicalElementsFile(
      mainConfigHandler::instance().getMattabDirectory() + "/" +
      insur::default_chemicalElementsFile);
    ...
    while (std::getline(chemicalElementsFile, lineString)) {
      ...
      if (elementName != "" && elementName[0] != '#') {
        ...
        ChemicalElement element =
            ChemicalElement(elementDensity, atomicNumber, atomicWeight);
  ...
  for (const auto& elemIt : allChemicalElements) {
    ...
    const double radRatio = (elem.getRadiationLength() - oldRad) / oldRad *
  100.; const double intRatio = (elem.getInteractionLength() - oldInt) / oldInt
  * 100.; std::cout << elemIt.first << " radRatio = " << radRatio << "intRatio =
  " << intRatio << std::endl;


    std::cout << "MaterialsTable::MaterialsTable finsihed computing all pure
  elem. load Elementary mat = " << elemIt.first; const ChemicalElement& elem =
  elemIt.second; std::cout << " elem.getDensity() = " << elem.getDensity()
  ...
  // CHEMICAL COMPOUNDS
  std::ifstream chemicalCompoundsFile(
      mainConfigHandler::instance().getMattabDirectory() + "/" +
      insur::default_chemicalCompoundsFile);
    ...
    while (std::getline(chemicalCompoundsFile, lineString)) {
      ...
      if (compoundName != "" && compoundName[0] != '#') {
        ...
        while (myLine >> element) {
          ...
          const auto delimiterPosition =
              element.find(insur::default_composition_delimiter);
          if (delimiterPosition != std::string::npos) {
            const std::string elementName =
                element.substr(0, delimiterPosition);
            const std::string elementNumberString =
                element.substr(delimiterPosition +
                               insur::default_composition_delimiter.length());
            const int elementNumber = atoi(elementNumberString.c_str());
            // std::cout << "elementName = " << elementName << " elementNumber =
            // " << elementNumber;
            compoundFormula.push_back(
                std::make_pair(elementName, elementNumber));
          } else {
            std::cout << "Chemical compound " << compoundName
                      << ": could not find the : delimiter." << std::endl;
          }
        ...
        ChemicalMixture coumpound = ChemicalMixture(
            compoundDensity, compoundFormula, allChemicalElements);
  ...
  for (const auto& mixIt : allChemicalMixtures) {
    ...
    const double intRatio = (mix.getInteractionLength() - oldInt) / oldInt *
  100.; std::cout << mixIt.first << " radRatio = " << radRatio << "intRatio = "
  << intRatio << std::endl;


      std::cout << "MaterialsTable::MaterialsTable finsihed computing all
  chemical composites. load Composite = " << mixIt.first; const ChemicalMixture&
  mix = mixIt.second; std::cout << " mix.getDensity() = " << mix.getDensity()
  ...
  // CHEMICAL MIXTURES
  std::ifstream chemicalMixturesFile(
      mainConfigHandler::instance().getMattabDirectory() + "/" +
      insur::default_chemicalMixturesFile);

  ChemicalBaseMap alreadyDefinedMaterials;
  alreadyDefinedMaterials.insert(allChemicalElements.begin(),
                                 allChemicalElements.end());
  alreadyDefinedMaterials.insert(allChemicalMixtures.begin(),
                                 allChemicalMixtures.end());
  ...
  if (chemicalMixturesFile.good()) {
    ...
    while (std::getline(chemicalMixturesFile, lineString)) {
      ...
      if (mixtureName != "" && mixtureName[0] != '#') {
        ...
        while (myLine >> constituant) { // TO DO: !!!!! should check for (myLine
                                        // >> constituant) error

          const auto delimiterPosition =
              constituant.find(insur::default_composition_delimiter);
          if (delimiterPosition != std::string::npos) {
            const std::string constituantName =
                constituant.substr(0, delimiterPosition);
            const std::string constituantMassFractionString =
                constituant.substr(
                    delimiterPosition +
                    insur::default_composition_delimiter.length());
            const double constituantMassFraction =
                std::stod(constituantMassFractionString);
            // std::cout << "constituantName = " << constituantName << "
            // constituantMassFraction = " << constituantMassFraction;
            mixtureComposition.push_back(
                std::make_pair(constituantName, constituantMassFraction));
          } else {
            std::cout << "Chemical mixture " << mixtureName
                      << ": could not find the : delimiter. Please set "
                         "name:value, with no space between name and value."
                      << std::endl;
          }
        ...
        ChemicalMixture mixture = ChemicalMixture(
            mixtureDensity, mixtureComposition, alreadyDefinedMaterials);
  ...
  /*
    ...
    const double radRatio = (mix.getRadiationLength() - oldRad) / oldRad * 100.;
    const double intRatio = (mix.getInteractionLength() - oldInt) / oldInt *
    100.; const double densityRatio = (mix.getDensity() - oldDensity) /
    oldDensity * 100.; std::cout << mixIt.first << " radRatio = " << radRatio <<
    " intRatio = " << intRatio << " densityRatio = " << densityRatio <<
    std::endl;
    }
  */
  ...
  /*
    std::cout << "MaterialsTable::MaterialsTable finsihed computing all
    mixtures. load mixture = " << mixIt.first; const ChemicalMixture& mix =
    mixIt.second; std::cout << " mix.getDensity() = " << mix.getDensity()
    << " mix.getRadiationLength() = " << mix.getRadiationLength()
    << " mix.getInteractionLength() = " << mix.getInteractionLength()
    << " mix.hasChemicalFormula() = " << mix.hasChemicalFormula()
    << " mix.isChemicalElement() = " << mix.isChemicalElement();
...
int MaterialsTable::getMaxMaterialsTreeHierarchyLevel() const {
  ...
  for (const auto &mixIt : allChemicalMixtures) {
    maxMaterialsTreeHierarchyLevel =
        std::max(maxMaterialsTreeHierarchyLevel,
...
double MaterialsTable::getDensity(const std::string materialName) const {
  ...
  if (foundElem != allChemicalElements.end()) {
    density = foundElem->second.getDensity();
  } else {
    const auto foundMix = allChemicalMixtures.find(materialName);
    if (foundMix != allChemicalMixtures.end()) {
      density = foundMix->second.getDensity();
    } else {
      std::cout << "MaterialsTable::density: material " << materialName
                << " could not be found in MaterialsTable." << std::endl;
    }
  }
  return density;
}
...
double
MaterialsTable::getRadiationLength(const std::string materialName) const {
  const ChemicalElementMap &allChemicalElements = this->first;
  const ChemicalMixtureMap &allChemicalMixtures = this->second;
  ...
  const auto foundElem = allChemicalElements.find(materialName);
  if (foundElem != allChemicalElements.end()) {
    radiationLength = foundElem->second.getRadiationLength();
  } else {
    const auto foundMix = allChemicalMixtures.find(materialName);
    if (foundMix != allChemicalMixtures.end()) {
      radiationLength = foundMix->second.getRadiationLength();
    } else {
      std::cout << "MaterialsTable::radiationLength: material " << materialName
                << " could not be found in MaterialsTable." << std::endl;
    }
  }
  return radiationLength;
...
double
MaterialsTable::getInteractionLength(const std::string materialName) const {
  const ChemicalElementMap &allChemicalElements = this->first;
  const ChemicalMixtureMap &allChemicalMixtures = this->second;
  ...
  const auto foundElem = allChemicalElements.find(materialName);
  if (foundElem != allChemicalElements.end()) {
    interactionLength = foundElem->second.getInteractionLength();
  } else {
    const auto foundMix = allChemicalMixtures.find(materialName);
    if (foundMix != allChemicalMixtures.end()) {
      interactionLength = foundMix->second.getInteractionLength();
    } else {
      std::cout << "MaterialsTable::interactionLength: material "
                << materialName << " could not be found in MaterialsTable."
                << std::endl;
    }
  }
  return interactionLength;
...
const std::string MaterialTab::msg_no_mat_file =
    "Material tab file does not exist.";
const std::string MaterialTab::msg_no_mat_file_entry1 = "Material '";
const std::string MaterialTab::msg_no_mat_file_entry2 =
    "' not found in Material tab file.";

MaterialTab::MaterialTab() {
  std::string mattabFile(mainConfigHandler::instance().getMattabDirectory() +
                         "/" + insur::default_mattabfile);
  std::ifstream mattabStream(
      mainConfigHandler::instance().getMattabDirectory() + "/" +
      insur::default_mattabfile);
  std::string line;
  std::string material;
  ...
  if (mattabStream.good()) {
    while (!mattabStream.eof()) {
      ...
      if (material[0] != '#') {
        ...
        insert(make_pair(
            material, make_tuple(density, radiationLength, interactionLength)));
      }
