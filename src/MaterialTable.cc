 ...
 * @brief This is the implementation of the internal data structure for material
 * properties.
 */
...
double CompositeMaterial::getRadiationLength() {
  if (rlength_ <= 0) {
    for (std::vector<MaterialFraction>::const_iterator it =
             materialFractions_.begin();
         it != materialFractions_.end(); ++it) {
...
/**
 * Adds a material already bundled into a <i>MaterialRow</i> struct to the
 * container class.
 * @param mat The struct that will be copied and appended to the internal vector
 * container
 */
void MaterialTable::addMaterial(MaterialRow mat) { materials.push_back(mat); }

/**
 * Add a material to the container class by bundling the individual parts into a
 * struct.
 ...
 */
void MaterialTable::addMaterial(std::string tag, double density, double rlength,
                                double ilength) {
  MaterialRow row;
  row.tag = tag;
  row.density = density;
...
/**
 * Find a material by its name. If no material is found, the function throws an
 * exception.
 * @param tag The name of the requested material
 * @return A reference to the requested <i>MaterialRow</i> struct
 */
MaterialRow &
MaterialTable::getMaterial(std::string tag) { // throws std::runtime_error
  int index = findIndex(tag);
  if (index < 0)
    throw std::runtime_error("MaterialTable::getMaterialByTag(): " +
                             errEntryNotFound);
  return getMaterial(index);
}

/**
 * Find a material by its index within the collection. If the index is out of
 * range, the function throws an exception.
 * @param index The material index
 * @return A reference to the requested <i>MaterialRow</i> struct
 */
MaterialRow &
MaterialTable::getMaterial(int index) { // throws std::runtime_error
  if (index >= (int)materials.size())
    throw std::runtime_error("MaterialTable::getMaterialByIndex(): " +
                             errIndexOutOfRange);
 ...
 * @return True if the operation was successful, false if the material that was
 ...
 * @param newmat The <i>MaterialRow</i> struct containing the replacement
 * @return True if the operation was successful, false if the index was out of
 * range
 */
...
/**
 * This is a bookkeeping function that wraps around a basic function of
 * <i>std::vector</i>.
 * @return The number of entries in the table.
 */
unsigned int MaterialTable::rowCount() { return materials.size(); }
 ...
 */
bool MaterialTable::empty() { return materials.empty(); }

/**
 * Print the contents of the material table to <i>cout</i>.
 */
void MaterialTable::print() {
  std::cout << "The material table contains " << materials.size()
            << " elements:" << std::endl;
  for (unsigned int i = 0; i < materials.size(); i++) {
    std::cout << "Material: " << materials.at(i).tag
              << ", density: " << materials.at(i).density
              << ", radiation length: ";
    std::cout << materials.at(i).rlength
              << ", interaction length: " << materials.at(i).ilength
              << std::endl;
  }
  std::cout << std::endl;
}
 ...
 * @param tag The name of the material
 * @return The index of the requested material; -1 if no such material exists in
 * the table
 */
int MaterialTable::findIndex(std::string tag) {
  ...
  while (index < (int)materials.size() && !found) {
    if (tag.compare(materials.at(index).tag) == 0)
      found = true;
    else
      index++;
  }
