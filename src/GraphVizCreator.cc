...
int GraphVizCreator::getFileId(std::string istreamid) {
  if (graphVizFilesToNodes_[istreamid] == 0)
    graphVizFilesToNodes_[istreamid] = graphVizFilesToNodes_.size();
...
void GraphVizCreator::addNodeRename(std::string nodeName,
                                    std::string symbolic) {
...
std::string GraphVizCreator::createGraphVizFile() {
  ...
  for (const auto &it : graphVizFilesToNodes_) {
    ...
    std::string addBox = "";
    if (!nodeLocal_[it.first])
      addBox = " shape=box";
    if (nodeRenameMap_[it.first] == "") {
      result += indent_ + "" + getNodeName(it.second) + "[label=\"" + it.first +
                "\"" + URLString + " " + addBox + "];\n";
    } else {
      result += indent_ + "" + getNodeName(it.second) + "[label=\"" +
                nodeRenameMap_[it.first] + "\"" + addBox + URLString + "];\n";
    }
  }
  result += "\n";
  for (const auto &it : nodeLinks_) {
    ...
    for (const auto &childIt : linkedNodes) {
      graphVizConnections_ += indent_ + getNodeName(it.first) + ":e -> " +
                              getNodeName(childIt) + ":w;\n";
    }
  }
...
void GraphVizCreator::prepareNodeOutput(std::string absoluteFileName,
  ...
  if (isLocal == false)
    addNodeRename(absoluteFileName, "STD/" + relativeFileName); // @include-std
  else
    addNodeRename(absoluteFileName, relativeFileName); // @include

  // Prepare the correct link
  if (webOutput)
    addNodeUrl(absoluteFileName, destinationEncode(absoluteFileName));
  else
    addNodeUrl(absoluteFileName, "file://" + absoluteFileName);
}

std::string
GraphVizCreator::destinationEncode(const std::string &originalFileName) {
  boost::filesystem::path p(originalFileName);
  return std::to_string(getFileId(originalFileName)) + "-" +
         p.filename().string();
}

void GraphVizCreator::createEncodedFileList(
