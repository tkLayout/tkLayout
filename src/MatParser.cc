/**
 * @file MatParser.cc
 * @brief This parses the global material list and the tracker-specific
 * materials from files, initialising an instance of <i>MatCalc</i> in the
 * process
 */
 ...
 * @param materialfile The name and, if necessary, path of the global material
 * config file
...
bool MatParser::fillTable(std::string materialfile, MaterialTable &mattab) {
  ...
  if (materialfile.empty())
    materialfile = default_mattabfile;
    ...
    try {
      ...
      while (std::getline(infilestream, line)) {
        ...
        while (wordstream >> word) {
          // save everything that is not a comment word for word in a temporary
          // vector
          if ((word.compare(0, c_comment.size(), c_comment) == 0) ||
              (word.compare(0, shell_comment.size(), shell_comment) == 0))
            break;
          else
            tmp.push_back(word);
        }
        if (tmp.empty())
          ...
          // fill up necessary data with dummy values if there is too little
          // information or complain if there is too much
          if (tmp.size() < 4) {
            while (tmp.size() < 4)
              tmp.push_back(dummy_value);
          }
          if (tmp.size() > 4)
            std::cerr << warning_too_many_values << std::endl;
          // convert the information in the temporary vector to fill the fields
          // in the material row
          row.tag = tmp.at(0);
 ...
 * @param configfile The name and, if necessary, path of a material config file
 * for a tracker layout
 * @param calc A reference to an uninitialised material calculator
 * @param mattabdir The location of the global material table if it is not in
 * the default place
 * @return True if the config file was successfully parsed, false otherwise
 */
