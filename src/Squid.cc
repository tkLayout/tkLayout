  ...
  bool Squid::buildInactiveSurfaces(bool verbose) {
    ...
    if (getGeometryFile()!="") {
      if (std_ || pxd_) {
        ...
        if (std_) { 
          if (stdPasive_) delete stdPasive_;
          stdPasive_ = new InactiveSurfaces();
...
bool Squid::analyzeNeighbours(std::string graphout) {
  ...
  if (stdPasive_) {
    ...
    vizard_.writeNeighbourGraph(*stdPasive_, graphout);
    stopTaskClock();
    return true;
  }
  else {
...
bool Squid::reportResolutionSite() {
  ...
  if (std_ || pxd_) {
     ...
     vizard_.errorSummary(trackerAnalyzer_, webSite_, "trigger", true);
#else
...
bool Squid::reportTriggerPerformanceSite(bool extended) {
  ...
  bool Squid::reportNeighbourGraphSite() {
    if (vizard_.neighbourGraphSummary(*stdPasive_, webSite_)) return true;
    else {
