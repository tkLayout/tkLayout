 ...
 * @file Squid.cc
 * @brief This implements the main interface between the tkgeometry library
 * classes and the frontend
 */
 ...
 */
Squid::Squid()
    : mainConfiguration(mainConfigHandler::instance()), t2c(mainConfiguration),
      weightDistributionTracker(0.1), weightDistributionPixel(0.1) {
  tr = NULL;
  is = NULL;
  mb = NULL;
  px = NULL;
...
/**
 * Build a bare-bones geometry of active modules. The resulting tracker object
 * replaces the previously registered one, if such an object existed. It remains
 * in the squid until it is overwritten by a second call to this function or by
 * another one that creates a new tracker object. If the geometry file contains
 * a description of a pixel detector, that is created as well. It is treated
 * exactly like the tracker with respect to replacement and persistence.
 * @param geomfile The name and - if necessary - path of the geometry
 * configuration file
...
bool Squid::buildTracker() {
  ...
  if (ifs.fail()) {
    std::cerr << "ERROR: cannot open geometry file " << getGeometryFile()
              << std::endl;
    return false;
  }
  ...
  public:
    CoordExportVisitor(std::string trid) : barof(trid + "_barrel_coords.txt"),
  endof(trid + "_endcap_coords.txt") {} void visit(const BarrelModule& m)
  override { barof << m.center().Z() << ", " << m.center().Rho() << ", " <<
  m.center().Phi() << std::endl; } void visit(const EndcapModule& m) override {
  endof << m.center().Z() << ", " << m.center().Rho() << ", " <<
  m.center().Phi() << std::endl; }
  };
  ...
  public:
    ModuleDataVisitor(std::string ofname) : of(ofname + "_mods.txt") {
       of << "subdetectorName" << sep << "refZ" << sep << "refRho" << sep <<
  "refPhi" << sep
    ...
    void visit(const DetectorModule& m) override {
      ...
      of << m.subdetectorName() << sep << (int)m.posRef().z << sep <<
  (int)m.posRef().rho << sep << (int)m.posRef().phi << sep
         << m.center().Z() << sep << m.center().Rho() << sep << m.center().Phi()
  << sep
         << m.dsDistance() << sep << m.thickness() << sep
         << m.minWidth() << sep << m.maxWidth() << sep << m.length() << sep
         << m.moduleType() << sep
         << m.nominalResolutionLocalX() << sep << m.nominalResolutionLocalY() <<
  sep
         << m.numStripsAcrossEstimate() << sep <<
  m.innerSensor().numSegmentsEstimate() << sep <<
  m.outerSensor().numSegmentsEstimate() << std::endl;
    }
  };
  */
  ...
  try {
    ...
    std::for_each(childRange.first, childRange.second,
                  [&](const ptree::value_type &kv) {
    ...
    std::set<string> unmatchedProperties =
        PropertyObject::reportUnmatchedProperties();
    ...
    // Look for tag "Support" not associated with a concrete Tracker and build
    // supports
...
/**
 * Build an optical cabling map, which connects each module to a bundle, cable,
 * DTC. Can actually be reused for power cables routing. Please note that this
 * is independant from any cable Material Budget consideration, which is done
 * indepedently. The underlying cabling was designed for OT616, and is not
 * garanteed to work for any other layout (could/should require adaptations).
 */
bool Squid::buildOuterCablingMap(const bool outerCablingOption) {
  startTaskClock(
      "Building optical and power cabling map in the Outer Tracker.");
  if (tr) {
    try {
      // BUILD OUTER CABLING MAP.
      std::unique_ptr<const OuterCablingMap> map(new OuterCablingMap(tr));
      // std::unique_ptr<const OuterCablingMap> map = std::make_unique<const
      // OuterCablingMap>(tr);  // Switch to C++14 :)
      tr->setOuterCablingMap(std::move(map));
    } catch (PathfulException &e) {
      std::cerr << e.path() << " : " << e.what()
                << std::endl; // should improve this!
      stopTaskClock();
      return false;
    }
    stopTaskClock();
 ...
 * DTC. Can actually be reused for power cables routing. Please note that this
 * is independant from any cable Material Budget consideration, which is done
 * indepedently. The underlying cabling was designed for IT613, and will not
 * work for any other layout.
 */
bool Squid::buildInnerCablingMap(const bool innerCablingOption) {
  startTaskClock(
      "Building optical and power cabling map in the Inner Tracker.");
  if (px) {
    try {
      // BUILD INNER CABLING MAP.
      std::unique_ptr<const InnerCablingMap> map(new InnerCablingMap(px));
      // std::unique_ptr<const CablingMap> map = std::make_unique<const
      // CablingMap>(px);  // Switch to C++14 :)
...
/**
 * Build up the inactive surfaces around the previously created tracker
 * geometry. The resulting collection of inactive surfaces replaces the
 * previously registered one, if such an object existed. It remains in the squid
 * until it is overwritten by a second call to this function or by another one
 * that creates a new collection of inactive surfaces. If a pixel detector was
 * also created in a previous step, it gets a new set of inactive surfaces as
 * well.
 * @param verbose A flag that turns the final status summary of the inactive
 * surface placement algorithm on or off
 * @return True if there were no errors during processing, false otherwise
 */
...
/**
 * Calculate a material budget for the previously created tracker object and
 * collection of inactive surfaces. The resulting material budget replaces the
 * previously registered one, if such an object existed. It remains in the squid
 * until it is overwritten by a second call to this function or by another one
 * that creates a new material budget. This function succeeds if either the
 * tracker or both the tracker and the inactive surfaces exist. If a pixel
 * detector exists, it gets its own material budget at the same time. This
 * object is treated exactly the same as the one for the tracker.
 * @param verbose A flag that turns the final status summary of the material
 * budget on or off
 * @return True if there were no errors during processing, false otherwise
 */
bool Squid::createMaterialBudget(bool verbose) {
  if (tr) {
    ...
    // if (mp.initMatCalc(tkMaterialCalc,
    // mainConfiguration.getMattabDirectory())) {
    if (verbose)
      mb->print();

    if (px) {
      // if (mp.initMatCalc(pxMaterialCalc,
      // mainConfiguration.getMattabDirectory())) {
      if (!pi)
        pi = new InactiveSurfaces();
      if (pm)
...
/**
 * Build a full system consisting of tracker object, collection of inactive
 * surfaces and material budget from the given configuration files. All three
 * objects replace the previously registered ones, if they existed. They remain
 * in the squid  until they are overwritten by a second call to this function or
 * by another one that creates new instances of them.
 * @param geomfile The name and - if necessary - path of the geometry
 * configuration file
 * @param settingsfile The name and - if necessary - path of the module settings
 * configuration file
 * @param usher_verbose A flag that turns the final status summary of the
 * inactive surface placement algorithm on or off
 * @param mat_verbose A flag that turns the final status summary of the material
 * budget on or off
 * @return True if there were no errors during processing, false otherwise
 */
// bool Squid::buildFullSystem(bool usher_verbose, bool mat_verbose) {
//   if (buildInactiveSurfaces(usher_verbose)) return
//   createMaterialBudget(mat_verbose) && irradiateTracker(); return false;
// }

/**
 * Build the feeder/neighbour graph of the previously created collection of
 * inactive surfaces and save the results in a plain text file.
...
/**
 * Translate an existing full tracker and material budget to a series of XML
 * files that can be interpreted by CMSSW.
...
bool Squid::translateFullSystemToXML(std::string xmlout) {
  ...
  if (xmlOutputPath.at(xmlOutputPath.size() - 1) != '/')
    xmlOutputPath = xmlOutputPath + "/";

  std::string temporaryPath = xmlDirectoryPath + "/" + xml_tmppath + "/";
  if (bfs::exists(xmlOutputPath))
    bfs::rename(xmlOutputPath, temporaryPath);
  ...
  try {
    if (mb) {
      ...
      t2c.translate(
          tkMaterialCalc.getMaterialTable(), *mb, outerTrackerXmlTags,
          xmlDirectoryPath, xmlOutputPath, xmlOutputName,
          false); // false is setting a mysterious flag called wt which changes
                  // the way the XML is output. apparently setting it to true is
                  // of no use anymore.
      if (pm) {
        XmlTags pixelXmlTags = XmlTags(true);
        t2c.translate(pxMaterialCalc.getMaterialTable(), *pm, pixelXmlTags,
                      xmlDirectoryPath, xmlOutputPath, xmlOutputName, false);
      }
    } else {
      std::cout << "Squid::translateFullSystemToXML(): " << err_no_matbudget
                << std::endl;
      return false;
    }
    ...
    if (bfs::exists(xmlOutputPath))
      bfs::remove_all(xmlOutputPath);
    if (bfs::exists(temporaryPath))
      bfs::rename(temporaryPath, xmlOutputPath);
...
/**
 * Extract the filename from a path. If there is nothing to extract, a copy of
 * the input is returned.
 * @full The source string
 * @return A string containing everything after the last slash character, or the
 * entire input if there were no slashes
 */
...
bool Squid::prepareWebsite() {
  ...
  string trackerName;
  if (htmlDir_ != "")
    trackerName = htmlDir_;
  else {
    if (tr)
      trackerName = baseName_;
    else
      trackerName = default_trackername;
  }
...
/**
 * Analyze the previously created geometry and without no output  through
 * rootweb.
 * @return True if there were no errors during processing, false otherwise
 */
bool Squid::pureAnalyzeGeometry(int tracks) {
  if (tr) {
    ...
    std::cout << "Squid::pureAnalyzeGeometry(): " << err_no_tracker
              << std::endl;
    return false;
  }
}
 ...
 */
bool Squid::reportOuterCablingMapSite(const bool outerCablingOption,
                                      const std::string layoutName) {
  startTaskClock("Creating OT Cabling map report.");
  if (layoutName.find(default_cabledOTName) == std::string::npos)
    logERROR(
        "Cabling map is designed and implemented for OT616 only. Forcing it on "
        "another layout is at your own risks (could require adaptations).");
  if (tr) {
 ...
 */
bool Squid::reportInnerCablingMapSite(const bool innerCablingOption,
                                      const std::string layoutName) {
  startTaskClock("Creating IT Cabling map report.");
  if (layoutName.find(default_cabledITName) == std::string::npos)
    logERROR(
        "Cabling map is designed and implemented for IT613 only. Forcing it on "
        "another layout is at your own risks (could require adaptations).");
  if (px) {
    // CREATE REPORT ON WEBSITE.
...
bool Squid::analyzeTriggerEfficiency(int tracks, bool detailed) {
  // Call this before analyzetrigger if you want to have the map of suggested
  // spacings
  if (detailed) {
  ...
  a.analyzeTriggerEfficiency(*tr, mainConfiguration.getTriggerMomenta(),
 ...
 * @param tracks The number of tracks that should be fanned out across the
 * analysed region
 * @return True if there were no errors during processing, false otherwise
 */
bool Squid::pureAnalyzeMaterialBudget(int tracks, bool triggerResolution,
                                      bool triggerPatternReco,
                                      bool debugResolution) {
  if (mb) {
    //      startTaskClock(!trackingResolution ? "Analyzing material budget" :
    //      "Analyzing material budget and estimating resolution");
    // TODO: insert the creation of sample tracks here, to compute intersections
    // only once
    ...
    if (pm) {
      ...
      pixelAnalyzer.analyzeMaterialBudget(*pm, mainConfiguration.getMomenta(),
                                          tracks, NULL);
      stopTaskClock();
    }
    ...
    if (triggerResolution) {
      ...
      a.analyzeTaggedTracking(*mb, mainConfiguration.getMomenta(),
                              mainConfiguration.getTriggerMomenta(),
                              mainConfiguration.getThresholdProbabilities(),
                              false, debugResolution, tracks, pm);
      if (pm) {
        pixelAnalyzer.analyzeTaggedTracking(
            *pm, mainConfiguration.getMomenta(),
            mainConfiguration.getTriggerMomenta(),
            mainConfiguration.getThresholdProbabilities(), true,
            debugResolution, tracks, nullptr);
      }
      stopTaskClock();
    }
    if (triggerPatternReco) {
      startTaskClock("Estimating pattern recognition");
      bool analysisOK = a.analyzePatterReco(*mb, mainConfiguration, tracks, pm);
      stopTaskClock();
      if (!analysisOK)
        return false;
    }
...
bool Squid::reportGeometrySite(bool debugResolution) {
  if (tr) {
    ...
    if (px)
      v.geometrySummary(pixelAnalyzer, *px, pi, site, debugResolution, "pixel");
    stopTaskClock();
    return true;
  } else {
    logERROR(err_no_tracker);
...
bool Squid::reportMaterialBudgetSite() {
  if (mb) {
    ...
    if (pm)
      v.weigthSummary(pixelAnalyzer, *pm, weightDistributionPixel, site,
                      "pixel");
    stopTaskClock();
    return true;
  } else {
...
bool Squid::reportNeighbourGraphSite() {
  if (v.neighbourGraphSummary(*is, site))
    return true;
  else {
...
bool Squid::additionalInfoSite() {
  if (!tr) {
    ...
    v.additionalInfoSite(getSettingsFile(), a, pixelAnalyzer, *tr, px, site);
    stopTaskClock();
    return true;
  }
}

void Squid::setBasename(std::string newBasename) { baseName_ = newBasename; }
...
void Squid::setHtmlDir(std::string htmlDir) { htmlDir_ = htmlDir; }
...
void Squid::simulateTracks(const po::variables_map &varmap,
                           int seed) { // CUIDADO not ported to coderev -- yet?
  ...
  /*    TrackShooter ts;
      ...
      ts.setTrackerBoundaries(tr->getMaxR(), tr->getMaxBarrelZ(-1),
     tr->getMaxBarrelZ(+1)); for (std::vector<Layer*>::const_iterator lit =
     tr->getLayers().begin(); lit != tr->getLayers().end(); ++lit) {
        ModuleVector* mods = (*lit)->getModuleVector();
        for (ModuleVector::const_iterator mit = mods->begin(); mit !=
     mods->end(); ++mit) { ts.addModule(*mit);
        }
      }
...
void Squid::createAdditionalXmlSite(std::string xmlout) {
  std::string xmlPath = mainConfiguration.getXmlDirectory() + "/" +
                        (xmlout.empty() ? baseName_ : xmlout) + "/";
  std::string layoutPath =
      mainConfiguration.getLayoutDirectory() + "/" + baseName_ + "/";
