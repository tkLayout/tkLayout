 ...
 */
void OuterCablingMap::connectBundlesToCables(
    std::map<const int, std::unique_ptr<OuterBundle>> &bundles,
    std::map<const int, std::unique_ptr<OuterCable>> &cables,
    std::map<const std::string, std::unique_ptr<const OuterDTC>> &DTCs) {
  ...
  for (auto &b : bundles) {
    ...
    std::map<int, std::pair<int, int>> cablesPhiSectorRefAndSlot =
        computeCablesPhiSectorRefAndSlot(bundles);
    ...
    const int cableId = computeCableId(cablePhiSectorRef, cableTypeIndex, slot,
                                       isPositiveCablingSide);

    // BUILD CABLES and DTCS AND STORE THEM
    createAndStoreCablesAndDTCs(myBundle, cables, DTCs, cableId, phiSectorWidth,
                                cablePhiSectorRef, cableType, slot,
                                isPositiveCablingSide);
  }
 ...
 */
const Category
OuterCablingMap::computeCableType(const Category &bundleType) const {
  Category cableType = bundleType;
  if (bundleType == Category::PS10GA || bundleType == Category::PS10GB)
    cableType = Category::PS10G;
  return cableType;
}
 ...
 * 1 slot = 1 DTC.
 * A staggering of bundles is done on the fly, when the number of bundles per
 * cable is too high.
 */
const std::map<int, std::pair<int, int>>
OuterCablingMap::computeCablesPhiSectorRefAndSlot(
    const std::map<const int, std::unique_ptr<OuterBundle>> &bundles) const {
  std::map<int, std::pair<int, int>> cablesPhiSectorRefAndSlot;
  ...
  for (const auto &b : bundles) {
    ...
    const int nextPhiSectorRef =
        computeNextPhiSliceRef(phiSectorRef, numPhiSectors);
    const int previousPhiSectorRef =
        computePreviousPhiSliceRef(phiSectorRef, numPhiSectors);
    ...
    if (cableType == Category::PS10G) {
      ...
      if ((subDetectorName == outer_cabling_tbps && layerDiskNumber == 1 &&
           !myBundle->isTiltedPart()) ||
      ...
      else if ((subDetectorName == outer_cabling_tbps && layerDiskNumber == 1 &&
                myBundle->isTiltedPart()) ||
               (subDetectorName == outer_cabling_tedd1 &&
                layerDiskNumber == 2 && bundleType == Category::PS10GB) ||
      ...
      // BARREL FULL LAYER 2 + ENDCAPS DISK 2A
                ...
                layerDiskNumber == 2) ||
               (subDetectorName == outer_cabling_tedd1 &&
                layerDiskNumber == 2 && bundleType == Category::PS10GA)) {
        slot = 3;
      }
    }
      ...
      // STAGGERING
        ...
        if (subDetectorName == outer_cabling_tbps) {
          ...
          if (myBundle->isTiltedPart()) {
            int &myPhiSectorCounter =
                Layer3TiltedPhiSectorsCounter[phiSectorRef];
            myPhiSectorCounter += 1;
            // In case already 4 bundles from tilted part, assign to next phi
            // Sector
          ...
          // FLAT PART : assign TBPS bundles with TEDD bundles
            ...
            myPhiSectorCounter += 1;
            // In case already 4 bundles from flat part, assign to next phi
            // Sector
            if (myPhiSectorCounter > 4) {
              myPhiSectorCounter -= 1;
        ...
        // TEDD_2
          ...
          if (subDetectorName == outer_cabling_tedd1 && layerDiskNumber == 2)
            slot = 5;
    ...
    // 2S
      ...
      if (subDetectorName == outer_cabling_tb2s && layerDiskNumber == 4) {
        ...
        myPhiSectorCounter += 1;
        // In a few cases, need to reduce to 5 bundles (additional bundle from
        // Layer 5 will be added). As a result, the first bundle in the Phi
        // Sector is assigned to the previous phiSector.
      ...
      // STAGGERING TB2S LAYER 5
        ...
        // if (phiSectorRefThird == 0) : should have 5 bundles in Layer 4 (+ 1
        // added from Layer 5) if (phiSectorRefThird == 1) : should have 5
        // bundles in Layer 4 (+ 1 added from Layer 5) if (phiSectorRefThird ==
        // 2) : should have 6 bundles in Layer 4 (no bundle added from Layer 5)
        ...
        if (subDetectorName == outer_cabling_tb2s) {
          ...
          myPhiSectorCounter += 1;
          if (myPhiSectorCounter == 1 || myPhiSectorCounter == 5 ||
              myPhiSectorCounter == 8)
            slot = 4;
          else
            slot = 3;
        } else
      ...
      else if ((subDetectorName == outer_cabling_tedd1 &&
                layerDiskNumber == 1) ||
               (subDetectorName == outer_cabling_tedd2 &&
                layerDiskNumber == 4)) {
        slot = 5;
    ...
    if (slot == 0) {
      logERROR(any2str("Connection from ribbon to cable : ribbon category is "
                       "unknown. Slot was not defined properly.") +
    ...
    const std::pair<int, int> phiSectorRefAndSlot =
        std::make_pair(cablePhiSectorRef, slot);
    const int bundleId = b.first;
    cablesPhiSectorRefAndSlot.insert(
        std::make_pair(bundleId, phiSectorRefAndSlot));
  }
 ...
 */
void OuterCablingMap::createAndStoreCablesAndDTCs(
    OuterBundle *myBundle,
    std::map<const int, std::unique_ptr<OuterCable>> &cables,
    std::map<const std::string, std::unique_ptr<const OuterDTC>> &DTCs,
    const int cableId, const double phiSectorWidth, const int cablePhiSectorRef,
    const Category &cableType, const int slot,
    const bool isPositiveCablingSide) {

  auto found = cables.find(cableId);
  if (found == cables.end()) {
    std::unique_ptr<OuterCable> cable(
        new OuterCable(cableId, phiSectorWidth, cablePhiSectorRef, cableType,
 ...
 */
const int
OuterCablingMap::computeCableTypeIndex(const Category &cableType) const {
  int cableTypeIndex;
  if (cableType == Category::PS10G)
    cableTypeIndex = 0;
  else if (cableType == Category::PS5G)
 ...
 */
const int
OuterCablingMap::computeCableId(const int cablePhiSectorRef,
                                const int cableTypeIndex, const int slot,
                                const bool isPositiveCablingSide) const {
  int cablingSideIndex = (isPositiveCablingSide ? 0 : 1);

  const int cableId = cablingSideIndex * 1000 + cablePhiSectorRef * 100 +
                      cableTypeIndex * 10 + slot;
  return cableId;
}

/* Connect bundle to cable and vice-versa.
 */
void OuterCablingMap::connectOneBundleToOneCable(OuterBundle *bundle,
                                                 OuterCable *cable) const {
 ...
 */
void OuterCablingMap::checkBundlesToCablesCabling(
  ...
  for (const auto &c : cables) {
    ...
    if (type == Category::UNDEFINED || phiSectorRef <= -1 ||
        phiSectorRef >= outer_cabling_numNonants || (slot <= 0) ||
        (slot >= (outer_cabling_maxNumDTCsPerNonantPerZEnd / 2 + 1))) {
      logERROR(
          any2str(
              "Building cabling map : a cable was not correctly created. ") +
          "OuterCable " + any2str(c.first) + ", with cableType = " +
          any2str(type) + ", has phiSectorRef = " + any2str(phiSectorRef) +
          ", slot = " + any2str(slot));
    }
    ...
    if (numBundles > outer_cabling_maxNumBundlesPerCable) {
      logERROR(any2str("Building cabling map : Staggering bundles. ") +
               "OuterCable " + any2str(c.first) + " is connected to " +
               any2str(numBundles) + " bundles." +
               "Maximum number of bundles per cable allowed is " +
               any2str(outer_cabling_maxNumBundlesPerCable));
    }
  }
}

/* BUNDLES TO POWER SERVICE CHANNELS CONNECTIONS.
 * VERY IMPORTANT: connection scheme from modules to optical bundles =
 * connection scheme from modules to power cables. As a result, 1 single Bundle
 * object is used for both schemes. Regarding the connections to services
 * channels, each Bundle is then assigned:
 * - 1 Optical Services Channel Section (considering the Bundle as an optical
 * Bundle);
 * - 1 Power Services Channels section (making as if the Bundle is a power
 * cable);
 *
 * The optical channel mapping is done so that all bundles connected to the same
 * DTC, are routed through the same channel.
 *
 * The assignments of power cables to services channels must be done after the
 * bundles to DTCs connections are established. Indeed, the power channel
 * mapping is done so that all modules connected to the same DTC, have their
 * power cables routed through 2 consecutive channels sections at most.
 */
void OuterCablingMap::computePowerServicesChannels() {
  for (bool isPositiveCablingSide : {true, false}) {

    // BARREL ONLY: IN VIEW OF THE POWER CHANNEL ASSIGNMENT, SPLIT EACH NONANT
    // INTO 2 SEMI-NONANTS
    routeBarrelBundlesPoweringToSemiNonants(isPositiveCablingSide);

    // ASSIGN POWER SERVICES CHANNELS
    std::map<const int, std::unique_ptr<OuterCable>> &cables =
        (isPositiveCablingSide ? cables_ : negCables_);
    for (auto &c : cables) {
    ...
    const std::map<const int, std::unique_ptr<OuterBundle>> &bundles =
        (isPositiveCablingSide ? bundles_ : negBundles_);
    checkBundlesToPowerServicesChannels(bundles);
  }
}

/* Barrel only: in view of the power channel assignment, split each nonant into
   2 semi-nonants. The cooling pipes design should be invariant by rotation of
   180° around CMS_Y, to avoid different cooling designs on both (Z) side. The
   cooling pipes and power cables are assigned to similar channels slots (A or
   C). As a result, the channel assignmennt of power cables need to follow the
   same symmetry as the cooling pipes. Hence, the CHANNEL ASSIGNMENNT OF POWER
   CABLES NEED TO BE INVARIANT BY ROTATION OF 180DEG AROUND CMS_Y. This is a
   priori not trivial, since the power cables scheme follow the bundles scheme,
   hence the optical map mirror symmetry. This is only possible if, for each phi
   nonant, one define a semi-nonant Phi boundary in a certain way. This is what
   is done here: the semi-nonant Phi boundaries are defined so that ALL NONANTS
   AND SEMI-NONANTS PHI BOUNDARIES ARE INVARIANT BY ROTATION OF 180° AROUND
   CMS_Y.
 */
void OuterCablingMap::routeBarrelBundlesPoweringToSemiNonants(
    const bool isPositiveCablingSide) {
  std::map<const int, std::unique_ptr<OuterBundle>> &bundles =
      (isPositiveCablingSide ? bundles_ : negBundles_);
  const std::map<const int, std::unique_ptr<OuterBundle>> &stereoBundles =
      (isPositiveCablingSide ? negBundles_ : bundles_);
  ...
  // 'stereo' means on the other cabling side, by a rotation of 180° around
  // CMS_Y.
  ...
  for (auto &b : bundles) {
    ...
    if (isBarrel && !isBarrelPSFlatPart) {
      ...
      // in the trigonometric sense in the (XY) plane in CMS global frame of
      // reference.
      ...
      // Get the bundle located on the other cabling side, by a rotation of 180°
      // around CMS_Y.
      ...
      if (found != stereoBundles.end()) {
        ...
        const int stereoPhiSectorRef =
            myStereoBundle->getCable()->phiSectorRef();

        // Decisive point!!
        // As soon as a change in the identifier of the stereoBundle Phi nonant
        // is detected, one assigns the bundle to the upper semi-nonant.
        if (stereoPhiSectorRefMarker != -1 &&
    ...
    // NB: All this is possible because the bundles and stereoBundles are sorted
    // by their Ids. One relies on 2 characteristics of the BundleId scheme:
    // - all Bundles connected to the same DTC will have consecutive Ids.
    // - the Id increment is in Phi, in the (XY) plane in CMS global frame of
    // reference.
  }

  // FLAT PART OF TBPS
  // For a given bundle, connected to untilted modules:
  // Take the same semi-nonant assignment as the bundle located at the same Phi
  // and connected to the tilted modules.
  ...
  for (auto &b : bundles) {
    ...
    if (isBarrelPSFlatPart) {
      ...
      // Get the bundle located at the same Phi, but connected to the tilted
      // modules.
      ...
      if (found != bundles.end()) {
        ...
        const bool isLower =
            myTiltedBundle->isPowerRoutedToBarrelLowerSemiNonant();

        // Lastly, assign the semi-nonant attribution decision to the bundle.
        myBundle->setIsPowerRoutedToBarrelLowerSemiNonant(isLower);
      } else {
        logERROR(
            any2str("Could not find bundle connected to tilted modules, id ") +
 ...
 */
void OuterCablingMap::checkBundlesToPowerServicesChannels(
    const std::map<const int, std::unique_ptr<OuterBundle>> &bundles) {
  ...
  for (const auto &b : bundles) {
    ...
    if (fabs(myChannelNumber) == 0 ||
        fabs(myChannelNumber) > outer_cabling_numServicesChannels) {
      logERROR(any2str("Invalid channel number = ") + any2str(myChannelNumber) +
    ...
    if (myChannelSlot != ChannelSlot::A && myChannelSlot != ChannelSlot::C) {
      logERROR(any2str("Power cable: Invalid channel slot = ") +
               any2str(myChannelSlot) + any2str(". Should be ") +
               any2str(ChannelSlot::A) + any2str(" or ") +
               any2str(ChannelSlot::C) + any2str("."));
    }

    // Compute number of power cables per channel section.
    std::pair<const int, const ChannelSlot> myChannel =
        std::make_pair(myChannelNumber, myChannelSlot);
  ...
  for (const auto &c : channels) {
    if (c.second > outer_cabling_maxNumPowerCablesPerChannel) {
      logERROR(any2str("Services channel ") + any2str(c.first.first) +
               ...
               any2str(" has ") + any2str(c.second) +
               any2str(" power cables.") +
