...
OuterDTC::OuterDTC(const std::string name, const double phiSectorWidth,
                   const int phiSectorRef, const Category &type, const int slot,
                   const bool isPositiveCablingSide)
    : name_(name), phiSectorWidth_(phiSectorWidth), phiSectorRef_(phiSectorRef),
      type_(type), slot_(slot), isPositiveCablingSide_(isPositiveCablingSide) {
  const int oneCablingSideId =
      computeOneCablingSideId(phiSectorRef, type, slot);
...
/** Memo: DTC CMMSW Id numbering scheme.
    In each Outer Tracker (Z) end, there are 9 Phi-sectors. In each Phi-sector,
   there are 12 DTC slots. Hence, on each Outer Tracker (Z) end, there are a
   total of 9*12 = 108 DTCs.
    ...
    Hence the DTC ids range from 1 to 108 (Tracker (+Z) end) and 109 to 216
   (Tracker (-Z) end).
    ...
    Hence this DTC is connected to modules located in the (240 deg, 280 deg) phi
   sector, and in PS 10G category.
*/
const int OuterDTC::computeCMSSWId(const int oneCablingSideId,
...
const int OuterDTC::computeOneCablingSideId(const int phiSectorRef,
                                            const Category &type,
                                            const int slot) const {
  int oneCablingSideId = slot;
  if (type == Category::SS)
    oneCablingSideId += outer_cabling_maxNumDTCsPerNonantPerZEnd / 2;
