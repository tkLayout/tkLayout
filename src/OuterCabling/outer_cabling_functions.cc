...
/* Compute the int n for which we have: phi ~= (n * phiSegmentWidth +
 * phiSegmentStart).
 */
const int computePhiSegmentRef(const double phi, const double phiSegmentStart,
                               const double phiSegmentWidth) {
  int phiSegmentRef =
      round(femod(phi - phiSegmentStart, 2. * M_PI) / phiSegmentWidth);
  return phiSegmentRef;
}

/* Compute the int n for which we have: (phiSliceStart + n*phiSliceWidth) <= phi
 * < (phiSliceStart + (n+1)*phiSliceWidth).
 */
const int computePhiSliceRef(const double phi, const double phiSliceStart,
                             const double phiSliceWidth) {
  double phiSliceRefExact =
      femod(phi - phiSliceStart, 2. * M_PI) / phiSliceWidth;
  int phiSliceRef = 0;
  // In case phiSliceRefExact is an integer, round it to an int!
  if (fabs((phiSliceRefExact - round(phiSliceRefExact))) <
      outer_cabling_roundingTolerance)
    phiSliceRef = fabs(round(phiSliceRefExact));
  else
    phiSliceRef = std::floor(phiSliceRefExact);
 ...
 */
const int computeNextPhiSliceRef(const int phiSliceRef,
