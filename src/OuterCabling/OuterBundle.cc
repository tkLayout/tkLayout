...
OuterBundle::OuterBundle(const int id, const int stereoBundleId,
                         const Category &type,
                         const std::string subDetectorName,
                         const int layerDiskNumber,
                         const PhiPosition &phiPosition,
                         const bool isPositiveCablingSide,
                         const bool isTiltedPart)
    : type_(type), subDetectorName_(subDetectorName),
      layerDiskNumber_(layerDiskNumber), phiPosition_(phiPosition),
      isPositiveCablingSide_(isPositiveCablingSide),
      isTiltedPart_(isTiltedPart), stereoBundleId_(stereoBundleId) {
  myid(id);
...
const double OuterBundle::minPhi() const {
  ...
  for (const auto &m : modules_) {
    min = MIN(min, femodRounded(m->center().Phi(), 2. * M_PI));
  }
  return min;
}
...
const double OuterBundle::maxPhi() const {
  double max = 0.;
  for (const auto &m : modules_) {
    max = MAX(max, femodRounded(m->center().Phi(), 2. * M_PI));
  }
  return max;
}
...
const std::vector<Module *>::iterator OuterBundle::minPhiModule() {
  const auto modIt = std::min_element(
      modules_.begin(), modules_.end(), [](Module *a, Module *b) {
        return (femodRounded(a->center().Phi(), 2. * M_PI) <=
                femodRounded(b->center().Phi(), 2. * M_PI));
      });
  return modIt;
}

const std::vector<Module *>::iterator OuterBundle::maxPhiModule() {
  const auto modIt = std::max_element(
      modules_.begin(), modules_.end(), [](Module *a, Module *b) {
        return (femodRounded(a->center().Phi(), 2. * M_PI) <=
                femodRounded(b->center().Phi(), 2. * M_PI));
      });
...
const int
OuterBundle::computePlotColor(const int id,
                              const bool isPositiveCablingSide) const {
  int plotColor = 0;
  int plotId = (isPositiveCablingSide ? id : (id - 20000));
  int plotType =
      2 + plotId % 2; // Barrel : Identifies Flat vs Tilted. Endcap : Identifies
                      // PS10GA vs PG10GB vs PS5G vs 2S type.
  int dizaine = plotId / 10;
  int plotPhi =
      dizaine %
      3; // Barrel : Identifies phiSegmentRef. Endcap : Identifies phiRegionRef.
...
const ChannelSection *OuterBundle::opticalChannelSection() const {
  ...
  if (cable_)
    opticalSection = cable_->opticalChannelSection();
  if (!cable_ || !opticalSection)
    throw PathfulException("cable_ or opticalSection is nullptr");
  return opticalSection;
}

/* Untilted TBPS only: Id of the bundle located on the same Phi, but connected
 * to the tilted modules.
 */
const int OuterBundle::tiltedBundleId() const {
  if (!isBarrelPSFlatPart())
    logERROR("Tried to access tiltedBundleId, but not in TBPS flat part.");
  const int bundleId = myid();
  const int tiltedBundleId = bundleId - femod(bundleId, 10);
  return tiltedBundleId;
}

/* Id of the bundle located on the other cabling side, by rotation of 180°
 * around CMS_Y.
 */
const int OuterBundle::stereoBundleId() const {
  if (!isBarrel())
    logERROR("Tried to access stereoBundleId in TEDD, where it is not "
             "implemented (only implemented for Barrel).");
  return stereoBundleId_;
}

void OuterBundle::setIsPowerRoutedToBarrelLowerSemiNonant(const bool isLower) {
  if (!isBarrel())
    logERROR("Tried to set Barrel power routing on a Bundle located in TEDD.");
...
const bool OuterBundle::isPowerRoutedToBarrelLowerSemiNonant() const {
  if (!isBarrel())
    logERROR(
        "Tried to access Barrel power routing from a Bundle located in TEDD.");
