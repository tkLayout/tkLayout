 ...
 * @brief This is the implementation of the container class for inactive tracker
 * elements
 */
 ...
 * @param service The element that is appended to the list of barrel service
 * parts
 */
...
/**
 * Remove a single barrel element identified by its index from the list. If the
 * removed barrel service part was the last on the list or the given index is
 * out of range, the returned iterator will point to <i>end()</i>.
 ...
 */
std::vector<InactiveElement>::iterator
InactiveSurfaces::removeBarrelServicePart(int index) {
  if ((index >= 0) && ((unsigned int)index < barrelservices.size()))
    return barrelservices.erase(barrelservices.begin() + index);
 ...
 * @param service The element that is appended to the list of endcap service
 * parts
 */
...
/**
 * Remove a single endcap element identified by its index from the list. If the
 * removed endcap service part was the last on the list or the given index is
 * out of range, the returned iterator will point to <i>end()</i>.
 ...
 */
std::vector<InactiveElement>::iterator
InactiveSurfaces::removeEndcapServicePart(int index) {
  if ((index >= 0) && ((unsigned int)index < endcapservices.size()))
    return endcapservices.erase(endcapservices.begin() + index);
 ...
 */
InactiveElement &
InactiveSurfaces::getSupportPart(int index) { // throws exception
  return supports.at(index);
}

/**
 *Remove a single element identified by its index from the list. If the removed
 *support part was the last on the list or the given index is out of range, the
 *returned iterator will point to <i>end()</i>.
 ...
 */
std::vector<InactiveElement>::iterator
InactiveSurfaces::removeSupportPart(int index) {
  if ((index >= 0) && ((unsigned int)index < supports.size()))
    return supports.erase(supports.begin() + index);
  return supports.end();
}
 ...
 */
std::vector<InactiveElement> &
InactiveSurfaces::getSupports() { // may return empty vector
  return supports;
}
...
void InactiveSurfaces::print(bool full_summary = true) {
  std::cout << "Number of barrel service elements: " << barrelservices.size()
            << std::endl;
  if (full_summary) {
  ...
  std::cout << "Number of endcap service elements: " << endcapservices.size()
            << std::endl;
  if (full_summary) {
