...
GraphBag::GraphBag() { parameterMeaning_ = "unknown"; }

GraphBag::GraphBag(string meaning) { parameterMeaning_ = meaning; }
...
int GraphBag::clearGraphs(const int &attributeMask) {
  int deleteCounter = 0;
  for (auto it = taggedGraphMap_.begin();
       it !=
       taggedGraphMap_
           .end();) { // first clear tagged graphs with the specified attribute
...
std::map<int, TGraph> &GraphBag::getTaggedGraphs(int attribute,
                                                 const string &tag) {
  tagSet_.insert(tag);
  return taggedGraphMap_[std::make_pair(attribute, tag)];
}

TGraph &GraphBag::getTaggedGraph(const int &attribute, const string &tag,
                                 const int &parameter) {
...
std::map<double, TProfile> &
profileBag::getNamedProfiles(const std::string &name) {
