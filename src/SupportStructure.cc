...
const std::map<std::string, SupportStructure::Type>
    SupportStructure::typeStringMap = {
        {"custom", CUSTOM}, {"auto", AUTO}, {"top", TOP}, {"bottom", BOTTOM}};
const std::map<std::string, SupportStructure::Direction>
    SupportStructure::directionStringMap = {{"horizontal", HORIZONTAL},
                                            {"vertical", VERTICAL}};
...
void SupportStructure::buildInTracker(const std::string subdetectorName) {
  ...
  if (supportType_ == CUSTOM) {
    if (customZMin.state() && customRMin.state() && customLength.state() &&
        customDir.state()) {
      try {
      ...
      buildInactiveElementPair(direction_, customZMin(), customRMin(),
                               customLength());
      logINFO("Building custom support structure, positioned at Z: " +
              any2str(customZMin()) + " R: " + any2str(customRMin()) +
              " of length: " + any2str(customLength()));
    } else {
      logERROR("Property customZMin, customRMin, customLength, or customDir "
               "not set.");
      return;
    }
  } else {
    logERROR("Support type \"" + type() +
             "\" not supported at the Tracker level.");
    return;
  }
...
void SupportStructure::buildInBarrel(Barrel &barrel) {
  ...
  case AUTO:
    if (autoPosition.state()) {
      ...
      public:
        LayerVisitor(std::set<double>& layerRadiuses, const double&
      autoLayerMarginUpper, const double& autoLayerMarginLower) :
        ...
        void visit(Layer& layer) {
          layerRadiuses_.insert(layer.minRwithHybrids() -
      autoLayerMarginUpper_); layerRadiuses_.insert(layer.maxRwithHybrids() +
      autoLayerMarginLower_);
        }
      };

      LayerVisitor visitor(layerRadiuses, autoLayerMarginUpper,
      autoLayerMarginLower); barrel.accept(visitor); */
      ...
      if (layerRadii.size() < 4) {
        logERROR("Barrel with only one or zero layers. Auto support impossible "
                 "to build.");
        return;
      }

      // delete minimum and maximum useless radiuses (support structure only in
      // the inner spaces)
      layerRadii.erase(layerRadii.begin());
      layerRadii.erase(std::prev(layerRadii.end()));

      // build inactiveElements inside spaces
      for (std::set<double>::iterator minIter = layerRadii.begin(),
                                      maxIter = ++layerRadii.begin();
           minIter != layerRadii.end();
           std::advance(minIter, 2), std::advance(maxIter, 2)) {
        buildInactiveElementPair(direction_, autoPosition(), *minIter,
                                 *maxIter - *minIter);
      }
      logINFO("Building barrel support structure vertically oriented, "
              "positioned at Z: " +
              any2str(autoPosition()));
    } else {
  ...
  case TOP:
    ...
    buildInactiveElementPair(direction_, MAX(0, barrel.minZwithHybrids()),
                             barrel.maxRwithHybrids() + autoLayerMarginLower,
                             barrel.maxZwithHybrids() -
                                 MAX(0, barrel.minZwithHybrids()));
    // std::cout << ">>Top barrel support>> " << "maxRwithHybrids: " <<
    // endcap.maxRwithHybrids() + insur::geom_support_margin_top << "
    // minZwithHybrids: " << MAX(0, endcap.minZwithHybrids()) << "
    // maxZwithHybrids: " << endcap.maxZwithHybrids() << std::endl;
    logINFO("Building barrel top support structure horizontally oriented");
    break;
  case BOTTOM:
    direction_ = HORIZONTAL;
    buildInactiveElementPair(
        direction_, MAX(0, barrel.minZwithHybrids()),
        barrel.minRwithHybrids() - inactiveElementWidth - autoLayerMarginUpper,
        barrel.maxZwithHybrids() - MAX(0, barrel.minZwithHybrids()));
    // std::cout << ">>Bottom barrel support>> " << "minRwithHybrids: " <<
    // endcap.minRwithHybrids() - insur::geom_support_margin_bottom << "
    // minZwithHybrids: " << MAX(0, endcap.minZwithHybrids()) << "
    // maxZwithHybrids: " << endcap.maxZwithHybrids() << std::endl;
  ...
  default:
    logERROR("Support type \"" + type() +
             "\" not supported at the barrel level.");
    return;
  }
  cleanup();
}
...
void SupportStructure::buildInEndcap(Endcap &endcap) {
  ...
  case TOP:
    ...
    direction_ = HORIZONTAL;
    buildInactiveElementPair(direction_, MAX(0, endcap.minZwithHybrids()),
                             endcap.maxRwithHybrids() + autoLayerMarginLower,
                             endcap.maxZwithHybrids() -
                                 MAX(0, endcap.minZwithHybrids()));
    // std::cout << ">>Top end-cap support>> " << "maxRwithHybrids: " <<
    // endcap.maxRwithHybrids() + insur::geom_support_margin_top << "
    // minZwithHybrids: " << MAX(0, endcap.minZwithHybrids()) << "
    // maxZwithHybrids: " << endcap.maxZwithHybrids() << std::endl;
  ...
  default:
    logERROR("Support type \"" + type() +
             "\" not supported at the end-cap level.");
    return;
  }
  cleanup();
}

void SupportStructure::updateInactiveSurfaces(
...
void SupportStructure::populateMaterialProperties(
...
void SupportStructure::buildInactiveElementPair(Direction direction,
                                                double startZ, double startR,
                                                double length) {
...
SupportStructure::Component::Component(const std::string subdetectorName)
    : componentsNode("Component", parsedOnly()),
      elementsNode("Element", parsedOnly()), subdetectorName_(subdetectorName) {
}
...
void SupportStructure::Component::populateMaterialProperties(
    MaterialProperties &materialProperties) const {
  for (const Component *currComponent : components_) {
    currComponent->populateMaterialProperties(materialProperties);
  }
...
const std::string SupportStructure::Element::msg_no_valid_unit =
    "No valid unit: ";

const std::map<std::string, SupportStructure::Element::Unit>
    SupportStructure::Element::unitStringMap = {
        {"g", GRAMS}, {"mm", MILLIMETERS}, {"g/m", GRAMS_METER}};

double SupportStructure::Element::quantityInGrams(double length,
...
void SupportStructure::Element::populateMaterialProperties(
    MaterialProperties &materialProperties) const {
  double quantity;

  if (debugInactivate() == false) {
    quantity = quantityInGrams(materialProperties.getLength(),
                               materialProperties.getSurface());
    if (subdetectorName() == "")
      std::cout << "SupportStructure::Element::populateMaterialProperties "
                   "subdetectorName() = "
