/**
 * @file Analyzer.cc
 * @brief This is the implementation of the class that analyses a material
 * budget
 */
 ...
 * @return True if <i>p.first</i> is smaller than <i>q.first</i>, false
 * otherwise
 */
 ...
 * @param p The first pair
 * @param q The second pair
 * @return True if <i>p.second</i> is smaller than <i>q.second</i>, false
 * otherwise
 */
...
Analyzer::Analyzer() {
  ...
  // colorPicker("ptIn");
  geomLite = nullptr;
  geomLiteCreated = false;
  geomLiteXY = nullptr;
  geomLiteXYCreated = false;
  geomLiteYZ = nullptr;
  geomLiteYZCreated = false;
  geomLiteEC = nullptr;
  geomLiteECCreated = false;
...
/* Get shape of luminous region, used for all analysis (except Material Budget
 * Analysis). IP is always assumed to be on the (Z) axis. Shape of luminous
 * region in Z is defined by user in SimParms cfg files. Shape of luminous
 * region in Z is always assumed to be centered around Z = 0. return: XYZVector
 * defining the IP position (in CMS official frame of reference).
 */
const XYZVector Analyzer::getLuminousRegion() {
  ...
  double dz = 0.;
  if (lumiShape == LumiRegShape::SPOT)
    dz = 0.;
  else if (lumiShape == LumiRegShape::FLAT)
    dz = (myDice.Rndm() * 2. - 1.) * zError;
  else if (lumiShape == LumiRegShape::GAUSSIAN)
    dz = myDice.Gaus(0., zError);
  else
    logERROR(
        "Shape of luminous region specified in SimParms is not supported.");
 ...
 * return: XYZVector defining the IP position (in CMS official frame of
 * reference).
 */
const XYZVector Analyzer::getLuminousRegionInMatBudgetAnalysis() {
  ...
  if (lumiShape != LumiRegShape::SPOT)
    logERROR("Non-ponctual IP in Material Budget Analysis not supported.");
 ...
 * @param mb A reference to the instance of <i>MaterialBudget</i> that is to be
 * analysed
 * @param momenta A list of momentum values for which to perform the efficiency
 * measurements
 * @param etaSteps The number of wedges in the fan of tracks covered by the eta
 * scan
 * @param pm A pointer to a second material budget associated to a pixel
 * detector; may be <i>NULL</i>
 * @return the total crossed material amount
 */
RILength Analyzer::findAllHits(MaterialBudget &mb, MaterialBudget *pm,
                               Track &track) {
  ...
  //      services, barrel
  totalMaterial += findHitsInactiveSurfaces(
      mb.getInactiveSurfaces().getBarrelServices(), track);
  //      services, endcap
  totalMaterial += findHitsInactiveSurfaces(
      mb.getInactiveSurfaces().getEndcapServices(), track);
  //      supports
  totalMaterial +=
      findHitsInactiveSurfaces(mb.getInactiveSurfaces().getSupports(), track);
  ...
  if (pm != NULL) {
    ...
    totalMaterial += findHitsInactiveSurfaces(
        pm->getInactiveSurfaces().getBarrelServices(), track, true);
    totalMaterial += findHitsInactiveSurfaces(
        pm->getInactiveSurfaces().getEndcapServices(), track, true);
    totalMaterial += findHitsInactiveSurfaces(
        pm->getInactiveSurfaces().getSupports(), track, true);
  }
  return totalMaterial;
}

/* TODO: finish this :-)
void Analyzer::createTaggedTrackCollection(std::vector<MaterialBudget*>
materialBudgets, int etaSteps, double maxEta, bool forceClean = false) {
...
void Analyzer::analyzeTaggedTracking(
    MaterialBudget &mb, const std::vector<double> &momenta,
    const std::vector<double> &triggerMomenta,
    const std::vector<double> &thresholdProbabilities, bool isPixel,
    bool &debugResolution, int etaSteps, MaterialBudget *pm) {
  ...
  // prepare etaStep, phiStep, nTracks, nScans
  if (etaSteps > 1)
    etaStep = getEtaMaxTrigger() / (double)(etaSteps - 1);
  else
    etaStep = getEtaMaxTrigger();
  nTracks = etaSteps;

  // prepareTriggerPerformanceHistograms(nTracks, getEtaMaxTrigger(),
  // triggerMomenta, thresholdProbabilities);
  ...
  for (int i_eta = 0; i_eta < nTracks; i_eta++) {
    ...
    material.radiation =
        0.0022761 /
        sin(theta); // was 0.0023, adapted to fit CMSSW 81X 2016/11/30
    material.interaction =
        0.0020334 /
        sin(theta); // was 0.0019, adapted to fit CMSSW 81X 2016/11/30
    ...
    if (!track.hasNoHits()) {
      for (string tag : track.getTags()) {
        ...
        if (SimParms::getInstance().useIPConstraint()) {
          ...
          track.addIPConstraint(SimParms::getInstance().rphiErrorCollider(),
                                SimParms::getInstance().lumiRegZError());
        }
        ...
        for (const auto &pIter : momenta) {
          int parameter =
              pIter /
              Units::MeV; // Store p or pT in MeV as int (key to the map)
          ...
          if (idealTrackPt->getNActiveHits(tag, useIPConstraint) >=
              3) { // Only keep tracks which have minimum 3 active hits
            idealTrackPt->fillModuleLocalResolutionStats();
            TrackCollectionMap &myMapIdeal =
                taggedTrackPtCollectionMapIdeal[tag];
          ...
          trackP->resetPt(pT);
          // trackP.pruneHits();                // Remove hits from a track that
          // is not able to reach a given radius due to its limited momentum
          if (trackP->getNActiveHits(tag, useIPConstraint) >=
              3) { // Only keep tracks which have minimum 3 active hits
          ...
          if (idealTrackP->getNActiveHits(tag, useIPConstraint) >=
              3) { // Only keep tracks which have minimum 3 active hits
            idealTrackP->fillModuleLocalResolutionStats();
            TrackCollectionMap &myMapIdealII =
                taggedTrackPCollectionMapIdeal[tag];
  ...
  if (!isPixel) {
    ...
    for (/*const*/ auto &ttcmIt : taggedTrackPtCollectionMap) {
      ...
      for (const auto &tcmIt : myTrackCollection) {
        ...
        calculateGraphsConstPt(parameter, myCollection, GraphBag::RealGraph,
                               myTag);
      }
    }
    for (/*const*/ auto &ttcmIt : taggedTrackPtCollectionMapIdeal) {
      ...
      for (const auto &tcmIt : myTrackCollection) {
        const int &parameter = tcmIt.first;
        const TrackCollection &myCollection = tcmIt.second;
        calculateGraphsConstPt(parameter, myCollection, GraphBag::IdealGraph,
                               myTag);
      }
    }
    ...
    for (/*const*/ auto &ttcmIt : taggedTrackPCollectionMap) {
      ...
      for (const auto &tcmIt : myTrackCollection) {
        ...
        const TrackCollection &myCollection = tcmIt.second;
        // std::cout << myCollection.size() << std::endl;
        calculateGraphsConstP(parameter, myCollection, GraphBag::RealGraph,
                              myTag);
      }
    }
    for (/*const*/ auto &ttcmIt : taggedTrackPCollectionMapIdeal) {
      ...
      for (const auto &tcmIt : myTrackCollection) {
        const int &parameter = tcmIt.first;
        const TrackCollection &myCollection = tcmIt.second;
        calculateGraphsConstP(parameter, myCollection, GraphBag::IdealGraph,
                              myTag);
      }
    }
  }
 ...
 * scan
 */
void Analyzer::analyzeTriggerEfficiency(
    Tracker &tracker, const std::vector<double> &triggerMomenta,
    const std::vector<double> &thresholdProbabilities, int etaSteps) {
  ...
  for (int i_eta = 0; i_eta < nTracks; i_eta++) {
    ...
    if (nHits) {
      ...
      track.addEfficiency();
      if (track.getNActiveHits("all") >
          0) { // At least 3 points are needed to measure the arrow
...
//
bool Analyzer::checkFile(const std::string &fileName,
...
//
// Is starting triplet from different layers (avoid using overlapping modules in
// one layer)
//
bool Analyzer::isTripletFromDifLayers(Track &track, int iHit,
                                      bool propagOutIn) {
  ...
  for (int i = 0; i <= iHit; i++) {
    ...
    if (!propagOutIn)
      hitID = track.getMeasurableOrIPHit(i)->getDetName() +
              std::string(track.getMeasurableOrIPHit(i)->isBarrel() ? "_L_"
                                                                    : "_D_") +
              any2str(track.getMeasurableOrIPHit(i)->getLayerOrDiscID());
    else
      hitID = track.getRMeasurableOrIPHit(i)->getDetName() +
              std::string(track.getRMeasurableOrIPHit(i)->isBarrel() ? "_L_"
                                                                     : "_D_") +
...
//
// Analyze tracker pattern recognition capabilities. The quantities used to
// qualify the pattern reco capabilities are meant for primary tracks only
// rather than for secondary tracks, as one always starts in the innermost layer
// (+ IP contraint) or outermost layer in case of out-in approach. The
// extrapolation part of pattern recognition uses formulae calculated in
// parabolic approximation and particle fluences (occupancies) scaled to given
// pile-up scenario. The fluences are read in from an external file and are
// assumed to be simulated by Fluka for given p-p collision energies & rough
// detector (material) setup.
//
bool Analyzer::analyzePatterReco(MaterialBudget &mb,
                                 mainConfigHandler &mainConfig, int nTracks,
                                 MaterialBudget *pm) {
  ...
  std::cout << "Reading in: " << directory + "/" + default_fluence_file
            << std::endl;
  if (fluenceMapOK)
    fluenceMap = new IrradiationMap(directory + "/" + default_fluence_file);

  // Set nTracks
  if (nTracks <= 0) {

    std::ostringstream message;
    message << "PatternReco: Number of simulation tracks zero or negative: "
            << nTracks << " or zero number of trackers to be initialized";
    ...
    for (const auto &pIter : mainConfig.getMomenta()) {
      ...
      hisPatternRecoInOutPt.push_back(new TProfile(
          name.c_str(), name.c_str(), nBins, 0, geom_max_eta_coverage));
      name = "hisBkgContInOut_p" + any2str(pIter / Units::GeV);
      hisPatternRecoInOutP.push_back(new TProfile(
          name.c_str(), name.c_str(), nBins, 0, geom_max_eta_coverage));

      // Out-In
      name = "hisBkgContOutIn_pT" + any2str(pIter / Units::GeV);
      hisPatternRecoOutInPt.push_back(new TProfile(
          name.c_str(), name.c_str(), nBins, 0, geom_max_eta_coverage));
      name = "hisBkgContOutIn_p" + any2str(pIter / Units::GeV);
      hisPatternRecoOutInP.push_back(new TProfile(
          name.c_str(), name.c_str(), nBins, 0, geom_max_eta_coverage));
    }
  }
      ...
      for (const auto &pIter : mainConfig.getMomenta()) {
        ...
        for (int pOption = 0; pOption < 2; pOption++) {
          ...
          for (int approachOption = 0; approachOption < 2; approachOption++) {
            ...
            int nMeasuredHits = 0;
            if (!propagOutIn)
              nMeasuredHits = track.getNMeasuredHits("all", !useIP);
            else
              nMeasuredHits = track.getNMeasuredHits("all", !useIP);

            // track.printHits();
            // std::cout << ">> " << nMeasuredHits << " dD0=" <<
            // track.getDeltaD(0.0)/Units::um << " dZ0=" <<
            // track.getDeltaZ(0.0)/Units::um << std::endl;

            // Start with first 3 hits and end with N-1 hits (C counting from
            // zero)
            bool testTriplet = true;
            for (int iHit = 2; iHit < nMeasuredHits - 1; iHit++) {

              // Start with triplet coming from 3 different layers -> avoid
              // using 2 close measurements from overlapping modules in 1 layer
              if (testTriplet &&
                  !isTripletFromDifLayers(track, iHit, propagOutIn))
                continue;
              else
                testTriplet = false;
              ...
              // Keep first/last N hits (based on approach) and find paramaters
              // of the next hit
              double nextRPos = 0;
              double nextZPos = 0;
              ...
              if (!propagOutIn) {
                ...
                if (SimParms::getInstance().isMagFieldConst())
                  A = track.getMeasurableOrIPHit(iHit + 1)->getRPos() / 2. /
                      track.getRadius(track.getMeasurableOrIPHit(iHit + 1)
                                          ->getZPos()); // r_i/2R

                iHitID =
                    track.getMeasurableOrIPHit(iHit + 1)->getDetName() +
                    std::string(track.getMeasurableOrIPHit(iHit + 1)->isBarrel()
                                    ? "_L_"
                                    : "_D_") +
                    any2str(track.getMeasurableOrIPHit(iHit + 1)
                                ->getLayerOrDiscID());
              } else {
                ...
                if (SimParms::getInstance().isMagFieldConst())
                  A = track.getRMeasurableOrIPHit(iHit + 1)->getRPos() / 2. /
                      track.getRadius(track.getRMeasurableOrIPHit(iHit + 1)
                                          ->getZPos()); // r_i/2R
                iHitID = track.getRMeasurableOrIPHit(iHit + 1)->getDetName() +
                         std::string(
                             track.getRMeasurableOrIPHit(iHit + 1)->isBarrel()
                                 ? "_L_"
                                 : "_D_") +
                         any2str(track.getRMeasurableOrIPHit(iHit + 1)
                                     ->getLayerOrDiscID());
              }

              // Calculate dRPhi & dZ
              int nPU = SimParms::getInstance().numMinBiasEvents();
              // TODO: Assumed that map on ZxR grid defined in cm -> needs to be
              // fixed
              double flux = fluenceMap->calculateIrradiation(std::make_pair(
                                nextZPos / Units::mm, nextRPos / Units::mm)) *
                            1 / Units::cm2 / Units::s * nPU;

              double corrFactor = cos(nextHitTilt) + track.getCotgTheta() /
                                                         sqrt(1 - A * A) *
                                                         sin(nextHitTilt);
              double dZProj =
                  track.getDeltaZ(nextRPos, propagOutIn) / corrFactor;
              ...
              // coverge F(mu + n*sigma) - F(mu - n*sigma) = erf(n/sqrt(2)) In
              // 2D we assume independent measurement in r-phi & Z, hence 0.95 =
              // erf(n/sqrt(2))*erf(n/sqrt(2)) assuming the same number of
              // sigmas (n) in both r-phi & z Hence n =
              // InverseErf(sqrt(0.95))*sqrt(2)
              static double nSigmaFactor =
                  TMath::ErfInverse(sqrt(0.95)) * sqrt(2);

              // Calculate errorEllipse multiplied by
              // nSigmaFactor(RPhi)*nSigmaFactor(Z)
              double errorEllipse =
                  M_PI * dZProj * dDProj * nSigmaFactor * nSigmaFactor;

              // Accumulate probibilites accross all hits not to be contaminated
              // anywhere -> final probability of being contaminated is 1 -
              // pContam
              ...
              //
              // Fill d0proj, z0proj & pContam for individual layers/discs to
              // histograms

              // Probability of inner tracker only
              // if (iHitID.find("Inner")!=std::string::npos) pNotContamTotInner
              // *= 1-pContam;

              // Create artificial map identifier with extra characters to have
              // innermost first, then outermost & then fwd
              std::string iHitIDMap = "";
              if (iHitID.find("Inner") != std::string::npos)
                iHitIDMap = "A_" + iHitID;
              else if (iHitID.find("Outer") != std::string::npos)
                iHitIDMap = "B_" + iHitID;
              else
                iHitIDMap = "C_" + iHitID;
              ...
              if (pOption == 0 && approachOption == 0) {
                ...
                if (hisPtHitDProjInOut.find(iHitIDMap) ==
                    hisPtHitDProjInOut.end()) {
                  const int numMomemta = mainConfig.getMomenta().size();
                  for (int iMom = 0; iMom < numMomemta; iMom++) {

                    std::string name = "hisPt_" + any2str(iMom) + "_Hit_" +
                                       iHitID + "_DProjInOut";
                    hisPtHitDProjInOut[iHitIDMap].push_back(
                        new TProfile(name.c_str(), name.c_str(), nBins, 0,
                                     geom_max_eta_coverage));

                    name = "hisPt" + any2str(iMom) + "_Hit_" + iHitID +
                           "_ZProjInOut";
                    hisPtHitZProjInOut[iHitIDMap].push_back(
                        new TProfile(name.c_str(), name.c_str(), nBins, 0,
                                     geom_max_eta_coverage));

                    name = "hisPt" + any2str(iMom) + "_Hit_" + iHitID +
                           "_ProbContamInOut";
                    hisPtHitProbContamInOut[iHitIDMap].push_back(
                        new TProfile(name.c_str(), name.c_str(), nBins, 0,
                                     geom_max_eta_coverage));
                  }
                }
                hisPtHitDProjInOut[iHitIDMap][iMomentum]->Fill(
                    eta, dDProj / Units::um);
                hisPtHitZProjInOut[iHitIDMap][iMomentum]->Fill(
                    eta, dZProj / Units::um);
                hisPtHitProbContamInOut[iHitIDMap][iMomentum]->Fill(eta,
                                                                    pContam);
              }
              // P & InOut
              if (pOption == 1 && approachOption == 0) {
                ...
                // Create profile histograms if don't exist yet
                if (hisPHitDProjInOut.find(iHitIDMap) ==
                    hisPHitDProjInOut.end()) {
                  const int numMomemta = mainConfig.getMomenta().size();
                  for (int iMom = 0; iMom < numMomemta; iMom++) {

                    std::string name = "hisP_" + any2str(iMom) + "_Hit_" +
                                       iHitID + "_DProjInOut";
                    hisPHitDProjInOut[iHitIDMap].push_back(
                        new TProfile(name.c_str(), name.c_str(), nBins, 0,
                                     geom_max_eta_coverage));

                    name = "hisP" + any2str(iMom) + "_Hit_" + iHitID +
                           "_ZProjInOut";
                    hisPHitZProjInOut[iHitIDMap].push_back(
                        new TProfile(name.c_str(), name.c_str(), nBins, 0,
                                     geom_max_eta_coverage));

                    name = "hisP" + any2str(iMom) + "_Hit_" + iHitID +
                           "_ProbContamInOut";
                    hisPHitProbContamInOut[iHitIDMap].push_back(
                        new TProfile(name.c_str(), name.c_str(), nBins, 0,
                                     geom_max_eta_coverage));
                  }
                }
                // TODO: Check that above zero!!!
                hisPHitDProjInOut[iHitIDMap][iMomentum]->Fill(
                    eta, dDProj / Units::um);
                hisPHitZProjInOut[iHitIDMap][iMomentum]->Fill(
                    eta, dZProj / Units::um);
                hisPHitProbContamInOut[iHitIDMap][iMomentum]->Fill(eta,
                                                                   pContam);
              }
              // Pt & OutIn
              if (pOption == 0 && approachOption == 1) {
                ...
                // Create profile histograms if don't exist yet
                if (hisPtHitDProjOutIn.find(iHitIDMap) ==
                    hisPtHitDProjOutIn.end()) {
                  const int numMomemta = mainConfig.getMomenta().size();
                  for (int iMom = 0; iMom < numMomemta; iMom++) {

                    std::string name = "hisPt_" + any2str(iMom) + "_Hit_" +
                                       iHitID + "_DProjOutIn";
                    hisPtHitDProjOutIn[iHitIDMap].push_back(
                        new TProfile(name.c_str(), name.c_str(), nBins, 0,
                                     geom_max_eta_coverage));

                    name = "hisPt" + any2str(iMom) + "_Hit_" + iHitID +
                           "_ZProjOutIn";
                    hisPtHitZProjOutIn[iHitIDMap].push_back(
                        new TProfile(name.c_str(), name.c_str(), nBins, 0,
                                     geom_max_eta_coverage));

                    name = "hisPt" + any2str(iMom) + "_Hit_" + iHitID +
                           "_ProbContamOutIn";
                    hisPtHitProbContamOutIn[iHitIDMap].push_back(
                        new TProfile(name.c_str(), name.c_str(), nBins, 0,
                                     geom_max_eta_coverage));
                  }
                }
                hisPtHitDProjOutIn[iHitIDMap][iMomentum]->Fill(
                    eta, dDProj / Units::um);
                hisPtHitZProjOutIn[iHitIDMap][iMomentum]->Fill(
                    eta, dZProj / Units::um);
                hisPtHitProbContamOutIn[iHitIDMap][iMomentum]->Fill(eta,
                                                                    pContam);
              }
              // P & OutIn
              if (pOption == 1 && approachOption == 1) {
                ...
                // Create profile histograms if don't exist yet
                if (hisPHitDProjOutIn.find(iHitIDMap) ==
                    hisPHitDProjOutIn.end()) {
                  const int numMomemta = mainConfig.getMomenta().size();
                  for (int iMom = 0; iMom < numMomemta; iMom++) {

                    std::string name = "hisP_" + any2str(iMom) + "_Hit_" +
                                       iHitID + "_DProjOutIn";
                    hisPHitDProjOutIn[iHitIDMap].push_back(
                        new TProfile(name.c_str(), name.c_str(), nBins, 0,
                                     geom_max_eta_coverage));

                    name = "hisP" + any2str(iMom) + "_Hit_" + iHitID +
                           "_ZProjOutIn";
                    hisPHitZProjOutIn[iHitIDMap].push_back(
                        new TProfile(name.c_str(), name.c_str(), nBins, 0,
                                     geom_max_eta_coverage));

                    name = "hisP" + any2str(iMom) + "_Hit_" + iHitID +
                           "_ProbContamOutIn";
                    hisPHitProbContamOutIn[iHitIDMap].push_back(
                        new TProfile(name.c_str(), name.c_str(), nBins, 0,
                                     geom_max_eta_coverage));
                  }
                }
                // TODO: Check that above zero!!!
                hisPHitDProjOutIn[iHitIDMap][iMomentum]->Fill(
                    eta, dDProj / Units::um);
                hisPHitZProjOutIn[iHitIDMap][iMomentum]->Fill(
                    eta, dZProj / Units::um);
                hisPHitProbContamOutIn[iHitIDMap][iMomentum]->Fill(eta,
                                                                   pContam);
              }
            } // Pattern reco loop

            //
            // Calculate total contamination based on different options: p/pT,
            // in-out/out-in
...
void Analyzer::fillAvailableSpacing(Tracker &tracker,
  ...
  for (std::set<double>::iterator it = foundSpacing.begin();
       it != foundSpacing.end(); ++it) {
    spacingOptions.push_back(*it);
  }
}

void Analyzer::createTriggerDistanceTuningPlots(
    Tracker &tracker, const std::vector<double> &triggerMomenta) {
  TriggerDistanceTuningPlotsVisitor v(myProfileBag, triggerMomenta);
...
void Analyzer::fillTriggerEfficiencyGraphs(
    const Tracker &tracker, const std::vector<double> &triggerMomenta,
    const TrackCollection &tracks) {

  // Prepare the graphs to record the number of triggered points
  // std::map<double, TGraph>& trigGraphs =
  // myGraphBag.getGraphs(GraphBag::TriggerGraph|GraphBag::TriggeredGraph);
  std::map<double, TProfile> &trigProfiles = myProfileBag.getProfiles(
      profileBag::TriggerProfile | profileBag::TriggeredProfile);
  std::map<double, TProfile> &trigFractionProfiles = myProfileBag.getProfiles(
      profileBag::TriggerProfile | profileBag::TriggeredFractionProfile);
  std::map<double, TProfile> &trigPurityProfiles = myProfileBag.getProfiles(
      profileBag::TriggerProfile | profileBag::TriggerPurityProfile);

  TProfile &totalProfile = trigProfiles[profileBag::Triggerable];

  std::map<std::string, std::map<std::string, TH1I *>>
      &stubEfficiencyCoverageProfiles = getStubEfficiencyCoverageProfiles();
  ...
  for (auto &iTrack : tracks) {
    ...
    for (auto &iMomentum : triggerMomenta) {
      ...
      double nExpectedTriggerPoints =
          iTrack->getExpectedTriggerPoints(iMomentum);
      if (nExpectedTriggerPoints >= 0) { // sanity check (! nan)

        myProfile.Fill(eta, nExpectedTriggerPoints);
        if (nHits > 0) {

          myFractionProfile.Fill(eta,
                                 nExpectedTriggerPoints * 100 / double(nHits));
          double curAvgTrue = 0;
          double curAvgInteresting = 0;
          double curAvgFake = 0;
          double bgReductionFactor; // Reduction of the combinatorial background
                                    // for ptPS modules by turning off the
                                    // appropriate pixels

          for (std::vector<std::unique_ptr<Hit>>::const_iterator itHit =
                   iTrack->getBeginHits();
               itHit != iTrack->getEndHits(); itHit++) {
            ...
            if (iHit->isTrigger() && !iHit->isIP() && iHit->isActive()) {
              ...
              if (hitModule == nullptr)
                logERROR("Track::fillTriggerEfficiencyGraphs: This SHOULD NOT "
                         "happen -> an active hit does not correspond to any "
                         "module!");

              PtErrorAdapter pterr(*hitModule);
              // Hits that we would like to have from tracks above this
              // threshold
              curAvgInteresting +=
                  pterr.getParticleFrequencyPerEventAbove(iMomentum);
              // ... out of which we only see these
              curAvgTrue +=
                  pterr.getTriggerFrequencyTruePerEventAbove(iMomentum);

              // The background is given by the contamination from low pT
              // tracks...
              curAvgFake +=
                  pterr.getTriggerFrequencyTruePerEventBelow(iMomentum);
              // ... plus the combinatorial background from occupancy (can be
              // reduced using ptPS modules)
              if (hitModule->reduceCombinatorialBackground())
                bgReductionFactor = hitModule->geometricEfficiency();
              else
                bgReductionFactor = 1;
              curAvgFake += pterr.getTriggerFrequencyFakePerEvent() *
                            SimParms::getInstance().numMinBiasEvents() *
                            bgReductionFactor;

              std::string layerName = hitModule->uniRef().subdetectorName +
                                      "_" + any2str(hitModule->uniRef().layer);
              if (iHit->getActiveHitType() == HitType::STUB) {

                std::string momentumString = any2str(iMomentum, 2);
                if (stubEfficiencyCoverageProfiles[layerName].count(
                        momentumString) == 0) {
                  stubEfficiencyCoverageProfiles[layerName][momentumString] =
                      new TH1I(Form("stubEfficiencyCoverageProfile%s%s",
                                    layerName.c_str(), momentumString.c_str()),
                               (layerName + ";#eta;Stubs").c_str(),
                               tracks.size(), 0.0, maxEta);
                }
                stubEfficiencyCoverageProfiles[layerName][momentumString]->Fill(
                    iTrack->getEta(), 1);
              }
            }
          } // For hits
          myPurityProfile.Fill(eta,
                               100 * curAvgTrue / (curAvgTrue + curAvgFake));
        }
      }
    } // For trigger momenta
  }   // For tracks
  ...
  //    std::cout << "--------------------- " << i.first << " ------------------
  //    " << std::endl; for (auto j : i.second) {
  //      std::cout << j.first << std::endl;
  //      j.second->Print("all");
  //    }
  //  }
  if (totalProfile.GetMaximum() < plotNumberOfTriggeredPointsMaxY)
    totalProfile.SetMaximum(plotNumberOfTriggeredPointsMaxY);
}

/**
 * The main analysis function provides a frame for the scan in eta, defers
 * summing up the radiation and interaction lengths for each volume category to
 * subfunctions, and sorts those results into the correct histograms.
 * @param mb A reference to the instance of <i>MaterialBudget</i> that is to be
 * analysed
 * @param momenta A list of momentum values for the tracks that are shot through
 * the layout
 * @param etaSteps The number of wedges in the fan of tracks covered by the eta
 * scan
 * @param A pointer to a second material budget associated to a pixel detector;
 * may be <i>NULL</i>
 */
void Analyzer::analyzeMaterialBudget(MaterialBudget &mb,
                                     const std::vector<double> &momenta,
                                     int etaSteps, MaterialBudget *pm) {
  ...
  // prepare etaStep, phiStep, nTracks, nScans
  if (etaSteps > 1)
    etaStep = getEtaMaxMaterial() / (double)(etaSteps - 1);
  else
    etaStep = getEtaMaxMaterial();
  nTracks = etaSteps;
  // reset the number of bins and the histogram boundaries (0.0 to
  // getEtaMaxMaterial()) for all histograms, recalculate the cell boundaries
  setHistogramBinsBoundaries(nTracks, 0.0, getEtaMaxMaterial());
  setCellBoundaries(nTracks, 0.0, geom_max_radius + geom_inactive_volume_width,
                    0.0, getEtaMaxMaterial());
  ...
  for (int i_eta = 0; i_eta < nTracks; i_eta++) {
    ...
    //      services, barrel
    tmp = analyzeInactiveSurfaces(mb.getInactiveSurfaces().getBarrelServices(),
                                  track, sumServicesComponentsRI,
                                  MaterialProperties::no_cat);

    //      services, endcap
    tmp = analyzeInactiveSurfaces(mb.getInactiveSurfaces().getEndcapServices(),
                                  track, sumServicesComponentsRI,
                                  MaterialProperties::no_cat);

    //      supports, barrel
    tmp = analyzeInactiveSurfaces(mb.getInactiveSurfaces().getSupports(), track,
                                  sumServicesComponentsRI,
                                  MaterialProperties::b_sup);

    //      supports, endcap
    tmp = analyzeInactiveSurfaces(mb.getInactiveSurfaces().getSupports(), track,
                                  sumServicesComponentsRI,
                                  MaterialProperties::e_sup);

    //      supports, tubes
    tmp = analyzeInactiveSurfaces(mb.getInactiveSurfaces().getSupports(), track,
                                  sumServicesComponentsRI,
                                  MaterialProperties::o_sup);

    //      supports, barrel tubes
    tmp = analyzeInactiveSurfaces(mb.getInactiveSurfaces().getSupports(), track,
                                  sumServicesComponentsRI,
                                  MaterialProperties::t_sup);

    //      supports, user defined
    tmp = analyzeInactiveSurfaces(mb.getInactiveSurfaces().getSupports(), track,
                                  sumServicesComponentsRI,
                                  MaterialProperties::u_sup);

    //      pixels, if they exist
    std::map<std::string, RILength> ignoredPixelSumComponentsRI;
    std::map<std::string, RILength> ignoredPixelSumServicesComponentsRI;
    if (pm != nullptr) {
      analyzeModules(pm->getBarrelModuleCaps(), track,
                     ignoredPixelSumComponentsRI, true);
      analyzeModules(pm->getEndcapModuleCaps(), track,
                     ignoredPixelSumComponentsRI, true);
      analyzeInactiveSurfaces(pm->getInactiveSurfaces().getBarrelServices(),
                              track, ignoredPixelSumServicesComponentsRI,
                              MaterialProperties::no_cat, true);
      analyzeInactiveSurfaces(pm->getInactiveSurfaces().getEndcapServices(),
                              track, ignoredPixelSumServicesComponentsRI,
                              MaterialProperties::no_cat, true);
      analyzeInactiveSurfaces(pm->getInactiveSurfaces().getSupports(), track,
                              ignoredPixelSumServicesComponentsRI,
                              MaterialProperties::b_sup, true);
    }
    ...
    if (!track.hasNoHits()) {
      ...
      if (nActiveHits > 0) {
        hadronTotalHitsGraph.SetPoint(hadronTotalHitsGraph.GetN(), eta,
                                      nActiveHits);
        double probability;
        std::vector<double> probabilities =
            track.getHadronActiveHitsProbability(trackingTag);
        ...
        for (unsigned int i = 0; i < hadronNeededHitsFraction.size(); ++i) {
          ...
          //}
          hadronGoodTracksFraction.at(i).SetPoint(
              hadronGoodTracksFraction.at(i).GetN(), eta, probability);
        }
      }
    }
    ...
    if (eta >= 0.) {
      ...
      for (std::vector<std::unique_ptr<Hit>>::const_iterator itHit =
               track.getBeginHits();
           itHit != track.getEndHits(); itHit++) {
        auto &hit = *itHit;
        const bool conditionOnHit =
            (!hit->isPixel() && !hit->isIP() && !hit->isBeamPipe());
        if (conditionOnHit) {
          fillRIPlotsPerMechanicalCategoryAndSubdetector(
              radiationAndInteractionLengthPlotsInFullVolume_, hit, eta, theta,
              nTracks, etaMax);
        }
      }
      ...
      if (mb.getTracker().myid() == outer_tracker_id) {
        ...
        // Is the hit within Inner Tracker Tracking Volume ? Outer Tracking
        // Volume ? In between ? Outside ?
        track.assignTrackingVolumesToHits();

        // 2) FILL THE TRACKING VOLUME MATERIAL BUDGET HISTOGRAMS
        computeTrackingVolumeMaterialBudget(
            track, nTracks, ignoredPixelSumComponentsRI, sumComponentsRI);
      }
    }
  ...
  for (unsigned int i = 0; i < cells.size(); i++) {
    for (unsigned int j = 1; j < cells.at(i).size(); j++) {
      cells.at(i).at(j).rlength =
          cells.at(i).at(j).rlength + cells.at(i).at(j - 1).rlength;
      cells.at(i).at(j).ilength =
          cells.at(i).at(j).ilength + cells.at(i).at(j - 1).ilength;
    }
  }
...
void Analyzer::computeTriggerFrequency(Tracker &tracker) {
  ...
  triggerFrequencyMisfilteredSummaries_ =
      v.triggerFrequencyMisfilteredSummaries;
  triggerFrequencyCombinatorialSummaries_ =
      v.triggerFrequencyCombinatorialSummaries;
  triggerFrequencyInterestingSummaries_ =
      v.triggerFrequencyInterestingSummaries;
...
void Analyzer::computeTriggerProcessorsBandwidth(Tracker &tracker) {
  TriggerProcessorBandwidthVisitor v(triggerDataBandwidths_,
 ...
 * @param tracker a reference to the <i>ModuleCap</i> vector of vectors that
 * sits on top of the tracker modules
 * @param result a map of summary tables to be filled
 */
void Analyzer::computeDetailedWeights(
    std::vector<std::vector<ModuleCap>> &tracker,
    std::map<std::string, SummaryTable> &result, bool byMaterial) {
  ...
  */
  ...
  for (layerIt = tracker.begin(); layerIt != tracker.end(); ++layerIt) {
    ...
    for (moduleIt = layerIt->begin(); moduleIt != layerIt->end(); ++moduleIt) {
      ...
      if (myModule->posRef().phi==1) {
        pair<int, int> myIndex = make_pair(myModule->tableRef().row,
  myModule->tableRef().col); tempString = myModule->subdetectorName(); if
  (!typeTaken[tempString][myIndex]) { typeTaken[tempString][myIndex]=true;
          // TODO: put this in a better place
          // (and make a better module typing)
          struct Visitor : public ConstGeometryVisitor {
            std::map<string, SummaryTable>& result;

            Visitor(std::map<std::string, SummaryTable>& result_) :
  result(result_) {} void visit(const BarrelModule& m) { string s = "Ring " +
  any2str(m.ring()); result[s].setCell(0, m.ring(), TagMaker::makePosTag(m));
            }
  ...
  localmassesBegin; it != localmassesEnd; ++it) {
          // if (byMaterial) materialTag =
  myModuleCap->getLocalTag(iLocalMasses); // sort by Material tag
          //  else materialTag = myModuleCap->getLocalTagComp(iLocalMasses); //
  sort by Component tag materialTag = it->first; materialTagIt =
  find(materialTagV.begin(), materialTagV.end(), materialTag); if
  (materialTagIt==materialTagV.end()) { materialTagV.push_back(materialTag);
          }
        }
      }
    }
  ...
  for (map<string, SummaryTable>::iterator it=result.begin(); it!=result.end();
  ++it) { for (unsigned int materialTag_i=0; materialTag_i<materialTagV.size();
  ++materialTag_i) { it->second.setCell(materialTag_i+1, 0,
  materialTagV[materialTag_i]);
    }
  ...
  for (layerIt = tracker.begin(); layerIt != tracker.end(); ++layerIt) {
    ...
    for (moduleIt = layerIt->begin(); moduleIt != layerIt->end(); ++moduleIt) {
      ...
      if (myModule->posRef().phi == 1) {
        ...
        pair<int, int> myIndex = make_pair(myModule->tableRef().row/,
      myModule->tableRef().col); tempString = myModule->subdetectorName(); if
      (!typeWritten[tempString][myIndex]) {
          ...
          if (tempString!="") {
            ...
            if (myModule->subdet() == BARREL) {
              ...
              cerr << "ERROR in Analyzer::detailedWeights(): "
                  << "I found a module which is neither endcap nor barrel!" <<
      std::endl;
            }
            ...
            for (unsigned int materialTag_i=0;
      materialTag_i<materialTagV.size(); ++materialTag_i) { materialTag =
      materialTagV[materialTag_i]; if (byMaterial) { // table by materials try {
      localMaterial = myModuleCap->getLocalMass(materialTag); } catch (exception
      e) { localMaterial = 0; } } else { // table by components try {
      localMaterial = myModuleCap->getLocalMassComp(materialTag); } catch
      (exception e) { localMaterial = 0; }
              }
              //cout << materialTag << "\t"
              //<< localMaterial << "\t"
              //<< endl;
              ...
              tempSS << std::dec << std::fixed << std::setprecision(1) <<
      localMaterial; result[tempString].setCell(materialTag_i+1,
      myModule->tableRef().col, tempSS.str());
            }
            localMaterial = myModuleCap->getLocalMass();
            tempSS.str("");
            tempSS << std::dec << std::fixed << std::setprecision(1) <<
      localMaterial; result[tempString].setCell(materialTagV.size()+1,
      myModule->tableRef().col, tempSS.str()); } else { cerr << "ERROR in
      Analyzer::detailedWeights(): "
                << "I found a module with no reference to the container name."
      << endl;
          }
        }
        }*/
...
void Analyzer::computeWeightSummary(MaterialBudget &mb) {
  ...
  computeDetailedWeights(mb.getBarrelModuleCaps(), barrelComponentWeights,
                         false);
  endcapComponentWeights.clear();
  computeDetailedWeights(mb.getEndcapModuleCaps(), endcapComponentWeights,
                         false);
...
// protected
/**
 * The layer-level analysis function for modules forms the frame for sending a
 * single track through the active modules. It loops through all layers and adds
 * up the results returned from the analysis of each of them.
 * @param tr A reference to the <i>ModuleCap</i> vector of vectors that sits on
 * top of the tracker modules
 ...
 * @param A boolean flag to indicate which set of active surfaces is analysed:
 * true if the belong to a pixel detector, false if they belong to the tracker
 * @return The summed up radiation and interaction lengths for the given track,
 * bundled into a <i>std::pair</i>
 */
RILength
Analyzer::analyzeModules(std::vector<std::vector<ModuleCap>> &tr, Track &track,
                         std::map<std::string, RILength> &sumComponentsRI,
                         bool isPixel) {
...
void printPosRefString(std::ostream &os, const Module &m,
                       const string &delim = " ") {
  os << "subdetectorId=" << m.posRef().subdetectorId << delim
     << "z=" << m.posRef().z << delim << "rho=" << m.posRef().rho << " ("
     << m.center().Rho() << ")" << delim << "phi=" << m.posRef().phi << delim
     << "side=" << m.side();
}

/**
 * The module-level analysis function loops through all modules of a given
 * layer, checking for collisions with the given track. If one is found, the
 * radiation and interaction lengths are scaled with respect to theta, then
 * summed up into a grand total, which is returned. As phi is fixed at the
 * moment and the tracks hit the modules orthogonally with respect to it, it is
 * so far not used to scale the results further.
 * @param layer A reference to the <i>ModuleCap</i> vector linking the
 * collection of material properties to the current layer
 ...
 * @param A boolean flag to indicate which set of active surfaces is analysed:
 * true if the belong to a pixel detector, false if they belong to the tracker
 * @return The scaled and summed up radiation and interaction lengths for the
 * given layer and track, bundled into a <i>std::pair</i>
 */
RILength
Analyzer::findModuleLayerRI(std::vector<ModuleCap> &layer, Track &track,
                            std::map<std::string, RILength> &sumComponentsRI,
                            bool isPixel) {
  ...
  while (iter != guard) {
    // collision detection: rays are in z+ only, so consider only modules that
    // lie on that side only consider modules that have type BarrelModule or
    // EndcapModule
    if (iter->getModule().maxZ() > 0) {
      // same method as in Tracker, same function used
      // TODO: in case origin==0,0,0 and phi==0 just check if sectionYZ and
      // minEta, maxEta
      ...
      if (h.second != HitType::NONE) {
        ...
        // radiation and interaction length scaling for endcaps
          ...
          tmp.radiation =
              tmp.radiation / cos(track.getTheta() + tiltAngle - M_PI / 2);
          tmp.interaction =
              tmp.interaction / cos(track.getTheta() + tiltAngle - M_PI / 2);
        }
        ...
        const std::map<LocalElement, RILength, ComponentNameCompare>
            &modulesComponentsRI = iter->getComponentsRI();
        for (const auto &moduleVolumeIt : modulesComponentsRI) {
          ...
          const RILength &correctedMat =
              computeCorrectedMat(uncorrectedMat, theta, tiltAngle);

          sumComponentsRI[componentName].radiation += correctedMat.radiation;
          sumComponentsRI[componentName].interaction +=
              correctedMat.interaction;

          // TO DO: what the hell is this duplicated work? also, the sum might
          // not even be ok.
        ...
        if (!isPixel)
          fillCell(r, track.getEta(), track.getTheta(), tmp);
        res += tmp;
 ...
 * It loops through all layers and adds up the results returned from the
 ...
 * bundled into a <i>std::pair</i>
 */
RILength Analyzer::findHitsModules(
    std::vector<std::vector<ModuleCap>> &tr,
...
/**
 * The module-level analysis function loops through all modules of a given
 * layer, finds hits and connects them to the track. If one is found, the
 * radiation and interaction lengths are scaled with respect to theta, then
 * summed up into a grand total, which is returned. As phi is fixed at the
 * moment and the tracks hit the modules orthogonally with respect to it, it is
 * so far not used to scale the results further.
 ...
 * given layer and track, bundled into a <i>std::pair</i>
 */
RILength Analyzer::findHitsModuleLayer(std::vector<ModuleCap> &layer, Track &t,
                                       bool isPixel) {
  std::vector<ModuleCap>::iterator iter = layer.begin();
  std::vector<ModuleCap>::iterator guard = layer.end();
  RILength res, tmp;
  ...
  while (iter != guard) {
    ...
    if (h.second != HitType::NONE) {
      ...
      if (iter->getModule().subdet() == BARREL) {
        tmp.radiation =
            tmp.radiation / sin(t.getTheta() + iter->getModule().tiltAngle());
        tmp.interaction =
            tmp.interaction / sin(t.getTheta() + iter->getModule().tiltAngle());
      }
      // radiation and interaction length scaling for endcaps
      else {
...
/**
 * The analysis function for inactive volumes loops through the given vector of
 * elements, checking for collisions with the given track. If one is found, the
 * radiation and interaction lengths are scaled with respect to theta, then
 * summed up into a grand total, which is returned. As all inactive volumes are
 * symmetric with respect to rotation around the z-axis, the track angle phi is
 * not necessary.
 * @param elements A reference to the collection of inactive surfaces that is to
 * be checked for collisions with the track
 * @param eta The pseudorapidity of the current track
 * @param theta The track angle in the yz-plane
 * @param t A reference to the current track object
 * @param cat The category of inactive surfaces that need to be considered
 * within the collection; none if the function is to look at all of them
 * @param A boolean flag to indicate which set of active surfaces isa nalysed:
 * true if the belong to a pixel detector, false if they belong to the tracker
 * @return The scaled and summed up radiation and interaction lengths for the
 * given collection of elements and track, bundled into a <i>std::pair</i>
 */

RILength Analyzer::analyzeInactiveSurfaces(
    std::vector<InactiveElement> &elements, Track &track,
    std::map<std::string, RILength> &sumServicesComponentsRI,
    MaterialProperties::Category cat, bool isPixel) {
  ...
  */
  ...
  while ((iter != guard)) {
    // if  ((iter->getInteractionLength() > 0) && (iter->getRadiationLength() >
    // 0)) {
    // collision detection: rays are in z+ only, so only volumes in z+ need to
    // be considered only volumes of the requested category, or those without
    // one (which should not exist) are examined
      ...
      if ((tmp.first < track.getEta()) && (tmp.second > track.getEta())) {
        ...
        if (eta<0.01) {
          std::cout << "Hitting an inactive surface at z=("
                    << iter->getZOffset() << " to " <<
        iter->getZOffset()+iter->getZLength()
                    << ") r=(" << iter->getInnerRadius() << " to " <<
        iter->getInnerRadius()+iter->getRWidth() << ")" << std::endl; const
        std::map<std::string, double>& localMasses = iter->getLocalMasses(); for
        (auto massIt : localMasses) std::cerr   << "       localMass" <<
        massIt.first << " = " << any2str(massIt.second) << " g" << std::endl;
        }
        */
        ...
        if (iter->isVertical()) {
          ...
          corr.interaction =
              iter->getInteractionLength() / cos(track.getTheta());
          res += corr;
          if (!isPixel) {
        ...
        // radiation and interaction length scaling for horizontal volumes
          ...
          // special treatment for user-defined supports; should not be
          // necessary for now as all user-defined supports are vertical, but
          // just in case...

          corr.radiation = iter->getRadiationLength() / sin(track.getTheta());
          corr.interaction =
              iter->getInteractionLength() / sin(track.getTheta());
          res += corr;
          if (!isPixel) {
            RILength thisLength;
            thisLength.radiation = corr.radiation;
        ...
        if ((iter->getCategory() != MaterialProperties::b_sup) &&
          ...
          sumServicesComponentsRI["Services : others"].radiation +=
              corr.radiation;
          sumServicesComponentsRI["Services : others"].interaction +=
              corr.interaction;
        }
 ...
 * The analysis function for inactive volumes loops through the given vector of
 * elements, checking for collisions with the given track. If one is found, the
 * radiation and interaction lengths are scaled with respect to theta. All hits
 * are added to the given track The total crossed material is returned. As all
 * inactive volumes are symmetric with respect to rotation around the z-axis,
 * the track angle phi is not necessary.
 * @param elements A reference to the collection of inactive surfaces that is to
 * be checked for collisions with the track
 * @param t A reference to the current track object
 ...
 */
RILength
Analyzer::findHitsInactiveSurfaces(std::vector<InactiveElement> &elements,
                                   Track &t, bool isPixel) {
...
void Analyzer::clearGraphsPt(int graphAttributes, const std::string &graphTag) {
  std::map<int, TGraph> &thisRhoGraphs_Pt =
      graphTag.empty()
          ? myGraphBag.getGraphs(graphAttributes | GraphBag::RhoGraph_Pt)
          : myGraphBag.getTaggedGraphs(graphAttributes | GraphBag::RhoGraph_Pt,
                                       graphTag);
  std::map<int, TGraph> &thisPhiGraphs_Pt =
      graphTag.empty()
          ? myGraphBag.getGraphs(graphAttributes | GraphBag::PhiGraph_Pt)
          : myGraphBag.getTaggedGraphs(graphAttributes | GraphBag::PhiGraph_Pt,
                                       graphTag);
  std::map<int, TGraph> &thisDGraphs_Pt =
      graphTag.empty()
          ? myGraphBag.getGraphs(graphAttributes | GraphBag::DGraph_Pt)
          : myGraphBag.getTaggedGraphs(graphAttributes | GraphBag::DGraph_Pt,
                                       graphTag);
  std::map<int, TGraph> &thisCtgThetaGraphs_Pt =
      graphTag.empty()
          ? myGraphBag.getGraphs(graphAttributes | GraphBag::CtgthetaGraph_Pt)
          : myGraphBag.getTaggedGraphs(
                graphAttributes | GraphBag::CtgthetaGraph_Pt, graphTag);
  std::map<int, TGraph> &thisZ0Graphs_Pt =
      graphTag.empty()
          ? myGraphBag.getGraphs(graphAttributes | GraphBag::Z0Graph_Pt)
          : myGraphBag.getTaggedGraphs(graphAttributes | GraphBag::Z0Graph_Pt,
                                       graphTag);
  std::map<int, TGraph> &thisPGraphs_Pt =
      graphTag.empty()
          ? myGraphBag.getGraphs(graphAttributes | GraphBag::PGraph_Pt)
          : myGraphBag.getTaggedGraphs(graphAttributes | GraphBag::PGraph_Pt,
                                       graphTag);
  std::map<int, TGraph> &thisLGraphs_Pt =
      graphTag.empty()
          ? myGraphBag.getGraphs(graphAttributes | GraphBag::LGraph_Pt)
          : myGraphBag.getTaggedGraphs(graphAttributes | GraphBag::LGraph_Pt,
                                       graphTag);
  std::map<int, TGraph> &thisBetaGraphs_Pt =
      graphTag.empty()
          ? myGraphBag.getGraphs(graphAttributes | GraphBag::BetaGraph_Pt)
          : myGraphBag.getTaggedGraphs(graphAttributes | GraphBag::BetaGraph_Pt,
                                       graphTag);
  std::map<int, TGraph> &thisOmegaGraphs_Pt =
      graphTag.empty()
          ? myGraphBag.getGraphs(graphAttributes | GraphBag::OmegaGraph_Pt)
          : myGraphBag.getTaggedGraphs(
                graphAttributes | GraphBag::OmegaGraph_Pt, graphTag);
...
void Analyzer::clearGraphsP(int graphAttributes, const std::string &graphTag) {
  std::map<int, TGraph> &thisRhoGraphs_P =
      graphTag.empty()
          ? myGraphBag.getGraphs(graphAttributes | GraphBag::RhoGraph_P)
          : myGraphBag.getTaggedGraphs(graphAttributes | GraphBag::RhoGraph_P,
                                       graphTag);
  std::map<int, TGraph> &thisPhiGraphs_P =
      graphTag.empty()
          ? myGraphBag.getGraphs(graphAttributes | GraphBag::PhiGraph_P)
          : myGraphBag.getTaggedGraphs(graphAttributes | GraphBag::PhiGraph_P,
                                       graphTag);
  std::map<int, TGraph> &thisDGraphs_P =
      graphTag.empty()
          ? myGraphBag.getGraphs(graphAttributes | GraphBag::DGraph_P)
          : myGraphBag.getTaggedGraphs(graphAttributes | GraphBag::DGraph_P,
                                       graphTag);
  std::map<int, TGraph> &thisCtgThetaGraphs_P =
      graphTag.empty()
          ? myGraphBag.getGraphs(graphAttributes | GraphBag::CtgthetaGraph_P)
          : myGraphBag.getTaggedGraphs(
                graphAttributes | GraphBag::CtgthetaGraph_P, graphTag);
  std::map<int, TGraph> &thisZ0Graphs_P =
      graphTag.empty()
          ? myGraphBag.getGraphs(graphAttributes | GraphBag::Z0Graph_P)
          : myGraphBag.getTaggedGraphs(graphAttributes | GraphBag::Z0Graph_P,
                                       graphTag);
  std::map<int, TGraph> &thisPGraphs_P =
      graphTag.empty()
          ? myGraphBag.getGraphs(graphAttributes | GraphBag::PGraph_P)
          : myGraphBag.getTaggedGraphs(graphAttributes | GraphBag::PGraph_P,
                                       graphTag);
  std::map<int, TGraph> &thisLGraphs_P =
      graphTag.empty()
          ? myGraphBag.getGraphs(graphAttributes | GraphBag::LGraph_P)
          : myGraphBag.getTaggedGraphs(graphAttributes | GraphBag::LGraph_P,
                                       graphTag);
  std::map<int, TGraph> &thisBetaGraphs_P =
      graphTag.empty()
          ? myGraphBag.getGraphs(graphAttributes | GraphBag::BetaGraph_P)
          : myGraphBag.getTaggedGraphs(graphAttributes | GraphBag::BetaGraph_P,
                                       graphTag);
  std::map<int, TGraph> &thisOmegaGraphs_P =
      graphTag.empty()
          ? myGraphBag.getGraphs(graphAttributes | GraphBag::OmegaGraph_P)
          : myGraphBag.getTaggedGraphs(graphAttributes | GraphBag::OmegaGraph_P,
                                       graphTag);
...
/**
 * Calculate the error graphs for the radius curvature, the distance and the
 * angle, for each momentum, and store them internally for later visualisation.
 * @param parameter The list of different momenta that the error graphs are
 * calculated for
 */
void Analyzer::calculateGraphsConstPt(const int &parameter,
  ...
  TGraph &thisRhoGraph_Pt =
      graphTag.empty()
          ? myGraphBag.getGraph(graphAttributes | GraphBag::RhoGraph_Pt,
                                parameter)
          : myGraphBag.getTaggedGraph(graphAttributes | GraphBag::RhoGraph_Pt,
                                      graphTag, parameter);
  TGraph &thisPhiGraph_Pt =
      graphTag.empty()
          ? myGraphBag.getGraph(graphAttributes | GraphBag::PhiGraph_Pt,
                                parameter)
          : myGraphBag.getTaggedGraph(graphAttributes | GraphBag::PhiGraph_Pt,
                                      graphTag, parameter);
  TGraph &thisDGraph_Pt =
      graphTag.empty()
          ? myGraphBag.getGraph(graphAttributes | GraphBag::DGraph_Pt,
                                parameter)
          : myGraphBag.getTaggedGraph(graphAttributes | GraphBag::DGraph_Pt,
                                      graphTag, parameter);
  TGraph &thisCtgThetaGraph_Pt =
      graphTag.empty()
          ? myGraphBag.getGraph(graphAttributes | GraphBag::CtgthetaGraph_Pt,
                                parameter)
          : myGraphBag.getTaggedGraph(graphAttributes |
                                          GraphBag::CtgthetaGraph_Pt,
                                      graphTag, parameter);
  TGraph &thisZ0Graph_Pt =
      graphTag.empty()
          ? myGraphBag.getGraph(graphAttributes | GraphBag::Z0Graph_Pt,
                                parameter)
          : myGraphBag.getTaggedGraph(graphAttributes | GraphBag::Z0Graph_Pt,
                                      graphTag, parameter);
  TGraph &thisPGraph_Pt =
      graphTag.empty()
          ? myGraphBag.getGraph(graphAttributes | GraphBag::PGraph_Pt,
                                parameter)
          : myGraphBag.getTaggedGraph(graphAttributes | GraphBag::PGraph_Pt,
                                      graphTag, parameter);
  TGraph &thisLGraph_Pt =
      graphTag.empty()
          ? myGraphBag.getGraph(graphAttributes | GraphBag::LGraph_Pt,
                                parameter)
          : myGraphBag.getTaggedGraph(graphAttributes | GraphBag::LGraph_Pt,
                                      graphTag, parameter);
  TGraph &thisBetaGraph_Pt =
      graphTag.empty()
          ? myGraphBag.getGraph(graphAttributes | GraphBag::BetaGraph_Pt,
                                parameter)
          : myGraphBag.getTaggedGraph(graphAttributes | GraphBag::BetaGraph_Pt,
                                      graphTag, parameter);
  TGraph &thisOmegaGraph_Pt =
      graphTag.empty()
          ? myGraphBag.getGraph(graphAttributes | GraphBag::OmegaGraph_Pt,
                                parameter)
          : myGraphBag.getTaggedGraph(graphAttributes | GraphBag::OmegaGraph_Pt,
                                      graphTag, parameter);
  ...
  // Prepare plots: pt
  thisRhoGraph_Pt.SetTitle("p_{T} resolution versus #eta - const P_{T} across "
                           "#eta;#eta;#delta p_{T}/p_{T} [%]");
  aName.str("");
  aName << "pt_vs_eta" << momentum << graphTag;
  thisRhoGraph_Pt.SetName(aName.str().c_str());
  // Prepare plots: phi
  thisPhiGraph_Pt.SetTitle("Track azimuthal angle error - const P_{T} across "
                           "#eta;#eta;#delta #phi [deg]");
  aName.str("");
  aName << "phi_vs_eta" << momentum << graphTag;
  thisPhiGraph_Pt.SetName(aName.str().c_str());
  // Prepare plots: d
  thisDGraph_Pt.SetTitle("Transverse impact parameter error - const P_{T} "
                         "across #eta;#eta;#delta d_{0} [#mum]");
  aName.str("");
  aName << "d_vs_eta" << momentum << graphTag;
  thisDGraph_Pt.SetName(aName.str().c_str());
  // Prepare plots: ctg(theta)
  thisCtgThetaGraph_Pt.SetTitle("Track polar angle error - const P_{T} across "
                                "#eta;#eta;#delta ctg(#theta)");
  aName.str("");
  aName << "ctgTheta_vs_eta" << momentum << graphTag;
  thisCtgThetaGraph_Pt.SetName(aName.str().c_str());
  // Prepare plots: z0
  thisZ0Graph_Pt.SetTitle("Longitudinal impact parameter error - const P_{T} "
                          "across #eta;#eta;#delta z_{0} [#mum]");
  aName.str("");
  aName << "z_vs_eta" << momentum << graphTag;
  thisZ0Graph_Pt.SetName(aName.str().c_str());
  // Prepare plots: p
  thisPGraph_Pt.SetTitle(
      "p resolution versus #eta - const P_{T} across #eta;#eta;#delta p/p [%]");
  aName.str("");
  aName << "p_vs_eta" << momentum << graphTag;
  thisPGraph_Pt.SetName(aName.str().c_str());
  // Prepare plots: L
  thisLGraph_Pt.SetTitle("c#tau resolution versus #eta - const P_{T} across "
                         "#eta;#eta;#delta c#tau [#mum]");
  aName.str("");
  aName << "l_vs_eta" << momentum << graphTag;
  thisLGraph_Pt.SetName(aName.str().c_str());
  // Prepare plots: Beta
  thisBetaGraph_Pt.SetTitle("#beta - const P_{T} across #eta;#eta;#beta [rad]");
  aName.str("");
  aName << "beta_vs_eta" << momentum << graphTag;
  thisBetaGraph_Pt.SetName(aName.str().c_str());
  // Prepare plots: Omega
  thisOmegaGraph_Pt.SetTitle(
      "#omega - const P_{T} across #eta;#eta;#omega [rad]");
  aName.str("");
  aName << "omega_vs_eta" << momentum << graphTag;
  ...
  for (const auto &myTrack : aTrackCollection) {
    ...
    /*std::vector<std::pair<Module*,HitType>> hitModules =
    myTrack.getHitModules(); if ( hitModules.at(0).first->getObjectKind() ==
    Active) { std::cout << "hitModules.at(0).first->getResolutionLocalX() = " <<
    hitModules.at(0).first->getResolutionLocalX() << std::endl;
    }*/

    //    std::vector<Hit*> hitModules = myTrack.getHitV();
    //    //std::cout << "hitModules.at(0)->getObjectKind() = " <<
    //    hitModules.at(0)->getObjectKind() << std::endl;
    ...
    if (dctg > 0) {
      ...
      thisCtgThetaGraph_Pt.SetPoint(thisCtgThetaGraph_Pt.GetN(), eta,
                                    graphValue);
    }
    if (dz0 > 0) {
    ...
    if ((dd0 > 0) && (dz0 > 0)) {
      double resolution = (cos(theta) * cos(theta) + 1) / dd0 / dd0 +
                          sin(theta) * sin(theta) / dz0 / dz0;
      resolution = sqrt(2 / resolution) / Units::um; // resolution in um
      thisLGraph_Pt.SetPoint(thisLGraph_Pt.GetN(), eta, resolution);

      double beta = (cos(theta) * cos(theta) + 1) * dz0 * dz0 * dz0 /
                    (sin(theta) * sin(theta) * dd0 * dd0 * dd0);
      beta = atan(beta);
      thisBetaGraph_Pt.SetPoint(thisBetaGraph_Pt.GetN(), eta, beta);

      double omega = ((cos(theta) * cos(theta) + 1) -
                      sin(theta) * sin(theta) * dz0 * dz0 / dd0 / dd0) /
                     dz0 / dd0;
      omega = atan(omega);
...
/**
 * Calculate the error graphs for case II with const P across eta: the radius
 * curvature, the distance and the angle, for each momentum, and store them
 * internally for later visualisation.
 * @param parameter The list of different momenta that the error graphs are
 * calculated for
 */
void Analyzer::calculateGraphsConstP(const int &parameter,
  ...
  // Get graphs from graphBag
  TGraph &thisRhoGraph_P =
      graphTag.empty()
          ? myGraphBag.getGraph(graphAttributes | GraphBag::RhoGraph_P,
                                parameter)
          : myGraphBag.getTaggedGraph(graphAttributes | GraphBag::RhoGraph_P,
                                      graphTag, parameter);
  TGraph &thisPhiGraph_P =
      graphTag.empty()
          ? myGraphBag.getGraph(graphAttributes | GraphBag::PhiGraph_P,
                                parameter)
          : myGraphBag.getTaggedGraph(graphAttributes | GraphBag::PhiGraph_P,
                                      graphTag, parameter);
  TGraph &thisDGraph_P =
      graphTag.empty()
          ? myGraphBag.getGraph(graphAttributes | GraphBag::DGraph_P, parameter)
          : myGraphBag.getTaggedGraph(graphAttributes | GraphBag::DGraph_P,
                                      graphTag, parameter);
  TGraph &thisCtgThetaGraph_P =
      graphTag.empty()
          ? myGraphBag.getGraph(graphAttributes | GraphBag::CtgthetaGraph_P,
                                parameter)
          : myGraphBag.getTaggedGraph(graphAttributes |
                                          GraphBag::CtgthetaGraph_P,
                                      graphTag, parameter);
  TGraph &thisZ0Graph_P =
      graphTag.empty()
          ? myGraphBag.getGraph(graphAttributes | GraphBag::Z0Graph_P,
                                parameter)
          : myGraphBag.getTaggedGraph(graphAttributes | GraphBag::Z0Graph_P,
                                      graphTag, parameter);
  TGraph &thisPGraph_P =
      graphTag.empty()
          ? myGraphBag.getGraph(graphAttributes | GraphBag::PGraph_P, parameter)
          : myGraphBag.getTaggedGraph(graphAttributes | GraphBag::PGraph_P,
                                      graphTag, parameter);
  TGraph &thisLGraph_P =
      graphTag.empty()
          ? myGraphBag.getGraph(graphAttributes | GraphBag::LGraph_P, parameter)
          : myGraphBag.getTaggedGraph(graphAttributes | GraphBag::LGraph_P,
                                      graphTag, parameter);
  TGraph &thisBetaGraph_P =
      graphTag.empty()
          ? myGraphBag.getGraph(graphAttributes | GraphBag::BetaGraph_P,
                                parameter)
          : myGraphBag.getTaggedGraph(graphAttributes | GraphBag::BetaGraph_P,
                                      graphTag, parameter);
  TGraph &thisOmegaGraph_P =
      graphTag.empty()
          ? myGraphBag.getGraph(graphAttributes | GraphBag::OmegaGraph_P,
                                parameter)
          : myGraphBag.getTaggedGraph(graphAttributes | GraphBag::OmegaGraph_P,
                                      graphTag, parameter);
  ...
  // Prepare plots: pt
  thisRhoGraph_P.SetTitle("p_{T} resolution versus #eta - const P across "
                          "#eta;#eta;#delta p_{T}/p_{T} [%]");
  aName.str("");
  aName << "pt_vs_eta" << momentum << graphTag;
  thisRhoGraph_P.SetName(aName.str().c_str());
  // Prepare plots: phi
  thisPhiGraph_P.SetTitle("Track azimuthal angle error - const P across "
                          "#eta;#eta;#delta #phi [deg]");
  aName.str("");
  aName << "phi_vs_eta" << momentum << graphTag;
  thisPhiGraph_P.SetName(aName.str().c_str());
  // Prepare plots: d
  thisDGraph_P.SetTitle("Transverse impact parameter error - const P across "
                        "#eta;#eta;#delta d_{0} [#mum]");
  aName.str("");
  aName << "d_vs_eta" << momentum << graphTag;
  thisDGraph_P.SetName(aName.str().c_str());
  // Prepare plots: ctg(theta)
  thisCtgThetaGraph_P.SetTitle(
      "Track polar angle error - const P across #eta;#eta;#delta ctg(#theta)");
  aName.str("");
  aName << "ctgTheta_vs_eta" << momentum << graphTag;
  thisCtgThetaGraph_P.SetName(aName.str().c_str());
  // Prepare plots: z0
  thisZ0Graph_P.SetTitle("Longitudinal impact parameter error - const P across "
                         "#eta;#eta;#delta z_{0} [#mum]");
  aName.str("");
  aName << "z_vs_eta" << momentum << graphTag;
  thisZ0Graph_P.SetName(aName.str().c_str());
  // Prepare plots: p
  thisPGraph_P.SetTitle(
      "p resolution versus #eta - const P across #eta;#eta;#delta p/p [%]");
  aName.str("");
  aName << "p_vs_eta" << momentum << graphTag;
  thisPGraph_P.SetName(aName.str().c_str());
  // Prepare plots: L
  thisLGraph_P.SetTitle("c#tau resolution versus #eta - const P across "
                        "#eta;#eta;#delta c#tau [#mum]");
  aName.str("");
  aName << "l_vs_eta" << momentum << graphTag;
...
/**
 * Creates the modules' parametrized spatial resolution profiles and
 * distributions
 * @param taggedTrackPtCollectionMap Tagged collections of tracks
 */
void Analyzer::calculateParametrizedResolutionPlots(
    std::map<std::string, TrackCollectionMap> &taggedTrackPtCollectionMap) {
  ...
  for (auto &ttcmIt : taggedTrackPtCollectionMap) {
    ...
    parametrizedResolutionLocalXBarrelMap[myTag].SetNameTitle(
        "resoXBarMap",
        "Resolution on local X coordinate vs cotg(alpha) (barrel modules)");
    parametrizedResolutionLocalXBarrelMap[myTag].SetBins(
        nBins, incidentAngleXBarrelMin, incidentAngleXBarrelMax, nBins,
        resoXMin, resoXMax);
    parametrizedResolutionLocalXBarrelMap[myTag].GetXaxis()->SetTitle(
        "cotg(alpha)");
    parametrizedResolutionLocalXBarrelMap[myTag].GetYaxis()->SetTitle(
        "resolutionLocalX [um]");
    ...
    parametrizedResolutionLocalXEndcapsMap[myTag].SetNameTitle(
        "resoXEndMap",
        "Resolution on local X coordinate vs cotg(alpha) (endcaps modules)");
    parametrizedResolutionLocalXEndcapsMap[myTag].SetBins(
        nBins, incidentAngleXEndcapsMin, incidentAngleXEndcapsMax, nBins,
        resoXMin, resoXMax);
    parametrizedResolutionLocalXEndcapsMap[myTag].GetXaxis()->SetTitle(
        "cotg(alpha)");
    parametrizedResolutionLocalXEndcapsMap[myTag].GetYaxis()->SetTitle(
        "resolutionLocalX [um]");
    ...
    parametrizedResolutionLocalYBarrelMap[myTag].SetNameTitle(
        "resoYBarMap",
        "Resolution on local Y coordinate vs |cotg(beta)| (barrel modules)");
    parametrizedResolutionLocalYBarrelMap[myTag].SetBins(
        nBins, incidentAngleYBarrelMin, incidentAngleYBarrelMax, nBins,
        resoYMin, resoYMax);
    parametrizedResolutionLocalYBarrelMap[myTag].GetXaxis()->SetTitle(
        "|cotg(beta)|");
    parametrizedResolutionLocalYBarrelMap[myTag].GetYaxis()->SetTitle(
        "resolutionLocalY [um]");
    ...
    parametrizedResolutionLocalYEndcapsMap[myTag].SetNameTitle(
        "resoYEndMap",
        "Resolution on local Y coordinate vs |cotg(beta)| (endcaps modules)");
    parametrizedResolutionLocalYEndcapsMap[myTag].SetBins(
        nBins, incidentAngleYEndcapsMin, incidentAngleYEndcapsMax, nBins,
        resoYMin, resoYMax);
    parametrizedResolutionLocalYEndcapsMap[myTag].GetXaxis()->SetTitle(
        "|cotg(beta)|");
    parametrizedResolutionLocalYEndcapsMap[myTag].GetYaxis()->SetTitle(
        "resolutionLocalY [um]");
    ...
    parametrizedResolutionLocalXBarrelDistribution[myTag].SetNameTitle(
        "resoXBarDistr",
        "Distribution of resolutions on local X coordinate (barrel modules)");
    parametrizedResolutionLocalXBarrelDistribution[myTag].SetBins(
        nBins, resoXMin, resoXMax);
    parametrizedResolutionLocalXBarrelDistribution[myTag].GetXaxis()->SetTitle(
        "resolutionLocalX [um]");
    parametrizedResolutionLocalXBarrelDistribution[myTag]
        .GetXaxis()
        ->CenterTitle();

    parametrizedResolutionLocalXEndcapsDistribution[myTag].Reset();
    parametrizedResolutionLocalXEndcapsDistribution[myTag].SetNameTitle(
        "resoXEndDistr",
        "Distribution of resolutions on local X coordinate (endcaps modules)");
    parametrizedResolutionLocalXEndcapsDistribution[myTag].SetBins(
        nBins, resoXMin, resoXMax);
    parametrizedResolutionLocalXEndcapsDistribution[myTag].GetXaxis()->SetTitle(
        "resolutionLocalX [um]");
    parametrizedResolutionLocalXEndcapsDistribution[myTag]
        .GetXaxis()
        ->CenterTitle();

    parametrizedResolutionLocalYBarrelDistribution[myTag].Reset();
    parametrizedResolutionLocalYBarrelDistribution[myTag].SetNameTitle(
        "resoYBarDistr",
        "Distribution of resolutions on local Y coordinate (barrel modules)");
    parametrizedResolutionLocalYBarrelDistribution[myTag].SetBins(
        nBins, resoYMin, resoYMax);
    parametrizedResolutionLocalYBarrelDistribution[myTag].GetXaxis()->SetTitle(
        "resolutionLocalY [um]");
    parametrizedResolutionLocalYBarrelDistribution[myTag]
        .GetXaxis()
        ->CenterTitle();

    parametrizedResolutionLocalYEndcapsDistribution[myTag].Reset();
    parametrizedResolutionLocalYEndcapsDistribution[myTag].SetNameTitle(
        "resoYEndDistr",
        "Distribution of resolutions on local Y coordinate (endcaps modules)");
    parametrizedResolutionLocalYEndcapsDistribution[myTag].SetBins(
        nBins, resoYMin, resoYMax);
    parametrizedResolutionLocalYEndcapsDistribution[myTag].GetXaxis()->SetTitle(
        "resolutionLocalY [um]");
    parametrizedResolutionLocalYEndcapsDistribution[myTag]
        .GetXaxis()
        ->CenterTitle();

    // Incident angles distributions (view from modules)
    incidentAngleLocalXBarrelDistribution_[myTag].Reset();
    incidentAngleLocalXBarrelDistribution_[myTag].SetNameTitle(
        "angleXBarDistr",
        "Distribution of incident angles alpha (barrel modules)");
    incidentAngleLocalXBarrelDistribution_[myTag].SetBins(
        nBinsDistr, incidentAngleXBarrelMin, incidentAngleXBarrelMax);
    incidentAngleLocalXBarrelDistribution_[myTag].GetXaxis()->SetTitle(
        "cotg(alpha)");
    incidentAngleLocalXBarrelDistribution_[myTag].GetXaxis()->CenterTitle();

    incidentAngleLocalXEndcapsDistribution_[myTag].Reset();
    incidentAngleLocalXEndcapsDistribution_[myTag].SetNameTitle(
        "angleXEndDistr",
        "Distribution of incident angles alpha (endcaps modules)");
    incidentAngleLocalXEndcapsDistribution_[myTag].SetBins(
        nBinsDistr, incidentAngleXEndcapsMin, incidentAngleXEndcapsMax);
    incidentAngleLocalXEndcapsDistribution_[myTag].GetXaxis()->SetTitle(
        "cotg(alpha)");
    incidentAngleLocalXEndcapsDistribution_[myTag].GetXaxis()->CenterTitle();

    incidentAngleLocalYBarrelDistribution_[myTag].Reset();
    incidentAngleLocalYBarrelDistribution_[myTag].SetNameTitle(
        "angleYBarDistr",
        "Distribution of incident angles beta (barrel modules)");
    incidentAngleLocalYBarrelDistribution_[myTag].SetBins(
        nBinsDistr, incidentAngleYBarrelMin, incidentAngleYBarrelMax);
    incidentAngleLocalYBarrelDistribution_[myTag].GetXaxis()->SetTitle(
        "|cotg(beta)|");
    incidentAngleLocalYBarrelDistribution_[myTag].GetXaxis()->CenterTitle();

    incidentAngleLocalYEndcapsDistribution_[myTag].Reset();
    incidentAngleLocalYEndcapsDistribution_[myTag].SetNameTitle(
        "angleYEndDistr",
        "Distribution of incident angles beta (endcaps modules)");
    incidentAngleLocalYEndcapsDistribution_[myTag].SetBins(
        nBinsDistr, incidentAngleYEndcapsMin, incidentAngleYEndcapsMax);
    incidentAngleLocalYEndcapsDistribution_[myTag].GetXaxis()->SetTitle(
        "|cotg(beta)|");
    ...
    trackPhiBarrelDistribution_[myTag].SetNameTitle(
        "trackPhiBarDistr",
        "Distribution of incident tracks Phi (barrel modules)");
    ...
    trackPhiEndcapsDistribution_[myTag].SetNameTitle(
        "trackPhiEndDistr",
        "Distribution of incident tracks Phi (endcaps modules)");
    ...
    trackEtaBarrelDistribution_[myTag].SetNameTitle(
        "trackEtaBarDistr",
        "Distribution of incident tracks Eta (barrel modules)");
    ...
    trackEtaEndcapsDistribution_[myTag].SetNameTitle(
        "trackEtaEndDistr",
        "Distribution of incident tracks Eta (endcaps modules)");
    ...
    for (const auto &tcmIt : myTrackCollection) {
      ...
      for (const auto &myTrack : myCollection) {
        ...
        // hit loop
        for (std::vector<std::unique_ptr<Hit>>::const_iterator iHit =
                 myTrack->getBeginHits();
             iHit != myTrack->getEndHits(); iHit++) {

          // In case the tag is "tracker", takes only the outer tracker
          if (myTag != "tracker" ||
              (myTag == "tracker" && !(*iHit)->isPixel())) {
            ...
            if ((*iHit)->isActive() && (*iHit)->getHitModule()) {
              ...
              // If any parameter for resolution on local X coordinate specified
              // for hitModule, fill maps and distributions
              if (hitModule->hasAnyResolutionLocalXParam()) {
                // trackPhi is misleading, as actually the dependency is also in
                // eta (in the forward).
                double trackPhi = myTrack->getPhi();
                double cotAlpha = 1. / tan(hitModule->alpha(trackDirection));
                double resolutionLocalX =
                    hitModule->resolutionLocalX(trackDirection) /
                    Units::um; // um
                if (hitModule->subdet() == BARREL) {
                  trackPhiBarrelDistribution_[myTag].Fill(
                      femod(trackPhi, 2. * M_PI));
                  incidentAngleLocalXBarrelDistribution_[myTag].Fill(cotAlpha);
                  parametrizedResolutionLocalXBarrelDistribution[myTag].Fill(
                      resolutionLocalX);
                  parametrizedResolutionLocalXBarrelMap[myTag].Fill(
                      cotAlpha, resolutionLocalX);
                }
                if (hitModule->subdet() == ENDCAP) {
                  trackPhiEndcapsDistribution_[myTag].Fill(
                      femod(trackPhi, 2. * M_PI));
                  incidentAngleLocalXEndcapsDistribution_[myTag].Fill(cotAlpha);
                  parametrizedResolutionLocalXEndcapsDistribution[myTag].Fill(
                      resolutionLocalX);
                  parametrizedResolutionLocalXEndcapsMap[myTag].Fill(
                      cotAlpha, resolutionLocalX);
                }
              }
              // If any parameter for resolution on local Y coordinate specified
              // for hitModule, fill maps and distributions
              if (hitModule->hasAnyResolutionLocalYParam()) {
                // trackEta is misleading, as actually the dependency is also in
                // track vector's phi (in both barrel and forward).
                double trackEta = myTrack->getEta();
                double absCotBeta =
                    fabs(1. / tan(hitModule->beta(trackDirection)));
                double resolutionLocalY =
                    hitModule->resolutionLocalY(trackDirection) /
                    Units::um; // um
                if (hitModule->subdet() == BARREL) {
                  trackEtaBarrelDistribution_[myTag].Fill(trackEta);
                  incidentAngleLocalYBarrelDistribution_[myTag].Fill(
                      absCotBeta);
                  parametrizedResolutionLocalYBarrelDistribution[myTag].Fill(
                      resolutionLocalY);
                  parametrizedResolutionLocalYBarrelMap[myTag].Fill(
                      absCotBeta, resolutionLocalY);
                }
                if (hitModule->subdet() == ENDCAP) {
                  trackEtaEndcapsDistribution_[myTag].Fill(trackEta);
                  incidentAngleLocalYEndcapsDistribution_[myTag].Fill(
                      absCotBeta);
                  parametrizedResolutionLocalYEndcapsDistribution[myTag].Fill(
                      resolutionLocalY);
                  parametrizedResolutionLocalYEndcapsMap[myTag].Fill(
                      absCotBeta, resolutionLocalY);
                }
              }
            }
...
void Analyzer::clearMaterialBudgetHistograms() {
  ...
  mapInteractionCount.SetTitle(
      "Interaction length hit count map;z [mm];r [mm]");
  ...
  while (hadronTotalHitsGraph.GetN())
    hadronTotalHitsGraph.RemovePoint(0);
  hadronTotalHitsGraph.SetName("hadronTotalHitsGraph");
  while (hadronAverageHitsGraph.GetN())
    hadronAverageHitsGraph.RemovePoint(0);
  ...
  std::sort(hadronNeededHitsFraction.begin(), hadronNeededHitsFraction.end());
  ...
  for (unsigned int i = 0; i < hadronNeededHitsFraction.size(); ++i) {
    tempSS.str("");
    tempSS << "hadronGoodTracksFraction_at" << hadronNeededHitsFraction.at(i);
...
void Analyzer::fillTriggerPerformanceMaps(Tracker &tracker) {
  ...
  SpacingCutVisitor scv(suggestedSpacingMap, suggestedSpacingMapAW,
                        nominalCutMap, moduleOptimalSpacings);
...
void Analyzer::prepareTriggerPerformanceHistograms(
    const int &nTracks, const double &myEtaMax,
    const std::vector<double> &triggerMomenta,
  ...
  // Single maps here
  TH2D &thicknessMap =
      myMapBag.getMaps(mapBag::thicknessMap)[mapBag::dummyMomentum];
  ...
  prepareTrackerMap(suggestedSpacingMap, "suggestedSpacingMap",
                    "Map of nearest available spacing [using standard window]");
  prepareTrackerMap(
      suggestedSpacingMapAW, "suggestedSpacingMapAW",
      "Map of nearest available spacing [using selected windows]");
  ...
  for (vector<double>::const_iterator iter = triggerMomenta.begin();
    ...
    // trigGraphs[*iter].SetTitle("Average triggered points;#eta;Triggered
    // points <N>");
    trigProfiles[*iter].SetTitle(
        "Average triggered points;#eta;Triggered points <N>");
    trigFractionProfiles[*iter].SetTitle(
        "Average trigger efficiency;#eta;Efficiency [%]");
    trigPurityProfiles[*iter].SetTitle("Average stub purity;#eta;Purity [%]");
    aName.str("");
    aName << "triggered_vs_eta" << *iter << "_profile";
    trigProfiles[*iter].SetName(aName.str().c_str());
    aName.str("");
    aName << "triggered_vs_eta" << *iter << "_fractionProfile";
    trigFractionProfiles[*iter].SetName(aName.str().c_str());
    aName.str("");
    aName << "triggered_vs_eta" << *iter << "_purityProfile";
  ...
  // TGraph totalGraph;
  char dummyName[256];
  sprintf(dummyName, "dummyName%d", bsCounter++);
  ...
  // trigGraphs[GraphBag::Triggerable].SetTitle("Average triggered
  // points;#eta;Triggered points <N>");
  trigProfiles[profileBag::Triggerable].SetTitle(
      "Average triggered points;#eta;Triggered points <N>");
  // aName.str(""); aName << "triggerable_vs_eta_graph";
  // trigGraphs[GraphBag::Triggerable].SetName(aName.str().c_str());
  aName.str("");
  aName << "triggerable_vs_eta_profile";
...
void Analyzer::prepareTriggerProcessorHistograms() {
  ...
  TH2D &moduleConnectionEtaMap =
      myMapBag.getMaps(mapBag::moduleConnectionEtaMap)[mapBag::dummyMomentum];
  prepareTrackerMap(moduleConnectionEtaMap, "moduleConnectionEtaMap", "Map");

  myMapBag.clearMaps(mapBag::moduleConnectionPhiMap);
  TH2D &moduleConnectionPhiMap =
      myMapBag.getMaps(mapBag::moduleConnectionPhiMap)[mapBag::dummyMomentum];
  prepareRadialTrackerMap(moduleConnectionPhiMap, "moduleConnectionPhiMap",
                          "Map");

  myMapBag.clearMaps(mapBag::moduleConnectionEndcapPhiMap);
  TH2D &moduleConnectionEndcapPhiMap = myMapBag.getMaps(
      mapBag::moduleConnectionEndcapPhiMap)[mapBag::dummyMomentum];
  prepareRadialTrackerMap(moduleConnectionEndcapPhiMap,
                          "moduleConnectionEndcapPhiMap", "Map");
...
void Analyzer::clearGeometryHistograms() {
  ...
  mapPhiEtaDTC.Reset();
  mapPhiEtaDTC.SetNameTitle("mapPhiEtaDTC",
                            "Number of distinct DTCs per track;#phi;#eta");
  etaProfileCanvas.SetName("etaProfileCanvas");
  etaProfileCanvas.SetTitle("Eta Profiles");
  hitDistribution.Reset();
  hitDistribution.SetNameTitle("hitDistribution", "Hit distribution");
  // geomLite->SetName("geometryLite");   geomLite->SetTitle("Modules
  // geometry"); geomLiteXY->SetName("geometryLiteXY");
  // geomLiteXY->SetTitle("Modules geometry (XY Section)");
  // geomLiteYZ->SetName("geometryLiteYZ"); geomLiteYZ->SetTitle("Modules
  // geometry (EC Section)"); geomLiteEC->SetName("geometryLiteEC");
  // geomLiteEC->SetTitle("Modules geometry (Endcap)");

  // Power density
  while (powerDensity.GetN())
    powerDensity.RemovePoint(0);
  powerDensity.SetName("powerdensity");
  powerDensity.SetTitle(
      "Power density;Total area [m^{2}];Power density [kW / m^{2}]");
}

/**
 * This convenience function sets all values in the internal array <i>cells</i>
 * to zero.
 */
...
/**
 * This convenience function sets the number of bins and the lower and upper
 * range for their contents for each of the available histograms.
...
void Analyzer::setHistogramBinsBoundaries(int bins, double min, double max) {
  // isolines
  isor.SetBins(bins, 0.0, geom_max_length, bins / 2, 0.0,
               geom_max_radius + geom_inactive_volume_width);
  isoi.SetBins(bins, 0.0, geom_max_length, bins / 2, 0.0,
               geom_max_radius + geom_inactive_volume_width);
  // Material distribution maps
  const double materialMapMaxY =
      (geom_max_radius + geom_inactive_volume_width) * geom_safety_factor;
  const double materialMapMaxX = geom_max_length * geom_safety_factor;
  const int materialMapNumBinsY =
      materialMapMaxY / 0.5; // every half a mm (used to be every 5 mm)
  const int materialMapNumBinsX =
      materialMapMaxX / 1.; // every mm (used to be every 5 mm)
  mapRadiation.SetBins(materialMapNumBinsX, 0., materialMapMaxX,
                       materialMapNumBinsY, 0., materialMapMaxY);
  mapInteraction.SetBins(materialMapNumBinsX, 0., materialMapMaxX,
                         materialMapNumBinsY, 0., materialMapMaxY);
  mapRadiationCount.SetBins(materialMapNumBinsX, 0., materialMapMaxX,
                            materialMapNumBinsY, 0., materialMapMaxY);
  mapInteractionCount.SetBins(materialMapNumBinsX, 0., materialMapMaxX,
                              materialMapNumBinsY, 0., materialMapMaxY);
  mapRadiationCalib.SetBins(materialMapNumBinsX, 0., materialMapMaxX,
                            materialMapNumBinsY, 0., materialMapMaxY);
  mapInteractionCalib.SetBins(materialMapNumBinsX, 0., materialMapMaxX,
                              materialMapNumBinsY, 0., materialMapMaxY);
}

/**
 * This convenience function sets the number of bins and the lower and upper
 * range for their contents for each of the cells that make up the basis for the
 * 2D histograms.
 * @param bins The number of bins in eta; the number of bins in r will be half
 * that
 ...
 */
void Analyzer::setCellBoundaries(int bins, double minr, double maxr,
 ...
 */
void Analyzer::fillMapRT(const double &r, const double &theta,
                         const RILength &mat) {
 ...
 * @param il The local interaction length
 */
void Analyzer::fillMapRZ(const double &r, const double &z,
                         const RILength &mat) {
  if (mat.radiation > 0) {
    mapRadiation.Fill(z, r, mat.radiation);
    mapRadiationCount.Fill(z, r);
...
TH2D &Analyzer::getHistoMapRadiation() {
  ...
  for (int iBin = 1; iBin <= nBins; ++iBin) {
    ...
    if (count == 1)
      ...
      mapRadiationCalib.SetBinContent(iBin, content / double(count));
...
TH2D &Analyzer::getHistoMapInteraction() {
  ...
  for (int iBin = 1; iBin <= nBins; ++iBin) {
    ...
    if (count == 1)
      ...
      mapInteractionCalib.SetBinContent(iBin, content / double(count));
...
/**
 * This function assigns the local radiation and interaction lengths of a
 * detected hit to their position in the (eta, r) space.
 * @param r The radius at which the hit was detected
...
void Analyzer::fillCell(double r, double eta, double theta, RILength mat) {
  ...
  if (cells.size() > 0) {
    for (rindex = 0; (unsigned int)rindex < cells.at(0).size(); rindex++) {
      if ((cells.at(0).at(rindex).rmin <= r) &&
          (cells.at(0).at(rindex).rmax > r))
        break;
    }
    if ((unsigned int)rindex < cells.at(0).size()) {
      for (etaindex = 0; (unsigned int)etaindex < cells.size(); etaindex++) {
        if ((cells.at(etaindex).at(rindex).etamin <= eta) &&
            (cells.at(etaindex).at(rindex).etamax > eta))
          break;
      }
      if ((unsigned int)etaindex < cells.size()) {
        cells.at(etaindex).at(rindex).rlength =
            cells.at(etaindex).at(rindex).rlength + rl;
        cells.at(etaindex).at(rindex).ilength =
            cells.at(etaindex).at(rindex).ilength + il;
      }
    }
  }
}
...
void Analyzer::transformEtaToZ() {
  ...
  for (int i = 1; i <= size_z; i++) {
    ...
    for (int j = 1; j <= size_r; j++) {
      ...
      if ((etaindex >= 0) && (rindex >= 0))
        isor.Fill(z, r, cells.at(etaindex).at(rindex).rlength);
    }
  }
  // init: sizes and boundaries
  ...
  for (int i = 0; i < size_z; i++) {
    ...
    for (int j = 0; j < size_r; j++) {
      ...
      if ((etaindex >= 0) && (rindex >= 0))
        isoi.Fill(z, r, cells.at(etaindex).at(rindex).ilength);
    }
  }
}
...
// private
/**
 * This is a convenience function that converts a radius to an index into the
 * second dimension of <i>cells</i>.
  ...
  if (r >= 0) {
    if (cells.size() > 0) {
      ...
      while (index < (int)cells.at(0).size()) {
        if (cells.at(0).at(index).rmax < r)
          index++;
        else
          break;
      }
      if (index == (int)cells.at(0).size())
        index = -1;
      return index;
    }
  }
...
/**
 * This is a convenience function that converts an eta value to an index into
 * the first dimension of <i>cells</i>.
  ...
  if (eta >= 0) {
    ...
    while (index < (int)cells.size()) {
      if (cells.at(index).size() > 0) {
        if (cells.at(index).at(0).etamax < eta)
          index++;
        else
          break;
      } else {
...
Analyzer::computeMinMaxTracksEta(const Tracker &t) const {
  std::pair<double, double> etaMinMax = t.computeMinMaxEta();
  if (SimParms::getInstance().maxTracksEta.state())
    etaMinMax.second = SimParms::getInstance().maxTracksEta();
  if (SimParms::getInstance().minTracksEta.state())
    etaMinMax.first = SimParms::getInstance().minTracksEta();
  return etaMinMax;
}
 ...
 * @param nTracker the number of tracks to be used to analyze the coverage
 * (defaults to 1000)
 */
void Analyzer::analyzeGeometry(Tracker &tracker, int nTracks /*=1000*/) {
  ...
  std::map<std::string, int>
      moduleTypeCount; // counts hit per module type -- if any of the sensors
                       // (or both) are hit, it counts 1
  std::map<std::string, int>
      sensorTypeCount; // counts hit per sensor on module type -- if one sensor
                       // is hit, it counts 1, if both sensors are hit, it
                       // counts 2
  std::map<std::string, int>
      moduleTypeCountStubs; // counts stubs per module type -- if both sensors
                            // are hit and a stub is formed, it counts 1
  ...
  double randomSpan =
      (etaMinMax.second - etaMinMax.first) * (1. + randomPercentMargin);
  double randomBase = etaMinMax.first - (etaMinMax.second - etaMinMax.first) *
                                            (randomPercentMargin) / 2.;
  ...
  TH2I mapPhiEtaCount("mapPhiEtaCount ", "phi Eta hit count", nBlocks,
                      -1 * M_PI, M_PI, nBlocks, -maxEta, maxEta);
  ...
  totalEtaProfile.SetTitle(
      "Number of modules with at least one hit;#eta;Number of hit modules");
  ...
  if (!tracker.isPixelTracker()) {
    totalEtaProfileStubs.SetTitle(
        "Number of modules with a stub;#eta;Number of stubs");
  } else {
    totalEtaProfileStubs.SetTitle("Number of stubs;#eta;Number of stubs");
  }
  ...
  const int plotMaxNumberOfStubs =
      (!tracker.isPixelTracker() ? plotMaxNumberOfOuterTrackerStubs
                                 : plotMaxNumberOfInnerTrackerStubs);
  for (int numberOfStubs = 0; numberOfStubs <= plotMaxNumberOfStubs;
       numberOfStubs++) {
    TProfile stubProfile = TProfile(
        Form("layerEtaCoverageProfileNumberOfStubs%d", numberOfStubs),
  ...
  totalEtaProfileLayers.SetTitle(
      "Number of layers with at least a hit;#eta;Number of layers");
  ...
  std::map<std::string, int>
      modulePlotColors; // CUIDADO quick and dirty way of creating a map with
                        // all the module colors (a cleaner way would be to have
                        // the map already created somewhere else)
  ...
  for (int i = 0; i < nTracksPerSide; i++) {
    for (int j = 0; j < nTracksPerSide; j++) {
      ...
      std::vector<std::pair<Module *, HitType>> hitModules =
          trackHit(getLuminousRegion(), aLine.first, tracker.modules());
      ...
      for (auto &mh : hitModules) {
        ...
        if (mh.second == HitType::INNER || mh.second == HitType::OUTER) {
          // Important NB: Concerning Inner Tracker modules, all hits are of
          // type INNER
      ...
      for (auto &mh : hitModules) {
        ...
        if (myDTC)
          hitModulesDTC.insert(myDTC->name());
      }
      ...
      // Fill other plots
      mapPhiEta.Fill(aLine.first.Phi(), aLine.second,
                     hitModules.size()); // phi, eta 2d plot
      mapPhiEtaDTC.Fill(aLine.first.Phi(), aLine.second,
                        hitModulesDTC.size()); // phi, eta  DTC 2d plot
      mapPhiEtaCount.Fill(aLine.first.Phi(),
                          aLine.second); // Number of shot tracks

      totalEtaProfile.Fill(fabs(aLine.second),
                           hitModules.size()); // Total number of hits
      totalEtaProfileSensors.Fill(fabs(aLine.second), numHits);

      // COVERAGE PER LAYER (fill 'per layer' plots and compute parameters of
      // interest).
      const std::pair<int, int> &coveragePerLayer = computeCoveragePerLayer(
          aLine, hitModules, layerNames, tracker.isPixelTracker(), maxEta);
      ...
      const int numStubsPerTrack =
          (!tracker.isPixelTracker() ? numOuterTrackerStubs
                                     : numInnerTrackerStubs);

      // COVERAGE FOR ALL LAYERS (use the 'per layer' information)
      computeCoveragePlotsAllLayers(aLine, numLayersWithAtLeastOneHit,
                                    numStubsPerTrack, plotMaxNumberOfStubs);
    }
  }
  ...
  const double plotNumberOfHitModulesMaxY =
      (!tracker.isPixelTracker() ? plotNumberOfOuterTrackerHitModulesMaxY
                                 : plotNumberOfInnerTrackerHitModulesMaxY);
  if (totalEtaProfile.GetMaximum() < plotNumberOfHitModulesMaxY)
    totalEtaProfile.SetMaximum(plotNumberOfHitModulesMaxY);
  if (totalEtaProfileSensors.GetMaximum() < plotNumberOfHitsMaxY)
    totalEtaProfileSensors.SetMaximum(plotNumberOfHitsMaxY);
  if (totalEtaProfileStubs.GetMaximum() < plotNumberOfStubsMaxY)
    totalEtaProfileStubs.SetMaximum(plotNumberOfStubsMaxY);
  const double plotNumberOfHitLayersMaxY =
      (!tracker.isPixelTracker() ? plotNumberOfOuterTrackerHitLayersMaxY
                                 : plotNumberOfInnerTrackerHitLayersMaxY);
  if (totalEtaProfileLayers.GetMaximum() < plotNumberOfHitLayersMaxY)
    totalEtaProfileLayers.SetMaximum(plotNumberOfHitLayersMaxY);
  ...
  for (std::map<std::string, TProfile>::iterator it = etaProfileByType.begin();
    ...
    savingGeometryV.push_back(*myProfile); // TODO: remove savingGeometryV
                                           // everywhere :-) [VERY obsolete...]
  ...
  for (std::map<std::string, TProfile>::iterator it =
           etaProfileByTypeSensors.begin();
  ...
  for (auto aModule : tracker.modules()) {
    ...
    typeToPower[aSensorType] +=
        aModule->sensorsIrradiationPowerMean() / 1e3; // in kW
  }
  ...
  for (std::map<std::string, int>::iterator typesIt = typeToCount.begin();
    ...
    powerDensity.SetPoint(iPoints++, typeToSurface[aSensorType],
                          typeToPower[aSensorType] /
                              typeToSurface[aSensorType]);
  }

  // Normalize the counts of IT stubs with 3 hits in ring transitions with the
  // number of tracks.
  for (auto &diskIt : stubWith3HitsCountPerDiskAndRing_) {
    ...
    for (auto &ringTransitionIndexIt : stubWith3HitsCountPerRing) {
      ...
      stubWith3HitsCountPerDiskAndRing_[diskName][ringTransitionIndex] /=
          nTracks;
    }
  }
...
// Moreover now we create them by calling the tracker object, which seems
// improper
void Analyzer::createGeometryLite(Tracker &tracker) {
  if (!(geomLiteCreated && geomLiteXYCreated && geomLiteYZCreated &&
        geomLiteECCreated)) {
 ...
 */
int Analyzer::createResetCounters(Tracker &tracker,
 ...
 */
std::pair<XYZVector, double> Analyzer::shootDirection(double minEta,
                                                      double spanEta) {
  ...
  // Direction
  result.first =
      XYZVector(cos(phi) * sin(theta), sin(phi) * sin(theta), cos(theta));
  result.second = eta;
  return result;
}
 ...
 */
std::vector<std::pair<Module *, HitType>>
Analyzer::trackHit(const XYZVector &origin, const XYZVector &direction,
                   Tracker::Modules &moduleV) {
  std::vector<std::pair<Module *, HitType>> result;
  static const double BoundaryEtaSafetyMargin =
      5.; // track origin shift in units of zError to compute boundaries
  ...
  for (auto &m : moduleV) {
    ...
    if (m->couldHit(direction, SimParms::getInstance().lumiRegZError() *
                                   BoundaryEtaSafetyMargin)) {
...
void Analyzer::computeBandwidth(Tracker &tracker) {
  BandwidthVisitor bv(chanHitDistribution, bandwidthDistribution,
                      bandwidthDistributionSparsified);
  SimParms::getInstance().accept(bv);
  tracker.accept(bv);
}

std::vector<double> Analyzer::average(TGraph &myGraph,
 ...
 * - number of Inner Tracker 'stubs' (in the Inner Tracker, a 'stub' on a layer
 * is defined by at least 2 hits on that layer). NB: This method is called for
 * each track!!
 */
const std::pair<int, int> Analyzer::computeCoveragePerLayer(
    const std::pair<XYZVector, double> &aLine,
    const std::vector<std::pair<Module *, HitType>> &hitModules,
    const LayerNameVisitor &layerNames, const bool isPixelTracker,
    const double maxEta) {
  ...
  for (const auto &layerName : layerNames.data) {
    ...
    // TEPX only: luminosity measurements. This is used to count IT stubs with 3
    // hits.
    const bool isTEPX =
        (layerName.find(insur::lumi_subdetector) != std::string::npos);
    ...
    for (auto mh : hitModules) {
      ...
      if (layerName == (ur.subdetectorName + " " + any2str(ur.layer))) {
        ...
        // OT stubs
        if (mh.second == HitType::STUB)
          hasLayerAtLeastOneStub = 1;

        // Luminosity measurements
        if (isTEPX) {
          hitTEPXRingsIndexes.push_back(ur.ring);
        } // add hit rings indexes
      }
    } // end of loop on hit modules

    // Compute Inner Tracker stubs info
    if (isPixelTracker) {
      computeInnerTrackerStubsInfoPerLayer(
          numHitsPerLayer, isTEPX, hitTEPXRingsIndexes, layerName,
          numInnerTrackerStubs, hasLayerAtLeastOneStub,
          hasLayerOneStubWith3Hits);
    }

    // Compute number of layers with at least one hit
    if (hasLayerAtLeastOneHit)
      numLayersWithAtLeastOneHit++;
    ...
    const int plotMaxNumberOfHitsPerLayer =
    ...
    for (const auto &numHitsIndexIt : hitCoveragePerLayerDetails_[layerName]) {
      ...
      if (numHitsIndex == numHitsPerLayer ||
          (numHitsIndex == plotMaxNumberOfHitsPerLayer &&
           numHitsPerLayer >= plotMaxNumberOfHitsPerLayer)) {
        result = 1;
      }
      hitCoveragePerLayerDetails_[layerName][numHitsIndex].Fill(aLine.second,
                                                                result);
    }
    ...
    if (isPixelTracker) {
      stubWith3HitsCoveragePerLayer_[layerName].Fill(
          eta, hasLayerOneStubWith3Hits); // fill plots
    }
  } // loop on all layers
 ...
 */
void Analyzer::computeInnerTrackerStubsInfoPerLayer(
    const int numHitsPerLayer, const bool isTEPX,
    const std::vector<int> &hitTEPXRingsIndexes, const std::string layerName,
    int &numInnerTrackerStubs, int &hasLayerAtLeastOneStub,
    int &hasLayerOneStubWith3Hits) {

  // IT STUBS WITH >= 2 HITS / LAYER
  const int numInnerTrackerStubsPerLayer =
      (numHitsPerLayer >= 2 ? 1 : 0); // 1 stub <-> (>= 2 hits / layer).
  numInnerTrackerStubs +=
      numInnerTrackerStubsPerLayer; // this is the definition of a IT stub
                                    // generally used.
  if (numInnerTrackerStubsPerLayer >= 1)
    hasLayerAtLeastOneStub = 1;

  // IT STUBS WITH >= 3 HITS / LAYER
  // TEPX only: luminosity measurements.
  const int numInnerTrackerStubsWith3HitsPerLayer =
      (numHitsPerLayer >= 3 ? 1 : 0); // 1 stub <-> (>= 3 hits / layer).
  if (numInnerTrackerStubsWith3HitsPerLayer >= 1) {
    hasLayerOneStubWith3Hits = 1;
    if (isTEPX) {
      const auto &hitTEPXRingsIndexesMinMax = std::minmax_element(
          hitTEPXRingsIndexes.begin(), hitTEPXRingsIndexes.end());
      if (hitTEPXRingsIndexesMinMax.first != hitTEPXRingsIndexes.end() &&
          hitTEPXRingsIndexesMinMax.second != hitTEPXRingsIndexes.end()) {
        const int minRingIndex =
            *hitTEPXRingsIndexesMinMax.first; // MIN hit ring index
        const int maxRingIndex =
            *hitTEPXRingsIndexesMinMax.second; // MAX hit ring index
        // Hit rings by a given track should be adjacent.
        if (maxRingIndex != (minRingIndex + 1)) {
          logERROR("Found a stub with 3 hits. But MIN(hitTEPXRingsIndexes) = " +
                   any2str(minRingIndex) +
                   " and MAX(hitTEPXRingsIndexes) = " + any2str(maxRingIndex));
        }
        stubWith3HitsCountPerDiskAndRing_[layerName][minRingIndex] +=
            1.; // KEY POINT: If there is a stub with 3 hits,
...
void Analyzer::createCoveragePerLayerPlots(const LayerNameVisitor &layerNames,
  ...
  for (const auto &layerName : layerNames.data) {
    // HITS (>=1) PER LAYER
    TProfile hitsPerLayer =
        TProfile(Form("hitCoveragePerLayer%s", layerName.c_str()),
                 layerName.c_str(), 200, maxEta, maxEta);
    ...
    for (int numberOfHits = 1; numberOfHits <= plotMaxNumberOfHitsPerLayer;
         numberOfHits++) {
      TProfile hitsCountPerLayer =
          TProfile(Form("hitCoveragePerLayerDetails%s%d", layerName.c_str(),
                        numberOfHits),
                   layerName.c_str(), 100, maxEta, maxEta);
    ...
    const std::string stubsPerLayerXAxisTitle =
        (!isPixelTracker ? "#eta" : "|#eta|");
    // For Inner Tracker stubs per layer, all results gathered on (+Z) side.
    // Indeed, the worry is not about hermetic coverage as for OT, but to have
    // very good resolution picks to distinguish rings.
    TProfile stubsPerLayer =
        TProfile(Form("stubCoveragePerLayer%s", layerName.c_str()),
                 layerName.c_str(), 400, stubsPerLayerMinX, maxEta);
    ...
    // IT only: 1 stub <-> (>= 3 hits / layer).
    if (isPixelTracker) {
      TProfile stubsWith3HitsPerLayer =
          TProfile(Form("stubWith3HitsCoveragePerLayer%s", layerName.c_str()),
                   layerName.c_str(), 600, stubsPerLayerMinX, maxEta);
      // For Inner Tracker stubs per layer, all results gathered on (+Z) side.
      // Indeed, the worry is not about hermetic coverage as for OT, but to have
      // very good resolution picks to distinguish rings.
...
/*
 * Compute the coverage plots for all layers. These are more specifically the
 * plots which need the 'per layer' information: 'Number of layers with at least
 * one hit' plot. 'Stub coverage' plot (because of the Inner Tracker stubs,
 * which are defined 'per layer'). NB: This method is called for each track!!
 */
void Analyzer::computeCoveragePlotsAllLayers(
    const std::pair<XYZVector, double> &aLine,
    const int numLayersWithAtLeastOneHit, const int numStubsPerTrack,
    const int plotMaxNumberOfStubs) {
  ...
  for (const auto &numStubsIndexIt : tracksDistributionPerNumberOfStubs_) {
    ...
    if (numStubsIndex == numStubsPerTrack ||
        (numStubsIndex == plotMaxNumberOfStubs &&
         numStubsPerTrack > plotMaxNumberOfStubs)) {
      result = 1;
    }
    tracksDistributionPerNumberOfStubs_[numStubsIndex].Fill(fabs(aLine.second),
                                                            result);
  }
}
 ...
 */
void Analyzer::computeTrackingVolumeMaterialBudget(
    const Track &track, const int nTracks,
    const std::map<std::string, RILength> &innerTrackerModulesComponentsRI,
    const std::map<std::string, RILength> &outerTrackerModulesComponentsRI) {
  const double eta = track.getEta();
  const double theta = track.getTheta();
  const double etaMax = getEtaMaxMaterial();

  // BEAM PIPE MATERIAL BUDGET
  // Material belonging to the Beam Pipe, and located before an active hit on
  // the Tracker.
  for (std::vector<std::unique_ptr<Hit>>::const_iterator itHit =
           track.getBeginHits();
       itHit != track.getEndHits(); itHit++) {
    ...
    if (hit->isTotalTrackingVolume() && hit->isBeamPipe()) {
      ...
      fillRIComponentsHistos(rComponentsBeamPipe, iComponentsBeamPipe,
                             beam_pipe, correctedMat, eta, nTracks, etaMax);
    }

    // MATERIAL BUDGET UNDER INNER TRACKER TRACKING VOLUME
    // Material not belonging to the Beam Pipe, and located before an active hit
    // on the Inner Tracker.
    if (hit->isPixelIntersticeVolume() && !hit->isBeamPipe()) {
      const RILength &correctedMat = hit->getCorrectedMaterial();
      fillRIComponentsHistos(rComponentsPixelInterstice,
                             iComponentsPixelInterstice,
                             services_under_pixel_tracking_volume, correctedMat,
                             eta, nTracks, etaMax);
    }
  }

  // MATERIAL BUDGET WITHIN INNER TRACKER TRACKING VOLUME: A + B + C
  // Material (IT or OT) located between first and last active hit on the Inner
  // Tracker.
  ...
  for (std::vector<std::unique_ptr<Hit>>::const_iterator itHit =
    ...
    if (hit->isPixelTrackingVolume()) {
      ...
      fillRIPlotsPerMechanicalCategoryAndSubdetector(
          radiationAndInteractionLengthPlotsInPixelTrackingVolume_, hit, eta,
          theta, nTracks, etaMax);
    }

    // MATERIAL BUDGET BETWEEN INNER TRACKER AND OUTER TRACKER TRACKING VOLUMES
    // Material (IT or OT) located between last active hit on the Inner Tracker
    // and first active hit on the Outer Tracker.
    if (hit->isIntersticeVolume()) {
      const RILength &correctedMat = hit->getCorrectedMaterial();
      fillRIComponentsHistos(rComponentsInterstice, iComponentsInterstice,
                             services_and_supports_in_interstice, correctedMat,
                             eta, nTracks, etaMax);
    }
  }

  // MATERIAL BUDGET WITHIN OUTER TRACKER TRACKING VOLUME: D + E + F
  // Material (IT or OT) located between first and last active hit on the Outer
  // Tracker.
  ...
  for (std::vector<std::unique_ptr<Hit>>::const_iterator itHit =
    ...
    if (hit->isOuterTrackingVolume()) {
      ...
      fillRIPlotsPerMechanicalCategoryAndSubdetector(
          radiationAndInteractionLengthPlotsInOuterTrackingVolume_, hit, eta,
          theta, nTracks, etaMax);
    }
  }
}
 ...
 * One needs to correct the MB, ie take into account the angle of the track
 * crossing the volumes. Then Analyzer::fillRIComponentsHistos is directly used.
 */
void Analyzer::fillRIPlotsPerMechanicalCategoryAndSubdetector(
    RIPlotsPerComponentAndPerSubdetectorAndPerMechanicalCategory
        &radiationAndInteractionLengthPerMechanicalCategoryAndSubdetector,
    const std::unique_ptr<Hit> &hit, const double eta, const double theta,
    const int nTracks, const double etaMax) const {
  ...
  if (hitInactive == nullptr && hitModule == nullptr) {
    logERROR("Analyzer::fillRIServicesDetailsHistos : Hit neither on module "
             "nor on inactive element!! ");
  }
    ...
    for (const auto &volumeElementsIt : allComponentsRI) {
      ...
      fillRIComponentsHistos(
          radiationAndInteractionLengthPerMechanicalCategoryAndSubdetector
              [mechanicalCategory][subdetectorName]
                  .first,
          radiationAndInteractionLengthPerMechanicalCategoryAndSubdetector
              [mechanicalCategory][subdetectorName]
                  .second,
          componentName, correctedMat, eta, nTracks, etaMax);
    }
  }
}

/* Fill histograms with corrected Material Budget: left pad for Radiation
 * Length, right pad for Interaction Length. The MB is split by component
 * category.
 */
void Analyzer::fillRIComponentsHistos(
    std::map<std::string, TH1D *> &rComponentsHistos,
    std::map<std::string, TH1D *> &iComponentsHistos,
    const std::string componentName, const RILength &correctedMat,
    const double eta, const int nTracks, const double etaMax) const {
...
/* Calculate corrected MB, ie the MB which takes into account the angle of the
 * track crossing the volumes.
 */
const RILength Analyzer::computeCorrectedMat(const RILength &uncorrectedMat,
                                             const double theta,
  ...
  if ((theta + tiltAngle) < insur::geom_zero) {
    logERROR("Unsupported case: theta + tiltAngle ~ 0. Assume all corrected MB "
             "is wrong!");
    return RILength();
  }
  // When theta is in [Pi/2 Pi], tiltAngle must be oriented negative.
  const double orientedTiltAngle =
      (theta <= M_PI / 2. ? tiltAngle : -tiltAngle);
...
TH1D &Analyzer::getHistoOptimalSpacing(bool actualWindow) {
  if (actualWindow)
    return optimalSpacingDistributionAW;
  else
    return optimalSpacingDistribution;
}
