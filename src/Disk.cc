...
const std::vector<double> Disk::scanSmallDeltas() const {
  ...
  for (int i = 1; i <= numRings(); i++) {
    ...
    if (ringNode.count(i) > 0)
      ringTemplate->store(ringNode.at(i));
...
const std::vector<double> Disk::scanDsDistances() const {
  ...
  for (int i = 1; i <= numRings(); i++) {
    ...
    if (ringNode.count(i) > 0)
      modTemplate->store(ringNode.at(i));
...
const double Disk::scanSensorThickness() const {
  ...
  int i = numRings(); // Look at only 1 Ring, assumes sensorThickness is
                      // identical everywhere in the Disk!
  RectangularModule *modTemplate = GeometryFactory::make<RectangularModule>();
  modTemplate->store(propertyTree());
  if (ringNode.count(i) > 0)
    modTemplate->store(ringNode.at(i));
...
/** Scan Property Tree and gathers relevant info which is needed for building a
 * disk.
 */
const ScanDiskInfo Disk::scanPropertyTree() const {
  ...
  const ScanDiskInfo &diskInfo =
      std::make_pair(ringsSmallDeltas, ringsDsDistances);
  return diskInfo;
}
...
void Disk::check() {
  ...
  if (numRings.state() && innerRadius.state())
    throw PathfulException(
        "Only one between numRings and innerRadius can be specified");
  else if (!numRings.state() && !innerRadius.state())
    throw PathfulException(
        "At least one between numRings and innerRadius must be specified");
}

const double Disk::getRingInfo(const vector<double> &ringsInfo,
                               int ringNumber) const {
  const int numRings = ringsInfo.size();
  if (ringNumber > numRings) {
    throw PathfulException(
        Form("When building disk, tried to access information from ring %i",
             ringNumber));
  }
...
void Disk::cutAtEta(double eta) {
  for (auto &r : rings_)
    r.cutAtEta(eta);
  //  rings_.erase_if([](const Ring& r) { return r.numModules() == 0; }); // get
  //  rid of rods which have been completely pruned
  rings_.erase_if([](const Ring &r) {
    return r.modules().size() == 0;
...
/** This is used for building ring (i+1) from ring (i).
    ...
    These Z belong either to the innermost, either to the outermost Disk in the
   Endcap. In what follows, 'innermost' corresponds to 'lower Z', and
   'outermost' corresponds to 'bigger Z' (Z+ side).
    * @param i : ringNumber
    * @param parity : ring's parity
    * @param extremaDisksInfo : info from the innermost and outermost disks in
   the Endcap
    * return : Z in Ring (i+1), and Z in Ring (i)
*/
std::pair<double, double>
Disk::computeStringentZ(int i, int parity,
  ...
  // Case where Ring (i+1) is the innermost ring, and Ring (i) is the outermost
  // ring.
  if (parity > 0) {
    // IN THIS CASE, THE INNERMOST DISK OF THE ENDCAPS BLOCK IS THE MOST
    // STRINGENT. AS A RESULT, GEOMETRY INFORMATION IS TAKEN FROM THAT DISK.
    ...
    lastZ = buildZ() - zHalfLength() - bigDelta() - lastSmallDelta -
            lastDsDistance / 2.;
    newZ = buildZ() - zHalfLength() + bigDelta() + newSmallDelta +
           newDsDistance / 2.;
  }

  // Case where Ring (i+1) is the outermost ring, and Ring (i) is the innermost
  // ring.
  else {
    // IN THIS CASE, THE OUTERMOST DISK OF THE ENDCAPS BLOCK IS THE MOST
    // STRINGENT. AS A RESULT, GEOMETRY INFORMATION IS TAKEN FROM THAT DISK.
 ...
 */
double Disk::computeNextRho(const int parity, const double zError,
                            const double rSafetyMargin, const double lastZ,
                            const double newZ, const double lastRho,
                            const double oneBeforeLastRho) {
...
void Disk::buildTopDown(const ScanEndcapInfo &extremaDisksInfo) {
  ...
  for (int i = numRings(), parity = -bigParity(); i > 0; i--, parity *= -1) {
    ...
    // ALL THIS IS TO AUTOMATICALLY CALCULATE RING RADIUSHIGH FOR RING (i) FROM
    // RIMG (i+1)
    else {

      // 1) FIND THE Z OF THE MOST STRINGENT POINTS IN RING (i+1) AND RING (i)
      std::pair<double, double> stringentZ =
          computeStringentZ(i, parity, extremaDisksInfo);
      double lastZ =
          stringentZ.first; // Z of the most stringent point in Ring (i+1)
      double newZ =
          stringentZ.second; // Z of the most stringent point in Ring (i)

      // 2) CALCULATES RING (i) RADIUSHIGH USING RING (i+1)
      double zError = ring->zError();
      double rSafetyMargin = ring->rSafetyMargin();
      double nextRho = computeNextRho(parity, zError, rSafetyMargin, lastZ,
                                      newZ, lastRho, oneBeforeLastRho);

      // 3) NOW, CAN ASSIGN THE CALCULATED RADIUS TO RING (i) !
      ring->buildStartRadius(nextRho);
    }

    // NOW THAT RADIUS HAS BEEN CALCULATED, FINISH BUILDING THE RING AND STORE
    // IT
    ring->myid(i);
...
const std::map<int, std::vector<const Module *>>
Disk::getSurfaceModules() const {

  class SurfaceSplitterVisitor : public ConstGeometryVisitor {
  public:
    const std::map<int, std::vector<const Module *>> surfaceModules() const {
      return surfaceModules_;
    }
  ...
  accept(v);
  const std::map<int, std::vector<const Module *>> &surfaceModules =
      v.surfaceModules();
  return surfaceModules;
}

/** Binds the 2 points which are provided as arguments, and returns
 * corresponding Z of intersection with (Z) axis.
 */
const std::pair<double, bool>
Disk::computeIntersectionWithZAxis(double lastZ, double lastRho, double newZ,
                                   double newRho) const {
  // Slope of the line that binds the most stringent points of ring (i) and ring
  // (i+1).
  ...
  zIntersection =
      newZ - newRho / slope; // Intersection of the line with (Z) axis.
...
void Disk::computeActualZCoverage() {
  ...
  for (int i = numRings(), parity = -bigParity(); i > 0; i--, parity *= -1) {
    ...
    if (i != numRings()) {
      ...
      // Calculation : Min coordinates of ring (i+1) with max coordinates of
      // ring (i)
      std::pair<double, bool> intersectionWithZAxis =
          computeIntersectionWithZAxis(lastMinZ, lastMinRho, newMaxZ,
                                       newMaxRho);
      ...
      if (parity > 0.) {
        if (isPositiveSlope)
          zErrorCoverage = zIntersection;
        else
          zErrorCoverage = -std::numeric_limits<double>::infinity();
      }
    ...
    // Keep for next calculation : radii and Z of the most stringent point in
    // ring (i+1).
...
void Disk::computeActualCoverage() {
  ...
  for (auto &r : rings_)
    r.computeActualPhiCoverage();
}

void Disk::translateZ(double z) {
  centerZ_ += z;
  for (auto &r : rings_)
    r.translateZ(z);
}
...
void Disk::rotateToNegativeZSide() {
  centerZ_ = -centerZ_;
  for (auto &r : rings_)
    r.rotateToNegativeZSide();
}

const MaterialObject &Disk::materialObject() const { return materialObject_; }
