  ...
  bool Vizard::geometrySummary(Analyzer& analyzer, Tracker& tracker, SimParms& simparms, InactiveSurfaces* inactive, RootWSite& site, bool& debugResolution, std::string name) {
	...
	if (l.isTilted() && l.isTiltedAuto()) {
	  ...
	  for (int i=0; i < l.tiltedRingsGeometry().size(); i++) {
	    ...
	    //if (i > 0) {
	    tiltedLayerTable->setContent(15, 0, "deltaZ" + subStart + "Inner" + subEnd + " (Ring i & i-1)");
	    tiltedLayerTable->setContent(15, i+1, l.tiltedRingsGeometryInfo().deltaZInner()[ringNumber], coordPrecision);
	    tiltedLayerTable->setContent(16, 0, "deltaZ" + subStart + "Outer" + subEnd + " (Ring i & i-1)");
	    ...
	    tiltedLayerTable->setContent(17, 0, "zError" + subStart + "Inner" + subEnd + " (Ring i & i-1)");
	    tiltedLayerTable->setContent(17, i+1, l.tiltedRingsGeometryInfo().zErrorInner()[ringNumber], coordPrecision);
	    tiltedLayerTable->setContent(18, 0, "zError" + subStart + "Outer" + subEnd + " (Ring i & i-1)");
