  ...
  bool Vizard::geometrySummary(Analyzer& analyzer, Tracker& tracker, SimParms& simparms, InactiveSurfaces* inactive, RootWSite& site, bool& debugResolution, std::string name) {
	...
	if (l.isTilted() && l.isTiltedAuto()) {
	  ...
	  for (int i=0; i < l.tiltedRingsGeometry().size(); i++) {
	    ...
	    tiltedPartTable->setContent(2, 0, "theta_g (°)");
	    tiltedPartTable->setContent(2, i+1, l.tiltedRingsGeometry()[ringNumber]->theta_g(), anglePrecision);
	    tiltedPartTable->setContent(3, 0, "r" + subStart + "Inner" + subEnd);
	    tiltedPartTable->setContent(3, i+1, l.tiltedRingsGeometry()[ringNumber]->innerRadius(), coordPrecision);
	    tiltedPartTable->setContent(4, 0, "z" + subStart + "Inner" + subEnd);
	    tiltedPartTable->setContent(4, i+1, l.tiltedRingsGeometry()[ringNumber]->zInner(), coordPrecision);
	    tiltedPartTable->setContent(5, 0, "r" + subStart + "Outer" + subEnd);
	    tiltedPartTable->setContent(5, i+1, l.tiltedRingsGeometry()[ringNumber]->outerRadius(), coordPrecision);
	    tiltedPartTable->setContent(6, 0, "z" + subStart + "Outer" + subEnd);
	    tiltedPartTable->setContent(6, i+1, l.tiltedRingsGeometry()[ringNumber]->zOuter(), coordPrecision);
	    tiltedPartTable->setContent(7, 0, "tiltAngleIdeal" + subStart + "Inner" + subEnd + " (°)");
	    tiltedPartTable->setContent(7, i+1, l.tiltedRingsGeometry()[ringNumber]->tiltAngleIdealInner(), anglePrecision);
	    tiltedPartTable->setContent(8, 0, "deltaTiltIdeal" + subStart + "Inner" + subEnd + " (°)");
	    tiltedPartTable->setContent(8, i+1, l.tiltedRingsGeometry()[ringNumber]->deltaTiltIdealInner(), anglePrecision);
	    tiltedPartTable->setContent(9, 0, "tiltAngleIdeal" + subStart + "Outer" + subEnd + " (°)");
	    tiltedPartTable->setContent(9, i+1, l.tiltedRingsGeometry()[ringNumber]->tiltAngleIdealOuter(), anglePrecision);
	    tiltedPartTable->setContent(10, 0, "deltaTiltIdeal" + subStart + "Outer" + subEnd + " (°)");
	    tiltedPartTable->setContent(10, i+1, l.tiltedRingsGeometry()[ringNumber]->deltaTiltIdealOuter(), anglePrecision);
	    tiltedPartTable->setContent(11, 0, "averageR");
	    tiltedPartTable->setContent(11, i+1, l.tiltedRingsGeometry()[ringNumber]->averageR(), coordPrecision);
	    tiltedPartTable->setContent(12, 0, "averageZ");
	    tiltedPartTable->setContent(12, i+1, l.tiltedRingsGeometry()[ringNumber]->averageZ(), coordPrecision);
	    tiltedPartTable->setContent(13, 0, "gapR");
	    tiltedPartTable->setContent(13, i+1, l.tiltedRingsGeometry()[ringNumber]->gapR(), coordPrecision);
	    tiltedPartTable->setContent(14, 0, "phiOverlap");
	    tiltedPartTable->setContent(14, i+1, l.tiltedRingsGeometry()[ringNumber]->phiOverlap(), coordPrecision);
	    //if (i > 0) {
	    tiltedPartTable->setContent(15, 0, "deltaZ" + subStart + "Inner" + subEnd + " (Ring i & i-1)");
	    tiltedPartTable->setContent(15, i+1, l.tiltedRingsGeometryInfo().deltaZInner()[ringNumber], coordPrecision);
	    tiltedPartTable->setContent(16, 0, "deltaZ" + subStart + "Outer" + subEnd + " (Ring i & i-1)");
	    tiltedPartTable->setContent(16, i+1, l.tiltedRingsGeometryInfo().deltaZOuter()[ringNumber], coordPrecision);
	    //tiltedPartTable->setContent(16, 0, "coverage" + subStart + "Inner" + subEnd + " (°)");
	    //tiltedPartTable->setContent(16, i+1, l.tiltedRingsGeometryInfo().covInner()[ringNumber] * 180. / M_PI, coordPrecision);
