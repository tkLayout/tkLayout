...
/**
 * Get the surface of the element that is relevant for material budget
 * calculation.
 * @return The average cylinder surface for a horizontal tube, the disc surface
 * for a vertical disc
 */
double InactiveElement::getSurface() const {
  if (isVertical())
    return ((i_radius + w_radius) * (i_radius + w_radius) -
            i_radius * i_radius) *
           M_PI;
  else
    return 2 * M_PI * (i_radius + w_radius / 2.0) * z_length;
}
...
const double InactiveElement::tiltAngle() const {
  return (isVertical() ? M_PI / 2. : 0.);
}
...
/**
 * Check if the content of this element travels out of the tracking volume after
 * this.
...
/**
 * Set if the content of this element travels out of the tracking volume after
 * this.
 * @param final Should be true if the element is not a neighbour to anything and
 * false otherwise
 */
 ...
 * @param iradius The distance from the z-axis to the innermost point of the
 * element
 */
 ...
 * @return The distance from the innermost to the outermost point of the element
 * in the xy-plane
 */
 ...
 * @param rwidth The distance from the innermost to the outermost point of the
 ...
 * @return Max Rho.
 */
const double InactiveElement::getOuterRadius() const {
  return i_radius + w_radius;
}
...
/**
 * Calculate and return the Eta range of the element, with respect to the
 * origin.
...
std::pair<double, double> InactiveElement::getEtaMinMax() const {
  ...
  if (isVertical()) {
    // up point
    theta0 = atan((getInnerRadius() + getRWidth()) /
                  (getZLength() / 2.0 + getZOffset()));
    // down point
  ...
  // tubes
    ...
    // right point
    theta1 = atan((getRWidth() / 2.0 + getInnerRadius()) /
                  (getZOffset() + getZLength()));
  }

  // convert angle theta to pseudorapidity eta
  res.first =
      -1 * log(tan(theta0 / 2.0)); // TODO change to the default converters
  res.second =
      -1 * log(tan(theta1 / 2.0)); // TODO change to the default converters
  return res;
}

/**
 * Checks whether track hits the inactive element -> if yes, return true with
 * passed hit position vector & material. Only implemeted for eta >= 0. (should
 * work out of the box for eta < 0. but need to be tested in that case). Assumes
 * tracks are straight lines! IP should be on (Z) axis, and track not parallel
 * to (Z) axis.
 */
const bool InactiveElement::checkTrackHits(const XYZVector &trackOrig,
  ...
  // Calculate hit only if inactive element non-transparent, otherwise no effect
  // in tracking or material budget etc.
  if (getRadiationLength() != 0. || getInteractionLength() != 0.) {
    ...
    if (trackDir.Eta() >= 0.) {
      ...
      if (trackOrig.Rho() == 0. && trackDir.Rho() > geom_zero) {
        ...
        // IP position + track direction parameters of interest for the
        // calculation
        ...
        // PART A
        // COMPUTE THE PROJECTION OF THE TRACK IN THE 4 EXTREMA SURFACES OF THE
        // INACTIVE VOLUME. Hit on inner (Z) plane
        const double kInnerZPlane =
            (fabs(trackDirZ) > geom_zero ? (innerZ - trackOrigZ) / trackDirZ
                                         : -1.);
        XYZVector hitOnInnerZPlane;
        if (kInnerZPlane > 0.)
          hitOnInnerZPlane = trackOrig + kInnerZPlane * trackDir;

        // Hit on outer (Z) plane
        const double kOuterZPlane =
            (fabs(trackDirZ) > geom_zero ? (outerZ - trackOrigZ) / trackDirZ
                                         : -1.);
        XYZVector hitOnOuterZPlane;
        if (kOuterZPlane > 0.)
          hitOnOuterZPlane = trackOrig + kOuterZPlane * trackDir;

        // Hit on inner radius cylinder
        const double kInnerRhoCylinder = innerRho / trackDirRho;
        const XYZVector hitOnInnerRhoCylinder =
            trackOrig + kInnerRhoCylinder * trackDir;

        // Hit on outer radius cylinder
        const double kOuterRhoCylinder = outerRho / trackDirRho;
        const XYZVector hitOnOuterRhoCylinder =
            trackOrig + kOuterRhoCylinder * trackDir;

        // PART B
        // FIND OUT WHETHER THE TRACK CROSSES THE INSIDE OF THE VOLUME.
        // IF SO, COMPUTES HIT POSITION AND PATH LENGTH.
        double hitPathLength =
            0.; // length of the track path crossing the volume.

        // Track enters and exits the volume between extrema radii cylinders.
        if (kInnerZPlane > 0. && kOuterZPlane > 0. &&
            (hitOnInnerZPlane.Rho() >= innerRho &&
             hitOnInnerZPlane.Rho() <= outerRho) &&
            (hitOnOuterZPlane.Rho() >= innerRho &&
             hitOnOuterZPlane.Rho() <= outerRho)) {
          hitFound = true;
          hitPos = (hitOnInnerZPlane + hitOnOuterZPlane) / 2.;
          hitPathLength = (hitOnOuterZPlane - hitOnInnerZPlane).R();
        }
        // Top left corner.
        else if (kInnerZPlane > 0. && hitOnInnerZPlane.Rho() >= innerRho &&
                 hitOnInnerZPlane.Rho() <= outerRho) {
          const double exitFactor = outerRho / hitOnInnerZPlane.Rho();
          const XYZVector hitExit =
              trackOrig + (kInnerZPlane * exitFactor) * trackDir;
          hitFound = true;
          hitPos = (hitOnInnerZPlane + hitExit) / 2.;
          hitPathLength = (hitExit - hitOnInnerZPlane).R();
        }
        // Bottom right corner.
        else if (kOuterZPlane > 0. && hitOnOuterZPlane.Rho() >= innerRho &&
                 hitOnOuterZPlane.Rho() <= outerRho) {
          const double exitFactor = innerRho / hitOnOuterZPlane.Rho();
          const XYZVector hitExit =
              trackOrig + (kOuterZPlane * exitFactor) * trackDir;
          hitFound = true;
        ...
        else if ((hitOnInnerRhoCylinder.Z() >= innerZ &&
                  hitOnInnerRhoCylinder.Z() <= outerZ) &&
                 (hitOnOuterRhoCylinder.Z() >= innerZ &&
                  hitOnOuterRhoCylinder.Z() <= outerZ)) {
          hitFound = true;
          hitPos = (hitOnInnerRhoCylinder + hitOnOuterRhoCylinder) / 2.;
          hitPathLength = (hitOnOuterRhoCylinder - hitOnInnerRhoCylinder).R();
        }

        // If hit was found, the computed path length allows to compute Material
        // estimate.
        if (hitFound) {
          // Cross-check that hit is inside volume
          if (hitPos.Rho() >= innerRho && hitPos.Rho() <= outerRho &&
              hitPos.Z() >= innerZ && hitPos.Z() <= outerZ) {
            // Cross-check that path length is > 0.
            if (hitPathLength >= 0.) {
              // This normalizationFactor comes from the fact that MB is by
              // default multiplied by length or width. TO DO: Is that actually
              // true?
              const double normalizationFactor =
                  1. / (isVertical() ? getZLength() : getRWidth());
              // Compute MB
              hitMaterial.radiation =
                  getRadiationLength() * normalizationFactor * hitPathLength;
              hitMaterial.interaction =
                  getInteractionLength() * normalizationFactor * hitPathLength;
            } else {
              logERROR("InactiveElement::checkTrackHits : Computed a "
                       "hitPathLength < 0.");
            }
          } else {
            logERROR("InactiveElement::checkTrackHits : Created a hit which is "
                     "not inside Inactive Volume.");
          }
        }
        ...
        logERROR("InactiveElement::checkTrackHits : trackOrig.Rho() = " +
                 ...
                 " trackDir.Rho() = " + any2str(trackDir.Rho()) +
                 ". IP not on (Z) axis or track parallel to (Z) axis not "
                 "supported.");
      }
    } else {
      logERROR("InactiveElement::checkTrackHits : Try to compute inactive MB "
               "on eta < 0., which is not supported");
    }
  }
  return hitFound;
}
 ...
 * @return The index within the tracker object's layer or disc vector, or of the
 * service volume; -1 if there is none
 */
int InactiveElement::getFeederIndex() { return feeder_index; }

/**
 * Set the index of the element's feeder volume.
 * @param layer The index within the tracker object's layer or disc vector, or
 ...
 */
void InactiveElement::setNeighbourIndex(int previous) {
  neighbour_index = previous;
}
 ...
 */
InactiveElement::InType InactiveElement::getNeighbourType() {
  return neighbour_type;
}
 ...
 */
void InactiveElement::setNeighbourType(InactiveElement::InType type) {
  neighbour_type = type;
}
 ...
 * @param rlength The new overall radiation length, averaged over all the
 * different material that occur in the inactive element
 */
 ...
 * @param ilength The new overall interaction length, averaged over all the
 * different material that occur in the inactive element
 */
void InactiveElement::setInteractionLength(double ilength) {
  i_length = ilength;
}

/**
 * Print the geometry-specific parameters of the inactive element including the
 * orientation.
 */
void InactiveElement::print() {
  ...
  if (isVertical())
    std::cout << "Volume is vertical." << std::endl;
  else
    std::cout << "Volume is horizontal" << std::endl;
}
} // namespace insur
