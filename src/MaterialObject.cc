...
namespace material {
const std::map<MaterialObject::Type, const std::string>
    MaterialObject::typeString = {
        {MODULE, "module"}, {ROD, "rod"}, {LAYER, "layer"}};
...
MaterialObject::MaterialObject(Type materialType,
    ...
    : subdetectorName_(subdetectorName), materialType_(materialType),
      type_("type", parsedOnly()), destination_("destination", parsedOnly()),
...
void MaterialObject::build() {
  ...
  if (!debugInactivate_()) {
    ...
    //   std::cout << "[" << currentSensor.first << "]=" <<
    //   temporarySensor.numChannels() << "; ";
    //   sensorChannels[currentSensor.first] = temporarySensor.numChannels();
    // }
    // std::cout << "}" << std::endl;

    static std::map<MaterialObjectKey, Materials *>
        materialsMap_; // for saving memory
    for (auto &currentMaterialNode : materialsNode_) {
      store(currentMaterialNode.second);

      check();
      if (type_().compare(getTypeString()) == 0) {
        MaterialObjectKey myKey(
            subdetectorName(), currentMaterialNode.first, sensorChannels,
            destination_.state() ? destination_() : std::string(""));
        if (materialsMap_.count(myKey) == 0) {
          Materials *newMaterials =
              new Materials(materialType_, subdetectorName());
...
void MaterialObject::deployMaterialTo(
    MaterialObject &outputObject, const std::vector<std::string> &unitsToDeploy,
    bool onlyServices /*= false */, double gramsMultiplier /*= 1.*/,
    Location requestedLocation /*= Location::ALL*/) const {
  for (const Element *currElement : serviceElements_) {
    currElement->deployMaterialTo(outputObject, unitsToDeploy, onlyServices,
                                  gramsMultiplier, requestedLocation);
  }
...
void MaterialObject::populateMaterialProperties(
  ...
  for (const Element *currElement : serviceElements_) {
    ...
    if (currElement->debugInactivate() == false) {
      ...
      if (currElement->subdetectorName() == "")
        std::cout << "MaterialObject::populateMaterialProperties "
                     "currElement->subdetectorName() = "
                  << currElement->subdetectorName() << std::endl;

      if (currElement->componentName.state()) {
        /*if (currElement->componentName() == "High voltage lines") {
          std::cout << "currElement->componentName()" <<
          currElement->componentName() << "currElement->elementName() = " <<
          currElement->elementName() << "quantity = " << quantity << std::endl;
          }*/
        materialProperties.addLocalMass(currElement->subdetectorName(),
                                        currElement->elementName(),
                                        currElement->componentName(), quantity);
      } else {
        std::cout << "MaterialObject::populateMaterialProperties: No component "
                     "name, element name = "
                  << currElement->elementName() << std::endl;
        materialProperties.addLocalMass(currElement->subdetectorName(), "",
                                        currElement->elementName(), quantity);
      }
    }
  }
...
bool MaterialObject::isPopulated() const { return (materials_ != nullptr); }
...
MaterialObject::Materials::Materials(MaterialObject::Type newMaterialType,
    ...
    : componentsNode_("Component", parsedOnly()),
      materialType_(newMaterialType), subdetectorName_(subdetectorName){};

MaterialObject::Materials::~Materials() {}

double MaterialObject::Materials::totalGrams(double length,
...
void MaterialObject::Materials::build(
    const std::map<int, int> &newSensorChannels) {
  check();
...
void MaterialObject::Materials::deployMaterialTo(
    MaterialObject &outputObject, const std::vector<std::string> &unitsToDeploy,
    bool onlyServices, double gramsMultiplier /*= 1.*/,
    Location requestedLocation /*= Location::ALL*/) const {
...
void MaterialObject::Materials::populateMaterialProperties(
    MaterialProperties &materialProperties) const {
  for (const Component *currComponent : components_) {
...
void MaterialObject::Materials::getLocalElements(
    ElementsVector &elementsList) const {
  for (const Component *currComponent : components_) {
...
MaterialObject::Component::Component(MaterialObject::Type &newMaterialType,
                                     const std::string subdetectorName)
    : componentsNode_("Component", parsedOnly()),
      elementsNode_("Element", parsedOnly()), materialType_(newMaterialType),
      subdetectorName_(subdetectorName){};

MaterialObject::Component::~Component() {}

double MaterialObject::Component::totalGrams(double length,
                                             double surface) const {
  double result = 0.0;
...
void MaterialObject::Component::deployMaterialTo(
    MaterialObject &outputObject, const std::vector<std::string> &unitsToDeploy,
    bool onlyServices, double gramsMultiplier /*= 1.*/,
    Location requestedLocation /*= Location::ALL*/) const {
...
void MaterialObject::Component::populateMaterialProperties(
    MaterialProperties &materialProperties) const {
  for (const Component *currComponent : components_) {
    currComponent->populateMaterialProperties(materialProperties);
  }
...
void MaterialObject::Component::getLocalElements(
    ElementsVector &elementsList) const {
  for (const Component *currComponent : components_) {
    currComponent->getLocalElements(elementsList);
  }
...
MaterialObject::Element::Element(MaterialObject::Type &newMaterialType,
  ...
  if (location() == Location::ALL) {
    logERROR("location ALL can not be set to an element.");
  }
};
...
MaterialObject::Element::Element(const Element &original, double multiplier)
  ...
  scaleOnSensor(0);
  quantity(original.quantity() * original.scalingMultiplier() *
           multiplier); // apply the scaling in the copied object
  unit(original.unit());
...
const std::string MaterialObject::Element::msg_no_valid_unit =
    "No valid unit: ";

const std::map<std::string, MaterialObject::Element::Unit>
    MaterialObject::Element::unitStringMap = {
        {"g", GRAMS}, {"mm", MILLIMETERS}, {"g/m", GRAMS_METER}};

void MaterialObject::Element::deployMaterialTo(
    MaterialObject &outputObject, const std::vector<std::string> &unitsToDeploy,
    bool onlyServices /*= false*/, double gramsMultiplier /*= 1.*/,
    Location requestedLocation /*= Location::ALL*/) const {
  ...
  // * there is a specific requestedLocation AND IT IS MATCHING THE
  // MaterialObject::Element location.
  if (requestedLocation == Location::ALL || (requestedLocation == location())) {
        ...
        if (unit().compare(unitToDeploy) == 0) {
          ...
          if (service() == true && (unit().compare("g/m") != 0)) {
            logERROR(any2str("Definition of services (") +
                     any2str(elementName()) + any2str(") in ") +
                     any2str(unit()) +
...
double
MaterialObject::Element::quantityInGrams(const DetectorModule &module) const {
  return quantityInUnit("g", module.length(), module.area());
}

double MaterialObject::Element::quantityInGrams(
    const MaterialProperties &materialProperties) const {
  return quantityInUnit("g", materialProperties.getLength(),
                        materialProperties.getSurface());
}

double MaterialObject::Element::quantityInGrams(const double length,
                                                const double surface) const {
  return quantityInUnit("g", length, surface);
}

double MaterialObject::Element::quantityInUnit(
    const std::string desiredUnit,
    const MaterialProperties &materialProperties) const {
  return quantityInUnit(desiredUnit, materialProperties.getLength(),
                        materialProperties.getSurface());
}
...
double MaterialObject::Element::quantityInUnit(const std::string desiredUnit,
                                               const double length,
                                               const double surface) const {
  double returnVal = 0;
  ...
  MILLIMETERS}, density*surface},
    {{GRAMS_METER, GRAMS}, 1000/length}, {{GRAMS_METER, GRAMS_METER}, 1},
  {{GRAMS_METER, MILLIMETERS}, (density*surface*1000)/length},
    {{MILLIMETERS, GRAMS}, 1/(density*surface)}, {{MILLIMETERS, GRAMS_METER},
  length/(density*surface*1000)}, {{MILLIMETERS, MILLIMETERS}, 1}
  };

  try {
    returnVal = quantity() * conversionMatrix.at({unitStringMap.at(desiredUnit),
  unitStringMap.at(unit())}); } catch (const std::out_of_range& ex) {
...
double MaterialObject::Element::totalGrams(
    const MaterialProperties &materialProperties) const {
  return totalGrams(materialProperties.getLength(),
                    materialProperties.getSurface());
}

double MaterialObject::Element::totalGrams(double length,
                                           double surface) const {
...
double MaterialObject::Element::scalingMultiplier() const {
  ...
  if (sensorIndex != 0) {
    if (materialType_ == MODULE) {
      try {
        return sensorChannels_.at(sensorIndex) /
               referenceSensors_.at(sensorIndex)->numChannels();
      ...
      logUniqueERROR(
          "Is not possible to define scaling for materials not in module.");
    }
  }
  return 1.;
}
...
void MaterialObject::Element::build(
  ...
  //   std::cout << "DESTINATION " << destination() << " for " << elementName()
  //   << std::endl;
  ...
  std::cout << "    DATA "
      << " componentName " << (componentName.state() ? componentName() :
  "NOT_SET")
      << " nSegments " << (nSegments.state() ? std::to_string(nSegments()) :
  "NOT_SET")
      << " service " << service()
...
void MaterialObject::Element::populateMaterialProperties(
    MaterialProperties &materialProperties) const {
  double quantity;
  ...
  if (debugInactivate() == false) {
    if (service() == false) {
      ...
      if (subdetectorName() == "")
        std::cout
            << "canary: MaterialObject::Element::populateMaterialProperties "
               "subdetectorName() = "
            << subdetectorName() << std::endl;
      materialProperties.addLocalMass(subdetectorName(), elementName(),
                                      componentName(), quantity);
    }
  }
}

void MaterialObject::Element::getLocalElements(
    ElementsVector &elementsList) const {
  if (service() == false) {
...
void MaterialObject::ReferenceSensor::check() {
  ...
  if (!numStripsAcross.state() && !pitchEstimate.state())
    throw PathfulException("At least one between numStripsAcross and "
                           "pitchEstimate must be specified");
  if (numStripsAcross.state() && pitchEstimate.state())
    throw PathfulException(
        "Only one between numStripsAcross and pitchEstimate can be specified");
  if (!numSegments.state() && !stripLengthEstimate.state())
    throw PathfulException("At least one between numSegments and "
                           "stripLengthEstimate must be specified");
  if (numSegments.state() && stripLengthEstimate.state())
    throw PathfulException("Only one between numSegments and "
                           "stripLengthEstimate can be specified");
}
