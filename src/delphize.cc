...
void printEtaRange(double lowEta, double highEta) {
  (*osp) << Form("   (abs(eta) >= %.04f && abs(eta) < %.04f) * ", lowEta,
                 highEta);
}

void printPtRange(double lowPt, double highPt) {
  if (highPt > lowPt) {
    (*osp) << Form("(pt >= %.04f && pt < %.04f) * ", lowPt, highPt);
  } else {
    (*osp) << Form("(pt >= %.04f) * ", lowPt);
  }
}
...
void printNewLine(bool last) {
  if (!last)
    ...
    (*osp) << std::endl << "}" << std::endl;
  ;
}

void printResolutionScale(double resolution1, double pt1, double resolution2,
                          double pt2) {
  if (pt1 == pt2) {
    ...
    (*osp) << Form("(%f + "  // resolution1
                   "(pt-%f)" // pt1
                   "* %f)",  // (res2-res1) / (pt2-pt1)
                   resolution1, pt1, (resolution2 - resolution1) / (pt2 - pt1));
  }
}
...
void addProfile(TObject *anObject) {
  ...
  if (aClassName == "TProfile") {
    ...
    double aMomentum;
    if (sscanf(myProfile->GetName(), "pt_vs_eta%lftracker_profile",
               &aMomentum) == 1) {
      std::cerr << "Momentum [GeV]: " << aMomentum << std::endl;
      ptProfiles[aMomentum] = (TProfile *)myProfile->Clone();
    } else if (sscanf(myProfile->GetName(), "Total_pt_vs_eta%lftracker_profile",
                      &aMomentum) == 1) {
      std::cerr << "Momentum [GeV]: " << aMomentum << std::endl;
      ptProfiles[aMomentum] = (TProfile *)myProfile->Clone();
    } else if (sscanf(myProfile->GetName(),
                      "Resolution_(%[^)]).ptres_tracker_MS_Pt.pt_vs_eta%"
                      "lftracker_profile",
                      buffer, &aMomentum) == 2) {
      std::cerr << "Momentum [GeV]: " << aMomentum << " buffer: " << buffer
                << " (" << myProfile->GetName() << ")" << std::endl;
      ptProfiles[aMomentum] = (TProfile *)myProfile->Clone();
    }
  }
}
...
int main(int argc, char *argv[]) {
  ...
  help.add_options()("help,h", "Display help")(
      "input-file,i", boost::program_options::value<std::string>(),
      "Specify name of input root file (from tkLayout), containing canvas with "
      "pT profiles")("layout-name,n",
                     boost::program_options::value<std::string>(),
                     "Specify given layout name")(
      "author,a",
      boost::program_options::value<std::string>(&author)->default_value(
          "Unknown author"),
      "Specify author of DELPHES file (optional: default = Unknown author)")(
      "eta-slicing,s",
      boost::program_options::value<double>(&etaSlice)->default_value(0.2),
      "Specify eta slicing <0.0?; 1.0>, e.g. 0.1, 0.2, ... (optional: default "
      "= 0.2)");
  ...
  try {
    ...
    // Check
    if (etaSlice <= 0.0 || etaSlice > 1.0)
      throw boost::program_options::invalid_option_value("eta-slicing");
    else if (!varMap.count("input-file") && !varMap.count("help"))
      throw boost::program_options::error(
          "Forgot to define input root file???");
    else if (!varMap.count("layout-name") && !varMap.count("help"))
      throw boost::program_options::error("Forgot to define layout name???");
    else {
      ...
      // Check that the provided ROOT input file corresponds to an existing
      // file.
      rootFileName = varMap["input-file"].as<std::string>();
      inputFile = new TFile(rootFileName.c_str(), "READ");
      if (!inputFile) {
        std::cerr << "File '" << rootFileName << "' does not exist"
                  << std::endl;
        return 0;
      }
      ...
      if (outFile->is_open()) {
        ...
        std::cerr << "ERROR: could not open " << outFile
                  << " -> writing config to the screen" << std::endl;
        return -1;
      }
    }
  ...
  double myPt, nextPt;
  (*osp) << "#" << std::endl;
  (*osp) << "# Automatically generated tracker resolution formula for layout: "
         << layoutName << std::endl;
  (*osp) << "#" << std::endl;
  ...
  for (int iBin = 1; iBin <= newBins; ++iBin) {
    ...
    for (auto it = ptProfiles.begin(); it != ptProfiles.end(); ++it) {
      ...
      if (iBin == 1)
        std::cout << "Using momentum [GeV]: " << std::setw(6) << aMomentum
                  << std::endl;
