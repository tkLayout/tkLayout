...
/* Object destructor */
StopWatch::~StopWatch() { std::cout << std::endl; }
...
double StopWatch::stopCounter() {
  ...
  if (startTimes_.size()) {
    ...
    if (startTimes_.size() < verbosity_) {
      if (startTimes_.size() < lastVerbosity_)
        std::cout << std::endl;
      std::cout << "done";
      if (reportTime_)
        std::cout << " [in " << timeSeconds << " s]";
      std::cout << std::flush;
    ...
    timeSeconds = 0;
    logERROR(
        "A timer stop was requested, not corresponding to any timer start");
  }
  return timeSeconds;
}
