...
Color_t Palette::color(const std::string &type) {
  ...
  if (colorPickMap[type] == 0) {
    ...
    while (found) {
      ++iColor;
      found = false;
      for (std::map<std::string, int>::iterator it = colorPickMap.begin();
           it != colorPickMap.end(); ++it) {
        if (it->first == iColor) {
          found = true;
          break;
        }
...
Color_t Palette::color_int(const unsigned int &plotIndex, bool isTransparent) {
  ...
  if (plotIndex == 0)
    ...
    case 11:
      ...
      break;
    default:
      std::cerr << "ERROR: in Vizard::getNiceColor() n%12 is not an int "
                   "between 0 and 11! This should not happen."
                << std::endl;
      paletteIndex = kBlack;
      break;
    }
...
/*
  ...
  A shift in the color scheme is also done for each of the 9 possible phi
  sectors.
 */
Color_t Palette::colorDTC(const int &colorIndex, bool isTransparent) {
  ...
  const int zone =
      femod(colorIndex % 12, 12); // unit digit (in a numbering of base 12)
  // const int phiSector = (colorIndex - 1) / 12;  // dizain digit (in a
  // numbering of base 12)
  ...
  if (colorIndex == 0)
    ...
    paletteIndex -=
        (colorIndex % 10); // should be -= phiSector, but decision was made to
                           // keep things like this, since color scheme cannot
                           // be perfectly unique anyway.
    if (isTransparent)
      paletteIndex = Palette::GetColorTransparent(paletteIndex, 0.2);
  }
...
/* This allows to have one color for each of the 12 services channels.
   If 12 is added, the color is set to transparent (if transparent colors
   allowed by isTransparentActivated).
 */
Color_t Palette::colorChannel(const int &colorIndex,
                              bool isTransparentActivated) {

  const int zone =
      femod(colorIndex % 12, 12); // unit digit (in a numbering of base 12)
  const int shift =
      (colorIndex - 1) / 12; // dizain digit (in a numbering of base 12)
  ...
  if (colorIndex == 0)
    ...
    if (isTransparentActivated) {
      const bool isTransparent =
          (shift >= 1); // set transparent if 12 has been added
      if (isTransparent)
        paletteIndex = Palette::GetColorTransparent(paletteIndex, 0.1);
    }
  }
...
// TO DO : Why the hell is TColor::GetColorTransparent not recognized as a
// method of TColor ?? Temporary : use this instead.
Int_t Palette::GetColorTransparent(Int_t colorIndex, Float_t ratio) {
  ...
  if (color) {
    TColor *transColor =
        new TColor(gROOT->GetListOfColors()->GetLast() + 1, color->GetRed(),
                   color->GetGreen(), color->GetBlue());
    transColor->SetAlpha(ratio);
    transColor->SetName(Form("%s_transparent", color->GetName()));
    return transColor->GetNumber();
  } else {
    std::cerr << "TColor::GetColorTransparent : color with undefined index"
              << std::endl;
    return -1;
  }
}
