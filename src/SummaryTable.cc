...
template <>
void SummaryTable::setCell<std::string>(const int row, const int column,
                                        const std::string &content) {
  if (column > 0 && !hasCell(0, column))
    summaryTable[std::make_pair(0, column)] = any2str(column + columnOffset_);
  if (row > 0 && !hasCell(row, 0))
    summaryTable[std::make_pair(row, 0)] = any2str(row + rowOffset_);
...
template <>
void SummaryTable::setSummaryCell<std::string>(std::string label,
                                               const std::string &content) {
  if (!hasSummaryCell()) {
