 ...
 * @brief implementation of class that take care of creating and reading the
 * configuration file .tkgeometryrc
 */
#include <fstream>
#include <iomanip>
#include <iostream>
...
void mainConfigHandler::askMomenta() {
  ...
  cout << "*** Specify the list of transverse momenta to be used for the"
       << endl
...
void mainConfigHandler::askTriggerMomenta() {
  ...
  for (double &iMom : triggerMomenta_)
    iMom *= Units::GeV;
}
...
void mainConfigHandler::askThresholdProbabilities() {
  ...
  cout << "*** Specify the list of trigger efficiency to be used for the"
       << endl
       << "    pt threshold find test (in percent: write 100 for full "
          "efficiency)"
       << endl
       << "    Example: 1, 50, 90, 95 : ";
  cin >> tempString;
  ...
  for (vector<double>::iterator it = thresholdProbabilities_.begin();
       it != thresholdProbabilities_.end(); ++it)
    (*it) /= 100;
}

bool mainConfigHandler::createConfigurationFileFromQuestions(
  ...
  cout << "Answer to the following questions to have your configuration file "
          "automatically created."
       << endl;
  cout << "You will be later able to edit it manually, or you can just delete "
          "it and answer these questions again."
       << endl;
  cout << endl;
  ...
  if (!configFile.good()) {
    cout << "Could not open " << configFileName << " for writing. I quit."
         << endl;
    configFile.close();
    return false;
  } else {
    configFile << BINDIRECTORYDEFINITION << "=\"" << binDirectory_ << "\""
               << endl;
    configFile << LAYOUTDIRECTORYDEFINITION << "=\"" << layoutDirectory_ << "\""
               << endl;
    configFile << STANDARDDIRECTORYDEFINITION << "=\"" << standardDirectory_
               << "\"" << endl;

    configFile << MOMENTADEFINITION << "=\"";
    for (std::vector<double>::iterator it = momenta_.begin();
         it != momenta_.end(); ++it) {
      if (it != momenta_.begin())
        configFile << ", ";
    ...
    for (std::vector<double>::iterator it = triggerMomenta_.begin();
         it != triggerMomenta_.end(); ++it) {
      if (it != triggerMomenta_.begin())
        configFile << ", ";
      configFile << std::fixed << std::setprecision(2) << (*it) / Units::GeV;
    }
    configFile << "\"" << std::endl;
    ...
    for (std::vector<double>::iterator it = thresholdProbabilities_.begin();
         it != thresholdProbabilities_.end(); ++it) {
      if (it != thresholdProbabilities_.begin())
        configFile << ", ";
...
bool mainConfigHandler::parseLine(const char *codeLine, string &parameter,
                                  string &value) {
  ...
  if (tokens.empty()) {
    ...
    cerr << "ERROR: Cannot understand line: '" << codeLine
         << "' in the configuration file " << CONFIGURATIONFILENAME << endl;
    return false;
  } else {
...
bool mainConfigHandler::readConfigurationFile(string &configFileName) {
  ...
  while (configFileIs.good()) {
    ...
    if (parseLine(myLine, parameter, value)) {
      ...
      if (parameter == BINDIRECTORYDEFINITION) {
        ...
        cerr << "ERROR: Unknown parameter " << parameter
             << " in the configuration file " << CONFIGURATIONFILENAME << endl;
      }
    }
  }
  ...
  if (!binFound && layoutFound && xmlFound && momentaFound &&
      triggerMomentaFound && thresholdProbabilitiesFound) {
    askBinDirectory();
    if (checkDirectory(binDirectory_)) {
      ...
      if (!configFileOs.good()) {
        ...
        configFileOs << BINDIRECTORYDEFINITION << "=\"" << binDirectory_ << "\""
                     << endl;
  ...
  return (binFound && layoutFound && xmlFound && momentaFound &&
          triggerMomentaFound && thresholdProbabilitiesFound);
}
...
string mainConfigHandler::getConfigFileName() {
  ...
  if (specialConfigFile)
    return string(specialConfigFile);
  ...
  if (!configFile.good()) {
    ...
    if (goodConfig) {
      if (checkDirExists) {
        ...
        if (!checkDirectory(layoutDirectory_)) {
          cout << "You probably need to edit or delete the configuration file "
               << CONFIGURATIONFILENAME << endl;
          return false;
        }
        ...
        if (!checkDirectory(getXmlDirectory_()))
          return false;
        if (!checkDirectory(getMattabDirectory_()))
          return false;
      }
    } else { // not good config read
      cout << "Configuration file '" << configFileName
           << "' not properly formatted. You probably need to edit or delete it"
           << endl;
      return false;
    }
  }
...
std::vector<int> mainConfigHandler::getDetIdScheme(std::string schemeName) {
  getConfiguration();
  if (detIdSchemes_.size() == 0)
    readDetIdSchemes();
  std::vector<int> scheme;
  auto found = detIdSchemes_.find(schemeName);
  if (found != detIdSchemes_.end())
    scheme = found->second;
  else
    std::cerr << "detIdScheme " << schemeName
              << " description could not be found in the cfg files. No detId "
                 "for the associated subdetector will be calculated."
              << std::endl;
  return scheme;
}
...
string mainConfigHandler::getStyleDirectory_() {
  return layoutDirectory_ + "/" + insur::default_styledir;
}
string mainConfigHandler::getXmlDirectory_() {
  return standardDirectory_ + "/" + insur::default_xmlpath;
}
string mainConfigHandler::getMattabDirectory_() {
  return standardDirectory_ + "/" + insur::default_mattabdir;
}
string mainConfigHandler::getIrradiationDirectory_() {
  return standardDirectory_ + "/" + insur::default_irradiationdir;
}
string mainConfigHandler::getDefaultMaterialsDirectory_() {
  return standardDirectory_ + "/" + insur::default_materialsdir;
}
string mainConfigHandler::getDetIdSchemesDirectory_() {
  return standardDirectory_ + "/" + insur::default_detidschemesdir;
}
string mainConfigHandler::getStandardIncludeDirectory_() {
  return standardDirectory_ + "/" + insur::default_configdir + "/" +
         insur::default_stdincludedir;
}
string mainConfigHandler::getGeometriesDirectory_() {
  return standardDirectory_ + "/" + insur::default_geometriesdir;
}
...
string ConfigInputOutput::getIncludedFile(string fileName) {
  ...
  if (result.size() == 1)
    ...
    cerr << "ERROR: cannot find file " << fileName
         << " in any of the include path directories: " << endl;
    for (const auto &aPath : includePathList)
      cerr << "   * " << aPath << endl;
    return "";
  } else {
    cerr << "ERROR: file " << fileName
         << " found multiple times in the include path directories: " << endl;
    for (const auto &aFile : result)
      cerr << "   * " << aFile << endl;
    return "";
  }
}
...
std::set<string>
mainConfigHandler::preprocessConfiguration(ConfigInputOutput cfgInOut) {
  using namespace std;
  ...
  string absoluteFileNameDirectory =
      boost::filesystem::canonical(absoluteFileName).parent_path().string();
  ...
  if (cfgInOut.includePathList.size() == 0) {
    string firstFileName =
        boost::filesystem::canonical(cfgInOut.absoluteFileName).string();
    addNodeUrl(firstFileName, "file://" + cfgInOut.absoluteFileName);
    setNodeLocal(firstFileName, true);
  }
  if ((cfgInOut.includePathList.size() == 0) || (!standardInclude))
    cfgInOut.includePathList.insert(absoluteFileNameDirectory);
  ...
  std::string full_path =
      boost::filesystem::system_complete(absoluteFileName).string();
  ...
  while (getline(is, line).good()) {
    ...
    if ((includeStart = trimmed.find("tiltedLayerSpecFile")) != string::npos) {
      string rightPart =
          trimmed.substr(includeStart + strlen("tiltedLayerSpecFile"));
      string leftPart =
          trimmed.substr(0, includeStart + strlen("tiltedLayerSpecFile"));
      std::size_t lastSpace;
      for (lastSpace = 0;
           (rightPart[lastSpace] == ' ') && (lastSpace < rightPart.size());
           lastSpace++)
        ;
    ...
    if ((includeStart = trimmed.find("@include")) !=
        string::npos) { //@include @include-std
      trimmed = trimmed.substr(includeStart);
      std::size_t quoteStart, quoteEnd;
      string nextIncludeFileName;
      if ((quoteStart = trimmed.find_first_of("\"")) != string::npos &&
          (quoteEnd = trimmed.find_last_of("\"")) != string::npos) {
        nextIncludeFileName =
            ctrim(trimmed.substr(quoteStart, quoteEnd - quoteStart + 1), "\"");
      } else {
      ...
      bool includeStdOld =
          trimmed.find("@includestd") !=
          string::npos; // both @includestd (deprecated) and @include-std
                        // (preferred) are supported
      ...
      if (includeStdOld || includeStdNew) {
        fullIncludedFileName =
            getStandardIncludeDirectory() + "/" + nextIncludeFileName;
      } else {
      ...
      if (ifs) {
        ...
        nextIncludeInputOutput.standardInclude =
            (includeStdOld || includeStdNew);
        ...
        cerr << "ERROR: ignoring "
             ...
             << " directive in " << absoluteFileName << ":" << numLine
             << " : could not open included file : " << nextIncludeFileName
             << endl;
      }
    } else {
      os << line << endl;
...
void mainConfigHandler::readDetIdSchemes() {
  std::ifstream schemesStream(getDetIdSchemesDirectory_() + "/" +
                              insur::default_detidschemesfile);
  ...
  if (schemesStream.good()) {
    ...
    while (getline(schemesStream, line).good()) {
      ...
      if (schemeData.size() < 2)
        std::cerr << "Reading DetId scheme : no data was entered." << std::endl;
      else {
        // Scheme Name
        std::string schemeName = schemeData.at(0);
        // Geometry hierarchy levels sizes (in number of bits) associated to the
        // Scheme. For example, Tracker volume (hierarchy level 0) is assigned
        // size 4 (ie, 4 bits).
        ...
        // The sum of all sizes should be 32. Indeed, the DetId, made of all of
        // them, id a 32-bit integer.
        if (sum != 32)
          std::cerr
              << "DetId scheme " << schemeName
              << " : The sum of geometry hierarchy sizes is not equal to 32."
              << std::endl;
        else
          detIdSchemes_.insert(
              std::make_pair(schemeName, geometryHierarchySizes));
      }
    }
    schemesStream.close();
  } else {
    std::cerr << "No file defining a detId scheme has been found." << std::endl;
  }
}
