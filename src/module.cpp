...
const Polygon3d<4>& Module::getFacePolygon(size_t facen) const {
  if (facePolys_.size() <= facen) { facePolys_.resize(facen+1); }
  if (!facePolys_[facen].inited) {
    facePolys_[facen].value << corner_[0] << corner_[1] << corner_[2] << corner_[3];
    facePolys_[facen].value.translate(facen == 0 ? getNormal()*(-getStereoDistance()/2) : getNormal()*getStereoDistance()/2); // in case of single-sided modules stereodistance is 0 so the translation is null
    facePolys_[facen].inited = geometryLocked();
  }
  return facePolys_[facen].value;
}
