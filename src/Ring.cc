...
inline void Ring::check() {
  ...
  if (moduleShape() == ModuleShape::WEDGE && buildDirection() == TOPDOWN)
    throw PathfulException("Wedge-module rings cannot be built top down");
}
...
void Ring::cutAtEta(double eta) {
  modules_.erase_if(
      [eta](EndcapModule &m) { return fabs(m.center().Eta()) > eta; });
}

double Ring::computeTentativePhiAperture(double moduleWaferDiameter,
                                         double minRadius) {
  ...
  if (i >= MAX_WEDGE_CALC_LOOPS) {
    // logWarning("Maximum number of iterations hit while computing wedge
    // geometry");
  }
...
std::pair<double, int>
Ring::computeOptimalRingParametersWedge(double moduleWaferDiameter,
                                        double minRadius) {
  double delta = phiOverlap() /
                 minRadius; // SM: The needed overlap becomes an angle delta by
  //     checking the unsafest point (r=r_min)

  double tentativeAlpha =
      computeTentativePhiAperture(moduleWaferDiameter, minRadius) - delta;
  float tentativeNumMods = 2 * M_PI / tentativeAlpha;
  int optimalNumMods = (!requireOddModsPerSlice()
                            ? round(tentativeNumMods / phiSegments())
                            : roundToOdd(tentativeNumMods / phiSegments()) +
                                  additionalModules()) *
                       phiSegments();
...
std::pair<double, int>
Ring::computeOptimalRingParametersRectangle(double moduleWidth,
                                            double highRadius) {
  ...
  if ((modsPerSlice % 2) == 0 && requireOddModsPerSlice())
    modsPerSlice++;
...
void Ring::buildModules(EndcapModule *templ, int numMods, double smallDelta) {
  ...
  for (int i = 0, parity = smallParity(); i < numMods; i++, parity *= -1) {
    ...
    mod->myid(i + 1);
    mod->rotateZ(2. * M_PI * (i + alignmentRotation) /
                 numMods); // CUIDADO had a rotation offset of PI/2
    mod->rotateZ(zRotation());
    mod->translateZ(parity * smallDelta);
    mod->setIsSmallerAbsZModuleInRing(parity < 0);
    const bool isFlipped =
        (!isRingOn4Dees()
             ? (parity < 0) // Case where ring modules are placed on both sides
                            // of a same dee. Half of the ring modules are
                            // flipped: the ones placed with parity < 0, hence
                            // on the small |Z| side.
             : isSmallerAbsZRingInDisk()); // Case where ring modules are placed
                                           // on 4 dees. If the ring is on the
                                           // small |Z| side with respect to the
                                           // disk: all ring modules are
                                           // flipped.
...
void Ring::buildBottomUp() {
  ...
  if (ringOuterRadius() > 0) {
    logWARNING("outer radius was set for a bottom-up endcap building. Ignoring "
               "ringOuterRadius.");
  }
  if (ringInnerRadius() > 0) {
    if (ringGap() != 0)
      logWARNING(
          "innerRadius and ringGap were both specified. Ignoring ringGap.");
  ...
  if (moduleShape() == ModuleShape::WEDGE) {
    ...
    auto optimalRingParms = computeOptimalRingParametersWedge(
        wmod->waferDiameter(), buildStartRadius());
    ...
    auto optimalRingParms = computeOptimalRingParametersRectangle(
        rmod->width(), buildStartRadius() + rmod->length());
    numMods = optimalRingParms.second;
...
void Ring::buildTopDown() {
  double startRadius = buildStartRadius() - ringGap();
  if (ringInnerRadius() > 0) {
    logWARNING("inner radius was set for a top-down endcap building. Ignoring "
               "ringInnerRadius.");
  }
  if (ringOuterRadius() > 0) {
    if (ringGap() != 0)
      logWARNING(
          "outerRadius and ringGap were both specified. Ignoring ringGap.");
  ...
  auto optimalRingParms =
      computeOptimalRingParametersRectangle(rmod->width(), buildStartRadius());
  int numMods = optimalRingParms.second;

  EndcapModule *emod =
      GeometryFactory::make<EndcapModule>(rmod, subdetectorName());
  emod->store(propertyTree());
  emod->build();
...
void Ring::build() {
  ...
  // std::cout << "Ring::build()  : materialObject_.subdetectorName() = " <<
  // materialObject_.subdetectorName() << std::endl;
  materialObject_.build();

  if (materialObject_.isPopulated()) {
    logUniqueWARNING(
        "Is not possible to define rod material for disks, material ignored.");
  }

  try {
    logINFO(Form("Building %s", fullid(*this).c_str()));
    check();
    if (buildDirection() == BOTTOMUP)
      buildBottomUp();
    else
      buildTopDown();
...
/** This computes the actual coverage in Phi of a disk (after it is built).
    It calcualtes the actual phiOverlap, using the relevant coordinates of the
   disk.
 */
...
const MaterialObject &Ring::materialObject() const { return materialObject_; }
...
void TiltedRing::check() {
  ...
  if (ringZOverlap.state()) {
    if (zInner.state() || zOuter.state())
      throw PathfulException("Only one parameter among ringZOverlap, "
                             "ringInnerZ, and ringOuterZ can be specified.");
  } else {
    if (!zInner.state() && !zOuter.state())
      throw PathfulException("At least one parameter among ringZOverlap, "
                             "ringInnerZ, and ringOuterZ must be specified.");
    if (zInner.state() && zOuter.state())
      throw PathfulException("Only one parameter among ringZOverlap, "
                             "ringInnerZ, and ringOuterZ can be specified.");
  }
...
void TiltedRing::buildLeftRight(double lastThetaEnd) {
  ...
  if (thetaStart_ == (M_PI / 2.)) {
    throw PathfulException(
        "Start building tilted rings at thetaStart = M_PI/2.");
    ...
    if (ringZOverlap.state()) {
      ...
      thetaOuterUP_ =
          atan(outerRadius() /
               (outerRadius() / tan(thetaStart_) +
                dsDistance * cos(tilt) / (2. * tan(thetaStart_)) +
                lengthEff * sin(tilt) / (2. * tan(thetaStart_)) -
                dsDistance / 2. * sin(tilt) + lengthEff / 2. * cos(tilt)));
      ...
      /*std::cout << "thetaStart_ * 180. / M_PI = " << thetaStart_ * 180. / M_PI
        << std::endl; std::cout << "outerRadius() = " << outerRadius() <<
        std::endl; std::cout << "lengthEff = " << lengthEff << std::endl;
        std::cout << "ringZOverlap() = " << ringZOverlap() << std::endl;
        std::cout << "thetaOuterUP_  * 180. / M_PI = " << thetaOuterUP_  * 180.
        / M_PI << std::endl; std::cout << "thetaOuterDOWN_  * 180. / M_PI = " <<
        thetaOuterDOWN_  * 180. / M_PI << std::endl; std::cout << "thetaOuter_
        * 180. / M_PI = " << thetaOuter_  * 180. / M_PI << std::endl;
    ...
    // CASE B : ZINNER OR ZOUTER IS SPECIFIED IN INPUT
      ...
      if (zOuter.state()) {
        if (zOuter() == 0.)
          throw PathfulException("Start building tilted rings at zOuter = 0.");
        zInner(zOuter() - (outerRadius() - innerRadius()) / tan(theta_gRad));
      }

      // If zInner is set, calculate zOuter
      if (zInner.state())
        zOuter(zInner() + (outerRadius() - innerRadius()) / tan(theta_gRad));

      // Calculate thetaOuter
      thetaOuter_ = atan(outerRadius() / zOuter());

      // Calculate ringZOverlap
      double ringZOverlapUP =
          0.5 *
          (length - (outerRadius() / tan(thetaOuter_) -
                     outerRadius() / tan(thetaStart_) -
                     dsDistance * cos(tilt) / (2. * tan(thetaStart_)) +
                     dsDistance * sin(tilt) / 2.) /
                        (sin(tilt) / (2. * tan(thetaStart_)) + cos(tilt) / 2.));

      double ringZOverlapDOWN =
          0.5 *
          (length - (outerRadius() / tan(thetaOuter_) -
                     outerRadius() / tan(thetaStart_) +
                     dsDistance * cos(tilt) / (2. * tan(thetaStart_)) -
      ...
      // std::cout << " ringZOverlapUP = " <<  ringZOverlapUP <<
      // "ringZOverlapDOWN = " << ringZOverlapDOWN << std::endl;
  ...
  /*std::cout << "zH2ppUP = " << zH2ppUP << " zH2ppDOWN = " << zH2ppDOWN <<
  std::endl; std::cout << "rH2ppUP = " << rH2ppUP <<" rH2ppDOWN = " << rH2ppDOWN
  << std::endl; std::cout << "atan(rH2ppUP / zH2ppUP) = " << atan(rH2ppUP /
  zH2ppUP) << std::endl; std::cout << "MAX( atan(rH2ppUP / zH2ppUP),
  atan(rH2ppDOWN / zH2ppDOWN)) = " << MAX( atan(rH2ppUP / zH2ppUP),
  atan(rH2ppDOWN / zH2ppDOWN)) << std::endl;*/
  ...
  // thetaEndInner_ = MAX( atan(rH1ppUP / zH1ppUP), atan(rH1ppDOWN /
  // zH1ppDOWN));

  // FOR INFO

  // phiOverlapDEG_ = atan(width / (2.* rH2pUP)) + atan(width / (2.* rH1pUP))
  // - 2. * M_PI / numPhi();
  ...
  if (fabs(rH2ppUP_REAL / zH2ppUP_REAL) >
      fabs(rH2ppDOWN_REAL / zH2ppDOWN_REAL)) {
...
void TiltedRing::build(double lastThetaEnd) {
  ...
  try {
    ...
    std::cout
        << pe.what()
        << std::endl; // TO DO : should not be necessary to specify pe.what() !!
                      // Problem in fullid from capabilities.h ?
    pe.pushPath(fullid(*this));
    throw;
  }
