...
void DetectorModule::setup() {
  nominalResolutionLocalX.setup([this]() {
    ...
    if (!hasAnyResolutionLocalXParam()) {
      // std::cout << "nominalResolutionLocalX and resolutionLocalXBarrel
      // parameters are all unset. Use of default formulae." << std::endl;
      double res = 0;
      for (const Sensor &s : sensors())
        res += pow(meanWidth() / s.numStripsAcrossEstimate() / sqrt(12), 2);
  ...
  nominalResolutionLocalY.setup([this]() {
    ...
    if (!hasAnyResolutionLocalYParam()) {
      if (stereoRotation() != 0.)
        return nominalResolutionLocalX() / sin(stereoRotation());
      else {
        return length() / maxSegments() /
               sqrt(12); // NOTE: not combining measurements from both sensors.
                         // The two sensors are closer than the length of the
                         // longer sensing element, making the 2 measurements
                         // correlated. considering only the best measurement is
                         // then a reasonable approximation (since in case of a
                         // PS module the strip measurement increases the
                         // precision by only 0.2% and in case of a 2S the
                         // sensors are so close that they basically always
  ...
  for (Sensor &s : sensors_)
    s.parent(this); // set the parent for the sensors once again (in case the
                    // module's been cloned)
};
...
void DetectorModule::check() {
  ...
  if (nominalResolutionLocalX.state() && hasAnyResolutionLocalXParam())
    throw PathfulException("Only one between resolutionLocalX and "
                           "resolutionLocalXParameters can be specified.");

  if (hasAnyResolutionLocalXParam()) {
    if (!resolutionLocalXParam0.state() || !resolutionLocalXParam1.state() ||
        !resolutionLocalXParam2.state() || !resolutionLocalXParam3.state() ||
        !resolutionLocalXParam4.state() || !resolutionLocalXParam5.state() ||
        !resolutionLocalXParam6.state() || !resolutionLocalXParam7.state() ||
        !resolutionLocalXParam8.state() || !resolutionLocalXParam9.state())
      throw PathfulException(
          "Local Y spatial resolution, tilted module. Resolution cfg file not "
          "properly chosen. You did not assign a resolution cfg file specific "
          "to a tilted module.");
  }

  // LOCAL Y RESOLUTION PARAMETERS
  if (nominalResolutionLocalY.state() && hasAnyResolutionLocalYParam())
    throw PathfulException("Only one between resolutionLocalY and "
                           "resolutionLocalYParameters can be specified.");

  if (hasAnyResolutionLocalYParam()) {
    if (!resolutionLocalYParam0.state() || !resolutionLocalYParam1.state() ||
        !resolutionLocalYParam2.state() || !resolutionLocalYParam3.state() ||
        !resolutionLocalYParam4.state() || !resolutionLocalYParam5.state() ||
        !resolutionLocalYParam6.state() || !resolutionLocalYParam7.state() ||
        !resolutionLocalYParam8.state() || !resolutionLocalYParam9.state())
      throw PathfulException(
          "Local Y spatial resolution, tilted module. Resolution cfg file not "
          "properly chosen. You did not assign a resolution cfg file specific "
          "to a tilted module.");
  ...
  if (numSensors() > 0) {
    for (int i = 0; i < numSensors(); i++) {
      ...
      if (numSensors() == 1)
        ...
        if (i == 0)
          s->innerOuter(SensorPosition::LOWER);
        else if (i == 1)
          s->innerOuter(SensorPosition::UPPER);
        else
          s->innerOuter(SensorPosition::NO);
      }
      s->build();
    ...
    Sensor *s =
        GeometryFactory::make<Sensor>(); // fake sensor to avoid defensive
                                         // programming when iterating over the
                                         // sensors and the module is empty
    s->parent(this);
    s->myid(1);
    s->innerOuter(SensorPosition::NO);
    s->build();
...
std::map<std::string, double> DetectorModule::extremaWithHybrids() const {
  ...
  double expandedModLength =
      length() + outerSensorExtraLength() + 2 * frontEndHybridWidth();
  double expandedModThickness;
  if (!isPixelModule()) {
    expandedModThickness =
        dsDistance() + 2.0 * (supportPlateThickness() + sensorThickness());
  }
  // double expandedModThickness = dsDistance() +
  // supportPlateThickness()+sensorThickness(); SHOULD BE THIS !!!!
  else {
    expandedModThickness =
  ...
  XYZVector mx =
      (0.5 * (basePoly().getVertex(2) + basePoly().getVertex(3)) - center())
          .Unit();
  XYZVector my =
      (0.5 * (basePoly().getVertex(1) + basePoly().getVertex(2)) - center())
          .Unit();
...
DetectorModule::minMaxEtaWithError(double zError) const {
  if (cachedZError_ != zError) {
    ...
    // cachedMinMaxEtaWithError_ = std::make_pair(MIN(eta1, eta2), MAX(eta1,
    // eta2));
  }
...
bool DetectorModule::couldHit(const XYZVector &direction, double zError) const {
  ...
  if (eta > minEtaWithError(zError) && eta < maxEtaWithError(zError))
    withinEta = true;

  // Phi region is from <-pi;+3*pi> due to crossline at +pi -> need to check phi
  // & phi+2*pi
  if ((phi >= minPhi() && phi <= maxPhi()) ||
      (shiftPhi >= minPhi() && shiftPhi <= maxPhi()))
    withinPhi = true;

  // Checking that hit within a module region works for barrel-type modules
  // only!!!
  if (this->shape() == ModuleShape::RECTANGULAR)
    return (withinEta && withinPhi);
  // ATTENTION: For wedge shaped modules, min, max procedure will not work
  // correctly -> return true to avoid errors --> will be implemented in the
  // future
  else
    return true;
}

// bool DetectorModule::couldHit(const XYZVector& direction, double zError)
// const {
//  double eta = direction.Eta(), phi = direction.Phi();
//  bool withinEta = eta > minEtaWithError(zError) && eta <
//  maxEtaWithError(zError); bool withinPhi; if (minPhi() < 0. && maxPhi() > 0.
//  && maxPhi()-minPhi() > M_PI) // across PI
 ...
 */
double DetectorModule::resolutionEquivalentRPhi(double hitRho, double trackR,
                                                double resolutionLocalX,
                                                double resolutionLocalY) const {
  double A = hitRho / (2 * trackR);
  double B = A / sqrt(1 - A * A);
  return sqrt(pow((B * sin(skewAngle()) * cos(tiltAngle()) + cos(skewAngle())) *
                      resolutionLocalX,
                  2) +
              pow(B * sin(tiltAngle()) * resolutionLocalY, 2));
}

/*
 * RETURN GLOBAL SPATIAL RESOLUTION ON Y AXIS.
 */
double DetectorModule::resolutionEquivalentZ(double hitRho, double trackR,
                                             double trackCotgTheta,
                                             double resolutionLocalX,
                                             double resolutionLocalY) const {
  double A = hitRho / (2 * trackR);
  double D = trackCotgTheta / sqrt(1 - A * A);
  return sqrt(
      pow(((D * cos(tiltAngle()) + sin(tiltAngle())) * sin(skewAngle())) *
              resolutionLocalX,
          2) +
      pow((D * sin(tiltAngle()) + cos(tiltAngle())) * resolutionLocalY, 2));
}

/*
 * RETURN LOCAL SPATIAL RESOLUTION ON X AXIS.
 * This resolution is nominal by default, and parametrized if the
 * paramnetrization model is called.
 */
const double
DetectorModule::resolutionLocalX(const TVector3 &trackDirection) const {
  if (!hasAnyResolutionLocalXParam()) {
    return nominalResolutionLocalX();
  } else {
    return calculateParameterizedResolutionLocalX(trackDirection);
  }
}
 ...
 */
const double
DetectorModule::resolutionLocalY(const TVector3 &trackDirection) const {
  if (!hasAnyResolutionLocalYParam()) {
    return nominalResolutionLocalY();
  } else {
    return calculateParameterizedResolutionLocalY(trackDirection);
  }
}
 ...
 */
const double DetectorModule::calculateParameterizedResolutionLocalX(
    const TVector3 &trackDirection) const {
  const double tanLorentzAngle =
      (is3DPixelModule()
           ? 0.
           : 0.053 * SimParms::getInstance().magField() *
                 cos(tiltAngle()) // dependancy on tilt angle is here!!! :)
      );
  const double cotanAlpha =
      1. / tan(alpha(trackDirection)); // Riccardo's theta = alpha - Pi/2    =>
                                       // than(theta) = -cotan(alpha)
  const double fabsTanDeepAngle = fabs(-cotanAlpha - tanLorentzAngle);

  const bool isLocalXAxis = true;
  const double resolutionLocalX =
      calculateParameterizedResolutionLocalAxis(fabsTanDeepAngle, isLocalXAxis);
 ...
 */
const double DetectorModule::calculateParameterizedResolutionLocalY(
    const TVector3 &trackDirection) const {
  const double cotanBeta =
      1. / tan(beta(trackDirection)); // Riccardo's theta = beta - Pi/2    =>
                                      // than(theta) = -cotan(beta)
  const double fabsTanDeepAngle = fabs(-cotanBeta);

  const bool isLocalXAxis = false;
  const double resolutionLocalY =
      calculateParameterizedResolutionLocalAxis(fabsTanDeepAngle, isLocalXAxis);
 ...
 */
const double DetectorModule::calculateParameterizedResolutionLocalAxis(
    const double fabsTanDeepAngle, const bool isLocalXAxis) const {
  // Parameter values
  const double param0 =
      (isLocalXAxis ? resolutionLocalXParam0() : resolutionLocalYParam0());
  const double param1 =
      (isLocalXAxis ? resolutionLocalXParam1() : resolutionLocalYParam1());
  const double param2 =
      (isLocalXAxis ? resolutionLocalXParam2() : resolutionLocalYParam2());
  const double param3 =
      (isLocalXAxis ? resolutionLocalXParam3() : resolutionLocalYParam3());
  const double param4 =
      (isLocalXAxis ? resolutionLocalXParam4() : resolutionLocalYParam4());
  const double param5 =
      (isLocalXAxis ? resolutionLocalXParam5() : resolutionLocalYParam5());
  const double param6 =
      (isLocalXAxis ? resolutionLocalXParam6() : resolutionLocalYParam6());
  const double param7 =
      (isLocalXAxis ? resolutionLocalXParam7() : resolutionLocalYParam7());
  const double param8 =
      (isLocalXAxis ? resolutionLocalXParam8() : resolutionLocalYParam8());
  const double param9 =
      (isLocalXAxis ? resolutionLocalXParam9() : resolutionLocalYParam9());

  // |tan(deepAngle)|
  const double x = fabsTanDeepAngle;

  // Use of parametrization model
  const double resolutionLocalAxis =
      param0 + param1 * x +
...
/*
 * Chekc whether the value of at least one parameter to compute a parametrized
 * spatial resolution in X, is specified.
 */
const bool DetectorModule::hasAnyResolutionLocalXParam() const {
  return (resolutionLocalXParam0.state() || resolutionLocalXParam1.state() ||
          resolutionLocalXParam2.state() || resolutionLocalXParam3.state() ||
          resolutionLocalXParam4.state() || resolutionLocalXParam5.state() ||
          resolutionLocalXParam6.state() || resolutionLocalXParam7.state() ||
          resolutionLocalXParam8.state() || resolutionLocalXParam9.state());
...
/*
 * Chekc whether the value of at least one parameter to compute a parametrized
 * spatial resolution in Y, is specified.
 */
const bool DetectorModule::hasAnyResolutionLocalYParam() const {
  return (resolutionLocalYParam0.state() || resolutionLocalYParam1.state() ||
          resolutionLocalYParam2.state() || resolutionLocalYParam3.state() ||
          resolutionLocalYParam4.state() || resolutionLocalYParam5.state() ||
          resolutionLocalYParam6.state() || resolutionLocalYParam7.state() ||
          resolutionLocalYParam8.state() || resolutionLocalYParam9.state());
...
const double DetectorModule::alpha(const TVector3 &trackDirection) const {
  ...
  const TVector3 &projectedTrack =
      CoordinateOperations::projectv1OnPlaneOfNormalUnitv2(trackDirection,
                                                           localY);
  ...
  if (alpha >= M_PI) {
    logERROR("alpha angle should be in ]0 180[, but found alpha = " +
             any2str(alpha * 180. / M_PI));
  }

  /* Keep old formula.
     This old formula is only true in (CMS_X, CMS_Y) plane.
     It is is a projection of the new formula onto the (CMS_X, CMS_Y) plane
     (perfect match).

     const double trackPhi = trackDirection.Phi();

     // Oriented angle: between the normal to the sensor (oriented towards outer
     radius) and the track vector. const double sensorNormalToTrackDeltaPhi =
     trackPhi - (center().Phi() + skewAngle());

     // Oriented angle: between local X vector (= Lorentz drift) and the track
     vector. const double sensorXToTrackDeltaPhi = (!flipped() ? M_PI / 2. +
     sensorNormalToTrackDeltaPhi : M_PI / 2. - sensorNormalToTrackDeltaPhi);
...
const double DetectorModule::beta(const TVector3 &trackDirection) const {
  ...
  if (beta >= M_PI) {
    logERROR("beta angle should be in ]0 180[, but found beta = " +
             any2str(beta * 180. / M_PI));
  }

  /* Keep old formula.
     This old formula is not really correct.
     It does not take into account a dependency in track vector's phi (true
     story!!), which occurs for both the barrel and the forward. This is because
     we are interested in the PROJECTION of the track vector into the plane of
     normal localX. When we project the track vector into the plane of normal
     localX: In the event the hit is not in the (sensor center, CMS_Z) plane, or
     if the module is skewed:
     * the component in (CMS_X, CMS_Y) plane of the track vector gets a smaller
     norm.
     * the component along CMS_Z of the track vector stays the same.
     Hence, the projected track vector is more 'parallel to CMS_Z' than the
     track vector. This implies that beta angle is further from M_PI/2 than eta
     is the barrel, and closer to M_PI/2 than eta in the forward. The effect is
     quite negligible though, except for skewed ladders. It is way cleaner to
     have a vectorial formula amyway.

     const double beta = theta + tiltAngle();

     // Oriented angle: between local Y vector and the track vector.
     const double orientedBeta = (fabs(tiltAngle() - M_PI/2.) < insur::geom_zero
     ? beta : (M_PI - beta));
  */
...
double DetectorModule::stripOccupancyPerEventBarrel() const {
  ...
  double myOccupancyBarrel =
      (1.63e-4) + (2.56e-4) * rho - (1.92e-6) * rho * rho;
  ...
  double occupancy = myOccupancyBarrel / factor / (90 / 1e3) *
                     (dphideta / minNSegments) *
                     (modWidth / numStripsAcrossEstimate);
...
double DetectorModule::stripOccupancyPerEventEndcap() const {
  ...
  double myOccupancyEndcap =
      (-6.20e-5) + (1.75e-4) * rho - (1.08e-6) * rho * rho + (1.50e-5) * (z);
  ...
  double occupancy = myOccupancyEndcap / factor / (90 / 1e3) *
                     (dphideta / minNSegments) *
                     (modWidth / numStripsAcrossEstimate);
...
double DetectorModule::stripOccupancyPerEvent() const {
  if (fabs(tiltAngle()) < 1e-3)
    return stripOccupancyPerEventBarrel();
  else if (fabs(tiltAngle()) - M_PI / 2. < 1e-3)
    return stripOccupancyPerEventEndcap();
  else
    return stripOccupancyPerEventBarrel() * pow(cos(tiltAngle()), 2) +
           stripOccupancyPerEventEndcap() * pow(sin(tiltAngle()), 2);
};

double DetectorModule::geometricEfficiency() const {
  double inefficiency = fabs(
      dsDistance() /
      (zCorrelation() == SAMESEGMENT ? outerSensor().stripLength() : length()) /
      tan(center().Theta() +
          tiltAngle())); // fabs prevents the inefficiency from becoming
                         // negative when theta+tilt > 90 deg (meaning the
                         // geometrically inefficient area is on the other end
                         // of the module)
...
double DetectorModule::effectiveDsDistance() const {
  if (fabs(tiltAngle()) < 1e-3)
    ...
    return dsDistance() * sin(center().Theta()) /
           sin(center().Theta() + tiltAngle());
}

std::pair<XYZVector, HitType>
DetectorModule::checkTrackHits(const XYZVector &trackOrig,
                               const XYZVector &trackDir) {
  ...
  if (numSensors() == 1) {
    ...
    // <SMe>The following line used to return HitType::BOTH. Changing to INNER
    // in order to avoid double hit counting</SMe>
    if (segm.second > -1) {
      gc = segm.first;
      ht = HitType::INNER;
    }
    ...
    if (inSegm.second > -1 && outSegm.second > -1) {
      gc = inSegm.first; // in case of both sensors are hit, the inner sensor
                         // hit coordinate is returned
      ht = ((zCorrelation() == SAMESEGMENT &&
             (inSegm.second / (maxSegments() / minSegments()) ==
              outSegm.second)) ||
  ...
  // basePoly().isLineIntersecting(trackOrig, trackDir, gc); // this was just
  // for debug
  if (ht != HitType::NONE)
    numHits_++;
...
std::string DetectorModule::summaryType() const {
  ...
  result += moduleType();
  if (dsDistance() != 0)
    result += " " + any2str(dsDistance(), 1) + " mm";
  return result;
};
...
void BarrelModule::build() {
  try {
    ...
    tiltAngle_ = 0.;
    for (auto &s : sensors_) {
      s.subdet(ModuleSubdetector::BARREL);
    }
...
void EndcapModule::build() {
  try {
    ...
    tiltAngle_ = M_PI / 2.;
    for (auto &s : sensors_) {
      s.subdet(ModuleSubdetector::ENDCAP);
    }
  } catch (PathfulException &pe) {
    pe.pushPath(*this, myid());
    throw;
