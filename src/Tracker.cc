...
std::pair<double, double> Tracker::computeMinMaxEta() const {
  ...
  return std::make_pair(-4.0, 4.0); // CUIDADO to make it equal to the extended
                                    // pixel - make it better ASAP!!
}
...
void Tracker::build() {
  try {
    ...
    for (auto &mapel : barrelNode) {
      if (!containsOnly.empty() && containsOnly.count(mapel.first) == 0)
        continue;
  ...
  // All Tracker Disks are numbered by increasing fabs(Z). Numbering starts from
  // 1 on (+Z) side, and from 1 on (-Z) side.
  addLayerDiskNumbers();
...
/**
 * Add geometry hierarchy information to modules.
 * The Ids here have nothing to do with DetIds: they are the numbers which
...
void Tracker::addHierarchyInfoToModules() {
  ...
  public:
    void visit(Barrel &b) { subdetectorId_++; }
    void visit(Endcap &e) { subdetectorId_++; }
    void visit(Layer &l) { layerDiskId_ = l.myid(); }
    void visit(Disk &d) { layerDiskId_ = d.myid(); }
    void visit(RodPair &r) { rodRingId_ = r.myid(); }
    void visit(Ring &r) {
      rodRingId_ = r.myid();
    ...
    void visit(Module &m) { m.subdetectorId(subdetectorId_); }
    ...
    void visit(EndcapModule &m) {
      ...
      int smallDeltaIndex =
          (m.isSmallerAbsZModuleInRing() ? 0 : moduleIncrement);
 ...
 * Add Layers and Disks a global Tracker numbering.
 * All Tracker Layers are numbered by increasing radius.
 * All Tracker Disks are numbered by increasing fabs(Z). Numbering starts from 1
 * on (+Z) side, and from 1 on (-Z) side.
 */
void Tracker::addLayerDiskNumbers() {
  ...
  public:
    ...
    void postVisit() {
      std::sort(trackerLayers_.begin(), trackerLayers_.end(),
                [](const Layer *l1, const Layer *l2) {
                  return l1->minR() < l2->minR();
                });
      int layerNumber = 1;
      ...
      std::sort(trackerDisks_.begin(), trackerDisks_.end(),
                [](const Disk *d1, const Disk *d2) {
                  return fabs(d1->centerZ()) < fabs(d2->centerZ());
                });
...
void Tracker::buildDetIds() {
  // Barrel part
  std::vector<int> barrelScheme =
      mainConfigHandler::instance().getDetIdScheme(barrelDetIdScheme());
  if (barrelScheme.size() != 0) {
    BarrelDetIdBuilder v(isPixelTracker(), barrelScheme);
    accept(v);
  }
  // Endcap part
  std::vector<int> endcapScheme =
      mainConfigHandler::instance().getDetIdScheme(endcapDetIdScheme());
 ...
 * NB : If DetIds are replicated outside of the same Tracker, this will not be
 * noticed. Though, this cannot happen by construction.
 */
void Tracker::checkDetIds() {
  ...
  for (const auto &m : modules) {
    ...
    if (found != moduleDetIds.end() && myModuleDetId != 0)
      logWARNING(any2str(myid()) + ": Error while building DetIds !! DetId " +
                 any2str(myModuleDetId) + " is duplicated.");
    else
      moduleDetIds.insert(myModuleDetId);
    ...
    for (const auto &s : m->sensors()) {
      ...
      if (found != sensorDetIds.end() && mySensorDetId != 0)
        logWARNING(any2str(myid()) + ": Error while building DetIds !! DetId " +
                   any2str(mySensorDetId) + " is duplicated.");
      else
        sensorDetIds.insert(mySensorDetId);
    }
  }
}
