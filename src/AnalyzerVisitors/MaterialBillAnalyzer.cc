...
void MaterialBillAnalyzer::inspectInactiveElements(
    const std::vector<InactiveElement> &inactiveElements) {
...
void MaterialBillAnalyzer::inspectModules(
    std::vector<std::vector<insur::ModuleCap>> &tracker) {
  // loop over layers
    ...
    for (auto moduleIt : layerIt) {
      ...
      struct Visitor : public ConstGeometryVisitor {
        ...
        void visit(const BarrelModule &m) {
          id_ = m.subdetectorName() + "_L" + any2str(m.layer());
        }
        void visit(const EndcapModule &m) {
          id_ = m.subdetectorName() + "_D" + any2str(m.disk());
        }
      };
      Visitor v;
      myModule->accept(v);
      MaterialMap &layerMaterial = layerMaterialMap_[v.id_];
      const std::map<std::string, double> &localMasses =
          myModuleCap.getLocalMasses();
      for (const auto &it : localMasses)
        layerMaterial[it.first] += it.second;
    }
  }
}
