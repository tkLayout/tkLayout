...
double AnalyzerHelpers::calculatePetalAreaMC(const Tracker& tracker, const SimParms& simParms, double crossoverR) {
  ...
  double r = simParms.triggerPtCut()/(0.3*simParms.magneticField()) * 1e3; // curvature radius of particles with the minimum accepted pt
...
bool AnalyzerHelpers::isModuleInPhiSector(const SimParms& simParms, const DetectorModule& module, double crossoverR, int phiSector) {
  static const double curvatureR = simParms.triggerPtCut()/(0.3*simParms.magneticField()) * 1e3; // curvature radius of particles with the minimum accepted pt
