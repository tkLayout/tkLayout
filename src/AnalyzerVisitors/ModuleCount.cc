...
RootWTable *ModuleCountVisitor::makeTable() {
  ...
  for (auto aType : moduleTypes)
    moduleCountTable->setContent(++iType, 0, aType.first);
  for (auto aSub : subDetectors)
    moduleCountTable->setContent(0, ++iSub, aSub.first);
  ...
  for (auto aType : moduleTypes) {
    ...
    for (auto aSub : subDetectors) {
      ...
      if (cellContent != 0)
        moduleCountTable->setContent(iType, iSub, cellContent);
  ...
  for (auto aType : moduleTypes)
    moduleCountTable->setContent(++iType, totalSub, typeTotal[aType.first]);
  for (auto aSub : subDetectors)
    moduleCountTable->setContent(totalType, ++iSub, subTotal[aSub.first]);
