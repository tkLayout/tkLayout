...
void LayerDiskSummaryVisitor::visit(const Ring &r) {
  if (r.averageZ() < 0. || r.numModules() == 0)
    return;
  ++nRings;
  diskTable->setContent(3, nDisks, nRings);
  endcapTables.at(nEndcaps - 1)->setContent(0, nRings, r.myid());
  endcapTables.at(nEndcaps - 1)
      ->setContent(6, nRings, r.actualPhiOverlap(), coordPrecision);
  endcapTables.at(nEndcaps - 1)->setContent(7, nRings, r.numModules());
  zErrorTables.at(nDisks - 1)->setContent(0, nRings, r.myid());
  if (nRings != nRingsTotal)
    zErrorTables.at(nDisks - 1)
        ->setContent(1, nRings, r.actualZError(), coordPrecision);
}
...
void LayerDiskSummaryVisitor::visit(const Module &m) {
  ...
  tagMapMaxStripOccupancy[aSensorTag] = MAX(
      m.stripOccupancyPerEvent() * nMB, tagMapMaxStripOccupancy[aSensorTag]);
  tagMapMaxHitOccupancy[aSensorTag] =
      MAX(m.hitOccupancyPerEvent() * nMB, tagMapMaxHitOccupancy[aSensorTag]);
  ...
  // modules' spatial resolution along the local X axis is parametrized
    ...
    if (boost::accumulators::count(m.rollingParametrizedResolutionLocalX) >
        0) { // calculation only on hit modules
      tagMapSumXResolution[aSensorTag] +=
          sum(m.rollingParametrizedResolutionLocalX);
      tagMapSumSquaresXResolution[aSensorTag] +=
          moment<2>(m.rollingParametrizedResolutionLocalX) *
          boost::accumulators::count(m.rollingParametrizedResolutionLocalX);
      tagMapCountXResolution[aSensorTag] += double(
          boost::accumulators::count(m.rollingParametrizedResolutionLocalX));
    }
  }
  ...
  // modules' spatial resolution along the local Y axis is parametrized
    ...
    if (boost::accumulators::count(m.rollingParametrizedResolutionLocalY) >
        0) { // calculation only on hit modules
      tagMapSumYResolution[aSensorTag] +=
          sum(m.rollingParametrizedResolutionLocalY);
      tagMapSumSquaresYResolution[aSensorTag] +=
          moment<2>(m.rollingParametrizedResolutionLocalY) *
          boost::accumulators::count(m.rollingParametrizedResolutionLocalY);
      tagMapCountYResolution[aSensorTag] += double(
          boost::accumulators::count(m.rollingParametrizedResolutionLocalY));
    }
  }
  ...
  if (tagMapSensorPowerMax[aSensorTag] < m.sensorsIrradiationPowerMean())
    tagMapSensorPowerMax[aSensorTag] = m.sensorsIrradiationPowerMean();
  totCountMod++;
...
void LayerDiskSummaryVisitor::visit(const EndcapModule &m) {
  ...
  endcapTables.at(nEndcaps - 1)
      ->setContent(1, nRings, m.minR(), coordPrecision);
  endcapTables.at(nEndcaps - 1)
      ->setContent(
          2, nRings, sqrt(pow(m.minR(), 2) + pow(m.minWidth() / 2., 2)),
          coordPrecision); // Ugly, this should be accessible as a method
  endcapTables.at(nEndcaps - 1)
      ->setContent(3, nRings, m.center().Rho(), coordPrecision);
  endcapTables.at(nEndcaps - 1)
      ->setContent(4, nRings, m.minR() + m.length(), coordPrecision);
  endcapTables.at(nEndcaps - 1)
      ->setContent(5, nRings, m.maxR(), coordPrecision);
}
...
void SkewedLayersVisitor::visit(const Layer &l) {
  ...
  if (l.isSkewedForInstallation()) {
    ...
    layerTable->setContent(
        4, 0, "inner module Rho" + subStart + "center" + subEnd + " [mm]");
    layerTable->setContent(4, 1, l.placeRadiusHint() - l.bigDelta(),
                           coordPrecision);
    layerTable->setContent(
        5, 0, "outer module Rho" + subStart + "center" + subEnd + " [mm]");
    layerTable->setContent(5, 1, l.placeRadiusHint() + l.bigDelta(),
                           coordPrecision);
    layerTable->setContent(
        6, 0, "skewed module Rho" + subStart + "center" + subEnd + " [mm]");
    layerTable->setContent(6, 1, l.skewedModuleCenterRho(), coordPrecision);
    layerTable->setContent(
        7, 0, "skewed module Rho" + subStart + "min" + subEnd + " [mm]");
    layerTable->setContent(7, 1, l.skewedModuleMinRho(), coordPrecision);
    layerTable->setContent(
        8, 0, "skewed module Rho" + subStart + "max" + subEnd + " [mm]");
    ...
    layerTable->setContent(
        12, 0, "horizontal overlap with (X=0) plane (each IT half) [mm]");
    layerTable->setContent(12, 1, l.installationHorizontalOverlapLength(),
                           coordPrecision);
...
void TiltedLayersVisitor::visit(const Layer &l) {
  ...
  if (l.isTilted() && l.isTiltedAuto()) {
    ...
    tiltedLayerName->setContent(0, 0,
                                "Layer " + std::to_string(l.myid()) + " :");
    ...
    for (int i = 0; i < numTiltedRings; i++) {
      ...
      tiltedPartTable->setContent(
          1, i + 1, l.tiltedRingsGeometry()[ringNumber]->tiltAngle(),
          anglePrecision);
      tiltedPartTable->setContent(
          2, 0, "tiltAngleIdeal" + subStart + "Inner" + subEnd + " (°)");
      tiltedPartTable->setContent(
          2, i + 1, l.tiltedRingsGeometry()[ringNumber]->tiltAngleIdealInner(),
          anglePrecision);
      tiltedPartTable->setContent(
          3, 0, "deltaTiltIdeal" + subStart + "Inner" + subEnd + " (°)");
      tiltedPartTable->setContent(
          3, i + 1, l.tiltedRingsGeometry()[ringNumber]->deltaTiltIdealInner(),
          anglePrecision);
      tiltedPartTable->setContent(
          4, 0, "tiltAngleIdeal" + subStart + "Outer" + subEnd + " (°)");
      tiltedPartTable->setContent(
          4, i + 1, l.tiltedRingsGeometry()[ringNumber]->tiltAngleIdealOuter(),
          anglePrecision);
      tiltedPartTable->setContent(
          5, 0, "deltaTiltIdeal" + subStart + "Outer" + subEnd + " (°)");
      tiltedPartTable->setContent(
          5, i + 1, l.tiltedRingsGeometry()[ringNumber]->deltaTiltIdealOuter(),
          anglePrecision);
      tiltedPartTable->setContent(6, 0, "theta_g (°)");
      tiltedPartTable->setContent(
          6, i + 1, l.tiltedRingsGeometry()[ringNumber]->theta_g(),
          anglePrecision);
      tiltedPartTable->setContent(7, 0, "r" + subStart + "Inner" + subEnd);
      tiltedPartTable->setContent(
          7, i + 1, l.tiltedRingsGeometry()[ringNumber]->innerRadius(),
          coordPrecision);
      tiltedPartTable->setContent(8, 0, "r" + subStart + "Outer" + subEnd);
      tiltedPartTable->setContent(
          8, i + 1, l.tiltedRingsGeometry()[ringNumber]->outerRadius(),
          coordPrecision);
      tiltedPartTable->setContent(9, 0, "averageR (on Ring)");
      tiltedPartTable->setContent(
          9, i + 1, l.tiltedRingsGeometry()[ringNumber]->averageR(),
          coordPrecision);
      tiltedPartTable->setContent(10, 0, "gapR");
      tiltedPartTable->setContent(10, i + 1,
                                  l.tiltedRingsGeometry()[ringNumber]->gapR(),
                                  coordPrecision);
      tiltedPartTable->setContent(11, 0, "z" + subStart + "Inner" + subEnd);
      tiltedPartTable->setContent(11, i + 1,
                                  l.tiltedRingsGeometry()[ringNumber]->zInner(),
                                  coordPrecision);
      tiltedPartTable->setContent(12, 0, "z" + subStart + "Outer" + subEnd);
      tiltedPartTable->setContent(12, i + 1,
                                  l.tiltedRingsGeometry()[ringNumber]->zOuter(),
                                  coordPrecision);
      tiltedPartTable->setContent(13, 0, "averageZ (on Ring)");
      tiltedPartTable->setContent(
          13, i + 1, l.tiltedRingsGeometry()[ringNumber]->averageZ(),
          coordPrecision);
      tiltedPartTable->setContent(
          14, 0, "deltaZ" + subStart + "Inner" + subEnd + " (Ring i & i-1)");
      tiltedPartTable->setContent(
          14, i + 1, l.tiltedRingsGeometryInfo().deltaZInner()[ringNumber],
          coordPrecision);
      tiltedPartTable->setContent(
          15, 0, "deltaZ" + subStart + "Outer" + subEnd + " (Ring i & i-1)");
      tiltedPartTable->setContent(
          15, i + 1, l.tiltedRingsGeometryInfo().deltaZOuter()[ringNumber],
          coordPrecision);
      tiltedPartTable->setContent(16, 0, "phiOverlap");
      tiltedPartTable->setContent(
          16, i + 1, l.tiltedRingsGeometry()[ringNumber]->phiOverlap(),
          coordPrecision);
      tiltedPartTable->setContent(17, 0,
                                  "zOverlap" + subStart + "Outer" + subEnd);
      tiltedPartTable->setContent(
          17, i + 1, l.tiltedRingsGeometry()[ringNumber]->ringZOverlap(),
          zOverlapPrecision);
      double zErrorInner =
          l.tiltedRingsGeometryInfo().zErrorInner()[ringNumber];
      tiltedPartTable->setContent(
          18, 0, "zError" + subStart + "Inner" + subEnd + " (Ring i & i-1)");
      if (!std::isnan(zErrorInner))
        tiltedPartTable->setContent(18, i + 1, zErrorInner, coordPrecision);
      else
        tiltedPartTable->setContent(18, i + 1, "n/a");
      double zErrorOuter =
          l.tiltedRingsGeometryInfo().zErrorOuter()[ringNumber];
      tiltedPartTable->setContent(
          19, 0, "zError" + subStart + "Outer" + subEnd + " (Ring i & i-1)");
      if (!std::isnan(zErrorOuter))
        tiltedPartTable->setContent(19, i + 1, zErrorOuter, coordPrecision);
      else
        tiltedPartTable->setContent(19, i + 1, "n/a");
    }
    ...
    StraightRodPair *minusBigDeltaRod =
        (l.bigParity() > 0 ? l.flatPartRods().at(1) : l.flatPartRods().front());
    const auto &minusBigDeltaModules = minusBigDeltaRod->modules().first;
    StraightRodPair *plusBigDeltaRod =
        (l.bigParity() > 0 ? l.flatPartRods().front() : l.flatPartRods().at(1));
    ...
    for (const auto &m : minusBigDeltaModules) {
      ...
      flatPartTable->setContent(
          7, i + 1,
          (((minusBigDeltaRod->zPlusParity() * pow(-1, (i % 2))) > 0)
               ? l.flatPartPhiOverlapSmallDeltaPlus()
               : l.flatPartPhiOverlapSmallDeltaMinus()),
          coordPrecision);
      // In case beamSpotCover == false, zOverlap is the only parameter used as
      // a Z-coverage constraint in the geometry construction process. (There is
      // then no zError taken into account). As a result, it is interesting to
      // display zOverlap !
      int extraLine = 0;
      if (!l.flatPartRods().front()->beamSpotCover()) {
        ...
        flatPartTable->setContent(
            8, i + 1, l.flatPartRods().front()->zOverlap(), coordPrecision);
        // WARNING : zOverlap, in the geometry construction process, is one
        // value common for all flat part (or straight rod)
      }
      ...
      if (i > 0) {
        ...
        flatPartTable->setContent(8 + extraLine, 0,
                                  "zError" + subStart + "Inner" + subEnd +
                                      " (Ring i & i-1)");
        if (!std::isnan(zErrorInner))
          flatPartTable->setContent(8 + extraLine, i + 1, zErrorInner,
                                    coordPrecision);
        else
          flatPartTable->setContent(8 + extraLine, i + 1, "n/a");
        // Outer modules in the tilted rings
        double zErrorOuter = l.flatRingsGeometryInfo().zErrorOuter()[i];
        flatPartTable->setContent(9 + extraLine, 0,
                                  "zError" + subStart + "Outer" + subEnd +
                                      " (Ring i & i-1)");
        if (!std::isnan(zErrorOuter))
          flatPartTable->setContent(9 + extraLine, i + 1, zErrorOuter,
                                    coordPrecision);
        else
          flatPartTable->setContent(9 + extraLine, i + 1, "n/a");
      }
      i++;
    }
    i = 0;
...
void TrackerVisitor::preVisit() {
  // output_ <<
  // "Section/C:Layer/I:Ring/I:r_mm/D:z_mm/D:tiltAngle_deg/D:phi_deg/D:meanWidth_mm/D:length_mm/D:sensorSpacing_mm/D:sensorThickness_mm/D,
  // DetId/I" << std::endl;
  output_ << "DetId/U, BinaryDetId/B, Section/C, Layer/I, Ring/I, SensorCenter "
             "rho(mm), SensorCenter z(mm), tiltAngle_deg/D, skewAngle_deg/D, "
             "phi_deg/D, meanWidth_mm/D, length_mm/D, sensorSpacing_mm/D, "
             "sensorThickness_mm/D"
          << std::endl;
}

void TrackerVisitor::visit(const Barrel &b) { sectionName_ = b.myid(); }

void TrackerVisitor::visit(const Endcap &e) { sectionName_ = e.myid(); }

void TrackerVisitor::visit(const Layer &l) { layerId_ = l.myid(); }

void TrackerVisitor::visit(const Disk &d) { layerId_ = d.myid(); }

void TrackerVisitor::visit(const Module &m) {
  output_ << m.myDetId() << "," << m.myBinaryDetId() << "," << sectionName_
          << ", " << layerId_ << ", " << m.moduleRing() << ", " << std::fixed
          << std::setprecision(6) << m.center().Rho() << ", " << m.center().Z()
          << ", " << m.tiltAngle() * 180. / M_PI << ", "
          << m.skewAngle() * 180. / M_PI << ", "
          << m.center().Phi() * 180. / M_PI << ", " << m.meanWidth() << ", "
          << m.length() << ", " << m.dsDistance() << ", " << m.sensorThickness()
          << std::endl;
}
...
void BarrelVisitor::preVisit() {
  output_ << "DetId, BinaryDetId, Barrel-Layer name, SensorCenter rho(mm), "
             "SensorCenter z(mm), tiltAngle(deg), num mods, meanWidth(mm) "
             "(orthoradial), length(mm) (along Z), sensorSpacing(mm), "
             "sensorThickness(mm)"
          << std::endl;
}
void BarrelVisitor::visit(const Barrel &b) { barName_ = b.myid(); }
...
void BarrelVisitor::visit(const BarrelModule &m) {
  if (m.posRef().phi > 2)
    return;
  output_ << m.myDetId() << ", " << m.myBinaryDetId() << "," << barName_ << "-L"
          << layId_ << ", " << std::fixed << std::setprecision(6)
          << m.center().Rho() << ", " << m.center().Z() << ", "
          << m.tiltAngle() * 180. / M_PI << ", " << numRods_ / 2. << ", "
          << m.meanWidth() << ", " << m.length() << ", " << m.dsDistance()
          << ", " << m.sensorThickness() << std::endl;
}
...
void EndcapVisitor::preVisit() {
  output_ << "DetId, BinaryDetId, Endcap-Disc name, Ring, SensorCenter "
             "rho(mm), SensorCenter z(mm), tiltAngle(deg), phi(deg),  "
             "meanWidth(mm) (orthoradial), length(mm) (radial), "
             "sensorSpacing(mm), sensorThickness(mm)"
          << std::endl;
}

void EndcapVisitor::visit(const Endcap &e) { endcapName_ = e.myid(); }

void EndcapVisitor::visit(const Disk &d) { diskId_ = d.myid(); }

void EndcapVisitor::visit(const EndcapModule &m) {
  if (m.minZ() < 0.)
    return;

  output_ << m.myDetId() << ", " << m.myBinaryDetId() << "," << endcapName_
          << "-D" << diskId_ << ", " << m.ring() << ", " << std::fixed
          << std::setprecision(6) << m.center().Rho() << ", " << m.center().Z()
          << ", " << m.tiltAngle() * 180. / M_PI << ", "
          << m.center().Phi() * 180. / M_PI << ", " << m.meanWidth() << ", "
          << m.length() << ", " << m.dsDistance() << ", " << m.sensorThickness()
...
//*                                   //
//************************************//
void TrackerSensorVisitor::visit(Barrel &b) { sectionName_ = b.myid(); }

void TrackerSensorVisitor::visit(Endcap &e) { sectionName_ = e.myid(); }

void TrackerSensorVisitor::visit(Layer &l) { layerId_ = l.myid(); }

void TrackerSensorVisitor::visit(Disk &d) { layerId_ = d.myid(); }

void TrackerSensorVisitor::visit(Module &m) { moduleRing_ = m.moduleRing(); }

void TrackerSensorVisitor::visit(Sensor &s) {
  output_ << s.myDetId() << "," << s.myBinaryDetId() << "," << sectionName_
          << ", " << layerId_ << ", " << moduleRing_ << ", " << std::fixed
          << std::setprecision(6) << s.hitPoly().getCenter().Rho() << ", "
          << s.hitPoly().getCenter().Z() << ", "
          << s.hitPoly().getCenter().Phi() * 180. / M_PI << std::endl;
}
...
void ModulesToDTCsVisitor::preVisit() {
  output_ << "Module_DetId/i, Module_Section/C, Module_Layer/I, Module_Ring/I, "
             "Module_phi_deg/D, MFB/I, OPT_Services_Channel/I, "
             "PWR_Services_Channel/I, MFC/I, MFC_type/C, DTC_name/C, "
             "DTC_CMSSW_Id/i, DTC_Phi_Sector_Ref/I, type_/C, DTC_Slot/I, "
             "DTC_Phi_Sector_Width_deg/D"
          << std::endl;
}

void ModulesToDTCsVisitor::visit(const Barrel &b) { sectionName_ = b.myid(); }

void ModulesToDTCsVisitor::visit(const Endcap &e) { sectionName_ = e.myid(); }

void ModulesToDTCsVisitor::visit(const Layer &l) { layerId_ = l.myid(); }

void ModulesToDTCsVisitor::visit(const Disk &d) { layerId_ = d.myid(); }
...
void ModulesToDTCsVisitor::visit(const Module &m) {
  ...
  if (myBundle != nullptr) {
    if (myBundle->isPositiveCablingSide() == isPositiveCablingSide_) {
      ...
      moduleInfo << m.myDetId() << ", " << sectionName_ << ", " << layerId_
                 << ", " << m.moduleRing() << ", " << std::fixed
                 << std::setprecision(6) << m.center().Phi() * 180. / M_PI
                 << ", ";
      ...
      if (myCable != nullptr) {
        ...
        cableInfo << myCable->myid() << ", " << any2str(myCable->type())
                  << ", ";
        ...
        if (myDTC != nullptr) {
          ...
          DTCInfo << myDTC->name() << ", " << myDTC->getCMSSWId() << ", "
                  ...
                  << ", " << myDTC->slot() << ", " << std::fixed
                  << std::setprecision(6)
                  << myDTC->phiSectorWidth() * 180. / M_PI;
          output_ << moduleInfo.str() << bundleInfo.str() << cableInfo.str()
                  << DTCInfo.str() << std::endl;
        } else
          output_ << moduleInfo.str() << bundleInfo.str() << cableInfo.str()
                  << std::endl;
      } else
        output_ << moduleInfo.str() << bundleInfo.str() << std::endl;
    }
  }
}
...
void CMSSWOuterTrackerCablingMapVisitor::visit(const Module &m) {
  ...
  if (myDTC) {
    ...
    output_ << moduleInfo.str() << std::endl;
}
...
void InnerTrackerModulesToDTCsVisitor::preVisit() {
  output_ << "Module_DetId/i, Module_Section/C, Module_Layer/I, Module_Ring/I, "
             ...
             "MFB/I, DTC_Id/I, DTC_CMSSW_Id/U, IsPlusZEnd/O, IsPlusXSide/O"
          << std::endl;
}
...
void InnerTrackerModulesToDTCsVisitor::visit(const Module &m) {
  ...
  if (myPowerChain != nullptr) {
    std::stringstream moduleInfo;
    moduleInfo << m.myDetId() << "," << sectionName_ << ", " << layerId_ << ", "
               << m.moduleRing() << ", " << std::fixed << std::setprecision(6)
               << m.center().Phi() * 180. / M_PI << ", "
               << m.outerSensor().totalROCs() << ", " << m.totalChannels()
               << ", ";
    ...
    if (myGBT != nullptr) {
      ...
      GBTInfo << myGBT->numELinksPerModule() << "," << any2str(myGBT->GBTId())
              << "," << myGBT->getCMSSWId() << ",";
      ...
      if (myBundle != nullptr) {
        ...
        if (myDTC != nullptr) {
          std::stringstream DTCInfo;
          DTCInfo << myDTC->myid() << "," << myDTC->getCMSSWId() << ","
                  << myDTC->isPositiveZEnd() << "," << myDTC->isPositiveXSide();
          output_ << moduleInfo.str() << powerChainInfo.str() << GBTInfo.str()
                  << bundleInfo.str() << DTCInfo.str() << std::endl;
        } else
          output_ << moduleInfo.str() << powerChainInfo.str() << GBTInfo.str()
      ...
      output_ << moduleInfo.str() << powerChainInfo.str() << std::endl;
  }
}
...
void CMSSWInnerTrackerCablingMapVisitor::preVisit() {
  output_ << "Module_DetId/U, GBT_CMSSW_IdPerDTC/U, DTC_CMSSW_Id/U"
          << std::endl;
}
...
void CMSSWInnerTrackerCablingMapVisitor::visit(const Module &m) {
  ...
  if (myGBT) {
    ...
    if (myDTC) {
      std::stringstream DTCInfo;
      DTCInfo << myDTC->getCMSSWId();
      output_ << moduleInfo.str() << GBTInfo.str() << DTCInfo.str()
              << std::endl;
    } else
      output_ << moduleInfo.str() << GBTInfo.str() << std::endl;
  } else
    output_ << moduleInfo.str() << std::endl;
}
