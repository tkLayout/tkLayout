...
void IrradiationPowerVisitor::visit(DetectorModule& m) {
  ...
  const auto irradiation = getModuleIrradiationValues(irradiationMap_, m);
  // Get the dose on all the points of the module sensors
  const auto dose = getModuleIrradiationValues(doseMap_, m);
  ...
  const double sensorsPowerMean = computeSensorsPower(irradiation.at("mean"), alphaParam_,
									    volume, referenceTemp_, operatingTemp_, biasVoltage_);
  const double sensorsPowerMax = computeSensorsPower(irradiation.at("max"), alphaParam_,
									    volume, referenceTemp_, operatingTemp_, biasVoltage_);

  // C) STORE RESULTS
  // Results for each module
  //
  m.sensorsIrradiationCenter(irradiation.at("center"));  // 1MeV-equiv-neutrons / cm^2
  m.sensorsIrradiationMean(irradiation.at("mean"));      // 1MeV-equiv-neutrons / cm^2
  m.sensorsIrradiationMax(irradiation.at("max"));        // 1MeV-equiv-neutrons / cm^2
  m.sensorsDoseCenter(dose.at("center"));  // Gy
  m.sensorsDoseMean(dose.at("mean"));      // Gy
  m.sensorsDoseMax(dose.at("max"));        // Gy
  ...
  sensorsFluenceMean_[moduleRef] += irradiation.at("mean");
  sensorsDoseMean_[moduleRef] += dose.at("mean");
  // max
  sensorsPowerMax_[moduleRef] = std::max(sensorsPowerMax_[moduleRef], sensorsPowerMax);
  sensorsFluenceMax_[moduleRef] = std::max(sensorsFluenceMax_[moduleRef], irradiation.at("max"));
  sensorsDoseMax_[moduleRef] = std::max(sensorsDoseMax_[moduleRef], dose.at("max"));
  // counter
...
/**
 * @brief Calculates irradiation power levels across a detector module's sensor geometry.
 * 
 * For all sensors of a module, this function samples irradiation values from the provided map
 * at the geometric center, all polygon vertices, and the midpoints of all envelope edges.
 * Values are scaled by the integrated luminosity.
 *
 * @param irradiationMap Pointer to the manager containing spatial irradiation data (e.g. absorbed dose, fluence, etc.).
 * @param m The detector module to be sampled.
 * @return A map of sampled irradiation values
 */
std::unordered_map<std::string_view, double> IrradiationPowerVisitor::getModuleIrradiationValues(const IrradiationMapsManager* irradiationMap,
  ...
  for (const auto& s : m.sensors()) {
    ...
    double centerIrradiation = irradiationMap->calculateIrradiationPower(center) * timeIntegratedLumi_;
    ...
    for (int i = 0; i < envelopePoly.getNumSides(); i++) {
      ...
      double vertexIrradiation = irradiationMap->calculateIrradiationPower(vertex) * timeIntegratedLumi_;
      ...
      double midVertexIrradiation = irradiationMap->calculateIrradiationPower(midVertex) * timeIntegratedLumi_;
