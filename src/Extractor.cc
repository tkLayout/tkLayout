/**
 * @file Extractor.cc
 * @brief This class extracts the tracker geometry and material properties from
 * an existing setup and groups them in preparation for translation to CMSSW XML
 */
...
// public
/**
 * This is the public analysis function that extracts the information that is
 * necessary to convert a given material budget to a series of CMSSW XML files.
 * None of this information is written to file, though. Instead, it is stored in
 * a custom struct that consists of a series of vectors listing different types
 * of information chunks. Those can then be used later to write the specific XML
 * blocks in the output files. The function itself does bookkeeping of the input
 * and output data, not actual analysis. It delegates that to a series of
 * internal custom functions, making sure that each of them gets the correct
 * parameters to run.
 * @param mt A reference to the global material table; used as input
 * @param mb A reference to the material budget that is to be analysed; used as
 * input
 * @param d A reference to the bundle of vectors that will contain the
 * information extracted during analysis; used for output
 */
void Extractor::analyse(MaterialTable &mt, MaterialBudget &mb,
                        XmlTags &trackerXmlTags, CMSSWBundle &trackerData,
                        CMSSWBundle &otstData, bool wt) {
  ...
  // Analyse supports
  analyseSupports(is, isPixelTracker, trackerXmlTags, c, l, s, p,
                  otstComposites, otstLogic, otstShapes, otstPositions, t);
...
// protected
/**
 * This is one of the smaller analysis functions that provide the core
 * functionality of this class. It goes through the global material table,
 * treating each entry as an elementary material and copying or converting the
 * information for that material (density, standard radiation length, standard
 * interaction length) to the properties (density, atomic weight and atomic
 * number) used by CMSSW to describe elementary materials.
 * @param mattab A reference to the global material table; used as input
 * @param elems A reference to the collection of elementary material
 * information; used as output
 */
void Extractor::analyseElements(std::vector<Element> &elems,
                                std::vector<Composite> &allComposites) {
  ...
  // Z and A are calculated from radiation length and nuclear interaction
  lengths.
  // THIS IS BECAUSE RADIATION LENGTH AND NUCLEAR INTERACTION LENGTH CANNOT BE
  TRANSMITED DIRECTLY TO CMSSW.
  // Hence, Z and A (and density) only are transmitted to CMSSW.
  // On CMSSW side, radiation lengths and nuclear interaction lengths will be
  recomputed from this Z, A, and density info. elems.push_back(e);
  }
  */
  ...
  const int maxMaterialsTreeHierarchyLevel =
      myTable.getMaxMaterialsTreeHierarchyLevel();

  // This is to print the mixtures in a specific order: from lowest to highest
  // materialsTreeHierarchyLevel. This is because the materials a mixture is
  // made of, need to be printed first (on top of the XML file).
  for (int materialsTreeHierarchyLevel = 1;
       materialsTreeHierarchyLevel <= maxMaterialsTreeHierarchyLevel;
    ...
    for (const auto &mixIt : allChemicalMixtures) {
      ...
      if (mix.getMaterialsTreeHierarchyLevel() == materialsTreeHierarchyLevel) {
        ...
        for (const auto &fractionIt : fractions) {
          comp.elements.insert(
              std::make_pair(fractionIt.first, fractionIt.second));
        }

        comp.isMixture = true;
        allComposites.push_back(
            comp); // Add composite to the queue to be printed
      }
    }
  }
}
...
/**
 * This is one of the smaller analysis functions that provide the core
 * functionality of this class. Its main purpose is that of extracting a series
 * of (r, z) points that will be used later to extend the polycone volume
 * enclosing the entire pixel and tracker barrels. Since those points need to be
 * in order around the enclosing polygon, they are grouped into two different
 * vectors, one for z- and one for z+. Because of the way the points are
 * extracted, mirroring points in z- and z+ will be extracted bottom-to-top and
 * placed in vectors <i>up</i> and <i>down</i>, respectively. The idea is that
 * these two vectors should be traversed in opposite directions later: <i>up</i>
 * from first to last element, <i>down</i> from last to first.
 ...
 * @param down A reference to a vector listing polygon points by decreasing
 * radius
 */
void Extractor::analyseBarrelContainer(
    Tracker &t, XmlTags &trackerXmlTags,
  ...
  for (oiter = bc.begin(); oiter != bc.end(); oiter++) {
    ...
    for (iiter = oiter->begin(); iiter != oiter->end(); iiter++) {
      if (iiter->getModule().uniRef().side > 0 &&
          (iiter->getModule().uniRef().phi == 1 ||
           iiter->getModule().uniRef().phi == 2)) {
        ...
        mname << trackerXmlTags.tracker << lname.str() << xml_R << modRing
              << xml_barrel_module; // .e.g. OTLayer1R1Bmodule
        // parent module name
 ...
 *functionality of this class. Its main purpose is that of extracting a series
 *of (r, z) points that will be used later to extend the polycone volume
 *enclosing one of the pixel and tracker endcaps, namely those in z+. Since
 *those points need to be in order around the enclosing polygon, they are
 *grouped into two different vectors, one for for those lying to the left of an
 *imaginary line vertically bisecting the endcaps, the other for those lying to
 *the right of it. Because of the way the points are extracted, mirroring points
 *to the left and right will be extracted bottom-to-top and placed in vectors
 *<i>up</i> and <i>down</i>, respectively. The idea is that these two vectors
 *should be traversed in opposite directions later: <i>up</i> from first to last
 *element, <i>down</i> from last to first.
 * @param t A reference to the collection of topology information
 * @param up A reference to a vector listing polygon points by increasing radius
 * @param down A reference to a vector listing polygon points by decreasing
 *radius
 */
void Extractor::analyseEndcapContainer(
    std::vector<std::vector<ModuleCap>> &ec, Tracker &t,
    XmlTags &trackerXmlTags, std::vector<std::pair<double, double>> &up,
    std::vector<std::pair<double, double>> &down) {
  ...
  for (oiter = ec.begin(); oiter != ec.end(); oiter++) {
    ...
    for (iiter = oiter->begin(); iiter != oiter->end(); iiter++) {
      ...
      if (ridx.find(modRing) == ridx.end()) {
        ...
        // module name
        std::ostringstream mname;
        mname << dname.str() << xml_R << modRing
              << xml_endcap_module; // e.g. OTDisc6R1EModule
        // parent module name
        std::string parentName = mname.str();
        // build module volumes, with hybrids taken into account
        ModuleComplex modcomplex(mname.str(), parentName, *iiter);
...
/**
 * This is one of the two main analysis functions that provide the core
 * functionality of this class. It examines the barrel layers and the modules
 * within, extracting a great range of different pieces of information from the
 * geometry layout. The volumes considered are layers, rods, potentially tilted
 * rings, modules (with wafer, active surfaces, hybrids, support plate). They
 * form hierarchies of volumes, one inside the other. Output information are
 * volume hierarchy, material, shapes, positioning (potential use of algorithm
 * and rotations). They is also some topology, such as which volumes contain the
 * active surfaces, and how those active surfaces are subdivided and connected
 * to the readout electronics. Last but not least, overall radiation and
 * interaction lengths for each layer are calculated and stored; those are used
 * as approximative values for certain CMSSW functions later on.
 * @param mt A reference to the global material table; used as input
 * @param bc A reference to the collection of material properties of the barrel
 * modules; used as input
 * @param tr A reference to the tracker object; used as input
 * @param c A reference to the collection of composite material information;
 * used for output
 * @param l A reference to the collection of volume hierarchy information; used
 * for output
 ...
 * @param p A reference to the collection of volume positionings; used for
 * output
 * @param a A reference to the collection of algorithm calls and their
 * parameters; used for output
 * @param r A reference to the collection of rotations; used for output
 * @param t A reference to the collection of topology information; used for
 * output
 * @param ri A reference to the collection of overall radiation and interaction
 * lengths per layer or disc; used for output
 */
void Extractor::analyseLayers(
    MaterialTable &mt /*, std::vector<std::vector<ModuleCap> >& bc*/,
    Tracker &tr, XmlTags &trackerXmlTags, std::vector<Composite> &c,
    std::vector<LogicalInfo> &l, std::vector<ShapeInfo> &s,
    std::vector<ShapeOperationInfo> &so, std::vector<PosInfo> &p,
    std::vector<AlgoInfo> &a, std::map<std::string, Rotation> &r,
    std::vector<SpecParInfo> &t, std::vector<RILengthInfo> &ri, bool wt) {
  ...
  for (oiter = bc.begin(); oiter != bc.end(); oiter++) {
    ...
    // is the layer skewed?
    const bool isSkewedLayer =
        lagg.getBarrelLayers()->at(layer - 1)->isSkewedForInstallation();

    // check that the layer is not both tilted and skewed
    if (isTilted && isSkewedLayer) {
      logERROR("Layer is both tilted and skewed: this is not supported.");
    }

    bool isTimingLayer = lagg.getBarrelLayers()->at(layer - 1)->isTiming();
    // TO DO : NOT THAT EXTREMA OF MODULES WITH HYBRIDS HAVE BEEN INTRODUCED
    // INTO DETECTORMODULE (USED TO BE IN MODULE COMPLEX CLASS ONLY) ALL THIS
    // SHOULD BE DELETED AND REPLACED BY module->minRwithHybrids() GETTERS.

    // Calculate geometrical extrema of rod (straight layer), or of rod part +
    // tilted ring (tilted layer) straight layer : x and y extrema of rod
    double xmin = std::numeric_limits<double>::max();
    double xmax = 0;
    double ymin = std::numeric_limits<double>::max();
    double ymax = 0;
    // straight or tilted layer : z and r extrema of rod (straight layer), or of
    // {rod part + tilted ring} (tilted layer)
    double zmax = 0;
    ...
    int flatPartNumModules =
        lagg.getBarrelLayers()->at(layer - 1)->buildNumModulesFlat();
    double flatPartOneBeforeLastModuleMaxZ = 0;
    // straight or tilted layer : radii of rods (straight layer) or of rod parts
    // (tilted layer)
    double firstPhiRodRadius =
        0;                       // first rod encountered in phi by the visitor.
    ...
    for (iiter = oiter->begin(); iiter != oiter->end(); iiter++) {
      ...
      if (iiter->getModule().uniRef().side > 0 &&
          (
              ...
              || (!isSkewedLayer && (iiter->getModule().uniRef().phi == 1 ||
                                     iiter->getModule().uniRef().phi == 2)))) {

        if (isSkewedLayer &&
            (iiter->getModule().uniRef().phi == 1 ||
             iiter->getModule().uniRef().phi == 2) &&
            iiter->getModule().isSkewed()) {
          logERROR("Nothing gonna work, old code assumes that modules placed "
                   "with uniRef().phi == 1 or 2 are non-skewed.");
        }
        ...
        if (iiter->getModule().uniRef().phi == 1) {
          rodThickness.insert(
              {layer, modcomplex.getExpandedModuleThickness() / 2.});
        ...
        if (isTilted && (iiter->getModule().tiltAngle() == 0)) {
          ...
          if (flatPartNumModules >= 2 && modRing == (flatPartNumModules - 1)) {
            flatPartOneBeforeLastModuleMaxZ =
                MAX(flatPartOneBeforeLastModuleMaxZ, modcomplex.getZmax());
          }
        }
        ...
        if (iiter->getModule().uniRef().phi == 1 &&
            (modRing == 1 || modRing == 2)) {
          firstPhiRodRadius =
              firstPhiRodRadius + iiter->getModule().center().Rho() / 2;
        }
        // next rod encountered in phi
        if (iiter->getModule().uniRef().phi == 2 &&
            (modRing == 1 || modRing == 2)) {
          nextPhiRodRadius =
              nextPhiRodRadius + iiter->getModule().center().Rho() / 2;
        }
          ...
          if (!isAtPositiveXSide) {
            ...
            if (!iiter->getModule().isSkewed()) {
              ...
              if (!isPhiComputationAtMinusXSideInitialized) {
                unskewedLaddersAtMinusXSideCentersMinPhi =
                    iiter->getModule().center().Phi();
                unskewedLaddersAtMinusXSideCentersMaxPhi =
                    iiter->getModule().center().Phi();
                isPhiComputationAtMinusXSideInitialized = true;
              } else {
                unskewedLaddersAtMinusXSideCentersMinPhi = moduloMin(
                    iiter->getModule().center().Phi(),
                    unskewedLaddersAtMinusXSideCentersMinPhi, 2. * M_PI);
                unskewedLaddersAtMinusXSideCentersMaxPhi = moduloMax(
                    iiter->getModule().center().Phi(),
                    unskewedLaddersAtMinusXSideCentersMaxPhi, 2. * M_PI);
              }
              countUnskewedLaddersAtMinusXSide++;
            }
            // Skewed ladder
            else {
              skewedLadderAtMinusXSideCenterRadius =
                  iiter->getModule().center().Rho();
              skewedLadderAtMinusXSideCenterPhi =
                  iiter->getModule().center().Phi();
              skewedLadderAtMinusXSideSkewAngle =
                  iiter->getModule().skewAngle();
            }
          }
          ...
          // (+X) side
            ...
            if (!iiter->getModule().isSkewed()) {
              ...
              if (!isPhiComputationAtPlusXSideInitialized) {
                unskewedLaddersAtPlusXSideCentersMinPhi =
                    iiter->getModule().center().Phi();
                unskewedLaddersAtPlusXSideCentersMaxPhi =
                    iiter->getModule().center().Phi();
                isPhiComputationAtPlusXSideInitialized = true;
              } else {
                unskewedLaddersAtPlusXSideCentersMinPhi = moduloMin(
                    iiter->getModule().center().Phi(),
                    unskewedLaddersAtPlusXSideCentersMinPhi, 2. * M_PI);
                unskewedLaddersAtPlusXSideCentersMaxPhi = moduloMax(
                    iiter->getModule().center().Phi(),
                    unskewedLaddersAtPlusXSideCentersMaxPhi, 2. * M_PI);
              }
              countUnskewedLaddersAtPlusXSide++;
            }
            // Skewed ladder
            else {
              skewedLadderAtPlusXSideCenterRadius =
                  iiter->getModule().center().Rho();
              skewedLadderAtPlusXSideCenterPhi =
                  iiter->getModule().center().Phi();
    ...
    lname << trackerXmlTags.tracker << xml_layer << layer; // e.g. OTLayer1
    if (!isPixelTracker)
      ladderName << trackerXmlTags.tracker << xml_layer << layer
                 << xml_rod; // e.g. OTLayer1Rod
    else {
      ladderName << trackerXmlTags.tracker << xml_layer << layer << xml_rod
                 << xml_flipped; // e.g. OTLayer1RodFlipped
      unflippedLadderName << trackerXmlTags.tracker << xml_layer << layer
                          << xml_rod
                          << xml_unflipped; // e.g. OTLayer1RodUnflipped
    }

    std::string places_unflipped_mod_in_rod =
        (!isPixelTracker ? xml_OT_places_unflipped_mod_in_rod
                         : xml_PX_places_unflipped_mod_in_rod);
    std::string places_flipped_mod_in_rod =
        (!isPixelTracker ? xml_OT_places_flipped_mod_in_rod
                         : xml_PX_places_flipped_mod_in_rod);
    ...
    bool hasPhiForbiddenRanges =
        lagg.getBarrelLayers()->at(layer - 1)->phiForbiddenRanges.state();
    ...
    for (iiter = oiter->begin(); iiter != oiter->end(); iiter++) {
        ...
        for (const auto &i : phiForbiddenRangesIndexes) {
          if (iiter->getModule().uniRef().phi == i) {
            ...
            if (found == phiForbiddenRanges.end()) {
              phiForbiddenRanges.insert(
                  std::make_pair(i, iiter->getModule().center().Phi()));
            }
          }
        }
      }
      ...
      if (iiter->getModule().uniRef().side > 0 &&
        ...
        // std::cout << "iiter->getModule().uniRef().phi = " <<
        // iiter->getModule().uniRef().phi << " iiter->getModule().posRef().phi =
        // " << iiter->getModule().posRef().phi <<
        // "iiter->getModule().center().Phi() = " <<
        // iiter->getModule().center().Phi() * 180. / M_PI << "
        // iiter->getModule().center().Rho() = " <<
        // iiter->getModule().center().Rho() << " iiter->getModule().center().X()
        // = " << iiter->getModule().center().X() << "
        // iiter->getModule().center().Y() = " << iiter->getModule().center().Y()
        // << " iiter->getModule().center().Z() = " <<
        // iiter->getModule().center().Z() << " tiltAngle = " << tiltAngle << "
        // iiter->getModule().flipped() = " << iiter->getModule().flipped() << "
        // iiter->getModule().moduleType() = " << iiter->getModule().moduleType()
        // << std::endl;
        ...
        std::vector<ModuleCap>::iterator partner =
            findPartnerModule(iiter, oiter->end(), modRing);

        // ROD 1 (STRAIGHT LAYER), OR ROD 1 + MODULES WITH UNIREF PHI == 1 OF
        // THE TILTED RINGS (TILTED LAYER)
        if (iiter->getModule().uniRef().phi == 1) {
          ...
          // Only define a new module if no module of the same type already
          // defined. This is for shortening the size of the XML files.
          std::tuple<int, int, int, int> crystalProperties;
          std::string childName;
          if (iiter->getModule().isTimingModule()) {
            if (iiter->getModule().numSensors() > 0) {
              double crystalWidth =
                  iiter->getModule().sensors().front().crystalWidth();
              double crystalLength =
                  iiter->getModule().sensors().front().crystalLength();
              double crystalThickness =
                  iiter->getModule().sensors().front().crystalThickness();
              double crystalTiltAngle =
                  iiter->getModule().sensors().front().crystalTiltAngle();
              crystalProperties = std::make_tuple(
                  round(crystalWidth * 10.), round(crystalLength * 10.),
                  round(crystalThickness * 10.), round(crystalTiltAngle * 10.));

              // Define new timing module type.
              if (timingModuleNames.count(crystalProperties) == 0) {
                newTimingModuleType = true;
                timingModuleNames.insert(
                    std::make_pair(crystalProperties, mnameBase.str()));
              }
              // Uses an already defined timing module with same properties.
              else {
                newTimingModuleType = false;
              }
            } else {
              std::cout << "ERROR !! Barrel timing module with no sensor."
                        << std::endl;
            }
          }
          ...
          // For PosPart section in tracker.xml : module's positions in rod
          // (straight layer) or rod part (tilted layer)
          if (!isTilted || (isTilted && (tiltAngle == 0))) {
            ...
            // For timing barrel layer : Child, to place in a rod, can be a copy
            // of an existing module.
              ...
              // Uses an already defined timing module with same properties.
                ...
                childName = trackerXmlTags.nspace + ":" +
                            timingModuleNames.at(crystalProperties);
                timingModuleCopyNumber += 1;
              }
            } // end of timing layer special case
            pos.trans.dx =
                iiter->getModule().center().Rho() - firstPhiRodRadius;
            pos.trans.dz = iiter->getModule().center().Z();

            if (!iiter->getModule().flipped()) {
              pos.rotref =
                  trackerXmlTags.nspace + ":" + places_unflipped_mod_in_rod;
            } else {
              pos.rotref =
                  trackerXmlTags.nspace + ":" + places_flipped_mod_in_rod;
            }
            pos.copy =
                (!iiter->getModule().isTimingModule() ? 1
                                                      : timingModuleCopyNumber);
            if (iiter->getModule().isTimingModule())
              pos.child_tag = childName + xml_positive_z;
            p.push_back(pos);

            // This is a copy of the BModule on -Z side
            if (partner != oiter->end()) {
              pos.trans.dx =
                  partner->getModule().center().Rho() - firstPhiRodRadius;
              pos.trans.dz = partner->getModule().center().Z();

              if (!partner->getModule().flipped()) {
                pos.rotref =
                    trackerXmlTags.nspace + ":" + places_unflipped_mod_in_rod;
              } else {
                pos.rotref =
              ...
              pos.copy = (!iiter->getModule().isTimingModule()
                              ? 2
                              : timingModuleCopyNumber);
              if (iiter->getModule().isTimingModule())
                pos.child_tag = childName + xml_negative_z;
              p.push_back(pos);
            }
            // reset
            pos.copy = 1;
        ...
        if (iiter->getModule().uniRef().phi == 2) {
          if (!isTilted || (isTilted && (tiltAngle == 0))) {
            ...
            if (isPixelTracker) {
              pos.parent_tag =
                  trackerXmlTags.nspace + ":" + unflippedLadderName.str();
              pos.child_tag = trackerXmlTags.nspace + ":" + mname.str();
              pos.trans.dx =
                  iiter->getModule().center().Rho() - nextPhiRodRadius;
              pos.trans.dz = iiter->getModule().center().Z();
              if (!iiter->getModule().flipped()) {
                pos.rotref =
                    trackerXmlTags.nspace + ":" + places_unflipped_mod_in_rod;
              ...
              if (partner != oiter->end()) {
                pos.trans.dx =
                    partner->getModule().center().Rho() - nextPhiRodRadius;
                pos.trans.dz = partner->getModule().center().Z();
                if (!partner->getModule().flipped()) {
                  pos.rotref =
                      trackerXmlTags.nspace + ":" + places_unflipped_mod_in_rod;
        ...
        if (iiter->getModule().uniRef().phi == 1) {
          if (!iiter->getModule().isTimingModule() || newTimingModuleType) {
            ...
            if (iiter->getModule().numSensors() == 2) {
              ...
              if (iiter->getModule().isPixelModule())
                shape.name_tag = mname.str() + xml_InnerPixel + xml_base_waf;
              else if (iiter->getModule().isTimingModule())
                shape.name_tag = mname.str() + xml_timing + xml_base_waf;
              else {
                std::cerr << "Wafer : Unknown module type : "
                          << iiter->getModule().moduleType() << "."
                          << std::endl;
              }
            }
            ...
            shape.dx =
                iiter->getModule().area() / iiter->getModule().length() / 2.0;
            ...
            if (iiter->getModule().numSensors() == 2) {
              ...
              // SolidSection
              shape.name_tag = mname.str() + xml_base_lowerupper + xml_base_waf;
              shape.dy = (iiter->getModule().length() +
                          iiter->getModule().outerSensorExtraLength()) /
                         2.0;
              s.push_back(shape);
              ...
              pos.trans.dz =
                  pos.trans.dz +
                  /*2 * shape.dz +*/ iiter->getModule()
                      .dsDistance(); // CUIDADO: was with 2*shape.dz, but why???
              // pos.copy = 2;
              ...
              if (iiter->getModule().stereoRotation() != 0) {
                ...
                rot.thetay = 90.0;
                rot.phiy =
                    90.0 + iiter->getModule().stereoRotation() / M_PI * 180.;
                r.insert(std::pair<const std::string, Rotation>(rot.name, rot));
            ...
            if (!iiter->getModule().isTimingModule()) {
              ...
              if (iiter->getModule().moduleType() == "ptPS")
                shape.name_tag = mname.str() + xml_base_lowerupper +
                                 xml_base_ps + xml_base_pixel + xml_base_act;
              else if (iiter->getModule().moduleType() == "pt2S")
                shape.name_tag = mname.str() + xml_base_lowerupper +
                                 xml_base_2s + xml_base_act;
              else if (iiter->getModule().isTimingModule())
                shape.name_tag = mname.str() + xml_timing + xml_base_act;
              else if (iiter->getModule().isPixelModule()) {
                if (!iiter->getModule().is3DPixelModule()) {
                  shape.name_tag = mname.str() + xml_InnerPixel + xml_base_Act;
                } else {
                  shape.name_tag =
                      mname.str() + xml_InnerPixel + xml_3D + xml_base_Act;
                }
              } else {
                std::cerr << "Active surface : Unknown module type : "
                          << iiter->getModule().moduleType() << "."
                          << std::endl;
              }
              ...
              logic.shape_tag = trackerXmlTags.nspace + ":" + logic.name_tag;
              logic.material_tag =
                  xml_fileident + ":" + xml_tkLayout_material +
                  (iiter->getModule().isTimingModule() &&
                           iiter->getModule().subdet() == BARREL
                       ? xml_sensor_LYSO
                       : xml_sensor_silicon);
              l.push_back(logic);
              ...
              if (iiter->getModule().numSensors() == 2)
                pos.parent_tag = trackerXmlTags.nspace + ":" + mname.str() +
                                 xml_base_lowerupper + xml_base_waf;
              else {
                if (iiter->getModule().isTimingModule())
                  pos.parent_tag = trackerXmlTags.nspace + ":" + mname.str() +
                                   xml_timing + xml_base_waf;
                else if (iiter->getModule().isPixelModule())
                  pos.parent_tag = trackerXmlTags.nspace + ":" + mname.str() +
                                   xml_InnerPixel + xml_base_waf;
                else {
                  std::cerr
                      << "Positioning active surface : Unknown module type : "
                      << iiter->getModule().moduleType() << "." << std::endl;
                }
              }
              pos.child_tag = trackerXmlTags.nspace + ":" + shape.name_tag;
              pos.trans.dz = 0.0;
              ...
              minfo.rocrows = any2str<int>(
                  iiter->getModule()
                      .innerSensor()
                      .numROCRows()); // in case of single sensor module
                                      // innerSensor() and outerSensor() point
                                      // to the same sensor
              minfo.roccols =
                  any2str<int>(iiter->getModule().innerSensor().numROCCols());
              minfo.rocx =
                  any2str<int>(iiter->getModule().innerSensor().numROCX());
              minfo.rocy =
                  any2str<int>(iiter->getModule().innerSensor().numROCY());
              ...
              if (iiter->getModule().numSensors() == 2) {
                ...
                if (iiter->getModule().moduleType() == "ptPS")
                  shape.name_tag = mname.str() + xml_base_lowerupper +
                                   xml_base_ps + xml_base_strip + xml_base_act;
                else if (iiter->getModule().moduleType() == "pt2S")
                  shape.name_tag = mname.str() + xml_base_lowerupper +
                                   xml_base_2s + xml_base_act;
                else {
                ...
                mspec.partselectors.push_back(shape.name_tag);
                minfo.rocrows =
                    any2str<int>(iiter->getModule().outerSensor().numROCRows());
                minfo.roccols =
                    any2str<int>(iiter->getModule().outerSensor().numROCCols());
                minfo.rocx =
                    any2str<int>(iiter->getModule().outerSensor().numROCX());
                minfo.rocy =
                    any2str<int>(iiter->getModule().outerSensor().numROCY());
                mspec.moduletypes.push_back(minfo);
              } // End of replica for Pt-modules
            }

            // Barrel timing modules : add crystals volumes within the wafer
            // volume
            else {
              if (iiter->getModule().numSensors() > 0) {

                int numCrystalsX =
                    iiter->getModule().sensors().front().numCrystalsX();
                int numCrystalsY =
                    iiter->getModule().sensors().front().numCrystalsY();

                double alveolaWidth =
                    iiter->getModule().sensors().front().alveolaWidth();
                double alveolaLength =
                    iiter->getModule().sensors().front().alveolaLength();
                double alveolaShift =
                    iiter->getModule().sensors().front().alveolaShift();

                std::string crystalName =
                    mnameBase.str() + xml_timing + xml_base_act;
                ...
                logic.name_tag = shape.name_tag;
                logic.shape_tag = trackerXmlTags.nspace + ":" + logic.name_tag;
                logic.material_tag = xml_fileident + ":" +
                                     xml_tkLayout_material + xml_sensor_LYSO;
                l.push_back(logic);
                ...
                for (int j = 0; j < numCrystalsY; j++) {
                  for (int i = 0; i < numCrystalsX; i++) {
                    ...
                    pos.rotref = trackerXmlTags.nspace + ":" +
                                 xml_positive_z_tilted_mod_rot + tilt.str();
                    p.push_back(pos);

                    // -Z side
                    pos.parent_tag = trackerXmlTags.nspace + ":" +
                                     mnameNeg.str() + xml_timing + xml_base_waf;
                    pos.rotref = trackerXmlTags.nspace + ":" +
                                 xml_negative_z_tilted_mod_rot + tilt.str();
                    p.push_back(pos);
                ...
                // Topology
                if (std::find(mspec.partselectors.begin(),
                              mspec.partselectors.end(),
                              crystalName) == mspec.partselectors.end()) {
                  minfo.name = iiter->getModule().moduleType();
                  mspec.partselectors.push_back(crystalName);
                  minfo.rocrows = any2str<int>(
                      iiter->getModule().innerSensor().numROCRows());
                  minfo.roccols = any2str<int>(
                      iiter->getModule().innerSensor().numROCCols());
                  minfo.rocx =
                      any2str<int>(iiter->getModule().innerSensor().numROCX());
                  minfo.rocy =
                      any2str<int>(iiter->getModule().innerSensor().numROCY());
            ...
            if (iiter->getModule().isTimingModule()) {
              ...
              ModuleComplex modcomplexNeg(mnameNeg.str(), parentNameNeg,
                                          *partner);
            ...
            if (isTilted && (tiltAngle != 0)) {
              ...
              rinf.isZPlus = 0;
              rinf.z1 = -iiter->getModule()
                             .center()
                             .Z(); // WARNING: this assumes symmetry through
                                   // (XY) plane!
        ...
        if (isTilted && (iiter->getModule().uniRef().phi == 2)) {
          ...
          if (foundPlusZTiltedRingInfo != rinfoplus.end()) {
            ...
            myInfo.rminatzmin =
                MIN(myInfo.rminatzmin, modcomplex.getRminatZmin());
            myInfo.rmaxatzmax =
                MAX(myInfo.rmaxatzmax, modcomplex.getRmaxatZmax());
          }
          ...
          if (foundMinusZTiltedRingInfo != rinfominus.end()) {
            ...
            myInfo.z2 =
                -iiter->getModule()
                     .center()
                     .Z(); // WARNING: this assumes symmetry through (XY) plane!
            myInfo.startPhiAngle2 = iiter->getModule().center().Phi();
            // Update tilted ring extrema
            myInfo.rmin = MIN(myInfo.rmin, modcomplex.getRmin());
            myInfo.rmax = MAX(myInfo.rmax, modcomplex.getRmax());
    ...
    if (isTilted && !isPixelTracker)
      shape.name_tag = ladderName.str() + "Full";
    if (isTilted)
      shape.dx = (flatPartMaxY - flatPartMinY) / 2 + xml_epsilon;
    if (isPixelTracker || isTimingLayer)
      shape.dx = rodThickness.at(layer) + xml_epsilon;
    shape.dy = (xmax - xmin) / 2 + xml_epsilon;
    if (isTilted)
      shape.dy = (flatPartMaxX - flatPartMinX) / 2 + xml_epsilon;
    if (isPixelTracker || isTimingLayer)
      shape.dy = rodWidth.at(layer) + xml_epsilon;
    shape.dz = zmax + xml_epsilon;
    if (isTilted)
      shape.dz = flatPartMaxZ + xml_epsilon;
    s.push_back(shape);

    // Subtraction of an air volume from the flat part rod container volume, to
    // avoid collision with first tilted ring This trick is only used for the
    // Outer Tracker. For the Inner Tracker, this trick doesn't make sense,
    // since in priciple smallDelta = 0.
    ...
    const double innerLadderCenterRadius =
        MIN(firstPhiRodRadius, nextPhiRodRadius); // inner radius
    const double outerLadderCenterRadius =
        MAX(firstPhiRodRadius, nextPhiRodRadius); // outer radius
    ...
    if (!isPixelTracker) {
      if (!hasPhiForbiddenRanges) {
        ...
        alg.parameters.push_back(numericParam(
            xml_tilt, "90*deg")); // This "tilt" here has nothing to do with the
                                  // tilt angle of a tilted TBPS.
        // It is an angle used internally by PhiAltAlgo to shift in Phi the
        // startAngle ( in (X,Y) plane). 90 deg corresponds to no shift. SHOULD
        // NOT BE MODIFIED!! Is firstPhiRod placed at inner radius?
        const bool isFirstPhiRodAtInnerRadius =
            (fabs(firstPhiRodRadius - innerLadderCenterRadius) < xml_epsilon);
        // The algo (as implemeted in CMSSW) starts by placing the inner radius
        // rod, no matter what! So need to start placing rods from the inner rod
        // mean phi.
        const double algoStartPhi =
            (isFirstPhiRodAtInnerRadius ? firstPhiRodMeanPhi
                                        : nextPhiRodMeanPhi);
        pconverter.str("");
        pconverter << algoStartPhi * 180. / M_PI << "*deg";
        alg.parameters.push_back(
            numericParam(xml_startangle, pconverter.str()));
        pconverter.str("");
        ...
        alg.parameters.push_back(
            numericParam(xml_startangle, pconverter.str()));
        pconverter.str("");
        pconverter << (phiForbiddenRanges.at(forbiddenPhiUpperAIndex) -
                       phiForbiddenRanges.at(1)) *
                          180. / M_PI
                   << "*deg";
        alg.parameters.push_back(
            numericParam(xml_rangeangle, pconverter.str()));
        // WARNING: Set RadisuIn parameter to the radius of the firstPhiRod.
        // The algorithm will indeed start by placing (in phi) that firstPhiRod,
        // at radius whatever is assigned to radisuIn. RadiusIn is just a name,
        // and does not imply that the radius is low !!!! Look at PhiAltAlgo
        // implementation.
        pconverter.str("");
        ...
        // NOT BE MODIFIED!!
        pconverter.str("");
        pconverter << phiForbiddenRanges.at(forbiddenPhiLowerBIndex) * 180. /
                          M_PI
                   << "*deg";
        alg.parameters.push_back(
            numericParam(xml_startangle, pconverter.str()));
        pconverter.str("");
        pconverter << (phiForbiddenRanges.at(numRods) -
                       phiForbiddenRanges.at(forbiddenPhiLowerBIndex)) *
                          180. / M_PI
                   << "*deg";
        alg.parameters.push_back(
            numericParam(xml_rangeangle, pconverter.str()));
        ...
        alg.parameters.push_back(
            numericParam(xml_startcopyno, pconverter.str()));
        alg.parameters.push_back(numericParam(xml_incrcopyno, "1"));
        a.push_back(alg);
        pconverter.str("");
        alg.parameters.clear();
    ...
    // INNER TRACKER
      ...
      if (!isSkewedLayer) {
        ...
        const int numberLadders =
            lagg.getBarrelLayers()->at(layer - 1)->numRods() / 2;
        ...
        createAndStoreDDTrackerAngularAlgorithmBlock(
            a, nameSpace, parentName, firstPhiLadderName,
            firstPhiLadderCenterPhi, rangeAngle, firstPhiLadderRadius, center,
            numberLadders, firstPhiLadderStartCopyNumber, copyNumberIncrement);
        ...
        createAndStoreDDTrackerAngularAlgorithmBlock(
            a, nameSpace, parentName, nextPhiLadderName, nextPhiLadderCenterPhi,
            rangeAngle, nextPhiLadderRadius, center, numberLadders,
            nextPhiLadderStartCopyNumber, copyNumberIncrement);
      }
      ...
      // ALGO BLOCKS TO PLACE UNSKEWED LADDERS
        ...
        // (-X) SIDE
        if (countUnskewedLaddersAtMinusXSide <= 2 ||
            femod(countUnskewedLaddersAtMinusXSide, 2) == 0) {
          logERROR("Skewed layer: It is expected to have an odd number (>=3) "
                   "of non-skewed ladders per (X) side.");
        } else {
          ...
          // phi
          const double flippedLadderCenterPhi =
              unskewedLaddersAtMinusXSideCentersMinPhi; // WARNING: THIS ASSUMES
                                                        // that the non-skewed
                                                        // ladder placed at min
                                                        // phi is placed at
                                                        // inner radius and
                                                        // flipped!!!
          // number of flipped ladders
          const int countFlippedLaddersAtMinusXSide =
              std::floor(countUnskewedLaddersAtMinusXSide / 2.) +
              1; // WARNING: THIS ASSUMES
          ...
          // algo block
          createAndStoreDDTrackerAngularAlgorithmBlock(
              a, nameSpace, parentName, flippedLadderName,
              flippedLadderCenterPhi, rangeAngleAtMinusXSide,
              innerLadderCenterRadius, center, countFlippedLaddersAtMinusXSide,
              flippedLadderStartCopyNumber, copyNumberIncrement);
          ...
          const double deltaPhiAtMinusXSide =
              rangeAngleAtMinusXSide / (countUnskewedLaddersAtMinusXSide - 1);
          // phi
          const double unFlippedLadderCenterPhi =
              unskewedLaddersAtMinusXSideCentersMinPhi + deltaPhiAtMinusXSide;
          // number of non-flipped ladders
          const int countUnFlippedLaddersAtMinusXSide =
              std::floor(countUnskewedLaddersAtMinusXSide /
                         2.); // WARNING: THIS ASSUMES
                              // that there is an odd number of
                              // non-skewed ladders:
          ...
          // algo block
          createAndStoreDDTrackerAngularAlgorithmBlock(
              a, nameSpace, parentName, unFlippedLadderName,
              unFlippedLadderCenterPhi,
              rangeAngleAtMinusXSide - 2. * deltaPhiAtMinusXSide,
              outerLadderCenterRadius, center,
              countUnFlippedLaddersAtMinusXSide, unFlippedLadderStartCopyNumber,
              copyNumberIncrement);
        }
        ...
        if (countUnskewedLaddersAtMinusXSide <= 2 ||
          ...
          // phi
          const double flippedLadderCenterPhi =
              unskewedLaddersAtPlusXSideCentersMinPhi; // WARNING: THIS ASSUMES
                                                       // that the non-skewed
                                                       // ladder placed at min
                                                       // phi is placed at inner
                                                       // radius and flipped!!!
          // number of flipped ladders
          const int countFlippedLaddersAtPlusXSide =
              std::floor(countUnskewedLaddersAtPlusXSide / 2.) +
              1; // WARNING: THIS ASSUMES
                 // that there is an odd number of
                 // non-skewed ladders:
                 // 1 more ladder at inner radius.
          // start copy number
          const int flippedLadderStartCopyNumber =
              countUnskewedLaddersAtMinusXSide + 1;

          // algo block
          createAndStoreDDTrackerAngularAlgorithmBlock(
              a, nameSpace, parentName, flippedLadderName,
              flippedLadderCenterPhi, rangeAngleAtPlusXSide,
              innerLadderCenterRadius, center, countFlippedLaddersAtPlusXSide,
              flippedLadderStartCopyNumber, copyNumberIncrement);
          ...
          const std::string unFlippedLadderName = unflippedLadderName.str();
          // delta phi between 2 consecutive non-skewed ladders
          const double deltaPhiAtPlusXSide =
              rangeAngleAtPlusXSide / (countUnskewedLaddersAtPlusXSide - 1);
          // phi
          const double unFlippedLadderCenterPhi =
              unskewedLaddersAtPlusXSideCentersMinPhi + deltaPhiAtPlusXSide;
          // number of non-flipped ladders
          const int countUnFlippedLaddersAtPlusXSide =
              std::floor(countUnskewedLaddersAtPlusXSide /
                         2.); // WARNING: THIS ASSUMES
                              // that there is an odd number of
                              // non-skewed ladders:
                              // 1 less ladder at outer radius.
          // start copy number
          const int unFlippedLadderStartCopyNumber =
              countUnskewedLaddersAtMinusXSide + 2;
          ...
          createAndStoreDDTrackerAngularAlgorithmBlock(
              ...
              outerLadderCenterRadius, center, countUnFlippedLaddersAtPlusXSide,
              unFlippedLadderStartCopyNumber, copyNumberIncrement);
        }

        const int countTotalUnskewedLadders =
            countUnskewedLaddersAtMinusXSide + countUnskewedLaddersAtPlusXSide;

        // SKEWED LADDER, (-X) side
        pos.parent_tag = nameSpace + ":" + parentName;
        pos.child_tag = nameSpace + ":" +
                        unflippedLadderName.str(); // WARNING: THIS ASSUMES
        // that the skewed ladder is always non-flipped.

        pos.trans.dx = skewedLadderAtMinusXSideCenterRadius *
                       cos(skewedLadderAtMinusXSideCenterPhi);
        pos.trans.dy = skewedLadderAtMinusXSideCenterRadius *
                       sin(skewedLadderAtMinusXSideCenterPhi);
        pos.trans.dz = 0;

        const double skewRotationAngleInRadAtMinusXSide =
            skewedLadderAtMinusXSideCenterPhi +
            skewedLadderAtMinusXSideSkewAngle;
        const std::string skewRotationNameAtMinusXSide =
            "Z" + any2str(skewRotationAngleInRadAtMinusXSide * 180. / M_PI,
                          xml_angle_name_precision);
        addRotationAroundZAxis(r, skewRotationNameAtMinusXSide,
                               skewRotationAngleInRadAtMinusXSide);
        ...
        pos.trans.dx = skewedLadderAtPlusXSideCenterRadius *
                       cos(skewedLadderAtPlusXSideCenterPhi);
        pos.trans.dy = skewedLadderAtPlusXSideCenterRadius *
                       sin(skewedLadderAtPlusXSideCenterPhi);
        pos.trans.dz = 0;

        const double skewRotationAngleInRadAtPlusXSide =
            skewedLadderAtPlusXSideCenterPhi + skewedLadderAtPlusXSideSkewAngle;
        const std::string skewRotationNameAtPlusXSide =
            "Z" + any2str(skewRotationAngleInRadAtPlusXSide * 180. / M_PI,
                          xml_angle_name_precision);
        addRotationAroundZAxis(r, skewRotationNameAtPlusXSide,
                               skewRotationAngleInRadAtPlusXSide);
    ...
    if (!rinfoplus.empty() || !rinfominus.empty()) {
      ...
      if (!rinfoplus.empty()) {
        rinfototal.insert({"rinfoplus", rinfoplus});
      }
      if (!rinfominus.empty()) {
        rinfototal.insert({"rinfominus", rinfominus});
      }
      ...
      for (auto const &rinfoside : rinfototal) {
        ...
        for (auto const &ringinfo : rinfoside.second) {
          ...
          if (rinfo.modules > 0) {
            ...
            if (rinfo.isZPlus) {
              shape.rmin1 = rinfo.rminatzmin -
                            xml_epsilon * tan(rinfo.tiltAngle * M_PI / 180.0);
              shape.rmax1 = rinfo.rmaxatzmax +
                            2 * shape.dz * tan(rinfo.tiltAngle * M_PI / 180.0) +
                            xml_epsilon * tan(rinfo.tiltAngle * M_PI / 180.0);
              shape.rmin2 = rinfo.rminatzmin -
                            2 * shape.dz * tan(rinfo.tiltAngle * M_PI / 180.0) -
                            xml_epsilon * tan(rinfo.tiltAngle * M_PI / 180.0);
            ...
            // Please note that the layer's dimensions rely on the fact this
            // intersection is made, so that layer's extrema are ~rmin and ~rmax
            ...
            alg.parameters.push_back(stringParam(
                xml_childparam, trackerXmlTags.nspace + ":" + rinfo.childname));
            ...
            alg.parameters.push_back(
                numericParam(xml_radius, pconverter.str()));
            pconverter.str("");
            alg.parameters.push_back(
                vectorParam(0, 0, (rinfo.z1 - rinfo.z2) / 2.0));
            pconverter << rinfo.isZPlus;
            alg.parameters.push_back(
                numericParam(xml_iszplus, pconverter.str()));
            pconverter.str("");
            pconverter << rinfo.tiltAngle << "*deg";
            alg.parameters.push_back(
                numericParam(xml_tiltangle, pconverter.str()));
            pconverter.str("");
            pconverter << rinfo.bw_flipped;
            alg.parameters.push_back(
                numericParam(xml_isflipped, pconverter.str()));
            pconverter.str("");
            a.push_back(alg);
            alg.parameters.clear();
            ...
            pconverter.str("");
            alg.parameters.push_back(
                vectorParam(0, 0, (rinfo.z2 - rinfo.z1) / 2.0));
            pconverter << rinfo.isZPlus;
            alg.parameters.push_back(
                numericParam(xml_iszplus, pconverter.str()));
            pconverter.str("");
  ...
  if (!lspec.partselectors.empty())
    t.push_back(lspec);
  if (!rspec.partselectors.empty())
    t.push_back(rspec);
  if (!srspec.partselectors.empty())
    t.push_back(srspec);
  if (!trspec.partselectors.empty())
    t.push_back(trspec);
  if (!sspec.partselectors.empty())
    t.push_back(sspec);
  if (!mspec.partselectors.empty())
    t.push_back(mspec);
}

/**
 * This is one of the two main analysis functions that provide the core
 * functionality of this class. It examines the endcap discs in z+ and the rings
 * and modules within, extracting a great range of different pieces of
 * information from the geometry layout. These are shapes for individual
 * modules, but also for their enclosing volumes, divided into rings and then
 * discs. They form hierarchies of volumes, one inside the other. Output
 * information are volume hierarchy, material, shapes, positioning (potential
 * use of algorithm and rotations). They is also some topology, such as which
 * volumes contain the active surfaces, and how those active surfaces are
 * subdivided and connected to the readout electronics. Last but not least,
 * overall radiation and interaction lengths for each layer are calculated and
 * stored; those are used as approximative values for certain CMSSW functions
 * later on.
 * @param mt A reference to the global material table; used as input
 * @param ec A reference to the collection of material properties of the endcap
 * modules; used as input
 * @param tr A reference to the tracker object; used as input
 * @param c A reference to the collection of composite material information;
 * used for output
 ...
 * lengths per layer or disc; used for output
 */
void Extractor::analyseDiscs(
    MaterialTable &mt, std::vector<std::vector<ModuleCap>> &ec, Tracker &tr,
    XmlTags &trackerXmlTags, std::vector<Composite> &c,
    std::vector<LogicalInfo> &l, std::vector<ShapeInfo> &s,
    std::vector<ShapeOperationInfo> &so, std::vector<PosInfo> &p,
    std::vector<AlgoInfo> &a, std::map<std::string, Rotation> &r,
    std::vector<SpecParInfo> &t, std::vector<RILengthInfo> &ri, bool wt) {
  ...
  for (oiter = ec.begin(); oiter != ec.end(); oiter++) {
    ...
    if (lagg.getEndcapLayers()->at(layer - 1)->minZ() > 0) {
      ...
      std::set<int> ringsIndexes; // VERY UGLY !! TO DO : IMPLEMENT
                                  // ringsIndexes() IN CLASS DISK
      for (iiter = oiter->begin(); iiter != oiter->end(); iiter++) {
        int modRing = iiter->getModule().uniRef().ring;
        if (ringsIndexes.find(modRing) == ringsIndexes.end())
          ringsIndexes.insert(modRing);
      }
      ...
      for (iiter = oiter->begin(); iiter != oiter->end(); iiter++) {
        if (iiter->getModule().uniRef().side > 0 &&
          ...
          // disk name
          std::ostringstream dname;
          dname << trackerXmlTags.tracker << xml_disc
                << discNumber; // e.g. OTDisc6
          // module name
          std::ostringstream mname;
          mname << dname.str() << xml_R << modRing
                << xml_endcap_module; // e.g. OTDisc6R1EModule
          ...
          zmax = MAX(zmax, modcomplex.getZmax());
          ringzmin.at(modRing) =
              MIN(ringzmin.at(modRing), modcomplex.getZmin());
          ringzmax.at(modRing) =
              MAX(ringzmax.at(modRing), modcomplex.getZmax());
          if (iiter->getModule().isSmallerAbsZModuleInRing()) {
            ringSmallAbsZModulesZMax.at(modRing) =
                MAX(ringSmallAbsZModulesZMax.at(modRing), modcomplex.getZmax());
          } else {
            ringBigAbsZModulesZMin.at(modRing) =
                MIN(ringBigAbsZModulesZMin.at(modRing), modcomplex.getZmin());
          }
        }
      }

      double diskZ = 0;
      if (ringsIndexes.size() < 2)
        std::cout << "!!!!!!Disk with less than 2 rings, unexpected"
                  << std::endl;
      else {
        int firstRingIndex = *(ringsIndexes.begin());
        int secondRingIndex = *ringsIndexes.begin() + 1;
        diskZ = (ringzmin.at(firstRingIndex) + ringzmax.at(firstRingIndex) +
                 ringzmin.at(secondRingIndex) + ringzmax.at(secondRingIndex)) /
                4.;
      }
      ...
      for (iiter = oiter->begin(); iiter != oiter->end(); iiter++) {
        if (iiter->getModule().uniRef().side > 0 &&
          ...
          // std::cout << "iiter->getModule().uniRef().phi = " <<
          // iiter->getModule().uniRef().phi << "
          // iiter->getModule().uniRef().ring = " <<
          // iiter->getModule().uniRef().ring <<
          // "iiter->getModule().center().Phi() = " <<
          // iiter->getModule().center().Phi() * 180. / M_PI << "
          // iiter->getModule().center().Rho() = " <<
          // iiter->getModule().center().Rho() << "
          // iiter->getModule().center().X() = " <<
          // iiter->getModule().center().X() << " iiter->getModule().center().Y()
          // = " << iiter->getModule().center().Y() << "
          // iiter->getModule().center().Z() = " <<
          // iiter->getModule().center().Z() << " iiter->getModule().flipped() =
          ...
          if (iiter->getModule().uniRef().phi == 1) {
            ...
            if (shape.type == bx) {
              ...
              shape.dx = iiter->getModule().minWidth() / 2.0 +
                         iiter->getModule().serviceHybridWidth();
              shape.dxx = iiter->getModule().maxWidth() / 2.0 +
                          iiter->getModule().serviceHybridWidth();
              shape.dy = iiter->getModule().length() / 2.0 +
                         iiter->getModule().frontEndHybridWidth();
              shape.dyy = iiter->getModule().length() / 2.0 +
                          iiter->getModule().frontEndHybridWidth();
              shape.dz = iiter->getModule().thickness() / 2.0 +
                         iiter->getModule().supportPlateThickness();
            }
            s.push_back(shape);
            ...
            // matname << xml_base_actcomp << "D" << discNumber << "R" <<
            // modRing; c.push_back(createComposite(matname.str(),
            // compositeDensity(*iiter, true), *iiter, true));

            // Topology
            sspec.partselectors.push_back(mname.str());
            sspec.moduletypes.push_back(minfo_zero);

            // WAFER -- same x and y size of parent shape, but different
            // thickness
            string xml_base_lowerupper = "";
            if (iiter->getModule().numSensors() == 2) {
              xml_base_lowerupper = xml_base_lower;
            ...
            shape.dz =
                iiter->getModule().sensorThickness() /
                2.0; // CUIDADO WAS calculateSensorThickness(*iiter, mt) / 2.0;
            // if (iiter->getModule().numSensors() == 2) shape.dz = shape.dz
            // / 2.0; // CUIDADO calcSensThick returned 2x what getSensThick
            // returns, it means that now one-sided sensors are half as thick if
            // not compensated for in the config files
            s.push_back(shape);
            ...
            pos.child_tag = trackerXmlTags.nspace + ":" + shape.name_tag;
            if (iiter->getModule().uniRef().side > 0)
              pos.trans.dz = /*shape.dz*/ -iiter->getModule().dsDistance() /
                             2.0; // CUIDADO WAS getModule().moduleThickness()
            else
              pos.trans.dz = iiter->getModule().dsDistance() /
                             2.0 /*- shape.dz*/; // DITTO HERE
            p.push_back(pos);
            ...
            if (iiter->getModule().numSensors() == 2) {
              ...
              if (iiter->getModule().uniRef().side > 0)
                pos.trans.dz =
                    /*pos.trans.dz + 2 * shape.dz +*/ iiter->getModule()
                        .dsDistance() /
                    2.0; // CUIDADO removed pos.trans.dz + 2*shape.dz, added
                         // / 2.0
              else
                pos.trans.dz =
                    /* pos.trans.dz - 2 * shape.dz -*/ -iiter->getModule()
            ...
            if (iiter->getModule().moduleType() == "ptPS")
              shape.name_tag = mname.str() + xml_base_lowerupper + xml_base_ps +
                               xml_base_pixel + xml_base_act;
            else if (iiter->getModule().moduleType() == "pt2S")
              shape.name_tag = mname.str() + xml_base_lowerupper + xml_base_2s +
                               xml_base_act;
            else if (iiter->getModule().isTimingModule())
              shape.name_tag = mname.str() + xml_timing + xml_base_act;
            else if (iiter->getModule().isPixelModule()) {
            ...
            logic.name_tag = shape.name_tag;
            logic.shape_tag = trackerXmlTags.nspace + ":" + logic.name_tag;
            logic.material_tag = xml_fileident + ":" + xml_tkLayout_material +
                                 xml_sensor_silicon;
            l.push_back(logic);
            ...
            minfo.name = iiter->getModule().moduleType();
            minfo.rocrows =
                any2str<int>(iiter->getModule().innerSensor().numROCRows());
            minfo.roccols =
                any2str<int>(iiter->getModule().innerSensor().numROCCols());
            minfo.rocx =
                any2str<int>(iiter->getModule().innerSensor().numROCX());
            ...
            // Collect ring info
            const Ring *myRing =
                lagg.getEndcapLayers()->at(layer - 1)->ringsMap().at(modRing);
            ...
            myRingInfo.moduleThickness =
                modcomplex.getExpandedModuleThickness();
            ...
            myRingInfo.smallAbsZSurfaceZMax =
                ringSmallAbsZModulesZMax.at(modRing);
          ...
          if (iiter->getModule().uniRef().phi == 2) {
            ...
            // Hence that the surfaces the modules are assigned to are
            // alternated, as one goes along Phi.
      ...
      for (const auto &ringIndex : ridx) {
        ...
        if (found != rinfo.end()) {
          ...
          if (myRingInfo.numModules > 0) {
            ...
            // CASE WHERE SMALLDELTA > BIGDELTA: MODULES OF A GIVEN RING ARE
            // SPREAD ALONG 4 DEES.
            if (myRingInfo.isRingOn4Dees) {
              ...
              // Full ring cylinder
              shape.name_tag = ((hasAnInnerRing || hasAnOuterRing)
                                    ? (myRingInfo.name + "Full")
                                    : myRingInfo.name);
              shape.rmin = myRingInfo.radiusMin - xml_epsilon;
              shape.rmax = myRingInfo.radiusMax + xml_epsilon;
              shape.dz =
                  (myRingInfo.zMax - myRingInfo.zMin) / 2.0 + xml_epsilon;
              s.push_back(shape);
              ...
              if (hasAnInnerRing) {
                ...
                shape.dz = (myRingInfo.bigAbsZSurfaceZMin -
                            myRingInfo.smallAbsZSurfaceZMax) /
                               2.0 -
                           xml_epsilon;
                s.push_back(shape);
                ...
                shapeOp.name_tag =
                    (hasAnOuterRing
                         ? (myRingInfo.name + "SubtractionIntermediate")
                         : myRingInfo.name);
                shapeOp.type = substract;
            ...
            // CASE WHERE SMALLDELTA < BIGDELTA: MODULES OF A GIVEN RING ARE
            // PLACED ON BOTH SIDES OF THE SAME DEE. There are only 2 dees per
            // ring: one per (X) side.
            else {
          ...
          pos.parent_tag = trackerXmlTags.nspace + ":" +
                           dname.str(); // CUIDADO ended with: + xml_plus;
          pos.child_tag = logic.shape_tag;
          ...
          alg.parameters.push_back(
              stringParam(xml_childparam,
                          trackerXmlTags.nspace + ":" + myRingInfo.childname));
          ...
          pconverter.str("");
          alg.parameters.push_back(
              vectorParam(0, 0, myRingInfo.surface1ZMid - myRingInfo.zMid));
          ...
          pconverter.str("");
          alg.parameters.push_back(
              vectorParam(0, 0, myRingInfo.surface2ZMid - myRingInfo.zMid));
          pconverter << myRingInfo.isDiskAtPlusZEnd;
          alg.parameters.push_back(numericParam(xml_iszplus, pconverter.str()));
          pconverter.str("");
          alg.parameters.push_back(numericParam(xml_tiltangle, "90*deg"));
      ...
      //   pos.child_tag = trackerXmlTags.nspace + ":" + logic.name_tag;
      //   p.push_back(pos);
      // dspec.partselectors.push_back(logic.name_tag); // CUIDADO dspec still
      // needs to be duplicated for minus discs (I think)
      // dspec.partextras.push_back(logic.extra);
      discNumber++;
    }
    layer++;
  }
  if (!dspec.partselectors.empty())
    t.push_back(dspec);
  if (!rspec.partselectors.empty())
    t.push_back(rspec);
  if (!sspec.partselectors.empty())
...
/**
 * This is one of the smaller analysis functions that provide the core
 * functionality of this class. It does a number of things: it creates a
 * composite material information struct for each barrel service, and it adds
 * the remaining information about the volume to the collections of hierarchy,
 * shape, position and topology information. All of these future CMSSW XML
 * blocks are given unique names based on the properties of the barrel service
 * they came from.
 * @param is A reference to the collection of inactive surfaces that the barrel
 * services are a part of; used as input
 * @param c A reference to the collection of composite material information;
 * used for output
 * @param l A reference to the collection of volume hierarchy information; used
 * for output
 ...
 * output
 */
void Extractor::analyseServices(InactiveSurfaces &is, bool &isPixelTracker,
                                XmlTags &trackerXmlTags,
                                std::vector<Composite> &c,
                                std::vector<LogicalInfo> &l,
                                std::vector<ShapeInfo> &s,
                                std::vector<PosInfo> &p,
                                std::vector<SpecParInfo> &t, bool wt) {
  // container inits
  ShapeInfo shape;
  LogicalInfo logic;
  PosInfo pos;
  ...
  for (iter = bs.begin(); iter != guard; iter++) {
    ...
    if ((int)(iter->getZOffset()) == 0) {
      if (previousInnerRadius == (int)(iter->getInnerRadius()))
        continue;
      else
        previousInnerRadius = (int)(iter->getInnerRadius());
    }
#endif
...
#else
    matname << xml_base_serfcomp << "R" << (int)(iter->getInnerRadius()) << "Z"
            << (int)(fabs(iter->getZOffset() + iter->getZLength() / 2.0));
    shapename << xml_base_serf << "R" << (int)(iter->getInnerRadius()) << "Z"
              << (int)(fabs(iter->getZOffset() + iter->getZLength() / 2.0));
    if ((iter->getZOffset() + iter->getZLength()) > 0) {
      if (iter->getLocalMasses().size()) {
        c.push_back(
            createComposite(matname.str(), compositeDensity(*iter), *iter));

        // TO DO : CALCULATION OF OUTERMOST SHAPES BOUNDARIES
        double startEndcaps;
        if (!isPixelTracker)
          startEndcaps = xml_outerTrackerEndcapsMinZ;
        else
          startEndcaps =
              xml_innerTrackerEndcapsMinZ; // Tilted Inner Tracker :
                                           // startEndcaps =
                                           // xml_innerTiltedTrackerEndcapsMinZ;
        ...
        if ((iter->getZOffset() + iter->getZLength() / 2.0) < startEndcaps) {
          ...
          s.push_back(shape);
          if (shape.rmin < serviceBarrelRMin)
            serviceBarrelRMin = shape.rmin;
          if (shape.rmax > serviceBarrelRMax)
            serviceBarrelRMax = shape.rmax;
          ...
          pos.parent_tag =
              xml_pixbarident + ":" + trackerXmlTags.bar; // xml_tracker;
          pos.child_tag = logic.shape_tag;
        ...
        // ENDCAPS services
          ...
          // VERY IMPORTANT : Barrel timing Layer : Services from Tracker
          // endcaps removed. TEMPORARY !! This is because the Timing Layer is
          // put in the tracker, and hence tracker and timing services are
          // grouped togetehr.
          // if (!isTimingLayout || (isTimingLayout && (iter->getInnerRadius() +
          // iter->getRWidth()) <= 1160.)) { // BTL

          // cut in 2 the services that belong to both Barrel and Endcaps mother
          // volumes
          if (iter->getZOffset() < startEndcaps) {
            std::ostringstream shapenameBarrel, shapenameEndcaps;
            shapenameBarrel
                << xml_base_serf << "R" << (int)(iter->getInnerRadius()) << "Z"
                << (int)(fabs(iter->getZOffset() + iter->getZLength() / 2.0))
                << "BarrelPart";
            shapenameEndcaps
                << xml_base_serf << "R" << (int)(iter->getInnerRadius()) << "Z"
            ...
            logic.shape_tag =
                trackerXmlTags.nspace + ":" + shapenameBarrel.str();
            logic.material_tag = trackerXmlTags.nspace + ":" + matname.str();
            l.push_back(logic);
            ...
            shape.dz =
                (iter->getZOffset() + iter->getZLength() - startEndcaps) / 2.0 -
                xml_epsilon;
            shape.rmin = iter->getInnerRadius();
            shape.rmax = shape.rmin + iter->getRWidth();
            s.push_back(shape);
            if (shape.rmin < serviceEndcapsRMin)
              serviceEndcapsRMin = shape.rmin;
            if (shape.rmax > serviceEndcapsRMax)
              serviceEndcapsRMax = shape.rmax;

            logic.name_tag = shapenameEndcaps.str();
            logic.shape_tag =
                trackerXmlTags.nspace + ":" + shapenameEndcaps.str();
            logic.material_tag = trackerXmlTags.nspace + ":" + matname.str();
            l.push_back(logic);

            pos.parent_tag =
                xml_pixfwdident + ":" + trackerXmlTags.fwd; // xml_tracker;
            pos.child_tag = logic.shape_tag;
          ...
          //}
          // else { std::cout << "VERY IMPORTANT : Barrel timing Layer :
          // Services from Tracker endcaps removed. TEMPORARY !! This is because
          // the Timing Layer is put in the tracker, and hence tracker and timing
          // services are grouped togetehr. TO DO : Place timing layer in an
          // independant container." << std::endl; }
        }
        ...
        msg << shapename.str() << " is not exported to XML because it is empty."
            << std::ends;
        logWARNING(msg.str());
      }
    }
 ...
 * This is one of the smaller analysis functions that provide the core
 * functionality of this class. It does a number of things: it creates a
 * composite material information struct for each support volume, and it adds
 * the remaining information about that volume to the collections of hierarchy,
 * shape, position and topology information. All of these future CMSSW XML
 * blocks are given unique names based on the properties of the support
 * structures they came from.
 * @param is A reference to the collection of inactive surfaces that the
 * supports are a part of; used as input
 * @param c A reference to the collection of composite material information;
 * used for output
 * @param l A reference to the collection of volume hierarchy information; used
 * for output
 ...
 * output
 */
void Extractor::analyseSupports(
    InactiveSurfaces &is, bool &isPixelTracker, XmlTags &trackerXmlTags,
  ...
  for (iter = sp.begin(); iter != guard; iter++) {
    ...
    for (const auto &massIt : allMasses) {
      ...
      if (subdetectorName == xml_OTST) {
        isOTST = true;
      }
      if (isOTST && subdetectorName != xml_OTST) {
        std::cout << "VERY IMPORTANT!! NOT SUPPORTED: SAME volume contains "
                     "OTST element(s), but also other element(s) assigned to "
                     "different subdetector(s)."
                  << std::endl;
      }
    }

    if ((iter->getZOffset() + iter->getZLength()) >
        0) { // && shapename.str() != "supportR1191Z1325")
      if (iter->getLocalMasses().size()) {
        ...
        if ((!isPixelTracker && iter->getZOffset() < startEndcaps) ||
            (isPixelTracker && ((iter->getZOffset() +
                                 iter->getZLength() / 2.0) < startEndcaps)) ||
            isOTST) {
          ...
          if (!isOTST) {
            ...
            otstShapes.push_back(shape);
          }
          if (shape.rmin < supportBarrelRMin)
            supportBarrelRMin = shape.rmin;
          if (shape.rmax > supportBarrelRMax)
            supportBarrelRMax = shape.rmax;

          logic.name_tag = shapename.str();
          logic.shape_tag = (!isOTST ? trackerXmlTags.nspace : xml_otstident) +
                            ":" + shapename.str();
          logic.material_tag =
              (!isOTST ? trackerXmlTags.nspace : xml_otstident) + ":" +
              matname.str();
          if (!isOTST) {
            l.push_back(logic);
          } else {
            otstLogic.push_back(logic);
          }
        ...
        // ENDCAPS supports
          ...
          if (isPixelTracker && iter->getZOffset() < startEndcaps) {
            std::ostringstream shapenameBarrel, shapenameEndcaps;
            shapenameBarrel
                << xml_base_lazy << "R" << (int)(iter->getInnerRadius()) << "Z"
                << (int)(iter->getZLength() / 2.0 + iter->getZOffset())
                << "BarrelPart";
            shapenameEndcaps
                << xml_base_lazy << "R" << (int)(iter->getInnerRadius()) << "Z"
            ...
            shape.rmax = shape.rmin + iter->getRWidth();
            s.push_back(shape);
            if (shape.rmin < supportEndcapsRMin)
              supportEndcapsRMin = shape.rmin;
            if (shape.rmax > supportEndcapsRMax)
              supportEndcapsRMax = shape.rmax;
...
// This rotation is defined as the following : axial rotation, with axis X
// (local X of the module), and angle tiltAngle.
void Extractor::addTiltedModuleRot(std::map<std::string, Rotation> &rotations,
                                   double tiltAngle) {
  std::ostringstream tilt;
...
/* Add rotation around local Y axis.
   For example, this is used to skew modules: modules are rotated around their
   local Y axis.
*/
void Extractor::addRotationAroundZAxis(
    std::map<std::string, Rotation> &storedRotations,
    const std::string rotationName, const double rotationAngleInRad) const {
...
void Extractor::createAndStoreDDTrackerAngularAlgorithmBlock(
    std::vector<AlgoInfo> &storedAlgorithmBlocks, const std::string nameSpace,
    const std::string parentName, const std::string childName,
    const double startAngleInRad, const double rangeAngleInRad,
    const double radius, const XYZVector &center, const int numCopies,
    const int startCopyNumber, const int copyNumberIncrement) {
  AlgoInfo myAlgo; // would obviously be better to build the algo directly here
                   // instead of having a constructor with no argument!
  ...
  // Child volume name
  myAlgo.parameters.push_back(
      stringParam(xml_childparam, nameSpace + ":" + childName));

  // Volume with copy number 1, center phi angle
  myAlgo.parameters.push_back(numericParam(
      xml_startangle,
      any2str(startAngleInRad * 180. / M_PI, xml_angle_precision) + "*deg"));

  // Difference between first and last volumes' centers phi angles
  myAlgo.parameters.push_back(numericParam(
      xml_rangeangle,
      any2str(rangeAngleInRad * 180. / M_PI, xml_angle_precision) + "*deg"));

  // All volumes are assumed to be placed at same radius in that algorithm
  myAlgo.parameters.push_back(
      numericParam(xml_radius, any2str(radius) + "*mm"));
  ...
  // Start copy number
  myAlgo.parameters.push_back(
      numericParam(xml_startcopyno, any2str(startCopyNumber)));

  // Copy number increment
  myAlgo.parameters.push_back(
      numericParam(xml_incrcopyno, any2str(copyNumberIncrement)));
...
/**
 * Bundle the information for a composite material from a list of components
 * into an instance of a <i>Composite</i> struct. The function includes an
 * option to skip the sensor silicon in the list of elementary materials,
 * omitting it completely when assembling the information for the
 * <i>Composite</i> struct. This is useful for active volumes, where the sensor
 * silicon is assigned to a separate volume during the tranlation to CMSSW XML.
 * On the other hand, an inactive volume will want to include all of its
 * material components - regardless of what they're labelled.
 * @param name The name of the new composite material
 * @param density The overall density of the new composite material
 * @param mp A reference to the material properties object that provides the
 * list of elementary materials
 * @param nosensors A flag to include or exclude sensor silicon from the
 * material list; true if it is excluded, false otherwise
 * @return A new instance of a <i>Composite</i> struct that bundles the material
 * information for further processing
 */
Composite Extractor::createComposite(std::string name, double density,
                                     MaterialProperties &mp, bool nosensors) {
  Composite comp;
  comp.name = name;
  ...
  for (const auto &it : mp.getLocalMasses()) {
    if (!nosensors || ((it.first.compare(xml_sensor_silicon) != 0) &&
                       (it.first.compare(xml_sensor_LYSO) !=
                        0))) { // SenSi element is not treated here.
      if (comp.elements.find(it.first) !=
          comp.elements
              .end()) { // should not be necessary, but better safe than sorry
        throw PathfulException("Tried to insert several times " + it.first +
  ...
  // element info is the ratio of the mass of the element by the mass of the
  // composite
  for (auto &elem : comp.elements) {
    elem.second /= m;
  }

  comp.normalizedRIRatioPerMechanicalCategory =
      mp.getNormalizedRIRatioPerMechanicalCategory();

  return comp;
}

/**
 * Find the partner module of a given one in a layer, i.e. a module that is on
 * the same rod but on the opposite side of z=0.
 ...
 * @param find_first A flag indicating whether to stop the search at the first
 * module with the desired position, regardless of which side of z=0 it is on;
 * default is false
 * @return An iterator pointing to the partner module, or to one past the end of
 * the range if no partner is found
 */
std::vector<ModuleCap>::iterator
Extractor::findPartnerModule(std::vector<ModuleCap>::iterator i,
  ...
  if (i != g) {
    ...
    if (!find_first)
      plus =
          i->getModule().uniRef().side > 0; // i->getModule().center().Z() > 0;
    while (res != g) {
      if (res->getModule().uniRef().ring == ponrod) {
        if (find_first)
          break;
        else {
          if ((plus && res->getModule().uniRef().side <
                           0 /*(res->getModule().center().Z() < 0)*/) ||
              (!plus && res->getModule().uniRef().side >
                            0 /*(res->getModule().center().Z() > 0)*/))
            break;
        }
      }
 ...
 * @param start An iterator pointing to the start of a range of modules in a
 * vector
 * @param stop An iterator pointing to one past the end of a range of modules in
 * a vector
...
double Extractor::findDeltaR(std::vector<Module *>::iterator start,
                             std::vector<Module *>::iterator stop,
                             double middle) {
 ...
 * @param stop An iterator pointing to one past the end of a range of modules in
 * a vector
 * @param middle The midpoint in z of the disc that the ring in question belongs
 * to
 * @return Half the thickness of the ring volume, or zero if the module
...
/**
 * Given the name of a <i>SpecPar</i> block, find the corresponding index in the
 * vector of <i>SpecParInfo</i> structs.
 * @param specs The list of <i>SpecParInfo</i> structs collecting topological
 * information for the tracker
 * @param label The name of the requested <i>SpecPar</i> block
 * @return The index of the requested struct, or <i>-1</i> if the name was not
 * found in the collection
 */
int Extractor::findSpecParIndex(std::vector<SpecParInfo> &specs,
                                std::string label) {
  int idx = 0, size = (int)(specs.size());
  while (idx < size) {
    if (specs.at(idx).name.compare(label) == 0)
      return idx;
    idx++;
  }
  return -1;
...
/**
 * Calculate the thickness of the sensor material in a module from the amount of
 * sensor silicon <i>SenSi</i> and the dimensions of the module.
...
double Extractor::calculateSensorThickness(ModuleCap &mc, MaterialTable &mt) {
  ...
  for (std::map<std::string, double>::const_iterator it =
           mc.getLocalMasses().begin();
       it != mc.getLocalMasses().end(); ++it) {
    if (it->first.compare(xml_sensor_silicon) == 0)
      m += it->second;
  }
  try {
    d = mt.getMaterial(xml_sensor_silicon).density;
...
std::string Extractor::stringParam(std::string name, std::string value) {
  std::string res;
  res = xml_algorithm_string + name + xml_algorithm_value + value +
        xml_general_endline;
  return res;
}
...
std::string Extractor::numericParam(std::string name, std::string value) {
  std::string res;
  res = xml_algorithm_numeric + name + xml_algorithm_value + value +
        xml_general_endline;
  return res;
}
...
std::string Extractor::vectorParam(double x, double y, double z) {
  ...
  res << xml_algorithm_vector_open << x << "*mm, " << y << "*mm, " << z << "*mm"
      << xml_algorithm_vector_close;
  return res.str();
}

/**
 * Calculate the composite density of the material mix in a module. A boolean
 * flag controls whether the sensor silicon <i>SenSi</i> should be added to or
 * excluded from the overall mix.
 * @param mc A reference to the module materials
 * @param nosensors Then sensor silicon flag; true if that material should be
...
double Extractor::compositeDensity(ModuleCap &mc, bool nosensors) {
  ...
  if (nosensors) {
    ...
    for (std::map<std::string, double>::const_iterator it =
         ...
         it != mc.getLocalMasses().end(); ++it) {
      if (it->first.compare(xml_sensor_silicon) != 0 &&
          it->first.compare(xml_sensor_LYSO) != 0)
        m += it->second;
    }
    d = 1000 * m / d;
  } else
...
/**
 * Calculate the radial distance of an outer rod surface from the outer limit of
 * its layer.
...
/**
 * Calculate estimated atomic number and atomic weight from radiation and
 * nuclear interaction length.
 * @param radiationLength
 * @param interactionLength
 * @return Estimated atomic number and atomic weight.
 */
std::pair<double, int> Extractor::getAZ(double radiationLength,
                                        double interactionLength) {
  const double A_a = 31.645;
  const double A_b = 11.57238;
  double A_estimate = pow((interactionLength - A_b) / A_a,
                          3); // Wait, A value will be corrected in a few lines.
  ...
  // Bonus : calculate estimated radiation length and interaction lengths that
  // one can get from the provided Z and A values.
  // radEstimate = compute_X0(Z, A_estimate);
  // double intEstimate = pow(A_estimate, 1./3.)*A_a+A_b;

  // On CMSSW side, radiation lengths and nuclear interaction lengths will be
  // recomputed from Z, A and density. Now, a question is whether the computed
  // radiation and interaction lengths on CMSSW side (similar to radEstimate and
  // intEstimate) are closed to the values we initially had at hand (double
  // radiationLength and interactionLength) ! The estimated errors can be
  // calculated by : Error on radiation length :
  // (radEstimate-radiationLength)/radiationLength Error on nuclear interaction
  // length : (intEstimate-interactionLength)/interactionLength
...
/**
 * Calculate the closest atomic number corresponding to a given estimated atomic
 * weight and radiation length.
...
int Extractor::findClosest_Z(double pA, double radiationLength) {
  ...
  for (int i = 2; i < 85; ++i) {
    ...
    if (aDistance < distance)
      distance = aDistance, closest_Z = i;
  }
  return closest_Z;
}

/**
 * Calculate the estimated radiation length associated to an estimated atomic
 * number Z and estimated atomic weight A.
 */
double Extractor::compute_X0(int pZ, double pA) {
  ...
  double a_4 = a_2 * a_2;
  double a_6 = a_3 * a_3;
  double f_Z = a_2 * (1 / (1 + a_2) + 0.20206 - 0.0369 * a_3 + 0.0083 * a_4 -
                      0.002 * a_6);
...
const double ModuleComplexHelpers::computeExpandedModWidth(
    const double moduleWidth, const double serviceHybridWidth,
    const double deadAreaExtraWidth, const double chipNegativeXExtraWidth,
    const double chipPositiveXExtraWidth) {
  const double totalServiceHybridWidth = 2. * serviceHybridWidth; // OT case
  const double totalDeadAreaExtraWidth =
      2. * deadAreaExtraWidth; // IT case: around sensor
  const double totalChipExtraWidth =
      2. * MAX(chipNegativeXExtraWidth,
               chipPositiveXExtraWidth); // IT case: around chip
  const double expandedModWidth =
      moduleWidth + totalServiceHybridWidth +
      MAX(totalDeadAreaExtraWidth, totalChipExtraWidth);
  return expandedModWidth;
}

const double ModuleComplexHelpers::computeExpandedModLength(
    const double moduleLength, const double frontEndHybridWidth,
  ...
  const double expandedModLength =
      moduleLength + totalFrontEndHybridWidth + totalDeadAreaExtraLength;
  return expandedModLength;
}

ModuleComplex::ModuleComplex(std::string moduleName, std::string parentName,
                             ModuleCap &modcap)
    : modulecap(modcap), module(modcap.getModule()), moduleId(moduleName),
      parentId(parentName), modThickness(module.thickness()),
      ...
      hybridTotalMass(0.), hybridTotalVolume_mm3(-1.),
      hybridFrontAndBackVolume_mm3(-1.), hybridLeftAndRightVolume_mm3(-1.),
      deadAreaTotalVolume_mm3(-1.), moduleMassWithoutSensors_expected(0.),
      expandedModWidth(ModuleComplexHelpers::computeExpandedModWidth(
          modWidth, serviceHybridWidth, deadAreaExtraWidth,
          chipNegativeXExtraWidth, chipPositiveXExtraWidth)),

      expandedModLength(ModuleComplexHelpers::computeExpandedModLength(
          modLength, frontEndHybridWidth, deadAreaExtraLength)),
      center(module.center()), normal(module.normal()),
      prefix_material(xml_hybrid_comp) {
  if (!module.isPixelModule()) {
    if (!module.isTimingModule())
      expandedModThickness =
          sensorDistance + 2.0 * (supportPlateThickness + sensorThickness);
    else
      expandedModThickness =
          sensorThickness + 2.0 * MAX(supportPlateThickness, hybridThickness);
    prefix_xmlfile = xml_fileident + ":";
    nTypes = 9;
  } else {
    expandedModThickness =
        sensorThickness + 2.0 * MAX(chipThickness, hybridThickness);
...
void ModuleComplex::buildSubVolumes() {
  ...
  if (!module.isPixelModule()) {
    ...
    if (!module.isTimingModule()) {
      ...
      vol[xml_HybridFront] = new Volume(moduleId + "FSide", xml_HybridFront,
                                        parentId, dx, dy, dz, posx, posy, posz);

      posx = -(modWidth + serviceHybridWidth) / 2.;
      posy = 0.;
      posz = 0.;
      // Hybrid BackSide Volume
      vol[xml_HybridBack] = new Volume(moduleId + "BSide", xml_HybridBack,
                                       parentId, dx, dy, dz, posx, posy, posz);
      ...
      vol[xml_HybridLeft] = new Volume(moduleId + "LSide", xml_HybridLeft,
                                       parentId, dx, dy, dz, posx, posy, posz);
      ...
      vol[xml_HybridRight] = new Volume(moduleId + "RSide", xml_HybridRight,
                                        parentId, dx, dy, dz, posx, posy, posz);
      ...
      vol[xml_HybridBetween] =
          new Volume(moduleId + "Between", xml_HybridBetween, parentId, dx, dy,
                     dz, posx, posy, posz);
      ...
      // SupportPlate
      vol[xml_SupportPlate] =
          new Volume(moduleId + "SupportPlate", xml_SupportPlate, parentId, dx,
                     dy, dz, posx, posy, posz);
    }
    ...
    //     =========================     Chip    (3)
    //
    // Chip(3) volume can contain Bumps and any other material (supports, etc)
    // for simplification.
    //
    // Volumes (5), (6), (7), and (8) are volumes of same thickness as the
    // sensor, located around the sensor. They are used for Si inactive areas.
    ...
    vol[xml_PixelModuleHybrid] =
        new Volume(moduleId + "Hybrid", xml_PixelModuleHybrid, parentId,
    ...
    vol[xml_PixelModuleDeadAreaRight] = new Volume(
        moduleId + "DeadAreaRight", xml_PixelModuleDeadAreaRight, parentId,
    ...
    vol[xml_PixelModuleDeadAreaLeft] = new Volume(
        moduleId + "DeadAreaLeft", xml_PixelModuleDeadAreaLeft, parentId,
    ...
    vol[xml_PixelModuleDeadAreaFront] = new Volume(
        moduleId + "DeadAreaFront", xml_PixelModuleDeadAreaFront, parentId,
    ...
    vol[xml_PixelModuleDeadAreaBack] = new Volume(
        moduleId + "DeadAreaBack", xml_PixelModuleDeadAreaBack, parentId,
    ...
    const double myChipWidth =
        modWidth + chipNegativeXExtraWidth + chipPositiveXExtraWidth;
    const double myChipLength = modLength;
    const double myChipThickness = chipThickness;
    const double myChipPosX =
        (chipPositiveXExtraWidth - chipNegativeXExtraWidth) / 2.;
    const double myChipPosY = 0.;
    const double myChipPosZ = -sensorThickness / 2. - chipThickness / 2.;
    vol[xml_PixelModuleChip] = new Volume(
        moduleId + "Chip", xml_PixelModuleChip, parentId, myChipWidth,
        myChipLength, myChipThickness, myChipPosX, myChipPosY, myChipPosZ);
  }

  // =========================================================================================================
  // Finding Xmin/Xmax/Ymin/Ymax/Zmin/Zmax/Rmin/Rmax/RminatZmin/RmaxatZmax,
  // taking hybrid volumes into account
  // =========================================================================================================
  //
  // Module polygon
  //   top view
  ...
  //    ----------------- bottom
  //
  vector<double> xv; // x list (in global frame of reference) from which we will
                     // find min/max.
  vector<double> yv; // y list (in global frame of reference) from which we will
                     // find min/max.
  vector<double> zv; // z (in global frame of reference) list from which we will
                     // find min/max.
  vector<double> rv; // radius list (in global frame of reference) from which we
                     // will find min/max.
  vector<double> ratzminv; // radius list (in global frame of reference) at zmin
                           // from which we will find min/max.
  vector<double> ratzmaxv; // radius list (in global frame of reference) at zmax
                           // from which we will find min/max.

  // mx: (v2+v3)/2 - center, my: (v1+v2)/2 - center
  XYZVector mx =
      (0.5 * (module.basePoly().getVertex(2) + module.basePoly().getVertex(3)) -
       center)
          .Unit();
  XYZVector my =
      (0.5 * (module.basePoly().getVertex(1) + module.basePoly().getVertex(2)) -
       center)
          .Unit();
  ...
  v[0] = module.center() - expandedModWidth / 2. * mx -
         expandedModLength / 2. * my;
  v[1] = module.center() - expandedModWidth / 2. * mx +
         expandedModLength / 2. * my;
  v[2] = module.center() + expandedModWidth / 2. * mx +
         expandedModLength / 2. * my;
  v[3] = module.center() + expandedModWidth / 2. * mx -
         expandedModLength / 2. * my;
  ...
  for (meit = matElements.begin(); meit != matElements.end(); meit++) {
    ...
    if (el->componentName() == "Sensor" || el->componentName() == "Sensors" ||
      ...
      continue; // We will not handle sensors in this class.
                // Indeed, only Sensi is assigned to the Sensor volume. There is
                // no addition of different materials. Sensi is directly
                // assigned to the sensor volume within Extractor::analyseLayers
                // and Extractor::analyseDiscs.
    }
    ...
    if (!module.isPixelModule()) {
      if (el->targetVolume() == xml_InnerSensor ||
        ...
        continue; // Still to skip sensors, in case not detected by the
                  // component name.
        ...
        std::cerr << "!!!! ERROR !!!! : Found unexpected targetVolume."
                  << std::endl;
        std::cerr << "targetVolume " << el->targetVolume()
                  << " is not supported for Outer Barrel modules. Exit."
                  << std::endl;
        std::exit(1);
      }
      ...
      if (el->targetVolume() == xml_HybridFront ||
        ...
        vol[el->targetVolume()]->addMaterial(el->elementName(),
                                             el->componentName(),
                                             el->quantityInGrams(module));
        ...
        vol[xml_HybridFront]->addMaterial(
            el->elementName(), el->componentName(),
            el->quantityInGrams(module) * vol[xml_HybridFront]->getVolume() /
                hybridFrontAndBackVolume_mm3);
        vol[xml_HybridBack]->addMaterial(el->elementName(), el->componentName(),
                                         el->quantityInGrams(module) *
                                             vol[xml_HybridBack]->getVolume() /
                                             hybridFrontAndBackVolume_mm3);
        vol[xml_HybridFront]->addMass(el->quantityInGrams(module) *
                                      vol[xml_HybridFront]->getVolume() /
                                      hybridFrontAndBackVolume_mm3);
        vol[xml_HybridBack]->addMass(el->quantityInGrams(module) *
                                     vol[xml_HybridBack]->getVolume() /
                                     hybridFrontAndBackVolume_mm3);
        ...
        vol[xml_HybridLeft]->addMaterial(el->elementName(), el->componentName(),
                                         el->quantityInGrams(module) *
                                             vol[xml_HybridLeft]->getVolume() /
                                             hybridLeftAndRightVolume_mm3);
        vol[xml_HybridRight]->addMaterial(
            el->elementName(), el->componentName(),
            el->quantityInGrams(module) * vol[xml_HybridRight]->getVolume() /
                hybridLeftAndRightVolume_mm3);
        vol[xml_HybridLeft]->addMass(el->quantityInGrams(module) *
                                     vol[xml_HybridLeft]->getVolume() /
                                     hybridLeftAndRightVolume_mm3);
        vol[xml_HybridRight]->addMass(el->quantityInGrams(module) *
                                      vol[xml_HybridRight]->getVolume() /
                                      hybridLeftAndRightVolume_mm3);
      } else if (el->targetVolume() == xml_HybridFBLR_0 ||
                 el->targetVolume() ==
                     xml_HybridFBLR_3456) { // Uniformly Distribute
    ...
    // PIXEL MODULE
      ...
      if (el->targetVolume() == xml_PixelModuleSensor) {
        ...
        throw PathfulException(
            "!!!! ERROR !!!! : Found unexpected targetVolume, not supported "
            "for Pixel Barrel modules.");
      }
      moduleMassWithoutSensors_expected += el->quantityInGrams(module);
      ...
      if (el->targetVolume() == xml_PixelModuleHybrid ||
      ...
      else if (el->targetVolume() ==
               xml_PixelModuleDeadArea) { // Uniformly Distribute

        if (deadAreaTotalVolume_mm3 < 0) { // Need only once
          deadAreaTotalVolume_mm3 =
              vol[xml_PixelModuleDeadAreaRight]->getVolume() +
        ...
        vol[xml_PixelModuleDeadAreaRight]->addMaterial(
            el->elementName(), el->componentName(),
            el->quantityInGrams(module) *
                vol[xml_PixelModuleDeadAreaRight]->getVolume() /
                deadAreaTotalVolume_mm3);
        vol[xml_PixelModuleDeadAreaLeft]->addMaterial(
            el->elementName(), el->componentName(),
            el->quantityInGrams(module) *
                vol[xml_PixelModuleDeadAreaLeft]->getVolume() /
                deadAreaTotalVolume_mm3);
        vol[xml_PixelModuleDeadAreaFront]->addMaterial(
            el->elementName(), el->componentName(),
            el->quantityInGrams(module) *
                vol[xml_PixelModuleDeadAreaFront]->getVolume() /
                deadAreaTotalVolume_mm3);
        vol[xml_PixelModuleDeadAreaBack]->addMaterial(
            el->elementName(), el->componentName(),
            el->quantityInGrams(module) *
                vol[xml_PixelModuleDeadAreaBack]->getVolume() /
                deadAreaTotalVolume_mm3);
        ...
        // Uniform density distribution and consistent with total mass
        vol[xml_PixelModuleDeadAreaRight]->addMass(
            el->quantityInGrams(module) *
            vol[xml_PixelModuleDeadAreaRight]->getVolume() /
            deadAreaTotalVolume_mm3);
        vol[xml_PixelModuleDeadAreaLeft]->addMass(
            el->quantityInGrams(module) *
            vol[xml_PixelModuleDeadAreaLeft]->getVolume() /
            deadAreaTotalVolume_mm3);
        vol[xml_PixelModuleDeadAreaFront]->addMass(
            el->quantityInGrams(module) *
            vol[xml_PixelModuleDeadAreaFront]->getVolume() /
            deadAreaTotalVolume_mm3);
        vol[xml_PixelModuleDeadAreaBack]->addMass(
            el->quantityInGrams(module) *
            vol[xml_PixelModuleDeadAreaBack]->getVolume() /
            deadAreaTotalVolume_mm3);
      }
...
void ModuleComplex::addMaterialInfo(std::vector<Composite> &vec) {
  for (const auto &myVolume : volumes) {
    ...
    comp.normalizedRIRatioPerMechanicalCategory =
        myVolume->getNormalizedRIRatioPerMechanicalCategory();
...
void ModuleComplex::print() const {
  ...
  std::cout << "    center postion : (" << center.X() << "," << center.Y()
            << "," << center.Z() << ")" << std::endl;
  ...
  for (unsigned int i = 0; i < vertex.size(); i++) {
    if (i != vertex.size() - 1)
      std::cout << "(" << vertex[i].X() << "," << vertex[i].Y() << ","
                << vertex[i].Z() << "), ";
    else
      std::cout << "(" << vertex[i].X() << "," << vertex[i].Y() << ","
                << vertex[i].Z() << ")" << std::endl;
  ...
  std::cerr << "  Module Total Mass = " << moduleTotalMass << " ("
            << moduleMassWithoutSensors_expected << " is expected.)"
            << std::endl;
}
