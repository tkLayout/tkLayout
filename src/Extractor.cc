			...
			if (is_relevant) {
				...
				for (iiter = oiter->begin(); iiter != iguard; iiter++) {
					if (rings.find(iiter->getModule().getRing()) == rings.end()) {
						...
						minfo.rocrows	= any2str<int>(iiter->getModule().getNROCRows());
						minfo.roccols	= any2str<int>(iiter->getModule().getNROCCols());
						minfo.rocx		= any2str<int>(iiter->getModule().getNStripsAcross()/iiter->getModule().getNROCRows());
						minfo.rocy		= any2str<int>(iiter->getModule().getNMaxSegments());
