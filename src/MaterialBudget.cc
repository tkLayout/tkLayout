...
namespace insur {
/**
 * The constructor registers a tracker and a collection of inactive surfaces
 * with the material budget before creating a module cap object for each active
 * module found in the tracker.
 */
MaterialBudget::MaterialBudget(Tracker &tr, InactiveSurfaces &is) {
  ...
  public:
    CapsVisitor(Caps &capsbarrelmods, Caps &capsendmods)
        : capsbarrelmods_(capsbarrelmods), capsendmods_(capsendmods) {}
 ...
 * @return A reference to the vector of vectors listing the module caps that are
 * associated with the barrel modules
 */
std::vector<std::vector<ModuleCap>> &MaterialBudget::getBarrelModuleCaps() {
  return capsbarrelmods;
}

/**
 * Get the collection of endcap module caps.
 * @return A reference to the vector of vectors listing the module caps that are
 * associated with the endcap modules
 */
std::vector<std::vector<ModuleCap>> &MaterialBudget::getEndcapModuleCaps() {
  return capsendmods;
}
...
void MaterialBudget::print() {
  std::cout << std::endl
            << "----------Material Budget Internal State----------"
            << std::endl;
  ...
  for (unsigned int i = 0; i < inactive->getSupports().size(); i++) {
    if (inactive->getSupportPart(i).getCategory() == MaterialProperties::b_sup)
      a++;
    else if (inactive->getSupportPart(i).getCategory() ==
  ...
  for (unsigned int i = 0; i < capsbarrelmods.size(); i++)
    d = d + capsbarrelmods.at(i).size();
  for (unsigned int i = 0; i < capsendmods.size(); i++)
    f = f + capsendmods.at(i).size();
  std::cout << "MaterialBudget: " << d << " barrel modcaps in "
            << capsbarrelmods.size() << " vectors." << std::endl;
  std::cout << "MaterialBudget: " << f << " endcap modcaps in "
            << capsendmods.size() << " vectors." << std::endl;
  std::cout << "InactiveSurfaces: " << inactive->getBarrelServices().size()
            << " barrel services and ";
  std::cout << inactive->getEndcapServices().size() << " endcap services."
            << std::endl;
  std::cout << "InactiveSurfaces: " << inactive->getSupports().size()
            << " supports in total, " << a << " in barrels, ";
  std::cout << b << " in endcaps, " << g << " in barrel support tubes, " << h
            << " user defined, " << o;
  ...
  if (d > 0)
    std::cout << "Average module mass in barrels: " << (am / (double)d)
              << std::endl;
  if (d > 0)
    std::cout << "Average module radiation length in barrels: "
              << (ar / (double)d) << std::endl;
  if (d > 0)
    std::cout << "Average module interaction length in barrels: "
              << (ai / (double)d) << std::endl;
  for (unsigned int i = 0; i < capsbarrelmods.size(); i++) {
    std::cout << "Sample boundary module in layer " << (i + 1) << ":"
              << std::endl;
    try {
      capsbarrelmods.at(i).at(onBoundary(capsbarrelmods, i)).print();
    } catch (std::out_of_range &oor) {
      std::cerr << "Error: unable to find a module index for layer " << i << "."
                << std::endl;
  ...
  if (f > 0)
    std::cout << "Average module mass in endcaps: " << (bm / (double)f)
              << std::endl;
  if (f > 0)
    std::cout << "Average module radiation length in endcaps: "
              << (br / (double)f) << std::endl;
  if (f > 0)
    std::cout << "Average module interaction length in endcaps: "
              << (bi / (double)f) << std::endl;
  for (unsigned int i = 0; i < capsendmods.size(); i++) {
    std::cout << "Sample boundary module on disc " << i << ":" << std::endl;
    try {
      capsendmods.at(i).at(onBoundary(capsendmods, i)).print();
    } catch (std::out_of_range &oor) {
      std::cerr << "Error: unable to find a module index for disc " << i << "."
                << std::endl;
      std::cerr << oor.what() << std::endl;
    }
  }
  ...
  std::cout << "Total valid services in barrels: "
            << (inactive->getBarrelServices().size() - s) << std::endl;
  std::cout << "Total service mass in barrels: " << cm << std::endl;
  if (inactive->getBarrelServices().size() > 0)
    std::cout << "Average service mass in barrels: "
              << (cm / (double)(inactive->getBarrelServices().size()))
              << std::endl;
  if (inactive->getBarrelServices().size() > 0)
    std::cout << "Average service radiation length in barrels: "
              << (cr / (double)(inactive->getBarrelServices().size()))
              << std::endl;
  if (inactive->getBarrelServices().size() > 0)
    std::cout << "Average service interaction length in barrels: "
              << (ci / (double)(inactive->getBarrelServices().size()))
              << std::endl;
  std::cout << std::endl;
  ...
  std::cout << "Total valid services in endcaps: "
            << (inactive->getEndcapServices().size() - t) << std::endl;
  std::cout << "Total service mass in endcaps: " << dm << std::endl;
  if (inactive->getEndcapServices().size() > 0)
    std::cout << "Average service mass in endcaps: "
              << (dm / (double)(inactive->getEndcapServices().size()))
              << std::endl;
  if (inactive->getEndcapServices().size() > 0)
    std::cout << "Average service radiation length in endcaps: "
              << (dr / (double)(inactive->getEndcapServices().size()))
              << std::endl;
  if (inactive->getEndcapServices().size() > 0)
    std::cout << "Average service interaction length in endcaps: "
              << (di / (double)(inactive->getEndcapServices().size()))
              << std::endl;
  std::cout << std::endl;
  for (unsigned int i = 0; i < inactive->getSupports().size(); i++) {
    if (inactive->getSupportPart(i).getCategory() ==
  ...
  std::cout << "Total valid support parts: " << a << " in barrels, " << b
            << " in endcaps, ";
  std::cout << g << " in barrel support tubes, " << o
            << " in outer support tube and " << h;
  std::cout << " user defined." << std::endl << std::endl;
  std::cout << "Total support mass in barrels: " << (em + gm) << std::endl;
  if ((a + g) > 0)
    std::cout << "Average support mass in barrels: "
              << ((em + gm) / (double)(a + g)) << std::endl;
  if ((a + g) > 0)
    std::cout << "Average support radiation length in barrels: "
              << ((er + gr) / (double)(a + g)) << std::endl;
  if ((a + g) > 0)
    std::cout << "Average support interaction length in barrels: "
              << ((ei + gi) / (double)(a + g)) << std::endl;
  std::cout << std::endl;
  std::cout << "Total support mass in endcaps: " << fm << std::endl;
  if (b > 0)
    std::cout << "Average support mass in endcaps: " << (fm / (double)b)
              << std::endl;
  if (b > 0)
    std::cout << "Average support radiation length in endcaps: "
              << (fr / (double)b) << std::endl;
  if (b > 0)
    std::cout << "Average support interaction length in endcaps: "
              << (fi / (double)b) << std::endl;
  std::cout << std::endl;
  std::cout << "Total mass in outer support tube: " << lm << std::endl;
  if (o > 0)
    std::cout << "Average mass in outer support tube: " << (lm / (double)o)
              << std::endl;
  if (o > 0)
    std::cout << "Average radiation length in outer support tube: "
              << (lr / (double)o) << std::endl;
  if (o > 0)
    std::cout << "Average interaction length in outer support tube: "
              << (li / (double)o) << std::endl;
  std::cout << std::endl;
  std::cout << "Total mass in user defined supports: " << hm << std::endl;
  if (h > 0)
    std::cout << "Average mass in user defined supports: " << (hm / (double)h)
              << std::endl;
  if (h > 0)
    std::cout << "Average radiation length in user defined supports: "
              << (hr / (double)h) << std::endl;
  if (h > 0)
    std::cout << "Average interaction length in user defined supports: "
              << (hi / (double)h) << std::endl;
  std::cout << std::endl;
  ...
  std::cout << std::endl;
  std::cout << "Total mass in tracker: "
            << ((am + bm + cm + dm + em + fm + gm + hm + lm) / 1000.0) << "kg."
            << std::endl;
...
// protected
/**
 * This finds a sample module in the given layer that sits at the end of its
 * rod.
 * @param source The complete collection of barrel module caps
 * @param layer The layer within that the query applies to
 * @return The index of the sample module within the vector of module caps of
 * the given layer
 */
int MaterialBudget::onBoundary(std::vector<std::vector<ModuleCap>> &source,
                               int layer) { // throws exception
  int ring = 0, index = 0;
  if ((layer >= 0) && (layer < (int)source.size())) {
    for (unsigned int mod = 0; mod < source.at(layer).size(); mod++) {
      int myring =
          source.at(layer).at(mod).getModule().as<BarrelModule>()->ring();
      if (myring > ring) {
        ring = myring;
        index = mod;
      }
    ...
    throw std::out_of_range("Layer index is out of range: " + layer);
  return index;
}
...
std::vector<InactiveElement> MaterialBudget::getAllServices() {
  ...
  allServices.reserve(barrelServices.size() + endcapServices.size() +
                      supports.size()); // preallocate memory
  allServices.insert(allServices.end(), barrelServices.begin(),
                     barrelServices.end());
  allServices.insert(allServices.end(), endcapServices.begin(),
                     endcapServices.end());
