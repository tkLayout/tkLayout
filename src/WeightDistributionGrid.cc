...
void WeightDistributionGrid::addTotalGrams(
    double minZ, double minR, double maxZ, double maxR, double length,
    double surface, const MaterialObject &materialObject) {
  ...
  do {
    ...
    do {
      ...
      operator[](std::make_pair(binIndexZ, binIndexR)) +=
          (materialObject.totalGrams(length, surface) * zMul * rMul) /
          ((maxZ - minZ) * (maxR - minR));
...
double WeightDistributionGrid::binDimension() const { return binDimension_; }
} // namespace material
