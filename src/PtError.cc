...
double ptError::computeErrorBE(double p) {
  ...
  // double f5 = (1-g)/sin(Module_theta)/cos(Module_theta); // For material
  // interaction std::cout << "f1 = " << f1 <<std::endl; std::cout << "f2 = " <<
  // f2 <<std::endl; std::cout << "f3 = " << f3 <<std::endl; std::cout << "f4 =
  // "
  // << f4 <<std::endl; std::cout << "f5 = " << f5 <<std::endl;
  ...
  if (material.radiation == 0) {
    ...
    // in this case theta represents the average scattering angle in the (r,z)
    // plane (That is: i should divide p by sin(theta) [bigger momentum, less
    // scattering] to obtain the scattering angle in the plane containing the z
    // axis and the track and then I should use the distance between two hits
    // [larger lever arm] to obtain the error on phi. But the two effects
    // cancel, so I can directly use pT and deltaR of the hits
    scatteringAngle = (13.6 * Units::MeV) / (p / Units::MeV) *
                      sqrt(material.radiation) *
                      (1 + 0.038 * log(material.radiation));
  ...
  case BARREL:
    ...
    //#ifdef ADD_ERR
    // relativeError = f1 *
    // sqrt(2*pow(Module_pitch,2)+pow(Module_strip_l*0.01,2)) /sqrt(12) /
    // Module_d; #endif
    break;
  case ENDCAP: {
    //        static std::ofstream ofs("pterr.txt");
    //        ofs << Module_z << "\t" << Module_r << "\t" << Module_d << "\t" <<
    //        Module_ed << "\t" << Module_d*Module_r/Module_z << "...\t";
    relativeError2 = f1_sq *
                     pow(Module_pitch / (Module_d * Module_r / Module_z), 2) /
                     6; // * 1/tg(theta)
                        //        ofs << relativeError2 << "\t";
    relativeError2 += f2_sq * pow(deltaPhi, 2);
    //        ofs << relativeError2 << "\t";
    ...
    //        ofs << relativeError2 << "\t";
    if (material.radiation != 0)
      relativeError2 += pow(f5 * deltaTheta, 2);
  ...
  default:
    ...
    std::cerr << "Error: in TODO:addFunctionNameHere"
              << "I just found a module which is neither Barrel nor Endcap"
              << std::endl;
  }
...
double ptError::computeError(
    double p) { // CUIDADO TODO this should be refactored (remove type checks!),
                // but we have to refit the new occupancy data first
