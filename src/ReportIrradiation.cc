...
void ReportIrradiation::preparePowerHistograms() {
  prepareTrackerMap(sensorsIrradiationPowerMap, "sensorsIrradiationPowerMap",
                    "Map of power dissipation in sensors (after irradiation)");
  prepareTrackerMap(totalPowerConsumptionMap, "irradiationPowerConsumptionMap",
                    "Map of power dissipation in modules (after irradiation)");
}
...
void ReportIrradiation::computeChipPowerConsumptionTable() {
  ...
  for (auto it = v.typeMap.begin(); it != v.typeMap.end(); ++it) {
    ...
    for (auto &itPowerValue : it->second) {
      if (j == 1)
        chipPowerPerType.setCell(j, 0, "Totap power [mW]");
      else if (j == 2)
        chipPowerPerType.setCell(j, 0, "Power per module [mW]");
      else
        chipPowerPerType.setCell(
            j, 0, Form("Power per channel in sensor %d [mW]", j - 2));
...
std::string ReportIrradiation::createSensorsIrradiationCsv() {
  ...
  public:
    void preVisit() {
      output_
          << "Module DetId, Section, Layer, Ring, moduleType, dsDistance, "
             "isOuterRadiusRod_bool, operatingTemperature_Celsius, "
             "biasVoltage_V, meanWidth_mm, length_mm, sensorThickness_mm, "
             "sensor(s)Volume(totalPerModule)_mm3, sensorsPowerMean_W, "
             "sensorsPowerMax_W, sensorsFluenceMean_Hb, sensorsFluenceMax_Hb"
          << std::endl;
    }
    ...
    void visit(const Module &m) {
      output_ << m.myDetId() << ", " << sectionName_ << ", " << layerId_ << ", "
              << m.moduleRing() << ", " << m.moduleType() << ", "
              << m.dsDistance() << ", " << isOuterRadiusRod_ << ", "
              << std::fixed << std::setprecision(6) << m.operatingTemp() << ", "
              << m.biasVoltage() << ", " << m.meanWidth() << ", " << m.length()
              << ", " << m.sensorThickness() << ", " << m.totalSensorsVolume()
              << ", " << std::fixed << std::setprecision(3)
              << m.sensorsIrradiationPowerMean() << ", "
              << m.sensorsIrradiationPowerMax() << ", "
              << m.sensorsIrradiationMean() << ", " << m.sensorsIrradiationMax()
              << std::endl;
    }
...
void ReportIrradiation::dumpRadiationTableSummary(
    RootWPage &myPage, std::map<std::string, SummaryTable> &radiationSummaries,
    const std::string &title, std::string units) {
  for (std::map<std::string, SummaryTable>::iterator it =
           radiationSummaries.begin();
       it != radiationSummaries.end(); ++it) {
    RootWContent &myContent =
        myPage.addContent(title + std::string(" (") + it->first + ")", false);
    RootWTable *comments = new RootWTable();
    comments->setContent(0, 0,
                         "Values in table represent the mean value per module "
                         "with fully irradiated sensors [" +
                             units + "]");
...
void ReportIrradiation::visualizeTo(RootWSite &site) {
  ...
  RootWContent &summaryContent =
      myPage.addContent("Irradiation summary per module type");
  ...
  RootWContent &chipContent =
      myPage.addContent("Chip power consumption per module type", false);
  RootWTable &typesTable = chipContent.addTable();
  typesTable.setContent(chipPowerPerType.getContent());

  dumpRadiationTableSummary(myPage, powerSummaries,
                            "Power in irradiated sensors", "W");
  dumpRadiationTableSummary(myPage, fluenceSummaries, "Fluence on sensors",
                            "1-MeV-n-eq×cm" + superStart + "-2" + superEnd);
  ...
  struct SensorsIrradiationPower {
    double operator()(const Module &m) {
      return m.sensorsIrradiationPowerMean();
    } // W
  };

  struct TotalPower {
    double operator()(const Module &m) {
      return m.sensorsIrradiationPowerMean() + m.totalPower() * Units::mW;
    } // W (convert m.totalPower() from mW to W)
  };
  ...
  yzSensorsPowerDrawer.addModules<CheckType<BARREL | ENDCAP>>(
      tracker.modules().begin(), tracker.modules().end());
  yzTotalPowerDrawer.addModulesType(tracker.modules().begin(),
                                    tracker.modules().end(), BARREL | ENDCAP);
  ...
  yzSensorsPowerDrawer.drawFrame<HistogramFrameStyle>(
      *sensorsIrradiationPowerCanvas.get());
  yzSensorsPowerDrawer.drawModules<ContourStyle>(
      *sensorsIrradiationPowerCanvas.get());
  ...
  RootWImage &sensorsIrradiationPowerImage = myContent.addImage(
      std::move(sensorsIrradiationPowerCanvas), insur::vis_std_canvas_sizeX,
      insur::vis_min_canvas_sizeY);
  sensorsIrradiationPowerImage.setComment(
      "Power dissipation in irradiated sensors (due to leakage current) "
      "(average per module) (W)");
  sensorsIrradiationPowerImage.setName("sensorsIrradiationPowerMap");
  RootWImage &totalPowerImage = myContent.addImage(std::move(totalPowerCanvas),
                                                   insur::vis_std_canvas_sizeX,
                                                   insur::vis_min_canvas_sizeY);
  totalPowerImage.setComment(
      "Total power dissipation in irradiated modules (W)");
  ...
  myTextFile =
      new RootWTextFile(Form("sensorsIrradiation%s.csv", trackerName.c_str()),
                        "Sensors irradiation file");
