...
SimParms::SimParms()
    : numMinBiasEvents("numMinBiasEvents", parsedAndChecked()),
      ...
      lumiRegShapeInMatBudgetAnalysis("lumiRegShapeInMatBudgetAnalysis",
                                      parsedAndChecked()),
      ...
      alphaParam("alphaParam",
                 parsedAndChecked()), // radiation-damage coefficient, A/cm
...
void SimParms::build() {
  try {
    ...
    std::string irradiationMapDirectory_ =
        mainConfigHandler::instance().getIrradiationDirectory();
    // iter between irradiation map file names and feed the
    // irradiationMapsManager
    for (std::vector<std::string>::const_iterator iterMapFile =
             irradiationMapFiles.begin();
         iterMapFile != irradiationMapFiles.end(); ++iterMapFile) {
  ...
  if (useIPConstraint()) {
    ...
    if (lumiRegZError() == 0)
      nonZero = false;
    if (rphiErrorCollider() == 0)
      nonZero = false;
  }
  if (!nonZero)
    throw PathfulException("IP constraint required, but errors on beam spot "
                           "set to zero in R-Phi/Z!",
                           "SimParms");
