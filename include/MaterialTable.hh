...
// MaterialTable is nearly not used and was replaced by MaterialTab. This should
// be reworked.

/**
 * @file MaterialTable.h
 * @brief This is the header file for the internal data structure for material
 * properties.
 */
...
/**
 * @struct MaterialRow
 * @brief One instance of the MaterialRow struct contains the relevant
 * information about a single basic, i.e. non-mixture, material.
...
/**
 * @class MaterialTable
 * @brief Essentially, this is a collection class for <i>MaterialRow</i>
 * instances.
 *
 * It provides access functions for individual entries and a couple of
 * bookkeeping calls. The access functions may trow exceptions if an entry
 * doesn't exist or if an index is out of range.
 */
class MaterialTable {
public:
  MaterialTable() {}
  ...
  void addMaterial(std::string tag, double density, double rlength,
                   double ilength);
...
class BaseMaterial { // this wasn't made abstract because it's used as key class
                     // in a set, therefore it is necessary to create an
                     // instance of it to perform key-based lookups
protected:
  std::string tag_;
...
// TODO: restart the development of this "new" material table so that supports
// composite materials and syncs with CMSSW
class MaterialTable2 {
  std::map<std::string, BaseMaterial *> materials_;

public:
  void parseMaterial(std::string str);
  BaseMaterial *getMaterial(std::string tags) const {
    return materials_.at(tags);
  }
  const std::map<std::string, BaseMaterial *> &getMaterials() const {
    return materials_;
  }
  ~MaterialTable2() {
    for (std::map<std::string, BaseMaterial *>::iterator it =
             materials_.begin();
...
public:
  ElementaryMaterial()
      : density_(0.), z_(0.), a_(0.), rlength_(0.), ilength_(0.) {}
...
public:
  CompositeMaterial(const MaterialTable2 *const mTable)
      : mTable_(mTable), density_(0.), rlength_(0.), ilength_(0.) {}
  double getDensity() { return density_; }
  double getRadiationLength();
  double getInteractionLength();
  std::string toXML() { return std::string(); }
