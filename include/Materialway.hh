...
namespace material {
static const std::string inactiveElementError =
    "Section without inactiveElement.";
...
private:
  ...
  class Section;
  class Station; // because Train need to use Station and Section, and Station
                 // and Section need to use Train
  ...
  // typedef std::map<const Layer*, std::pair<std::vector<Section*>, Section*> >
  // LayerRodSectionsMap; typedef std::map<const Disk*,
  // std::pair<std::vector<Section*>, Section*> > DiskRodSectionsMap;
...
private:
  ...
  public:
    Section(int minZ, int minR, int maxZ, int maxR, Direction bearing,
            Section *nextSection, bool debug);
    Section(int minZ, int minR, int maxZ, int maxR, Direction bearing,
            Section *nextSection);
    Section(int minZ, int minR, int maxZ, int maxR, Direction bearing,
            Section *nextSection, const Location &location);
    Section(int minZ, int minR, int maxZ, int maxR, Direction bearing);
    ...
    virtual void
    getServicesAndPass(const MaterialObject &source,
                       const std::vector<std::string> &unitsToPass);

    bool debug_;

  private:
    int minZ_, minR_, maxZ_, maxR_;
    Location myLocation_; // Location of the section: undefined, external to the
                          // layer / double-disk, or inside dee.
    Direction bearing_;
    Section *nextSection_;
    InactiveElement *inactiveElement_; /**< The InactiveElement for hooking up
  ...
  public:
    Station(int minZ, int minR, int maxZ, int maxR, Direction bearing,
            ConversionStation &conversionStation, Section *nextSection);
    Station(int minZ, int minR, int maxZ, int maxR, Direction bearing,
  ...
  public:
    Boundary();
    // Boundary(const Visitable* containedElement, int minZ, int minR, int maxZ,
    // int maxR);
    Boundary(const Visitable *containedElement, const bool isBarrel, int minZ,
             int minR, int maxZ, int maxR,
             bool hasCShapeOutgoingServices = false,
             int cShapeMinZ = std::numeric_limits<int>::max());
    virtual ~Boundary();
    ...
    int maxZ() const;
    int maxR() const;
    const bool hasCShapeOutgoingServices() const {
      return hasCShapeOutgoingServices_;
    }
    ...
    bool operator==(Boundary *const &b) {
      return (minZ_ == b->minZ() && minR_ == b->minR() && maxZ_ == b->maxZ() &&
              maxR_ == b->maxR());
    }
  ...
  /**
   * @class OuterUsher
   * @brief Is the core of the functionality that builds sections across
   * boundaries starting from a point and ending to another section or to the
   * upper right angle
   */
  class OuterUsher {
  public:
    ...
    void go(Boundary *boundary,
            const Tracker &tracker); /**< start the process of section building,
                                        returns pointer to the first */
  private:
    SectionVector &sectionsList_;
    BoundariesSet &boundariesList_;

    Direction buildDirection(const int &startZ, const int &startR,
                             const bool &hasStepInEndcapsOuterRadius,
                             const int &numBarrels);
    void routeOutgoingServicesAlongCShape(Section *&firstSection,
                                          Section *&lastSection, int &startR,
                                          const Direction direction, int startZ,
                                          int cShapeMinZ);
    bool findBoundaryCollision(int &collision, int &border, int startZ,
                               int startR, const Tracker &tracker,
                               Direction direction);
    bool findSectionCollision(std::pair<int, Section *> &sectionCollision,
                              int startZ, int startR, int end,
                              Direction direction);
    bool buildSection(Section *&firstSection, Section *&lastSection,
                      int &startZ, int &startR, int end, Direction direction,
                      bool isTowardsBiggerZ = true,
                      bool isTowardsBiggerR = true);
    bool buildSectionPair(Section *&firstSection, Section *&lastSection,
                          int &startZ, int &startR, int collision, int border,
                          Direction direction);
  ...
  class InnerUsher {
  public:
    InnerUsher(SectionVector &sectionsList, StationVector &stationListFirst,
...
public:
  ...
  static const double
      gridFactor; /**< the conversion factor for using integers in the algorithm
  ...
  // static const double globalMaxZ_mm;                     /**< the Z
  // coordinate of the end point of the sections */ static const double
  // globalMaxR_mm;                   /**< the rho coordinate of the end point of
  // the sections */ static const int globalMaxZ; static const int globalMaxR;
...
private:
  ...
  SectionVector
      sectionsList_; /**< Vector for storing all the sections (also stations)*/
  ...
  bool buildBoundaries(const Tracker &tracker); /**< build the boundaries around
                                                   barrels and endcaps */
  void buildExternalSections(
      const Tracker &tracker); /**< build the sections outside the boundaries */
  void buildInternalSections(
      Tracker &tracker); /**< build the sections inside the boundaries */
  ...
  void
  populateAllMaterialProperties(Tracker &tracker,
                                WeightDistributionGrid &weightDistribution);
  // void calculateMaterialValues(Tracker& tracker);
  void buildInactiveSurface(Tracker &tracker,
                            InactiveSurfaces &inactiveSurface);
  void calculateMaterialValues(InactiveSurfaces &inactiveSurface,
                               Tracker &tracker);
  // InactiveElement* buildOppositeInactiveElement(InactiveElement*
  // inactiveElement);

  BarrelBoundaryMap barrelBoundaryAssociations_;
  EndcapBoundaryMap endcapBoundaryAssociations_;
  ModuleSectionMap
      moduleSectionAssociations_; /**< Map that associate each module with the
                                     section that it feeds */
  LayerRodSectionsMap layerRodSections_; /**< maps for sections of the rods */
  DiskRodSectionsMap diskRodSections_;
  // std::map<Boundary&, Section*> boundarySectionAssociations;         /**< Map
  // that associate each boundary with the outgoing section (for the
  // construction) */
};
