...
static const double celsius_to_kelvin =
    273.15; // T(K) = T(°C) + celsius_to_kelvin
static const double siliconEffectiveBandGap =
    1.21; // eV. Used in the Hamburg model (effect of temperature on sensor
          // leakage current).

/**
 * Geometry constants.
 */
static const std::vector<std::string> geom_name_eta_regions = {
    "", "C", "I", "F", "VF", "VVF"}; // Name tracker eta regions
static const std::vector<double> geom_range_eta_regions = {
    0.001, 0.8, 1.6, 2.4, 3.2, 4.0}; // Name tracker eta regions

static const double geom_zero = 1E-6; // mm
static const double geom_epsilon =
    0.1; // mm, standard distance between one solid object and the next.
static const double geom_inactive_volume_width =
    2.0; // mm, standard geometrical thickness of an inactive volume
static const double geom_conversion_station_width = 2.0; // mm
static const double geom_inner_pixel_radius =
    30.0; // mm, inner support tube starts immediately inside
static const double geom_inner_strip_radius =
    150.0; // mm, inner support tube starts immediately inside
static const double geom_outer_strip_radius =
    1200.0; // mm, outer support tube starts immediately outside
...
static const double geom_max_length =
    2800.0; // mm, maximum length, in +z, available to place the tracker
            // components

static const double geom_max_eta_coverage =
    geom_range_eta_regions[geom_range_eta_regions.size() -
                           1]; // Tracking performed from step_eta_epsilon to
                               // max_eta_coverage in steps

static const int default_n_tracks =
    100; // Default number of tracks simulated
         // (max_eta_coverage/default_n_tracks = etaStep)

static const double hits_negligible = 1.E-10;

/**
 * Visualisation constants: material parameters for active surfaces, services
 * and supports, plus top volume padding. The selected materials are completely
 * arbitrary and only meant to distinguish one type of surface from another
 * visually.
 ...
 * @param top_volume_pad The extra space that is added to the dimensions of the
 * top volume on each side of the cube
 * @param temperature_levels The number of different colour levels in 2D
 * histogram plots
 */
...
static const double mat_budget_overall_scaling_factor =
    1.; // WARNING: DO NOT CHANGE THIS UNLESS YOU ARE OBSOLUTELY SURE OF WHAT
        // YOU ARE DOING!! This will scale absolutely all weights by the
        // mentioned factor (modules + cabling + supports). Was added for CMSSW
        // Material Budget debug purposes.

/**
 * Display formatting parameters - eta ticks displayed with short step in range
 * 0 - short_eta_coverage, with long step in range short_eta_coverage -
 * long_eta_coverage
 */
...
static const int vis_n_bins =
    geom_max_eta_coverage / vis_eta_step; // Default number of bins in histogram
                                          // from eta=0  to max_eta_coverage
...
/**
 * Internal string constants for standard one-sided and specialised
 * double-sided, rotated types
 */
...
/**
 * Integers used for materials assignments to modules volumes (XML export
 * only!!). This is not placed in the XMLs dedicated file, as it might be needed
 * to move this to core tkLayout MB analysis.
 */
 ...
 * @param default_mattabfile List of materials and of their properties as
 * required by <i>MaterialTable</i>
...
static const std::string default_chemicalElementsFile =
    "chemical_elements.list";
static const std::string default_chemicalCompoundsFile =
    "chemical_compounds.list";
static const std::string default_chemicalMixturesFile =
    "chemical_mixtures.list";
