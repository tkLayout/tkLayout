...
/**
 * @file Squid.h
 * @brief This is the header file for the integrating wrapper class for the
 * material budget operations of <i>tkgeometry</i>
 */
...
static const std::string err_no_geomfile =
    "There is no recorded name for the geometry configuration file. Initialise "
    "the tracker first.";
static const std::string err_no_matfile =
    "The provided material configuration file does not exist.";
static const std::string err_no_matfile_pixel =
    "The material configuration file for the pixels does not exist.";
static const std::string err_init_failed =
    "Initialisation of the material calculator failed.";
static const std::string err_no_tracker =
    "The tracker object does not exist. The tracker must be created before "
    "calling this function.";
static const std::string err_no_inacsurf =
    "The collection of inactive surfaces does not exist. It must be created "
    "before calling this function";
static const std::string err_no_matbudget =
    "The material budget does not exist. It must be created before calling "
    "this function.";
static const std::string err_no_triggerSummary =
    "Could not report on the trigger performance.";
static const std::string warn_rootonly =
    "The collection of inactive surfaces does not exist. Only the .root file "
    "will be written.";
static const std::string warn_custom_matfile =
    "A customized material file was used for the tracker";
static const std::string warn_custom_matfile_pixel =
    "A customized material file was used for the pixel";

static const std::string default_trackername = "defaultTrackerName";

/**
 * @class Squid
 * @brief The Squid class integrates the components of the <i>tkmaterial</i>
 * application and provides an interface to its high-level functionality.
 *
 * It manages instances to all the necessary components internally. Its
 * functions bundle the steps that are required to carry out the main tasks of
 * the application. The idea is to allow access to the underlying via a series
 * of well-defined pathways contained in a single class. This class acts as a
 * sort of manager to the application and can be embedded into a main program
 * easily. It maintains internal instances of all the objects that are necessary
 * to run the modelling, analysis and visualisation steps.
 */
class Squid {
public:
  Squid();
  ...
  bool reportOuterCablingMapSite(const bool outerCablingOption,
                                 const std::string layoutName);
  bool reportInnerCablingMapSite(const bool innerCablingOption,
                                 const std::string layoutName);
  bool pureAnalyzeMaterialBudget(int tracks, bool triggerRes,
                                 bool triggerPatternReco, bool debugResolution);
