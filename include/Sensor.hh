...
public:
  ...
  // Timing barrel module : approximated as 1 single sensor made of crystals
  // (for the XML export)
  ...
  Sensor()
      : numStripsAcross("numStripsAcross", parsedOnly()),
        ...
        numROCX("numROCX", parsedOnly()), numROCY("numROCY", parsedOnly()),
  ...
  double sensorNormalOffset() const; // normal offset of the sensor center, in
                                     // the frame of reference of the module
  const XYZVector &center() const {
    return hitPoly().getCenter();
  } // center of the sensor
  const Polygon3d<4> &
  hitPoly() const; // sensor rectangle (in plane containing the sensor center)
  const Polygon3d<4> &
  hitMidPoly() const; // losange formed by the mid-points of hitPoly
  const Polygon3d<8> &envelopePoly() const; // sensor parallelepiped rectangle
  const Polygon3d<8> &
  envelopeMidPoly() const; // parallelepiped formed by hitMidPoly shifted by
                           // -sensorThickness/2 and + sensorThickness/2
  void clearPolys();

  std::pair<XYZVector, int> checkHitSegment(const XYZVector &trackOrig,
  ...
  void setup() {
    clearPolys();
    minR.setup(
        [&]() { return CoordinateOperations::computeMinR(envelopePoly()); });
    maxR.setup(
        [&]() { return CoordinateOperations::computeMaxR(envelopePoly()); });
    minZ.setup(
        [&]() { return CoordinateOperations::computeMinZ(envelopePoly()); });
    maxZ.setup(
        [&]() { return CoordinateOperations::computeMaxZ(envelopePoly()); });
  }

  void accept(SensorGeometryVisitor &v) { v.visit(*this); }
};
