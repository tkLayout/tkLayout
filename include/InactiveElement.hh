 ...
 * @brief This is the base class for the elements that make up the inactive
 * surfaces.
 *
 * Since all inactive elements are simplified to tube shapes in the geometrical
 * model, the geometry parameters have been packed into the base class. All of
 * these parameters apply to descendants of a different shape as well, though,
 * because they describe relations between the object and the origin, not
 * between points within the object.
 */
class InactiveElement : public MaterialProperties {
public:
  /**
   * @enum InType A list of the various types of neighbour or feeder an element
   * can have
   */
  ...
  const bool checkTrackHits(const XYZVector &trackOrig,
                            const XYZVector &trackDir, XYZVector &hitPos,
