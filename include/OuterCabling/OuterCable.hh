...
class OuterCable : public PropertyObject,
                   public Buildable,
                   public Identifiable<int> {
  typedef std::vector<OuterBundle *> Container;

public:
  OuterCable(const int id, const double phiSectorWidth, const int phiSectorRef,
  ...
  const ChannelSection *opticalChannelSection() const {
    if (!opticalChannelSection_)
      throw PathfulException("opticalChannelSection_ is nullptr");
...
private:
  void buildDTC(const double phiSectorWidth, const int phiSectorRef,
                const Category &type, const int slot,
                const bool isPositiveCablingSide);
  const std::string computeDTCName(const int phiSectorRef, const Category &type,
                                   const int slot,
  ...
  std::unique_ptr<const ChannelSection>
      opticalChannelSection_; // opticalChannelSection is owned by OuterCable
};
