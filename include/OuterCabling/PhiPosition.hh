...
/* This class contains several useful Phi identifiers.
   ...
   * phiSegment : Phi slice delimited by 2 consecutive (centers of) modules in
   Phi.
   * phiRegion : Phi slice of size defined in constructor. It is used for the
   following: Barrel : Phi slice containing bundles which can be connected to
   the same DTC. Endcap : Phi Slice contaning modules which can be connected to
   the same bundle.
   * phiSector : Phi slice of size outer_cabling_nonantWidth.

   There is the following classification, by the phi angle which is covered :
   phiSegment < phiRegion < phiSector.

   NB: These phi slices correspond to purely geometrical delimitations only.
   In exceptional cases, the modules can be artificially placed in a phi slice
   which does not correspond to its geometrical position : staggering. This is
   used to avoid that a bundle (or cable) connects to more modules than
   possible.
*/
class PhiPosition {
public:
  PhiPosition(const double phi, const int numPhiSegments, const bool isBarrel,
              const int layerDiskNumber, const std::string subDetectorName = "",
              const Category &bundleType = Category::UNDEFINED);
