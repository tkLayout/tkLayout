 ...
 * At the end of the process, 2 maps containing all the bundles which have been
 * built are returned.
 */
class ModulesToBundlesConnector : public GeometryVisitor {
public:
  std::map<int, OuterBundle *> getBundles() {
    return bundles_;
  } // positive cabling side
  std::map<int, OuterBundle *> getNegBundles() {
    return negBundles_;
...
private:
  // BUILDING
  const bool
  computeBarrelFlatPartRodCablingSide(const double rodPhi,
                                      const double phiSegmentWidth) const;

  const Category computeBundleType(const bool isBarrel,
                                   const std::string subDetectorName,
                                   const int layerDiskNumber,
                                   const int ringNumber = 0) const;
  void buildBundle(DetectorModule &m, std::map<int, OuterBundle *> &bundles,
                   std::map<int, OuterBundle *> &negBundles,
                   const Category &bundleType, const bool isBarrel,
                   ...
                   const int totalNumFlatRings = 0,
                   const bool isTiltedPart = false,
                   const bool isExtraFlatPart = false);
  const int computeBundleTypeIndex(const bool isBarrel,
                                   const Category &bundleType,
                                   const int totalNumFlatRings = 0,
                                   const bool isTilted = false,
                                   const bool isExtraFlatPart = false) const;
  const int computeBundleId(const bool isBarrel,
                            const bool isPositiveCablingSide,
                            const int layerDiskNumber, const int phiRef,
                            const int bundleTypeIndex) const;
  const int computeStereoBundleId(const bool isBarrel,
                                  const bool isPositiveCablingSide,
                                  const int layerDiskNumber, const int phiRef,
                                  const int bundleTypeIndex) const;
  ...
  // CHECKING
  void checkModulesToBundlesCabling(
      const std::map<int, OuterBundle *> &bundles) const;

  // ENDCAPS ONLY: ASSIGN THE MFB FANOUTS BRANCHES TO THE MODULES
  void connectEndcapModulesToBundlesFanoutBranches(
      std::map<int, OuterBundle *> &bundles);
  void checkEndcapModulesToBundlesFanoutBranchesCabling(
      const std::map<int, OuterBundle *> &bundles) const;
  ...
  int layerNumber_;
  int numRods_;
  int totalNumFlatRings_; // Total number of flat rings on both (+Z) side and
                          // (-Z) side
  double rodPhi_;
