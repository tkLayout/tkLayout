...
class OuterDTC : public PropertyObject,
                 public Buildable,
                 public Identifiable<int> {
  typedef std::vector<OuterCable *> Container;

public:
  OuterDTC(const std::string name, const double phiSectorWidth,
...
private:
  const int computeCMSSWId(const int oneCablingSideId,
                           const bool isPositiveCablingSide) const;
  const int computePlotColor(const int oneCablingSideId) const;
  const int computeOneCablingSideId(const int phiSectorRef,
                                    const Category &type, const int slot) const;
