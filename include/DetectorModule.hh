...
struct UniRef {
  ...
  int layer, ring, phi, side;
};

class DetectorModule
    : public Decorator<GeometricModule>,
      public ModuleBase,
      public DetIdentifiable { // implementors of the DetectorModuleInterface
                               // must take care of rotating the module based on
                               // which part of the subdetector it will be used
                               // in (Barrel, EC)
...
public:
  ...
  ReadonlyProperty<int, AutoDefault> numSparsifiedHeaderBits,
      numSparsifiedPayloadBits;
  ReadonlyProperty<int, AutoDefault> numTriggerDataHeaderBits,
      numTriggerDataPayloadBits;
  ...
  ReadonlyProperty<double, Computable> nominalResolutionLocalX,
      nominalResolutionLocalY;
  ...
  DetectorModule(Decorated *decorated, const std::string subdetectorName)
      : Decorator<GeometricModule>(decorated),
        ...
        reduceCombinatorialBackground("reduceCombinatorialBackground",
                                      parsedOnly(), false),
        ...
        subdetectorName_(subdetectorName), sensorNode("Sensor", parsedOnly()) {}
  ...
  double
  totalSensorsVolume() const { // Calculate total volume occupied by sensors
    double volume = 0.;
    for (const auto &s : sensors())
      volume += area() * s.sensorThickness();
    return volume;
  }
  ...
  const bool isAtPlusXSide() const {
    return (center().X() >= -insur::geom_zero);
  }
  ...
  double resolutionEquivalentZ(double hitRho, double trackR,
                               double trackCotgTheta, double resolutionLocalX,
                               double resolutionLocalY) const;
  double resolutionEquivalentRPhi(double hitRho, double trackR,
  ...
  accumulator_set<double, features<tag::count, tag::mean, tag::variance,
                                   tag::sum, tag::moment<2>>>
      rollingParametrizedResolutionLocalX;
  accumulator_set<double, features<tag::count, tag::mean, tag::variance,
                                   tag::sum, tag::moment<2>>>
      rollingParametrizedResolutionLocalY;

  void translate(const XYZVector &vector) {
    decorated().translate(vector);
    clearSensorPolys();
  }
  void mirror(const XYZVector &vector) {
    decorated().mirror(vector);
    clearSensorPolys();
  }
  void translateZ(double z) {
    decorated().translate(XYZVector(0, 0, z));
    clearSensorPolys();
  }
  ...
  void rotateX(double angle) {
    decorated().rotateX(angle);
    clearSensorPolys();
  }
  void rotateY(double angle) {
    decorated().rotateY(angle);
    clearSensorPolys();
  }
  void rotateZ(double angle) {
    decorated().rotateZ(angle);
    clearSensorPolys();
    rAxis_ = RotationZ(angle)(rAxis_);
  }
  void tilt(double angle) {
    rotateX(-angle);
    tiltAngle_ += angle;
  } // CUIDADO!!! tilt and skew can only be called BEFORE translating/rotating
    // the module, or they won't work as expected!!
  // void skew(double angle) { rotateY(-angle); skewAngle_ += angle; } // This
  // works for endcap modules only !! Skew is now defined at construction time
  // instead, before the module has had a chance to be translated/rotated!
  ...
  double maxZ() const {
    return maxget2(sensors_.begin(), sensors_.end(), &Sensor::maxZ);
  }
  double minZ() const {
    return minget2(sensors_.begin(), sensors_.end(), &Sensor::minZ);
  }
  double maxR() const {
    return maxget2(sensors_.begin(), sensors_.end(), &Sensor::maxR);
  }
  double minR() const {
    return minget2(sensors_.begin(), sensors_.end(), &Sensor::minR);
  }
  ...
  double planarMaxZ() const {
    return CoordinateOperations::computeMaxZ(basePoly());
  }
  double planarMinZ() const {
    return CoordinateOperations::computeMinZ(basePoly());
  }
  double planarMaxR() const {
    return CoordinateOperations::computeMaxR(basePoly());
  }
  double planarMinR() const {
    return CoordinateOperations::computeMinR(basePoly());
  }

  double phiAperture() const { return maxPhi() - minPhi(); }

  double maxEta() const {
    return MAX(basePoly().getVertex(0).Eta(), basePoly().getVertex(2).Eta());
  }
  double minEta() const {
    return MIN(basePoly().getVertex(0).Eta(), basePoly().getVertex(2).Eta());
  }
  double etaAperture() const { return maxEta() - minEta(); }
  double maxEtaWithError(double zError) const {
    return minMaxEtaWithError(zError).second;
  }
  double minEtaWithError(double zError) const {
    return minMaxEtaWithError(zError).first;
  }
  std::pair<double, double> minMaxEtaWithError(double zError) const;

  double maxTheta() const {
    return MAX(basePoly().getVertex(0).Theta(),
               basePoly().getVertex(2).Theta());
  }
  double minTheta() const {
    return MIN(basePoly().getVertex(0).Theta(),
               basePoly().getVertex(2).Theta());
  }
  double thetaAperture() const { return maxTheta() - minTheta(); }

  // Get local X orientation on sensor plane. Ie, for barrel modules, the
  // Lorentz drift orientation!! NB: This is not garanteed at all to match CMSSW
  // frame of reference orientation, which is independent.
  const TVector3 getLocalX() const {
    XYZVector localX = basePoly().getVertex(3) - basePoly().getVertex(0);
    if (flipped()) {
      localX *= -1;
    } // a flip operation does not move the polygon vertexes, hence desserves
      // special treatment.
    if (fabs(tiltAngle() - M_PI / 2.) < insur::geom_zero || !isPixelModule()) {
      localX *= -1;
    }
    return CoordinateOperations::convertCoordVectorToTVector3(localX).Unit();
  }
  // Get local Y orientation on sensor plane.
  // NB: This is not garanteed at all to match CMSSW frame of reference
  // orientation, which is independent.
  ...
  ElementsVector &getLocalElements() const {
    return materialObject_.getLocalElements();
  }
  int maxSegments() const {
    int segm = 0;
    for (const auto &s : sensors()) {
      segm = MAX(segm, s.numSegmentsEstimate());
    }
    return segm;
  } // CUIDADO NEEDS OPTIMIZATION (i.e. caching or just MAX())
  int minSegments() const {
    int segm = std::numeric_limits<int>::max();
    for (const auto &s : sensors()) {
      segm = MIN(segm, s.numSegmentsEstimate());
    }
    return segm;
  }
  ...
  int maxChannels() const {
    int max = 0;
    for (const auto &s : sensors()) {
      max = MAX(max, s.numChannels());
    }
    return max;
  }
  int minChannels() const {
    int min = std::numeric_limits<int>::max();
    for (const auto &s : sensors()) {
      min = MIN(min, s.numChannels());
    }
    return min;
  }
  ...
  int numStripsAcrossEstimate() const {
    return sensors().front().numStripsAcrossEstimate();
  } // CUIDADO this assumes both sensors have the same number of sensing
    // elements in the transversal direction - typically it is like that
  double pitch() const { return sensors().front().pitch(); }
  int numSegmentsEstimate() const {
    return sensors().front().numSegmentsEstimate();
  } // CUIDADO this assumes both sensors have the same number of sensing
    // elements in the transversal direction - typically it is like that
  double stripLength() const { return sensors().front().stripLength(); }
  double sensorThickness() const {
    return sensors().front().sensorThickness();
  } // CUIDADO this has to be fixed (called in Extractor.cc), sensor thickness
    // can be different for different sensors
  ...
  inline bool isPixelModule() const {
    return (moduleType().find(insur::type_pixel) != std::string::npos);
  }
  inline bool is3DPixelModule() const {
    return (isPixelModule() &&
            moduleType().find(insur::type_3D) != std::string::npos);
  }
  inline bool isTimingModule() const {
    return (moduleType().find(insur::type_timing) != std::string::npos);
  }

  bool couldHit(const XYZVector &direction, double zError) const;
  double trackCross(const XYZVector &PL, const XYZVector &PU) {
    return decorated().trackCross(PL, PU);
  }
  std::pair<XYZVector, HitType> checkTrackHits(const XYZVector &trackOrig,
  ...
  // OT CABLING
  const int
  isPositiveCablingSide() const; // Can be different from (Z) end.
                                 // 'Positive cabling side': the module end up
                                 // connected on a DTC on (Z+) end.
  ...
  void setEndcapFiberFanoutBranch(const int branchIndex) {
    endcapFiberFanoutBranch_ = branchIndex;
  } // Branch of the MFB fanout
  const int getEndcapFiberFanoutBranch() const {
    return endcapFiberFanoutBranch_;
  }
  ...
  void setPhiRefInPowerChain(int phiRefInPowerChain) {
    phiRefInPowerChain_ = phiRefInPowerChain;
  }
...
protected:
  ...
  calculateParameterizedResolutionLocalX(const TVector3 &trackDirection) const;
  virtual const double
  calculateParameterizedResolutionLocalY(const TVector3 &trackDirection) const;
  const double
  calculateParameterizedResolutionLocalAxis(const double fabsTanDeepAngle,
                                            const bool isLocalXAxis) const;
...
private:
  ...
  int numELinks_;
  GBT *GBT_ = nullptr;
  // The raw pointers are intended. DetectorModule is NOT owning the cabling
  // resources. All cabling ressources are owned by the CablingMap, which is a
  // member variable of Tracker. They get destructed when Tracker is destructed.
};
...
public:
  ...
  void setup() override {
    ...
    minPhi.setup([&]() {
      ...
      //
      //              x (inter. point)
      // --> problem if absolute difference in phi betwwen barrel corners higher
      // than phi
      if (!(fabs(basePoly().getVertex(0).Phi() -
                 basePoly().getVertex(2).Phi()) >= M_PI)) {

        min = MIN(basePoly().getVertex(0).Phi(), basePoly().getVertex(2).Phi());
      }
      // Module overlaps the crossline between -pi/2 & +pi/2 -> rotate by 180deg
      // to calculate min
      else {
        ...
        // Shift by extra 180deg to get back to its original position (i.e.
        // +2*pi with respect to the nominal position)
        min += M_PI;
      }
      // Return value in interval <-pi;+3*pi> instead of <-pi;+pi> to take into
      // account the crossline at pi/2.
      return min;
    });
    maxPhi.setup([&]() {
      double max = 0;
    ...
    nominalResolutionLocalX.setup([this]() {
      ...
      // std::cout <<  "hasAnyResolutionLocalXParam() = " <<
      // hasAnyResolutionLocalXParam() << std::endl;
      ...
      if (!hasAnyResolutionLocalXParam()) {
        ...
        for (const Sensor &s : sensors())
          res += pow(meanWidth() / s.numStripsAcross() / sqrt(12), 2);
    ...
    nominalResolutionLocalY.setup([this]() {
      ...
      if (!hasAnyResolutionLocalYParam()) {
        // std::cout << "resolutionLocalY and resolutionLocalYBarrel parameters
        // are all unset. Use of default formulae." << std::endl;
        if (stereoRotation() != 0.)
          return nominalResolutionLocalX() / sin(stereoRotation());
        else {
          return length() / maxSegments() /
                 sqrt(12); // NOTE: not combining measurements from both
                           // sensors. The two sensors are closer than the
                           // length of the longer sensing element, making the 2
                           // measurements correlated. considering only the best
                           // measurement is then a reasonable approximation
                           // (since in case of a PS module the strip
                           // measurement increases the precision by only 0.2%
                           // and in case of a 2S the sensors are so close that
                           // they basically always measure the same thing)
        }
      }
      // if model parameters specified, return -1
      else
  ...
  // double maxZ() const { return MAX(basePoly().getVertex(0).Z(),
  // basePoly().getVertex(2).Z()); } double minZ() const { return
  // MIN(basePoly().getVertex(0).Z(), basePoly().getVertex(2).Z()); } double
  // maxR() const { return MAX(basePoly().getVertex(0).Rho(),
  // basePoly().getVertex(2).Rho()); } double minR() const { return
  // center().Rho(); }//MIN(basePoly().getVertex(0).Rho(),
  // basePoly().getVertex(2).Rho()); }

  virtual ModuleSubdetector subdet() const { return BARREL; }

  PosRef posRef() const {
    return (PosRef){subdetectorId(), (side() > 0 ? ring() : -ring()), layer(),
                    rod()};
  }
  TableRef tableRef() const {
    return (TableRef){subdetectorName(), layer(), ring()};
  }
  UniRef uniRef() const {
    return UniRef{subdetectorName(), layer(), ring(), rod(), side()};
  }
};
...
public:
  ...
  int16_t blade() const {
    return (int16_t)myid();
  } // CUIDADO Think of a better name!
  int16_t side() const { return (int16_t)signum(center().Z()); }
  Property<int, AutoDefault> endcapDiskSurface;
  void setIsSmallerAbsZModuleInRing(const bool isSmallerAbsZModuleInRing) {
    isSmallerAbsZModuleInRing_ = isSmallerAbsZModuleInRing;
  }
  const bool isSmallerAbsZModuleInRing() const override {
    return isSmallerAbsZModuleInRing_;
  }
  const int diskSurface() const override { return endcapDiskSurface(); }
  const bool isAtSmallerAbsZSideInDee() const {
    return (femod(diskSurface(), 2) == 1);
  }
  const bool isAtSmallerAbsZDeeInDoubleDisk() const {
    return (diskSurface() <= 2);
  }
  ...
  void setup() override {
    ...
    minPhi.setup([&]() {
      ...
      // to calculate min
        ...
        if (polygon.getVertex(1).Phi() < 0 || polygon.getVertex(2).Phi() < 0) {
          ...
          logERROR("Endcap module min calculation failed - algorithm problem. "
                   "Check algorithm!");
      }
      // Return value in interval <-pi;+3*pi> instead of <-pi;+pi> to take into
      // account the crossline at pi/2.
    ...
    maxPhi.setup([&]() {
      ...
      // to calculate max.
        ...
        if (polygon.getVertex(1).Phi() < 0 || polygon.getVertex(2).Phi() < 0) {
          ...
          logERROR("Endcap module max calculation failed - algorithm problem. "
                   "Check algorithm!");
      }
      // Return value in interval <-pi;+3*pi> instead of <-pi;+pi> to take into
      // account the crossline at pi/2.
    ...
    nominalResolutionLocalX.setup([this]() {
      ...
      // hasAnyResolutionLocalXParam() << std::endl;
      if (!hasAnyResolutionLocalXParam()) {
        // std::cout << "nominalResolutionLocalX and resolutionLocalXEndcap
        // parameters are all unset. Use of default formulae." << std::endl;
        double res = 0;
        for (const Sensor &s : sensors())
          res += pow(meanWidth() / s.numStripsAcross() / sqrt(12), 2);
    ...
    nominalResolutionLocalY.setup([this]() {
      // only set up this if no model parameters not specified
      if (!hasAnyResolutionLocalYParam()) {
        // std::cout << "resolutionLocalY and resolutionLocalYEndcap parameters
        // are all unset. Use of default formulae." << std::endl;
        if (stereoRotation() != 0.)
          return nominalResolutionLocalX() / sin(stereoRotation());
        else {
  ...
  // double minZ() const { return center().Z(); } // CUIDADO not accounting for
  // sensor placement double maxZ() const { return center().Z(); } // ditto here
  // double maxR() const { return MAX(basePoly().getVertex(0).Rho(),
  // basePoly().getVertex(2).Rho()); } double minR() const { XYZVector side[2];
  //                      std::partial_sort_copy(basePoly().begin(),
  //                      basePoly().end(), std::begin(side), std::end(side),
  //                      [](const XYZVector& v1, const XYZVector& v2) { return
  //                      v1.Rho() < v2.Rho(); }); return
  //                      ((side[0]+side[1])/2).Rho(); }

  virtual ModuleSubdetector subdet() const { return ENDCAP; }

  PosRef posRef() const {
    return (PosRef){subdetectorId(), (side() > 0 ? disk() : -disk()), ring(),
                    blade()};
  }
  TableRef tableRef() const {
    return (TableRef){subdetectorName(), disk(), ring()};
  }
  UniRef uniRef() const {
    return UniRef{subdetectorName(), disk(), ring(), blade(), side()};
  }
