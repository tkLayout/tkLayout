...
public:
  ...
  void setup() {
    minZ.setup([this]() { double min = DBL_MAX; for (const Ring& r : rings_) { min = MIN(min, r.minZ()); } return min; });
    maxZ.setup([this]() { double max = -DBL_MAX; for (const Ring& r : rings_) { max = MAX(max, r.maxZ()); } return max; }); //TODO: Make this value nicer
    minR.setup([this]() { double min = DBL_MAX; for (const Ring& r : rings_) { min = MIN(min, r.minR()); } return min; });
