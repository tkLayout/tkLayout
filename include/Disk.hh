...
class Disk : public PropertyObject,
             public Buildable,
...
private:
  ...
  inline const double getRingInfo(const vector<double> &ringsInfo,
                                  int ringNumber) const;

  std::pair<double, double>
  computeStringentZ(int i, int parity, const ScanEndcapInfo &extremaDisksInfo);
  double computeNextRho(const int parity, const double zError,
...
public:
  ...
  void setup() {
    ...
    maxRingThickness.setup([this]() {
      double max = 0;
      for (const Ring &r : rings_) {
        max = MAX(max, r.thickness());
      }
      return max;
    });
    totalModules.setup([this]() {
      int cnt = 0;
      for (const Ring &r : rings_) {
        cnt += r.numModules();
      }
  ...
  const double deesCentersHalfZDistance() const {
    ...
    if (!rings_.empty()) {
      ...
      deesCentersHalfZDistance =
          (isRingOn4Dees ? firstRing.smallDelta() : bigDelta());
    } else {
      logERROR("Requested spacing between dees in double-disk, but the "
               "double-disk has no ring!");
    }
    return deesCentersHalfZDistance;
  }
  // Z of the 2 dees located at lowest |Z| within the double-disk.
  const double smallAbsZDeeCenterZ() const {
    return (centerZ_ - deesCentersHalfZDistance());
  }
  // Z of the 2 dees located at biggest |Z| within the double-disk.
  const double bigAbsZDeeCenterZ() const {
    return (centerZ_ + deesCentersHalfZDistance());
  }
  ...
  int numEmptyRings() const {
    return count_if(rings_.begin(), rings_.end(),
                    [](const Ring &r) { return r.numModules() == 0; });
  }

  const std::map<int, std::vector<const Module *>> getSurfaceModules() const;

  const std::pair<double, bool>
  computeIntersectionWithZAxis(double lastZ, double lastRho, double newZ,
                               double newRho) const;
