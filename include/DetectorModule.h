...
////////
  ...
  int minSegments() const { int segm = DBL_MAX; for (const auto& s : sensors()) { segm = MIN(segm, s.numSegments()); } return segm; }
  ...
  int minChannels() const { int min = DBL_MAX; for (const auto& s : sensors()) { min = MIN(min, s.numChannels()); } return min; } 
