...
class Endcap : public PropertyObject,
               public Buildable,
               public Identifiable<std::string>,
               public Visitable {
private:
...
public:
  Endcap()
      : barrelGap("barrelGap", parsedOnly()), diskNode("Disk", parsedOnly()),
        supportNode("Support", parsedOnly()),
        numDisks("numDisks", parsedAndChecked()), innerZ("minZ", parsedOnly()),
        outerZ("maxZ", parsedAndChecked()),
        distanceFromEndcapsModulesMaxZtoRoutedMaterial(
            "distanceFromEndcapsModulesMaxZtoRoutedMaterial", parsedOnly(),
            10.0), // Distance in Z
                   // between maxZwithHybrids() and minZ of cabling materials
                   // which are routed FROM OTHER SUBDETECTORS.
        distanceFromEndcapsModulesMaxRtoRoutedMaterial(
            "distanceFromEndcapsModulesMaxRtoRoutedMaterial", parsedOnly(),
            16.0), // Radial distance
  ...
  void setup() {
    ...
    minZwithHybrids.setup([&]() {
      ...
      for (const auto &d : disks_) {
        if (d.minZ() > 0)
          min = MIN(min, d.minZwithHybrids());
      }
      if (innerZ.state()) {
        min = MIN(min, innerZ());
      } // if minZ (or barrelGap) is specified in cfg file, this should be taken
