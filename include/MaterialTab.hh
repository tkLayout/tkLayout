...
public:
  ...
  const int getMaterialsTreeHierarchyLevel() const {
    return materialsTreeHierarchyLevel_;
  }
...
public:
  ChemicalElement(const double density, const int atomicNumber,
...
private:
  // TO DO: should probably set more accurate RL and IL values directly in the
  // cfg file?
  const double computeRadiationLength(const int atomicNumber,
                                      const double atomicWeight);
...
typedef std::map<std::string, ChemicalBase>
    ChemicalBaseMap; // to do: define ChemicalBase directly inside
                     // MaterialsTable to avoid this?
...
public:
  ChemicalMixture(const double density, const ChemicalFormula &formula,
                  const ChemicalElementMap &allChemicalElements);
  ChemicalMixture(const double density, const MassComposition &fractions,
...
private:
  // TO DO: should probably place more accurate RL and IL values directly in the
  // file?
  const MassComposition
  computeMassComposition(const ChemicalFormula &formula,
                         const ChemicalElementMap &allChemicalElements) const;
  const MassComposition
  computeMassComposition(const MassComposition &fractionsFromCfg) const;
  ...
  const int computeMaterialsTreeHierarchyLevel(
      const MassComposition &fractions,
      const ChemicalBaseMap &alreadyDefinedMaterials) const;

  const std::pair<double, double> computeRadiationAndInteractionLengths(
      const MassComposition &fractions,
      const ChemicalBaseMap &alreadyDefinedMaterials) const;
...
public:
  ...
  double getDensity(const std::string materialName)
      const; // TO DO: should return const double!!!
  ...
  const ChemicalElementMap getAllChemicalElements() const {
    return this->first;
  }
  const ChemicalMixtureMap getAllChemicalMixtures() const {
    return this->second;
  }
};

typedef std::map<std::string, std::tuple<double, double, double>>
    MaterialTabType;
