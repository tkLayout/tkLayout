...
template <class T> class Decorator {
  T *const
      decorated_; // the decorated type must be cloneable (i.e. implement a
                  // clone method to copy-construct a new instance on the heap)
protected:
  Decorator(T *const decorated) : decorated_(decorated) {}

public:
  typedef T Decorated;
  Decorator(const Decorator<T> &other)
      : decorated_(((T *)other.decorated_->clone())) {}
  ...
  template <class U> bool is() const {
    return typeid(*this) == typeid(U) || this->decorated().template is<U>();
  }
  ...
  template <class U> const U *as() const {
    return typeid(*this) == typeid(U) ? static_cast<U *const>(this)
                                      : this->decorated().template as<U>();
  }
...
public:
  ...
  bool isAll(std::set<std::type_index> &types) const {
    return types.size() == 1 && types.count(typeid(*this));
  }
  template <class U> const U *as() const {
    return typeid(*this) == typeid(U) ? reinterpret_cast<U *const>(this) : NULL;
  }
  template <class U> U *as() {
    return typeid(*this) == typeid(U) ? reinterpret_cast<U *const>(this) : NULL;
  }
