...
public:
  ...
  //! Check whether magnetic field const. or defined as a function -> for now
  //! assumed const.
  ...
  const IrradiationMapsManager &irradiationMapsManager() const {
    return irradiationMapsManager_;
  }

  void accept(GeometryVisitor &v) { v.visit(*this); }
  void accept(ConstGeometryVisitor &v) const { v.visit(*this); }

  double particleCurvatureR(double pt) const {
    return pt / (0.3 * magField()) * 1e3;
  }

private:
  //! Singleton private constructor -> initialize all variables to be read-out
  //! from configuration file & define if checker should be called after parsing
  SimParms();
