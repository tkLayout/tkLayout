...
template <class T, int D> class SparseMatrix {
...
template <class T> class SparseMatrix<T, 1> {
  std::map<int, T> elem_;
...
struct HitsP { // holder struct for TTree export
  ...
  HitsP()
      : glox(0), gloy(0), gloz(0), locx(0), locy(0), pterr(0), hitprob(0),
        deltas(0), cnt(0), z(0), rho(0), phi(0) {}
};
...
struct SmartBin {
  ...
  int8_t stacked; // how many events have been stacked on top of each other
                  // public:
  SmartBin(int count_ = 0, uint8_t eventid_ = 0, uint16_t hitmask_ = 0,
           uint8_t stacked_ = -1)
      : count(count_), eventid(eventid_), hitmask(hitmask_), stacked(stacked_) {
  }
...
class HoughTrack {
  ...
  void processHit(int evid, int hitid, double x, double y, double z, double pt,
                  double ptError, double yres);
...
public: //     invPt,phi0,  z0, theta
  HoughTrack()
      : histo_(seq<4>(1000)(1000)(100)(1000), seq<4>(-0.5)(-3.14)(-70.5)(0.),
