...
class TiltedRing : public PropertyObject,
                   public Buildable,
...
private:
  ...
  double thetaOuterUP_, thetaOuterDOWN_, thetaOuter_, tiltAngleIdealOuter_,
      deltaTiltIdealOuter_;
...
public:
  ...
  TiltedRing()
      : // materialObject_(MaterialObject::ROD),
        ...
        zInner("ringInnerZ", parsedOnly()), zOuter("ringOuterZ", parsedOnly()),
  ...
  double gapR() const {
    return (outerRadius() - innerRadius()) / sin(theta_g() * M_PI / 180.);
  }
  ...
  double thetaEndOuter_REAL() const {
    return atan(rEndOuter_REAL_ / zEndOuter_REAL_);
  }
  ...
  double thetaEndInner_REAL() const {
    return atan(rEndInner_REAL_ / zEndInner_REAL_);
  }
};
...
class Ring : public PropertyObject,
  ...
  double computeTentativePhiAperture(double moduleWaferDiameter,
                                     double minRadius);
  std::pair<double, int>
  computeOptimalRingParametersWedge(double moduleWaferDiameter,
                                    double minRadius);
  std::pair<double, int>
  computeOptimalRingParametersRectangle(double moduleWidth, double highRadius);
...
public:
  ...
  Property<int, NoDefault> numModules; // if set forces the number of modules
                                       // (in phi) to be exactly numModules
  ...
  void setup() {
    ...
    minZwithHybrids.setup([this]() {
      double min = std::numeric_limits<double>::max();
      for (const auto &m : modules_)
        min = MIN(min, m.minZwithHybrids());
      return min;
    });
    maxZwithHybrids.setup([this]() {
      double max = 0;
      for (const auto &m : modules_)
        max = MAX(max, m.maxZwithHybrids());
      return max;
    });
    minRwithHybrids.setup([this]() {
      double min = std::numeric_limits<double>::max();
      for (const auto &m : modules_)
        min = MIN(min, m.minRwithHybrids());
      return min;
    });
    maxRwithHybrids.setup([this]() {
      double max = 0;
      for (const auto &m : modules_)
        max = MAX(max, m.maxRwithHybrids());
      return max;
    });
  }
  ...
  double averageZ() const {
    double averageZ = 0;
    for (const auto &m : modules_) {
      averageZ = averageZ + m.center().Z();
    }
    averageZ /= numModules();
    return averageZ;
  }

  void setIsSmallerAbsZRingInDisk(const bool isSmallerAbsZRingInDisk) {
    isSmallerAbsZRingInDisk_ = isSmallerAbsZRingInDisk;
  }
  const bool isSmallerAbsZRingInDisk() const {
    return isSmallerAbsZRingInDisk_;
  }
  void setIsRingOn4Dees(const bool isRingOn4Dees) {
    isRingOn4Dees_ = isRingOn4Dees;
  }
  ...
  void removeModules() {
    modules_.erase_if([](DetectorModule &m) { return (m.removeModule()); });
