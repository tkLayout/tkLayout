...
typedef ROOT::Math::DisplacementVector2D<ROOT::Math::Cartesian2D<double>,
    ...
    XYVector; // CUIDADO The version of ROOT tkLayout is linked with misses this
              // typedef
...
struct Tracks { // holder struct for TTree export
  ...
  void push_back(int eventn_, int trackn_, double eta_, double phi0_,
                 double z0_, double pt_, int8_t nhits_) {
...
struct Hits { // holder struct for TTree export
  ...
  void push_back(double glox_, double gloy_, double gloz_, double locx_,
                 double locy_, float pterr_, float hitprob_, float deltas_,
                 int8_t cnt_, int8_t z_, int8_t rho_,
                 int8_t phi_ /*, double distx_, double disty_, double eta_*/) {
...
template <class T> class UniformValue : public Value<T> {
  TRandom &die_;
  T min_, max_;
...
template <class T> class BinaryValue : public Value<T> {
  TRandom &die_;
  T value0_, value1_;

public:
  BinaryValue(TRandom &die, T value0, T value1)
      : die_(die), value0_(value0), value1_(value1) {}
  T get() { return die_.Integer(2) ? value1_ : value0_; }
  std::string toString() const {
    return any2str(value0_) + "," + any2str(value1_);
  }
};
...
std::auto_ptr<Value<T>> valueFromString(TRandom &die, const std::string &str) {
  ...
  if (str.find(":") != std::string::npos && values.size() == 2)
    return std::auto_ptr<Value<T>>(
        new UniformValue<T>(die, str2any<T>(values[0]), str2any<T>(values[1])));
  else if (str.find(",") != std::string::npos && values.size() == 2)
    return std::auto_ptr<Value<T>>(
        new BinaryValue<T>(die, str2any<T>(values[0]), str2any<T>(values[1])));
  else
    return std::auto_ptr<Value<T>>(new ConstValue<T>(str2any<T>(values[0])));
}
...
class TrackShooter {
  ...
  typedef std::list<BarrelModule *>
      BarrelModules; // accessed sequentially. last step in hit localization
  typedef std::list<EndcapModule *> EndcapModules;
  typedef std::vector<BarrelModules>
      BarrelOctants; // these vectors are accessed randomly when the octant of
                     // the hit is known (function of the radius/z discovered in
                     // the previous step)
  typedef std::vector<EndcapModules> EndcapOctants;
  typedef std::map<double, BarrelOctants>
      BarrelRadii; // first step in hit localization. we fix radius/z and we
                   // calculate the rest of the coordinates of a hit
  typedef std::map<double, EndcapOctants> EndcapZs;
  BarrelRadii barrelModsByRadius_;
  EndcapZs endcapModsByZ_;

  TRandom3 die_;
  std::auto_ptr<Value<double>> eta_, phi0_, z0_, pt_,
      invPt_; // auto_ptr because I don't want to be bothered with deletion
  ...
  int detectCollisionSlanted(const Helix &helix, const Polygon3d<4> &poly,
                             std::vector<XYZVector> &collisions);
  int detectCollisionBarrel(const Helix &helix, const Polygon3d<4> &poly,
                            std::vector<XYZVector> &collisions);
  int detectCollisionEndcap(const Helix &helix, const Polygon3d<4> &poly,
                            std::vector<XYZVector> &collisions);

  XYVector convertToLocalCoords(const XYZVector &globalHit,
...
public:
  TrackShooter()
      : trackerMaxRho_(std::numeric_limits<double>::max()), barrelMinZ_(0.),
        barrelMaxZ_(0.) {
    setDefaultParameters();
  }
  void setOutput(std::ostream &output, const char *fieldSeparator = "\t",
                 const char *lineSeparator = "\n", bool synced = false);
  void setTrackerBoundaries(
      double trackerMaxRho, double barrelMinZ,
      double barrelMaxZ); // if not set the tracker is considered infinite in
                          // the rho direction and no particle can escape
                          // without ever curving back
