...
// ========================================================================================
// Here be STATISTICS
// If possible, additional statistics should be local to the plot drawer
// instantiation
// ========================================================================================
...
// If possible, additional value getters should be local to the plot drawer
// instantiation
// ==============================================================================================

template <class RetType, RetType (Module::*ModuleMethod)() const>
struct Method {
  double operator()(const Module &m) const {
    return (double)(m.*ModuleMethod)();
  }
};
...
struct TypeAutoColor { // Auto-assign colors
  ...
  double operator()(const Module &m) {
    std::pair<std::set<std::string>::iterator, bool> it =
        colorSet_.insert(m.moduleType());
    return Palette::color(std::distance(colorSet_.begin(), it.first) + 1);
  }
};

struct Type { // Module-maintained color
  double operator()(const Module &m) { return Palette::color(m.plotColor()); }
};

// OT CABLING
struct TypeFanoutBranchTransparentColor { // Module-maintained Bundle fanout
                                          // branch color
  double operator()(const Module &m) {
...
struct TypePowerChannelColor { // Module-maintained channel color
  double operator()(const Module &m) {
    ...
    return Palette::colorChannel(m.powerChannelSectionPlotColor(),
                                 isTransparentActivated);
  }
};
...
// =============================================================================================
// Here be DRAWSTYLES
// If possible, additional draw styles should be local to the plot drawer
// instantiation
// =============================================================================================
...
struct FillStyle {
  ...
  void operator()(TPolyLine &line, StatType &bin,
                  const DrawerPalette &palette) const {
...
// ===============================================================================================
// Here be VALIDATORS
// If possible, additional module validators should be local to the plot drawer
// instantiation
// ===============================================================================================

template <const int SubdetType> struct CheckType {
  bool operator()(const Module &m) const { return m.subdet() & SubdetType; }
};

template <const int PhiIndex> struct CheckPhiIndex {
...
struct XY : public std::pair<int, int>, private Rounder {
  ...
  XY(const Module &m)
      : std::pair<int, int>(round(m.center().X()), round(m.center().Y())),
        valid(m.center().Z() >= 0) {}
  // XY coordinates of vector v.
  XY(const XYZVector &v)
      : std::pair<int, int>(round(v.X()), round(v.Y())), valid(v.Z() >= 0) {}
  // XY coordinates of vector v, in the (XY) plane passing by the center of
  // module m.
  XY(const XYZVector &v, const Module &m) : XY(v) {}
  // bool operator<(const XY& other) const { return (x() < other.x()) || (x() ==
  // other.x() && y() < other.y()); }
...
struct XYRotateY180 : public std::pair<int, int>, private Rounder {
  const bool valid;
  // XY coordinates of the centre of module m.
  XYRotateY180(const Module &m)
      : std::pair<int, int>(round(-m.center().X()), round(m.center().Y())),
        valid(m.center().Z() >= 0) {}
  // XY coordinates of vector v.
  XYRotateY180(const XYZVector &v)
      : std::pair<int, int>(round(-v.X()), round(v.Y())), valid(v.Z() >= 0) {}
  // XY coordinates of vector v, in the (XY) plane passing by the center of
  // module m.
...
struct XYNeg : public std::pair<int, int>, private Rounder {
  const bool valid;
  // XY coordinates of the centre of module m.
  XYNeg(const Module &m)
      : std::pair<int, int>(round(m.center().X()), round(m.center().Y())),
        valid(m.center().Z() <= 0) {}
  // XY coordinates of vector v.
  XYNeg(const XYZVector &v)
      : std::pair<int, int>(round(v.X()), round(v.Y())), valid(v.Z() <= 0) {}
  // XY coordinates of vector v, in the (XY) plane passing by the center of
  // module m.
...
struct XYNegRotateY180 : public std::pair<int, int>, private Rounder {
  const bool valid;
  // XY coordinates of the centre of module m.
  XYNegRotateY180(const Module &m)
      : std::pair<int, int>(round(-m.center().X()), round(m.center().Y())),
        valid(m.center().Z() <= 0) {}
  // XY coordinates of vector v.
  XYNegRotateY180(const XYZVector &v)
      : std::pair<int, int>(round(-v.X()), round(v.Y())), valid(v.Z() <= 0) {}
  // XY coordinates of vector v, in the (XY) plane passing by the center of
  // module m.
...
struct YZ : public std::pair<int, int>, private Rounder {
  const bool valid;
  // RZ coordinates of the centre of module m, in the plane (RZ) defined by ((Z
  // axis), moduleCenter).
  YZ(const Module &m)
      : std::pair<int, int>(round(m.center().Z()), round(m.center().Rho())),
        valid(m.center().Z() >= 0) {}
  // RZ coordinates of vector v, in the plane (RZ) defined by ((Z axis), v).
  YZ(const XYZVector &v)
      : std::pair<int, int>(round(v.Z()), round(v.Rho())), valid(v.Z() >= 0) {}
  // RZ coordinates of vector v, in the plane (RZ) defined by ((Z axis),
  // moduleCenter).
  YZ(const XYZVector &v, const Module &m) : valid(v.Z() >= 0) {
    this->first = round(v.Z());

    // This calculates the projection of vector v into plane ((Z axis),
    // moduleCenter).
    XYZVector vProjected;
    XYZVector z(0., 0., 1.);

    XYZVector basePolyCenter = m.basePoly().getCenter();
    XYZVector normal = crossProduct(
        z, basePolyCenter); // normal of plane ((Z axis), moduleCenter).
    normal = normal.Unit(); // normalize.

    vProjected =
        v - v.Dot(normal) * normal; // Calculate projected vector. TO DO :
                                    // Introduce this as a method !!
    this->second =
        round(vProjected.Rho()); // Take the Rho() of the projected vector.
  }
  //  bool operator<(const YZ& other) const { return (y() < other.y()) || (y()
  //  == other.y() && z() < other.z()); }
  int y() const { return this->second; }
...
struct ZPhi : public std::pair<double, double>, private Rounder {
  ...
  ZPhi(const Module &m)
      : std::pair<double, double>(
            m.center().Z() * Rounder::mmFraction,
            (femod(m.center().Phi() + M_PI / 2., 2. * M_PI) - M_PI / 2.) *
                Rounder::mmFraction),
        valid(true) {}
  // ZPhi coordinates of vector v.
  ZPhi(const XYZVector &v)
      : std::pair<double, double>(
            v.Z() * Rounder::mmFraction,
            (femod(v.Phi() + M_PI / 2., 2. * M_PI) - M_PI / 2.) *
                Rounder::mmFraction),
        valid(true) {}
  // ZPhi coordinates of vector v, in the (ZPhi) plane passing by the center of
  // module m.
  ZPhi(const XYZVector &v, const Module &m) : valid(true) {
    this->first = v.Z() * Rounder::mmFraction;

    const double centerPhi =
        femod(m.center().Phi() + M_PI / 2., 2. * M_PI) - M_PI / 2.;
    this->second = (centerPhi + moduloDiff(v.Phi(), m.center().Phi(), M_PI)) *
                   Rounder::mmFraction;
    // It is not possible to consider only v.Phi(), and ignore m.center().Phi().
    // Indeed, the module corners must not be drawned at +-Pi, despite the fact
    // that we work modulo Pi. As a result, the phi of the drawn corners needs
    // to take into account where the module center is.
  }
...
public:
  ...
  LineGetter()
      : maxx_(std::numeric_limits<CoordTypeX>::min()),
        minx_(std::numeric_limits<CoordTypeX>::max()),
        maxy_(std::numeric_limits<CoordTypeY>::min()),
        miny_(std::numeric_limits<CoordTypeY>::max()) {}
  ...
  TPolyLine *operator()(const Module &m) {
    if (!isContour) {
      ...
      if (j == 4) { // close the poly line in case it's made of 4 distinct
                    // points, to get the closing line drawn
        x[j] = x[0];
        y[j++] = y[0];
      }
      ...
      for (int i = 0; i < contourSize; i++) {
        ...
        XYZVector contourGlobal =
            ex * contourLocal.X() + ey * contourLocal.Y() + center;
...
template <class CoordType> struct HistogramFrameStyle {
  void operator()(TH2C &frame, TCanvas &canvas, DrawerPalette &palette) const {
    ...
    canvas.Update();
    palette.setFramePalette(
        (TPaletteAxis *)frame.GetListOfFunctions()->FindObject("palette"));
...
/// PlotDrawer is the class drawing module plots on user-supplied canvas. Alas,
/// not visitor-enabled, since it was written before the new visitable geometry
/// was conceived Usage: 1) Construct a PlotDrawer object: PlotDrawer<CoordType,
/// ValueGetterType, StatType> drawer(viewportMaxX, viewportMaxY, valueGetter);
///    - CoordType is the coordinate class: XY, YZ or YZFull (for Z- and Z+
///    sections together)
///    - ValueGetterType obtains a value from modules to decide their color. Any
///    functor or lambda taking a Module& and returning a double can be used
///    here.
///      Some ValueGetters are pre-defined. In case of user-defined
///      ValueGetters, if possible use lambda or classes local to the
///      instantiation of your PlotDrawer to avoid polluting this header with
///      additional declarations
///    - StatType is the type of statistic to do on the module values obtained
///    with the ValueGetters, in case two modules occupy the same map bin.
///      Default is NoStat, where values overwrite each other and the last one
///      counts. Average, Max, Min, Sum are also available and custom statistics
///      can be defined by the user.
///    - viewportMaxX, viewportMaxY specify the size of the viewport on the
///    canvas. If set to 0, the viewport is automatically calculated to include
///    all the modules. Default is 0 for both.
///    - valueGetter is an instance of a ValueGetterType that can be used by the
///    user to pass a custom valueGetter. Default is valueGetter().
/// 2) Add modules to the internal maps, using either:
///    - void addModulesType(begin, end, moduleTypes); where the last argument
///    moduleTypes can be the constants BARREL, ENDCAP or BARREL | ENDCAP, to
///    restrict to one subdetector or both
///    - void addModules<ModuleValidator>(begin, end, isValid);  where the last
///    argument isValid is of ModuleValidator type.
///      A ModuleValidator is a lambda or functor taking a const Module& and
///      returning a bool, used to decide whether a module should be included or
///      not in the plot
/// 3) Draw the plot frame: void drawFrame<FrameStyleType>(canvas, frameStyle)
///   - FrameStyleType is the type of frame to draw. The predefined classes are
///   SummaryFrameStyle (which draws eta lines) or HistogramFrameStyle (which
///   draws the legend colour bar)
///   - canvas is the TCanvas to draw on. cd() is called automatically by the
///   PlotDrawer
///   - frameStyle is the instance of a FrameStyleType class, which can be used
///   in case of custom frame styles. Default is FrameStyleType<CoordType>()
/// 4) Draw the modules: void drawModules<DrawStyleType>(canvas, drawStyle)
///   - DrawStyleType is the style of module drawing. The predefined classes are
///   ContourStyle (which only draws the contours of modules) and FillStyle
///   (which draws solid modules).
///   - canvas is the TCanvas to draw on. cd() is called automatically by the
///   PlotDrawer
///   - drawStyle is the instance of a DrawStyleType class, which can be used in
///   case of custom draw styles. Default is DrawStyleType<CoordType>()
/// 5) Draw the modules with outer contour: void
/// drawModuleContours<DrawStyleType>(canvas, drawStyle)
///   - DrawStyleType is the style of module drawing. The predefined classes are
///   ContourStyle (which only draws the contours of modules) and FillStyle
///   (which draws solid modules).
///   - canvas is the TCanvas to draw on. cd() is called automatically by the
...
public:
  PlotDrawer(CoordTypeX viewportMaxX = 0, CoordTypeY viewportMaxY = 0,
             const ValueGetterType &valueGetter = ValueGetterType())
      : viewportMaxX_(viewportMaxX), viewportMaxY_(viewportMaxY),
        getValue(valueGetter) {
    getContour.isContour = true;
  }

  ~PlotDrawer();

  template <template <class> class FrameStyleType>
  void drawFrame(TCanvas &canvas, const FrameStyleType<CoordType> &frameStyle =
                                      FrameStyleType<CoordType>());
  template <class DrawStyleType>
  void drawModules(TCanvas &canvas,
                   const DrawStyleType &drawStyle = DrawStyleType());
  template <class DrawStyleType>
  void drawModuleContours(TCanvas &canvas,
                          const DrawStyleType &drawStyle = DrawStyleType());

  void add(const Module &m);
  template <class InputIterator>
  void addModulesType(InputIterator begin, InputIterator end,
                      int moduleTypes = BARREL | ENDCAP);
  void addModules(const Visitable &structure);
  void addModulesType(const Visitable &structure,
                      int moduleTypes = BARREL | ENDCAP);
  template <class ModuleValidator>
  void addModules(const Visitable &structure,
                  const ModuleValidator &isValid = ModuleValidator());
  template <class ModuleValidator, class InputIterator>
  void addModules(InputIterator begin, InputIterator end,
                  const ModuleValidator &isValid = ModuleValidator());
};

template <class CoordType, class ValueGetterType, class StatType>
PlotDrawer<CoordType, ValueGetterType, StatType>::~PlotDrawer() {
  for (typename std::map<CoordType, StatType *>::iterator it = bins_.begin();
       it != bins_.end(); ++it) {
    delete it->second;
    delete lines_[it->first];
    if (contour_[it->first])
      delete contour_[it->first];
  }
  bins_.clear();
  lines_.clear();
  contour_.clear();
...
template <class CoordType, class ValueGetterType, class StatType>
template <template <class> class FrameStyleType>
void PlotDrawer<CoordType, ValueGetterType, StatType>::drawFrame(
    TCanvas &canvas, const FrameStyleType<CoordType> &frameStyle) {
  double minValue = std::numeric_limits<double>::max();
  double maxValue = 0;
  for (typename std::map<CoordType, StatType *>::const_iterator it =
           bins_.begin();
       it != bins_.end(); ++it) {
  ...
  viewportMaxX_ =
      viewportMaxX_ == 0
          ? getLine.maxx() * 1.1
          : viewportMaxX_; // in case the viewport coord is 0, auto-viewport
                           // mode is used and getLine is queried for the
                           // farthest X or Y it has registered
...
template <class CoordType, class ValueGetterType, class StatType>
template <class DrawStyleType>
void PlotDrawer<CoordType, ValueGetterType, StatType>::drawModules(
    TCanvas &canvas, const DrawStyleType &drawStyle) {
  canvas.cd();
  for (typename std::map<CoordType, StatType *>::const_iterator it =
           bins_.begin();
       it != bins_.end(); ++it) {
...
template <class CoordType, class ValueGetterType, class StatType>
template <class DrawStyleType>
void PlotDrawer<CoordType, ValueGetterType, StatType>::drawModuleContours(
    TCanvas &canvas, const DrawStyleType &drawStyle) {
  canvas.cd();
  for (typename std::map<CoordType, StatType *>::const_iterator it =
           bins_.begin();
    ...
    if (contour)
      drawStyle(*contour, *bin, palette_);
  }
}
...
template <class CoordType, class ValueGetterType, class StatType>
template <class InputIterator>
void PlotDrawer<CoordType, ValueGetterType, StatType>::addModulesType(
    InputIterator begin, InputIterator end, int moduleTypes) {
...
template <class CoordType, class ValueGetterType, class StatType>
void PlotDrawer<CoordType, ValueGetterType, StatType>::addModules(
    const Visitable &structure) {
  class ModuleVisitor : public ConstGeometryVisitor {
  private:
    PlotDrawer<CoordType, ValueGetterType, StatType> *pd_;

  public:
    ModuleVisitor(PlotDrawer<CoordType, ValueGetterType, StatType> *pd) {
      pd_ = pd;
    };
...
template <class CoordType, class ValueGetterType, class StatType>
void PlotDrawer<CoordType, ValueGetterType, StatType>::addModulesType(
    const Visitable &structure, int moduleTypes) {
  class ModuleVisitor : public ConstGeometryVisitor {
  private:
    PlotDrawer<CoordType, ValueGetterType, StatType> *pd_;
    int moduleTypes_;
...
template <class ModuleValidator>
void PlotDrawer<CoordType, ValueGetterType, StatType>::addModules(
    const Visitable &structure, const ModuleValidator &isValid) {
  class ModuleVisitor : public ConstGeometryVisitor {
  private:
    PlotDrawer<CoordType, ValueGetterType, StatType> *pd_;
  ...
  public:
    ModuleVisitor(PlotDrawer<CoordType, ValueGetterType, StatType> *pd,
                  const ModuleValidator &isValid)
        : pd_(pd), isValid_(isValid){};
    void visit(const Module &m) {
      int subDet = m.subdet();
      if (isValid_(m))
        pd_->add(m);
...
template <class ModuleValidator, class InputIterator>
void PlotDrawer<CoordType, ValueGetterType, StatType>::addModules(
    InputIterator begin, InputIterator end, const ModuleValidator &isValid) {
  for (InputIterator it = begin; it != end; ++it) {
    if (isValid(**it))
      add(**it);
  }
