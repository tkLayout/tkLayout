...
public:
  ...
  ConversionStation(const std::string subdetectorName)
      : MaterialObject(MaterialObject::Type::STATION, subdetectorName),
        ...
        type_("type", parsedAndChecked()), minZ_("minZ", parsedOnly()),
        maxZ_("maxZ", parsedOnly()), subdetectorName_(subdetectorName),
        stationType_(ERROR), conversionsNode_("Conversion", parsedOnly()),
        nonConvertedMaterialsNode_("NonConvertedMaterials", parsedOnly()){};
  virtual ~ConversionStation(){};

  void build();
  void routeConvertedElements(MaterialObject &localOutput,
...
private:
  ...
  void addOutputElements(MaterialObject::Element *outputElement,
                         MaterialObject &localOutput,
                         MaterialObject &serviceOutput);
 ...
 * These are the materials that should not be scaled as a function of the input.
 * They are independent from the conversions!! The non converted materials are
 * of the same quantity, whether the input is nothing, 10 g of Cu, or 50 g of
 * ALN! These materials can be local (assigned to the station volume) or
 * services (routed from the station volume).
 *///////////////////////////////////////////////////
