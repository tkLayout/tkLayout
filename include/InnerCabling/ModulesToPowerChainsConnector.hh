 ...
 * The idea is to use generic information on the module location, and use it to
 * assign the module to a given power chain. Once the mapping is done, it is
 * cross-checked that power chains are not assigned more modules than what they
 * can afford.
 */
...
private:
  ...
  computeBarrelModuleZEnd(const int side, const int ring,
                          const int layerNumber) const;
  const bool computeBarrelCentralModuleZEnd(const int layerNumber) const;
  const std::pair<int, int>
  computeForwardModulePhiPowerChain(const double modPhi,
                                    const int numModulesInRing,
                                    const bool isPositiveZEnd) const;

  // BUILDING POWER CHAIN
  void buildPowerChain(DetectorModule &m,
                       std::map<int, PowerChain *> &powerChains,
                       ...
                       const int halfRingIndex = 0,
                       const bool isAtSmallerAbsZDeeInDoubleDisk = false,
                       const bool isAtSmallerAbsZSideInDee = false);
  const int computePowerChainId(const bool isPositiveZEnd,
  ...
  void connectModuleToPowerChain(DetectorModule &m,
                                 PowerChain *powerChain) const;

  // CHECKING POWER CHAIN
  void checkModulesToPowerChainsCabling(
