 ...
 * All modules connected to a given GBT can be accessed, as well as the Fiber
 * Bundle the GBT is connected to. General info on the GBT is also provided.
 */
...
public:
  GBT(PowerChain *myPowerChain, const std::string GBTId, const int myGBTIndex,
      const int myGBTIndexColor, const int numELinksPerModule);
  ...
  const InnerBundle *getBundle() const {
    if (!myBundle_)
      throw PathfulException("myBundle_ is nullptr");
    return myBundle_;
  }
  ...
  const PowerChain *getPowerChain() const {
    if (!myPowerChain_)
      throw PathfulException("myPowerChain_ is nullptr");
  ...
  const bool isSmallerAbsZHalfRing() const {
    return myPowerChain_->isSmallerAbsZHalfRing();
  }
  const int halfRingIndex() const { return myPowerChain_->halfRingIndex(); }
  const bool isAtSmallerAbsZDeeInDoubleDisk() const {
    return myPowerChain_->isAtSmallerAbsZDeeInDoubleDisk();
  }
  const bool isAtSmallerAbsZSideInDee() const {
    return myPowerChain_->isAtSmallerAbsZSideInDee();
  }
