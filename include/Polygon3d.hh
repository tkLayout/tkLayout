...
struct RotateX<XYZVector, FloatType>
    : public std::binary_function<XYZVector, FloatType, XYZVector> {
...
class AbstractPolygon { // any number of dimensions, any number of sides, convex
                        // or concave. it only has 2 properties
protected:
  FloatType area_;
...
public:
  ...
  AbstractPolygon(const Coords &vertex)
      : allocated_(0), centerDirty_(true), normalDirty_(true) {
    *this << vertex;
  }
  virtual ~AbstractPolygon(){};
  virtual AbstractPolygon<NumSides, Coords, Random, FloatType> &
  operator<<(const Coords &vertex);
  virtual AbstractPolygon<NumSides, Coords, Random, FloatType> &
  operator<<(const std::vector<Coords> &vertices);
  virtual AbstractPolygon<NumSides, Coords, Random, FloatType> &
  operator()(const Coords &vertex) {
    *this << vertex;
    return *this;
  ...
  double getDoubleArea() const; // we save a division by 2 by returning the
                                // double area. for our purposes it's the same
  ...
  AbstractPolygon<NumSides, Coords, Random, FloatType> &
  translate(const Coords &vector);
  AbstractPolygon<NumSides, Coords, Random, FloatType> &
  mirror(const Coords &vector);
  AbstractPolygon<NumSides, Coords, Random, FloatType> &
  rotateX(FloatType angle);
  AbstractPolygon<NumSides, Coords, Random, FloatType> &
  rotateY(FloatType angle);
  AbstractPolygon<NumSides, Coords, Random, FloatType> &
  rotateZ(FloatType angle);
};

template <int NumSides, class Coords, class Random, class FloatType>
inline bool
AbstractPolygon<NumSides, Coords, Random, FloatType>::isComplete() const {
  return allocated_ == NumSides;
}

template <int NumSides, class Coords, class Random, class FloatType>
inline const Coords *
AbstractPolygon<NumSides, Coords, Random, FloatType>::getVertices() const {
  return v_;
}

template <int NumSides, class Coords, class Random, class FloatType>
inline const Coords &
AbstractPolygon<NumSides, Coords, Random, FloatType>::getVertex(
    int index) const {
  return v_[index];
}
...
template <int NumSides, class Coords, class Random, class FloatType>
double
AbstractPolygon<NumSides, Coords, Random, FloatType>::getDoubleArea() const {
  return area_;
}

template <int NumSides, class Coords, class Random, class FloatType>
const Coords &
AbstractPolygon<NumSides, Coords, Random, FloatType>::getCenter() const {
  if (centerDirty_) {
...
template <int NumSides, class Coords, class Random, class FloatType>
const Coords &
AbstractPolygon<NumSides, Coords, Random, FloatType>::getNormal() const {
  if (normalDirty_) {
...
template <int NumSides, class Coords, class Random, class FloatType>
AbstractPolygon<NumSides, Coords, Random, FloatType> &
AbstractPolygon<NumSides, Coords, Random, FloatType>::translate(
    const Coords &vector) {
  std::transform(&v_[0], &v_[NumSides], &v_[0],
                 std::bind2nd(std::plus<Coords>(), vector));
  setGeomDirty(true);
  return *this;
}
...
template <int NumSides, class Coords, class Random, class FloatType>
AbstractPolygon<NumSides, Coords, Random, FloatType> &
AbstractPolygon<NumSides, Coords, Random, FloatType>::mirror(
    const Coords &vector) {
  for (int i = 0; i < NumSides; i++) {
    v_[i].SetCoordinates(v_[i].X() * vector.X(), v_[i].Y() * vector.Y(),
                         v_[i].Z() * vector.Z());
  }
  setGeomDirty(true);
  return *this;
...
AbstractPolygon<NumSides, Coords, Random, FloatType>::rotateX(FloatType angle) {
  std::transform(&v_[0], &v_[NumSides], &v_[0],
                 std::bind2nd(RotateY<Coords, FloatType>(), angle));
  setGeomDirty(true);
  return *this;
}
...
AbstractPolygon<NumSides, Coords, Random, FloatType>::rotateZ(FloatType angle) {
  std::transform(&v_[0], &v_[NumSides], &v_[0],
                 std::bind2nd(RotateZ<Coords, FloatType>(), angle));
  setGeomDirty(true);
  return *this;
}
...
template <int NumSides, class Coords, class Random, class FloatType>
AbstractPolygon<NumSides, Coords, Random, FloatType> &
AbstractPolygon<NumSides, Coords, Random, FloatType>::operator<<(
    const Coords &vertex) {
  if (!isComplete())
    v_[allocated_++] = vertex;
  if (isComplete())
    computeProperties();
  return *this;
}
...
AbstractPolygon<NumSides, Coords, Random, FloatType> &
AbstractPolygon<NumSides, Coords, Random, FloatType>::operator<<(
    const std::vector<Coords> &vertices) {
  for (typename std::vector<Coords>::const_iterator it = vertices.begin();
       (!isComplete()) && (it != vertices.end()); ++it) {
    *this << *it;
  }
  return *this;
}
...
class Polygon3d
    : public AbstractPolygon<NumSides, ROOT::Math::XYZVector,
                             TRandom> { // no checks are made on the convexity,
                                        // but the algorithms in the class only
                                        // work for convex polygons, so beware!
public:
...
public:
  ...
  Polygon3d(const ROOT::Math::XYZVector &vertex)
      : AbstractPolygon<NumSides, ROOT::Math::XYZVector, TRandom>(vertex) {}
  ...
  bool isLineIntersecting(const XYZVector &orig, const XYZVector &dir,
                          XYZVector &intersection) const;
  ROOT::Math::XYZVector generateRandomPoint(TRandom *die) const;
};

template <>
class Polygon3d<3>
    : public AbstractPolygon<3, ROOT::Math::XYZVector,
                             TRandom> { // a triangle can be defined with more
                                        // than 3 vertices. no error checking is
                                        // made, simply the additional vertices
                                        // besides the 3rd one are ignored
  void computeProperties();

public:
  Polygon3d() : AbstractPolygon<3, ROOT::Math::XYZVector, TRandom>() {}
  Polygon3d(const ROOT::Math::XYZVector &vertex)
      : AbstractPolygon<3, ROOT::Math::XYZVector, TRandom>(vertex) {}
  bool isPointInside(const ROOT::Math::XYZVector &p) const;
  ROOT::Math::XYZVector generateRandomPoint(TRandom *die) const;
};
...
bool Polygon3d<NumSides>::isPointInside(const XYZVector &p) const {
  ...
  for (const XYZVector *it = vbegin; it != vend; ++it) {
    ...
    sum += t.getDoubleArea();
    if (sum - this->getDoubleArea() > 1e-4)
      return false; // early quit if sum area is already bigger
  }
  return fabs(this->getDoubleArea() - sum) < 1e-4;
}

template <int NumSides>
bool Polygon3d<NumSides>::isLineIntersecting(const XYZVector &orig,
                                             const XYZVector &dir,
                                             XYZVector &intersection) const {
  ...
  if (normDir < 1e-3)
    return false; // no fabs because if normDir < 0 we want to return false, as
                  // we're matching with the module in the opposite direction
  intersection = orig + (((d - normOrig) / normDir) *
                         dir); // TODO: slightly inefficient here: we do not
                               // need to go 3D and then 2D again...
  return isPointInside(intersection);
}

template <int NumSides> void Polygon3d<NumSides>::computeProperties() {
  this->area_ = 0;
  for (const XYZVector *it = &this->v_[1]; it != &this->v_[NumSides - 1];
       ++it) {
    Triangle3d t;
    t << this->v_[0] << *it << *(it + 1);
...
XYZVector Polygon3d<NumSides>::generateRandomPoint(TRandom *die) const {
  ...
  for (it = getTriangulation().begin(); it != getTriangulation().end(); it++) {
    if (it->getDoubleArea() + prevBin >= binPicker)
      break; // found triangle
...
template <int NumSides>
inline const std::multiset<Triangle3d, PolygonLess<Triangle3d>> &
Polygon3d<NumSides>::getTriangulation() const {
  return trianglesByArea_;
}

inline void Triangle3d::computeProperties() {
  this->area_ =
      sqrt((this->v_[1] - this->v_[0]).Cross(this->v_[2] - this->v_[0]).Mag2());
}

inline bool Triangle3d::isPointInside(const XYZVector &p)
    const { // note: this is exactly the same as the non specialized case //
            // inlined to avoid linker issues
  double sum = 0;
...
inline XYZVector Triangle3d::generateRandomPoint(TRandom *die) const {
  ...
  return this->v_[0] + a * (this->v_[1] - this->v_[0]) +
         b * (this->v_[2] -
              this->v_[0]); // seriously C++??? you've been around for a while
                            // now, time to fix this horrid syntax??
}
