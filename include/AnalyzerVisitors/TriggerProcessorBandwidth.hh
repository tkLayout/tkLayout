...
std::pair<Circle, Circle> findCirclesTwoPoints(const Point &p1, const Point &p2,
                                               double r);
bool isPointInCircle(const Point &p, const Circle &c);
bool areClockwise(const Point &p1, const Point &p2);

double calculatePetalAreaMC(const Tracker &tracker, const SimParms &simParms,
                            double crossoverR);
double calculatePetalAreaModules(const Tracker &tracker,
                                 const SimParms &simParms, double crossoverR);
double calculatePetalCrossover(const Tracker &tracker,
                               const SimParms &simParms);

bool isModuleInPetal(const DetectorModule &module, double petalPhi,
                     double curvatureR, double crossoverR);
bool isModuleInCircleSector(const DetectorModule &module, double startPhi,
                            double endPhi);

bool isModuleInEtaSector(const SimParms &simParms, const Tracker &tracker,
                         const DetectorModule &module, int etaSector);
bool isModuleInPhiSector(const SimParms &simParms, const DetectorModule &module,
...
class TriggerProcessorBandwidthVisitor : public ConstGeometryVisitor {
  ...
  map<string, map<pair<int, int>, double>> &triggerDataBandwidths_,
...
public:
  SummaryTable processorConnectionSummary, processorInboundBandwidthSummary,
      processorInboundStubPerEventSummary;
  ...
  public:
    ...
    int totalCpuConnections() const {
      return phiCpuConnections_ * etaCpuConnections_;
    }
...
public:
  TriggerProcessorBandwidthVisitor(
      map<string, map<pair<int, int>, double>> &triggerDataBandwidths,
      map<string, map<pair<int, int>, double>> &triggerFrequenciesPerEvent)
