...
typedef std::map<const DetectorModule *, std::map<int, double>>
...
public:
  ...
  TriggerDistanceTuningPlotsVisitor(profileBag &myProfileBag,
      ...
      : myProfileBag_(myProfileBag), triggerMomenta_(triggerMomenta) {
  ...
  void visit(const DetectorModule &m) {
    if (m.sensorLayout() && m.dsDistance() > 0.)
      foundSpacing_.insert(m.dsDistance());
  }

  void visit(const BarrelModule &aModule) {

    std::string myName = aModule.subdetectorName() + "_L" +
                         any2str(aModule.layer()) + "R" +
                         any2str(aModule.ring());

    std::vector<const DetectorModule *> &theseBarrelModules =
        selectedModules_[myName];

    if (aModule.dsDistance() <= 0 || aModule.triggerWindow() == 0)
      return;

    // Prepare the variables to hold the profiles
    std::map<double, TProfile> &tuningProfiles =
        myProfileBag_.getNamedProfiles(profileBag::TriggerProfileName + myName);
    // Prepare the variables to hold the turn-on curve profiles
    std::map<double, TProfile> &turnonProfiles =
        myProfileBag_.getNamedProfiles(profileBag::TurnOnCurveName + myName);

    //  Profiles
    if (!preparedProfiles_[myName]) {
      preparedProfiles_[myName] = true;
      for (std::vector<double>::const_iterator it = triggerMomenta_.begin();
           it != triggerMomenta_.end(); ++it) {
        std::ostringstream tempSS;
        tempSS << "Trigger efficiency for " << myName.c_str()
               << ";Sensor spacing [mm];Efficiency [%]";
        tuningProfiles[*it].SetTitle(tempSS.str().c_str());
        tempSS.str("");
        tempSS << "TrigEff" << myName.c_str() << "_" << (*it) << "GeV";
        tuningProfiles[*it].SetName(tempSS.str().c_str());
        tuningProfiles[*it].SetBins(
            100, 0.5,
            6); // TODO: these numbers should go into some kind of const
      }
    }
    ...
    if (!preparedTurnOn_[myName]) {
      ...
      for (unsigned int iWindow = 0; iWindow < nWindows_; ++iWindow) {
        ...
        tempSS.str("");
        tempSS << "TurnOn" << myName.c_str() << "_window" << int(windowSize);
        turnonProfiles[windowSize].SetName(tempSS.str().c_str());
        turnonProfiles[windowSize].SetBins(
            100, 0.5,
            10); // TODO: these numbers should go into some kind of const
      }
    }

    XYZVector center = aModule.center();
    if ((center.Z() < 0) || (center.Phi() < 0) || (center.Phi() > M_PI / 2))
      return;
    ...
    for (double myPt = 0.5; myPt <= 10; myPt += 0.02) {
      for (unsigned int iWindow = 0; iWindow < nWindows_; ++iWindow) {
        ...
        double myValue =
            100 * pterr.getTriggerProbability(myPt, distance, int(windowSize));
        if ((myValue >= 0) && (myValue <= 100))
  ...
  void visit(const EndcapModule &aModule) {
    ...
    string myBaseName =
        any2str(aModule.subdetectorName()) + "_D" + any2str(aModule.disk());

    if ((aModule.dsDistance() <= 0) || (aModule.triggerWindow() == 0))
      return;

    XYZVector center = aModule.center();
    // std::cerr << myBaseName << " z=" << center.Z() << ", Phi=" <<
    // center.Phi() << ", Rho=" << center.Rho() << std::endl; // debug
    if ((center.Z() < 0) || (center.Phi() < 0) || (center.Phi() > M_PI / 2))
      return;
    ...
    if (!preparedProfiles_[myName]) {
      ...
      for (std::vector<double>::const_iterator it = triggerMomenta_.begin();
        ...
        std::ostringstream tempSS;
        tempSS << "Trigger efficiency for " << myName.c_str()
               << " GeV;Sensor spacing [mm];Efficiency [%]";
        tuningProfiles[*it].SetTitle(tempSS.str().c_str());
        tempSS.str("");
        tempSS << "TrigEff" << myName.c_str() << "_" << (*it);
        tuningProfiles[*it].SetName(tempSS.str().c_str());
        tuningProfiles[*it].SetBins(
            100, 0.5,
            6); // TODO: these numbers should go into some kind of const
    ...
    if (!preparedTurnOn_[myName]) {
      ...
      for (unsigned int iWindow = 0; iWindow < nWindows_; ++iWindow) {
        ...
        turnonProfiles[windowSize].SetTitle(tempSS.str().c_str());
        tempSS.str("");
        tempSS << "TurnOn" << myName.c_str() << "_window" << windowSize;
        turnonProfiles[windowSize].SetName(tempSS.str().c_str());
        turnonProfiles[windowSize].SetBins(
            100, 0.5,
            10); // TODO: these numbers should go into some kind of const
  ...
  void postVisit() {
    ...
    std::vector<double> spacingOptions(foundSpacing_.begin(),
                                       foundSpacing_.end());
    foundSpacing_.clear();

    unsigned int nSpacingOptions =
        spacingOptions.size(); // TODO: keep this here!!
    ...
    std::vector<std::string> profileNames =
        myProfileBag_.getProfileNames(profileBag::TriggerProfileName);
    for (std::vector<std::string>::const_iterator itName = profileNames.begin();
         itName != profileNames.end(); ++itName) {
      std::map<double, TProfile> &tuningProfiles =
          myProfileBag_.getNamedProfiles(*itName);
      ...
      triggerRangeHighLimit[*itName] =
          findXThreshold(highTuningProfile, 90, false);
    }
    ...
    XYZVector center;
    double myPt;
    TProfile tempProfileLow(
        "tempProfileLow", "", 100, 0.5,
        6); // TODO: these numbers should go into some kind of const
    TProfile tempProfileHigh(
        "tempProfileHigh", "", 100, 0.5,
        6); // TODO: these numbers should go into some kind of const

    // TODO: IMPORTANT!!!!!! clear the spacing tuning graphs and frame here
    spacingTuningFrame.SetBins(selectedModules_.size(), 0,
                               selectedModules_.size());
    ...
    for (std::map<std::string, ModuleVector>::iterator itTypes =
             selectedModules_.begin();
      ...
      for (unsigned int iWindow = 0; iWindow < nWindows_; ++iWindow) {
        ...
        for (ModuleVector::const_iterator itModule = myModules.begin();
             itModule != myModules.end(); ++itModule) {
          ...
          for (double dist = 0.5; dist <= 6;
               dist += 0.02) { // TODO: constant here
            // First with the high momentum
            myPt = (spacingTuningMomenta.second);
            double myValue =
                100 * pterr.getTriggerProbability(myPt, dist, windowSize);
            if ((myValue >= 0) && (myValue <= 100))
          ...
          if (minDistBelow >= 0) {
            if (windowSize == 5)
              optimalSpacingDistribution.Fill(minDistBelow);
            if (windowSize == aModule->triggerWindow())
              optimalSpacingDistributionAW.Fill(minDistBelow);
          }

          /*if ((myName=="ENDCAP_D02R05")&&(windowSize==5)) { // debug
            std::cout << myName << " - " << aModule->getTag() << " -
            minDistBelow = " << minDistBelow; std::cout <<  ", so[0]=" <<
            spacingOptions[0] << ", so[n-1]=" <<
            spacingOptions[nSpacingOptions-1];
            }*/
          if (minDistBelow < spacingOptions[0])
            minDistBelow = spacingOptions[0];
          else if (minDistBelow > spacingOptions[nSpacingOptions - 1])
            minDistBelow = spacingOptions[nSpacingOptions - 1];
          else {
            for (unsigned int iSpacing = 0; iSpacing < nSpacingOptions - 1;
                 ++iSpacing) {
              /*if ((myName=="ENDCAP_D02R05")&&(windowSize==5)) {// debug
                std::cout << " spacingOptions[" << iSpacing << "] = " <<
                spacingOptions[iSpacing]; std::cout << " spacingOptions[" <<
                iSpacing+1 << "] = " << spacingOptions[iSpacing+1];
                }*/
              if ((minDistBelow >= spacingOptions[iSpacing]) &&
          ...
          /*if ((myName=="ENDCAP_D02R05")&&(windowSize==5)) // debug
            std::cout << " - approx to " << minDistBelow << " for a window of "
            << windowSize << std::endl;*/
        ...
        // std::cerr << myName << ": " << lowEdge << " -> " << highEdge <<
        // std::endl; // debug
        double centerX;
        double sizeX;
        centerX = iType + (double(iWindow) + 0.5) / (double(nWindows_));
        sizeX = 1. / (double(nWindows_)) *
                0.8; // 80% of available space, so that they will not touch
        if (lowEdge < highEdge) {
          spacingTuningGraphs[iWindow].SetPoint(iType, centerX,
                                                (highEdge + lowEdge) / 2.);
          spacingTuningGraphs[iWindow].SetPointError(iType, sizeX / 2.,
                                                     (highEdge - lowEdge) / 2.);
        } else {
          spacingTuningGraphsBad[iWindow].SetPoint(iType, centerX,
                                                   (highEdge + lowEdge) / 2.);
          spacingTuningGraphsBad[iWindow].SetPointError(
              iType, sizeX / 2., (highEdge - lowEdge) / 2.);
        }
    ...
    int iPoints = 0;
    for (std::map<double, bool>::iterator it = availableThinkness.begin();
         it != availableThinkness.end(); ++it) {
      iPoints++;
