...
/**
 * @file MaterialProperties.h
 * @brief This is the header of a base class for elements that have an influence
 * on the material budget of a tracker layout
 */
...
static const MechanicalCategory
computeMechanicalCategory(const std::string componentName) {
  if (componentName.find(mechanical_module) != std::string::npos)
    return MechanicalCategory::MODULE;
  else if (componentName.find(mechanical_cabling) != std::string::npos)
    return MechanicalCategory::CABLING;
  else if (componentName.find(mechanical_support) != std::string::npos)
    return MechanicalCategory::SUPPORT_AND_COOLING;
  else if (componentName.find(mechanical_cooling) != std::string::npos)
    return MechanicalCategory::SUPPORT_AND_COOLING;
  else
    return MechanicalCategory::UNKNOWN;
}

/* Normalize the mechanical categories' contributions to RL and IL.
 */
static void
normalizeRIRatio(std::map<MechanicalCategory, std::pair<double, double>>
  ...
  for (const auto &mechanicalCategoryIt :
       normalizedRIRatioPerMechanicalCategory) {
    const std::pair<double, double> &nonNormalizedRI =
...
class LocalElement {
public:
  LocalElement(const std::string subdetectorName,
               const std::string componentName, const std::string elementName)
      : subdetectorName_(subdetectorName), componentName_(componentName),
...
struct ElementNameCompare {
  bool operator()(const LocalElement &localA,
                  const LocalElement &localB) const {
    if (localA.subdetectorName() != localB.subdetectorName())
      return (localA.subdetectorName() < localB.subdetectorName());
    else {
      if (localA.mechanicalCategory() != localB.mechanicalCategory())
        return (localA.mechanicalCategory() < localB.mechanicalCategory());
      else {
        if (localA.componentName() != localB.componentName())
          return (localA.componentName() < localB.componentName());
        else
          return (localA.elementName() < localB.elementName());
      }
    }
  }
...
/**
 * Errors and messages that may be reported during operations on member
 * variables
 */
static const std::string err_local_mass = "Local mass not found";
static const std::string msg_mattab_except_local =
    "Exception other than runtime_error occurred accessing material table for "
    "local masses: ";
/**
 * @class MaterialProperties
 * @brief This is the base class for collections of properties related to the
 * material budget.
 *
 * It encapsulates the main parameters of interest, namely the overall density,
 * radiation length and interaction length of a tracker element, as well as the
 * influence of the materials that make up this building block. Access functions
 * are provided where appropriate. But unless the object is cloned the overall
 * parameters should typically be calculated from a list of materials and their
 *  properties rather than set explicitly. Some of the access functions for
 * individual materials may throw exceptions if the requested material does not
 * appear on the list.
 */
class MaterialProperties {
public:
  /**
   * @enum Category A list of logical categories within the detector geometry; a
   * single element belongs to exactly one of them
   */
  ...
  // weights
  const std::map<LocalElement, double, ElementNameCompare>
  getLocalElementsDetails() const {
    return localMassesDetails_;
  }
  // RI
  const std::map<LocalElement, RILength, ComponentNameCompare> &
  getComponentsRI() const {
    return componentsRI_;
  }
  ...
  void addLocalMass(const std::string subdetectorName, const std::string tag,
                    const std::string comp, double ms, int minZ = -777);
  void addLocalMass(const std::string subdetectorName, const std::string tag,
                    double ms);
  // void addLocalMass(std::string tag, std::string comp, double ms, int minZ =
  // -777); void addLocalMass(std::string tag, double ms);
  ...
  void calculateInteractionLength(MaterialTable &materials,
                                  double offset = 0.0);
...
protected:
  ...
  // THIS SHOULD REPLACE localmasses, localmassesComp, and so on. All desired
  // info is accessed from LocalElementDetails:
  std::map<LocalElement, double, ElementNameCompare> localMassesDetails_;
  std::map<LocalElement, RILength, ComponentNameCompare>
      componentsRI_; // component-by-component radiation and interaction lengths
  ...
  std::map<std::string, std::map<std::string, double>>
      localCompMats; // format here is <component name string, <material name,
                     // mass> >
