...
public:
  void
  calculateFlatRingsGeometryInfo(std::vector<StraightRodPair *> flatPartRods,
                                 double bigParity);
...
public:
  ...
  TiltedRingsGeometryInfo(int numModulesFlat, double, double, double, double,
...
class Layer : public PropertyObject,
              public Buildable,
              public Identifiable<int>,
              public Clonable<Layer>,
              public Visitable {
public:
  ...
  Property<double, AutoDefault>
      skewedModuleMinRho; // takes sensor thickness into account.
                          // WARNING: min Rho is not compulsory reached at the
                          // skewed sensor edge!!
  Property<double, AutoDefault> skewedModuleCenterRho;
  Property<double, AutoDefault>
      skewedModuleMaxRho; // takes sensor thickness into account
  ...
  Layer(const std::string subdetectorName)
      : smallDelta("smallDelta", parsedAndChecked()),
        ...
        buildNumModules("numModules", parsedOnly()), maxZ("maxZ", parsedOnly()),
        ...
        rotateLayerByRodsDeltaPhiHalf("rotateLayerByRodsDeltaPhiHalf",
                                      parsedOnly(), false),
        ...
        flangeConversionStation_(nullptr), subdetectorName_(subdetectorName),
        phiOverlap("phiOverlap", parsedOnly()), // used to be parsedAndChecked()
        phiSegments("phiSegments",
                    parsedOnly()), // used to be parsedAndChecked(), and default
                                   // value = 4
        ringNode("Ring", parsedOnly()), stationsNode("Station", parsedOnly()) {
    setup();
  }
  ...
  void setup() {
    ...
    maxZwithHybrids.setup([this]() {
      double max = 0;
      for (const auto &r : rods_) {
        max = MAX(max, r.maxZwithHybrids());
      }
      return max;
    });
    minZwithHybrids.setup([this]() {
      double min = std::numeric_limits<double>::max();
      for (const auto &r : rods_) {
        min = MIN(min, r.minZwithHybrids());
      }
      return min;
    });
  ...
  int numModulesPerRodSide(int side) const {
    return rods_.front().numModulesSide(side);
  }
  int totalModules() const { return numModulesPerRod() * numRods(); }
  double rodThickness() const { return rods_.front().thickness(); }
  double flatPartRodThickness() const {
    return smallDelta() * 2. + rods_.front().maxModuleThickness();
  }

  const Container &rods() const { return rods_; }
  std::vector<StraightRodPair *> flatPartRods() const { return flatPartRods_; }
  double flatPartPhiOverlapSmallDeltaMinus() const {
    return flatPartPhiOverlapSmallDeltaMinus_;
  }
  double flatPartPhiOverlapSmallDeltaPlus() const {
    return flatPartPhiOverlapSmallDeltaPlus_;
  }
  double flatPartAverageR() const { return flatPartAverageR_; }
  FlatRingsGeometryInfo flatRingsGeometryInfo() const {
    return flatRingsGeometryInfo_;
  }
  TiltedRingsTemplate tiltedRingsGeometry() const {
    return tiltedRingsGeometry_;
  }
  TiltedRingsGeometryInfo tiltedRingsGeometryInfo() const {
    return tiltedRingsGeometryInfo_;
  }
...
private:
  ...
  double calculatePlaceRadius(int numRods, double bigDelta, double smallDelta,
                              double dsDistance, double moduleWidth,
                              double overlap);
  ...
  const bool placeAndStoreFirstRod(
  ...
  void placeAndStoreSecondRod(StraightRodPair *secondRod,
  ...
  void placeAndStoreRod(StraightRodPair *rod, const RodTemplate &rodTemplate,
                        const bool isPlusBigDeltaRod,
                        const double rodCenterPhi);
  void buildAndStoreClonedRodsInNonSkewedMode(const StraightRodPair *firstRod,
                                              const StraightRodPair *secondRod,
                                              const double rodCenterPhiShift);
  // dedicated to skewed mode
  const SkewedLayerPhiShifts buildSkewed();
  static const SkewedLayerInfo
  computeSkewedLayerInfo(const double layerCenterRho, const double bigDelta,
                         const int numRods, const double moduleWidth,
                         const double skewedModuleEdgeShift,
                         const double installationOverlapRatio);

  void buildAndStoreClonedRodsInSkewedMode(
  ...
  double buildAndStoreNonSkewedRodsInSkewedMode(
      const int rodId, const int numRodsPerXSide,
      const StraightRodPair *firstRod, const StraightRodPair *secondRod,
      const double commonRodCenterPhiShift);
  void buildAndStoreSkewedRods(const int numRodsPerXSide,
                               ...
                               const double skewedRodCenterPhiShift);
  StraightRodPair *buildRotatedByPiInPhiRod(const StraightRodPair *initialRod,
...
/*class TiltedLayer : public Layer, public PropertyObject, public Buildable,
 public Identifiable<int>, public Clonable<TiltedLayer>, public Visitable {
