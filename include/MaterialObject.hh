...
public:
  class Element; // forward declaration for getElementIfService(Element&
                 // inputElement)
  class Component;
  ...
  void deployMaterialTo(MaterialObject &outputObject,
                        const std::vector<std::string> &unitsToDeploy,
...
private:
  ...
  PropertyNodeUnique<std::string>
      materialsNode_; // TODO:see if is better PropertyNode instead
...
public:
  ...
  public:
    ...
    ReferenceSensor()
        : numStripsAcross("numStripsAcross", parsedOnly()),
          ...
          length("length", parsedOnly()) // not checked because a custom checker
                                         // is defined for RectangularModules
    {}
    int numStripsAcrossEstimate() const {
      if (numStripsAcross.state())
        return numStripsAcross();
      else
        return floor(width() / pitchEstimate() + 0.5);
    }
    int numSegmentsEstimate() const {
      if (numSegments.state())
        return numSegments();
      else
        return floor(length() / stripLengthEstimate() + 0.5);
    }
    int numChannels() const {
      return numStripsAcrossEstimate() * numSegmentsEstimate();
    }
  ...
  public:
    ...
    MaterialObjectKey(const std::string subdetectorName,
                      const std::string &newName,
                      std::map<int, int> newSensorChannels,
                      const std::string &newDestination)
        : subdetectorName_(subdetectorName), name(newName),
          sensorChannels(newSensorChannels), destination(newDestination) {}

    bool operator<(const MaterialObjectKey &r) const {
      if (this->subdetectorName_ < r.subdetectorName_)
        return true;
      if (this->subdetectorName_ > r.subdetectorName_)
        return false;
      if (this->sensorChannels < r.sensorChannels)
        return true;
      if (this->sensorChannels > r.sensorChannels)
        return false;
      if (this->name < r.name)
  ...
  public:
    ...
    Property<std::string, NoDefault>
        componentName; // only the inner component's name
    ...
    void deployMaterialTo(MaterialObject &outputObject,
                          ...
                          Location requestedLocation = ALL) const;
    ...
    double quantityInUnit(const std::string desiredUnit,
                          const MaterialProperties &materialProperties) const;
    double quantityInUnit(const std::string desiredUnit, const double length,
    ...
    void
    populateMaterialProperties(MaterialProperties &materialProperties) const;
  ...
  private:
    ...
    // static const std::map<std::string, Materialway::Train::UnitType>
    // unitTypeMap;
  };
  ...
  public:
    ...
    void deployMaterialTo(MaterialObject &outputObject,
                          ...
                          Location requestedLocation = ALL) const;
    void
    populateMaterialProperties(MaterialProperties &materialPropertie) const;
    void getLocalElements(ElementsVector &elementsList) const;
  ...
  // ATTENTION: Materials objects of the same structure are shared between
  // MaterialObject objects
  //   of the modules/layer/etc.. (for containing memory use).
  //   This is not for service routing objects.
  Materials *materials_;

  ElementsVector serviceElements_; // used for MaterialObject not from config
