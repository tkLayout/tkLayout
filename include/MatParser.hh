...
static const std::string msg_invalid_type =
    "Invalid type description found. Skipping line.";
static const std::string msg_unknown_type = "Unknown type description found.";
static const std::string msg_unexpected_type =
    "Unexpected type description found.";
...
static const std::string msg_m_line_err =
    "Error parsing M entry. Skipping line.";
static const std::string msg_s_line_err =
    "Error parsing S entry. Skipping line.";
static const std::string msg_d_line_err =
    "Error parsing D entry. Skipping line.";
static const std::string msg_x_line_err =
    "Error parsing X entry. Skipping line";
static const std::string msg_y_line_err =
    "Error parsing Y entry. Skipping line";
static const std::string msg_z_line_err =
    "Error parsing Z entry. Skipping line";
static const std::string msg_w_line_err =
    "Error parsing W entry. Skipping line";
static const std::string msg_v_line_err =
    "Error parsing V entry. Skipping line";
static const std::string warning_too_many_values =
    "Warning: ignoring values beyond the fourth.";

/**
 * @class MatParser
 * @brief This class provides a parser for the - very simple - material config
 * file formats.
 *
 * It encapsulates the parser code that reads the material list from file and
 * initialises the global material table. More importantly, it also provides the
 * functions that parse a material config file and stores the extracted
 * information within the internal fields of a <i>MatCalc</i> object. One of the
 * private functions may throw an exception if the string parameter does not
 * correspond to one of the predefined units. This exception is always caught by
 * the caller function and the unit set to the default <i>gr</i>.
 */
class MatParser {
public:
  MatParser();
