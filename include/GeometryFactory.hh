...
struct FactoryCloneAllocator {
  template <class U> static U *allocate_clone(const U &r) {
    return GeometryFactory::clone(r);
  } // replaces cloning operation for boost ptr_containers (so that setup() is
    // called where needed)
  template <class U> static void deallocate_clone(const U *r) {
    boost::delete_clone(r);
  } // standard boost clone deleter
};

template <class T>
using PtrVector = boost::ptr_vector<T, FactoryCloneAllocator>;

// template<class T> using PtrSet = boost::ptr_set<T, FactoryCloneAllocator>;

template <class K, class V>
using PtrMap = boost::ptr_multimap<K, V, std::less<K>, FactoryCloneAllocator>;
