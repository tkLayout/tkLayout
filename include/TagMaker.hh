...
public:
  ...
  static string makeSensorTag(const BarrelModule &m) {
    stringstream ss;
    ss << "Barrel"
       << "/Tag=" << m.subdetectorName() << "L" << setfill('0') << setw(2)
       << m.layer() // << "R" << m.ring();
       << "/Width=" << m.maxWidth() << "/Height=" << m.length()
       << "/Thickness=" << m.thickness() << "/Type=" << m.moduleType();
    return ss.str();
  }
  ...
  static string makeSensorTag(const EndcapModule &m) {
    stringstream ss;
    ss << "Endcap"
       << "/Tag=" << m.subdetectorName() << "R" << setfill('0') << setw(2)
       << m.ring() << "D" << m.disk() << "/WidthLo=" << m.minWidth()
       << "/WidthHi=" << m.maxWidth() << "/Height=" << m.length()
       << "/Thickness=" << m.thickness() << "/Type=" << m.moduleType();
    return ss.str();
  }
  ...
  static string makeSensorGeoTag(const BarrelModule &m) {
    ...
    for (const auto &s : m.sensors())
      ss << "/Segments=" << s.numSegmentsEstimate();
  ...
  static string makePosTag(const BarrelModule &m) {
    stringstream ss;
    ss << m.subdetectorName() << "L" << setfill('0') << setw(2)
       << m.layer() /*<< "R" << m.ring() */;
    return ss.str();
  }
