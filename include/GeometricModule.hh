...
protected:
  ...
  double triangleCross(const XYZVector &P1, const XYZVector &P2,
                       const XYZVector &P3, const XYZVector &PL,
                       const XYZVector &PU);

public:
  Property<double, Default>
      dsDistance; // a GeometricModule is a purely 2d geometric object
                  // represented in 3d space with just a polygon and an
                  // additional for thickness value for tracker geometry
                  // construction
  ...
  void rotateX(double angle) {
    basePoly_.rotateX(angle);
    tiltAngle_ += angle;
  }
  void rotateY(double angle) {
    basePoly_.rotateY(angle);
    skewAngle_ += angle;
  }
...
class RectangularModule : public GeometricModule,
                          public Clonable<RectangularModule> {
...
public:
  ...
  RectangularModule()
      : length_("length", parsedOnly()), // not checked because a custom checker
  ...
  virtual RectangularModule *clone() const {
    return Clonable<RectangularModule>::clone();
  }
  ...
  virtual void accept(GeometryVisitor &v) {
    v.visit(*this);
    v.visit(*(GeometricModule *)this);
  }
  virtual void accept(ConstGeometryVisitor &v) const {
    v.visit(*this);
    v.visit(*(const GeometricModule *)this);
  }
...
public:
  ...
  virtual double meanWidth() const override {
    return (maxWidth_ + minWidth_) / 2;
  }
