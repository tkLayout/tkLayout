...
/**
 * Messages related to file handling that may show up on the command line, the
 * status bar, or in exceptions
 */
...
static const QString msgErrEndcapTypeBlock =
    " Error parsing endcap type block.";
static const QString msgErrModuleDressing =
    " Error parsing additional module parameters.";
static const QString msgErrUnexpectedEndOfInput =
    " Unexpected end of input file.";
static const QString msgCleanupDirContents =
    "Cleanup: could not remove directory contents.";
 ...
 *
 * The FileHandler class is a customised helper class that encapsulates various
 * kinds of read and write operations specific to the tkgeometry GUI. Its main
 * functions provide access to the tkgeometry geometry and settings
 * configuration files (parsing to internal data structures and translation of
 * those internal data structures back to config file format). It also provides
...
public:
  ...
  void writeSettingsToFile(QFile &writeFile, const paramaggreg &paramrow,
...
private:
  ...
  QString assembleSettingsFile(const paramaggreg &paramrow,
                               const QString &relativeOutputPath);
  QStringList::const_iterator
  parseTrackerBlock(QStringList::const_iterator &iter,
                    const QStringList::const_iterator &end,
                    paramaggreg &paramrow);
  QStringList::const_iterator
  parseBarrelBlock(QStringList::const_iterator &iter,
                   const QStringList::const_iterator &end,
                   paramaggreg &paramrow);
  QStringList::const_iterator
  parseEndcapBlock(QStringList::const_iterator &iter,
                   const QStringList::const_iterator &end,
                   paramaggreg &paramrow);
  QStringList::const_iterator
  parseBarrelTypeBlock(QStringList::const_iterator &iter,
                       const QStringList::const_iterator &end,
                       paramaggreg &paramrow);
  QStringList::const_iterator
  parseEndcapTypeBlock(QStringList::const_iterator &iter,
                       const QStringList::const_iterator &end,
                       paramaggreg &paramrow);
  QStringList::iterator assembleTrackerBlock(QStringList::iterator &iter,
                                             QStringList &linelist,
                                             const paramaggreg &paramrow);
  void appendBarrelTypeBlocks(QStringList &fileContents,
                              const paramaggreg &paramrow);
  void appendEndcapTypeBlocks(QStringList &fileContents,
                              const paramaggreg &paramrow);
