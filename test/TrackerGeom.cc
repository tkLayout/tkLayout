...
bool parseValues(int argc, char **argv) {
  ...
  try {
    ...
    // All-tracker options
    ValueArg<string> nameArg("n", "trackerName", "Name of the tracker", false,
                             "aTestTracker", "string");
    cmd.add(nameArg);
    ValueArg<double> smallDeltaArg(
        "", "trackerSmallDelta",
        "Distance between modules on same layer surface", false, 2., "double");
    cmd.add(smallDeltaArg);
    ValueArg<double> bigDeltaArg("", "trackerBigDelta",
                                 "Distance between layer surfaces", false, 12.,
                                 "double");
    cmd.add(bigDeltaArg);
    ValueArg<double> barrelToEndcapArg("", "trackerBarrelToEndcap",
                                       "Distance between barrel and endcap",
                                       false, 200., "double");
    cmd.add(barrelToEndcapArg);
    ValueArg<double> zErrorArg(
        "", "trackerZError",
        "The sigma of the z coordinate of interaction point", false, 70.,
        "double");
    cmd.add(zErrorArg);
    ValueArg<double> overlapArg("", "trackerOverlap",
                                "Desired overlap of modules", false, 1.,
                                "double");
    cmd.add(overlapArg);
    ValueArg<double> etaCutArg("", "trackerEtaCut",
                               "Cut modules with centre over this eta", false,
                               2.5, "double");
    cmd.add(etaCutArg);

    // Barrel
    ValueArg<int> nBarrelArg("", "barrelLayers", "Number of barrel layers",
                             false, 6, "int");
    cmd.add(nBarrelArg);
    ValueArg<int> nBarrelModulesArg("", "barrelModules",
                                    "Number modules building one half string",
                                    false, 12, "int");
    cmd.add(nBarrelModulesArg);
    ValueArg<double> barrelRhoInArg("", "barrelInnerRadius",
                                    "Inner radius of barrel", false, 340.,
                                    "double");
    cmd.add(barrelRhoInArg);
    ValueArg<double> barrelRhoOutArg("", "barrelOuterRadius",
                                     "Outer radius of barrel", false, 1080.,
                                     "double");
    cmd.add(barrelRhoOutArg);
    //     ValueArg<int> barrelModulus("","barrelModulus","String number in a
    //     layer will be multiple of this",false,4,"int"); cmd.add(
    //     barrelModulus );

    // Endcap
    ValueArg<int> nEndcapArg(
        "", "endcapDisks", "Number of endcap disks per side", false, 6, "int");
    cmd.add(nEndcapArg);
    ValueArg<double> endcapRhoInArg("", "endcapInnerRadius",
                                    "Inner radius of disks", false, 323.,
                                    "double");
    cmd.add(endcapRhoInArg);
    ValueArg<double> endcapRhoOutArg("", "endcapOuterRadius",
                                     "Outer radius of disks", false, 1095.,
                                     "double");
    cmd.add(endcapRhoOutArg);
    ValueArg<double> endcapMaxZArg("", "endcapMaxZ",
                                   "Highest Z available for the endcaps", false,
                                   2650., "double");
    ValueArg<int> endcapDiskParityArg(
        "", "endcapDiskParity",
        "Endcap disk parity (if the inner ring is next or far from the "
        "interaction point). +1 and -1 accepted",
        false, +1, "int");
    cmd.add(endcapDiskParityArg);
    //     ValueArg<int> endcapModulus("","endcapModulus","Modules in a disk
    //     will be multiple of this",false,4,"int"); cmd.add( endcapModulus );

    // Directives
    MultiArg<string> directiveRing(
        "", "directiveRing",
        "Asks for a ring increase/decrease modules (e.g. : 3-2 will reduce by "
        "2*endcapModulus the modules in ring 3)",
        false, "string");
    cmd.add(directiveRing);
    MultiArg<string> directiveLayer(
        "", "directiveLayer",
        "Asks for a barrel layer radius. Syntax: layer#/directive (directive "
        "can be F(ix), S(hrink), E(nlarge), A(uto)or a radius)",
        false, "string");
    ...
    SwitchArg placeSwitch("p", "place", "Stops after having placed modules",
                          false);
    cmd.add(placeSwitch);

    SwitchArg fullSwitch("f", "fullGeometry", "Saves also the solid geometry",
                         false);
    cmd.add(fullSwitch);

    // Summary Parameters
    ValueArg<double> summaryPtCostArg(
        "", "ptCost", "Cost of pt silicon strip detector in CHF/cm^2", false,
        200., "double");
    ValueArg<double> summaryStripCostArg(
        "", "stripCost", "Cost of silicon strip detector in CHF/cm^2", false,
        40., "double");
    ValueArg<double> summaryPtPowerArg(
        "", "ptPower",
        "Power consumption of pt silicon strip detector in mW/Channel", false,
        0.3, "double");
    ValueArg<double> summaryStripPowerArg(
        "", "stripPower",
        "Power consumption of silicon strip detector in mW/Channel", false, 0.7,
        "double");
    ...
    for (ringDirectiveIt = ringDirective.begin();
         ringDirectiveIt != ringDirective.end(); ringDirectiveIt++) {
      aNamePart.str("");
    ...
    for (layerDirectiveIt = layerDirective.begin();
         layerDirectiveIt != layerDirective.end(); layerDirectiveIt++) {
      aNamePart.str("");
      aNamePart << "_L" << std::noshowpos << (*layerDirectiveIt).first
                << std::showpos;
      if ((*layerDirectiveIt).second == Layer::FIXED)
        aNamePart << "F";
      if ((*layerDirectiveIt).second == Layer::SHRINK)
        aNamePart << "S";
      if ((*layerDirectiveIt).second == Layer::ENLARGE)
        aNamePart << "E";
      if ((*layerDirectiveIt).second == Layer::AUTO)
        aNamePart << "A";
      if ((*layerDirectiveIt).second > 0)
        aNamePart << std::showpos << (*layerDirectiveIt).second;
      tkName += aNamePart.str();
    }
...
int main(int argc, char *argv[]) {
  ...
  BarrelModule *sampleBarrelModule =
      new BarrelModule(1.); // Square modules of kind rphi
  myTracker->buildBarrel(nBarrelLayers, barrelRhoIn, barrelRhoOut,
                         nBarrelModules, sampleBarrelModule, mySection,
                         true); // Actually build compressed layer

  delete sampleBarrelModule; // Dispose of the sample module

  std::cout << "Tracker stays in the z range " << myTracker->getMaxBarrelZ(-1)
            << ".." << myTracker->getMaxBarrelZ(+1) << std::endl;
  ...
  myTracker->buildEndcaps(nEndcapDisks, // nDisks (per side)
                          ...
                          endcapMaxZ, endcapRhoIn, endcapRhoOut, sampleModule,
                          endcapDiskParity, mySection);
  ...
  std::cout << "Eta coverage of the tracker (prior to module purging): "
            << std::endl
