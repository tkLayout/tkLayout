...
int main(int argc, char **argv) {
  ...
  RootWBinaryFile *myBinaryFile = new RootWBinaryFile(
      "testFile.png", "A test binary file", "/tmp/testme.png");
  ...
  for (vector<RootWItem *>::iterator it = itemV.begin(); it != itemV.end();
       ++it) {
  ...
  if (!exists(styleDirectory)) {
    cerr << "Warning: cannot find the style directory " << styleDirectory
         ...
         << "Using current directory as style directory (probably you will see
  the site without formatting)" << endl; styleDirectory=".";
  }
